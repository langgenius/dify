...
- For each feature module, keep a module-local `README.md` as a boundary note. Start with the module name, a brief one-sentence description, and an `External Modules` table: list project modules imported from outside the module and why they are used; omit npm packages, workspace package dependencies, and whitelisted plumbing modules. For modules inside the same overall feature, write paths from that feature root, such as `shared/domain/runtime-status`; for modules outside the feature, write repo-relative project paths, such as `web/app/components/base/skeleton`. Do not copy caller-relative import paths into the README.
