...
module.exports = async function knowledgeDeployGates({
  github,
  context,
  core,
  now = Date.now,
  ...
  if (
    ...
    !/^[0-9a-f]{40}$/.test(source.head_sha)
  ) {
  ...
  while (now() < deadline) {
    const { data } = await github.rest.actions.listWorkflowRuns({
      ...context.repo,
      workflow_id: 'knowledge-fs-ci.yml',
      branch,
      event: 'push',
    ...
    const run = data.workflow_runs
      .filter(
        (candidate) =>
          candidate.head_sha === source.head_sha &&
    ...
    if (run?.status === 'completed') {
      if (run.conclusion !== 'success')
        throw new Error(`KnowledgeFS CI did not succeed for ${source.head_sha}: ${run.conclusion}.`)
      if (!(await isCurrent())) return skip()
