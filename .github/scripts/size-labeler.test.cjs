...
for (const [n, label] of [
  ...
  test(`threshold ${n}`, async () =>
    assert.deepEqual(
      (await scenario([{ filename: "a.py", additions: n, deletions: 0 }]))
        .calls,
      [["add", `size:${label}`]],
    ));
}
test("count deletions", async () =>
  assert.deepEqual(
    (await scenario([{ filename: "a.py", additions: 0, deletions: 30 }])).calls,
    [["add", "size:M"]],
  ));
...
test("replace stale size only", async () =>
  assert.deepEqual(
    (
      await scenario(
        [{ filename: "a.py", additions: 10, deletions: 0 }],
        ["size:L", "bug", "web"],
      )
    ).calls,
    [
...
test("idempotent", async () =>
  assert.deepEqual((await scenario([], ["size:XS", "bug"])).calls, []));
