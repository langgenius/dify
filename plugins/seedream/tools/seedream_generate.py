...
from tools.acedata_client import AceDataSeedreamClient, AceDataSeedreamError


class SeedreamGenerateImageTool(Tool):
    def _invoke(
        ...
        payload: dict[str, Any] = {"action": "generate", "prompt": prompt.strip()}
        if model:
            payload["model"] = model
        if size:
            payload["size"] = size
