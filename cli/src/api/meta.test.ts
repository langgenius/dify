...
describe('MetaClient', () => {
  ...
  it('fetches /openapi/v1/_version without a bearer token', async () => {
    const client = new MetaClient(createHttpClient({ baseURL: openAPIBase(mock.url) }))
  ...
  it('throws when the host has no Dify on it', async () => {
    ...
    const client = new MetaClient(createHttpClient({ baseURL: openAPIBase('http://127.0.0.1:1') }))
