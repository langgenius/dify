...
export type WorkspaceObject = {
  mode: () => string
  raw: () => WorkspaceListResponseType
}

export function newWorkspaceObject(env: WorkspaceListResponseType): WorkspaceObject {
  return {
...
export function workspaceTableHandler(currentId: string): TableHandler {
  return {
    ...
    rows: (raw): readonly TableRow[] => {
      if (!isPayloadShape<WorkspaceListResponseType>(raw, 'workspaces'))
