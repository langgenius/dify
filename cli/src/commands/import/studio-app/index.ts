...
export default class ImportStudioApp extends DifyCommand {
  ...
  static override flags = {
    'from-file': Flags.string({
      description: 'import DSL from a local file (relative or absolute path)',
      char: 'f',
    }),
    ...
    'app-id': Flags.string({
      description: 'overwrite an existing app (workflow/advanced-chat only)',
    }),
  ...
  async run(argv: string[]) {
    ...
    const status =
      result.status === 'completed-with-warnings' ? 'completed (with warnings)' : result.status
