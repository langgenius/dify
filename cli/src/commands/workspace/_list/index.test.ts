// Skipped: `get workspace` moved out of the tree; see docs/superpowers/specs/2026-09-22-difyctl-v2-human-output-design.md
...
it.skip('marks the pinned workspace, exactly one true', async () => {
  const w = await testContext({ login: true, argv: ['get', 'workspace'] })
  worlds.push(w)
...
it.skip('a response with no data array is a server error, exit 1', async () => {
  ...
  expect(err).toMatchObject({
    code: 'server_error',
    message: 'get workspace response has no data array',
  })
