...
describe('awaitAuthorization', () => {
  ...
  it('rejects when clock signals cancelled', async () => {
    ...
    const clock = new FakeClock()
    clock.cancelAt = 2
    // Cancelling is neither an expiry (exit 4) nor a usage mistake (exit 2).
    const err = await awaitAuthorization(api, code, { clock }).catch((e: unknown) => e)
    expect(err).toMatchObject({ code: 'unknown', message: 'cancelled' })
    expect((err as BaseError).exit()).toBe(ExitCode.Generic)
  })

  it('realClock reports the run-wide cancellation flag, not a constant', async () => {
    expect(realClock().isCancelled).toBe(isCancelled)
    expect(realClock().isCancelled()).toBe(false)
  })
  ...
  it('propagates BaseError thrown by api.pollOnce', async () => {
    const api = {
      pollOnce: vi
        .fn()
        .mockRejectedValue(new BaseError({ code: ErrorCode.ServerError, message: 'old server' })),
    }
    const clock = new FakeClock()
