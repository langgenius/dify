...
const workflowRun = opDescriptor('console_app.workflow.run', ['console_app', 'workflow', 'run'])
const skillsInstall: Descriptor = commandRow(SkillsInstall, ['skills', 'install'])
...
it('renders an op descriptor with input, options, global and examples', () => {
  expect(descriptorView(workflowRun).text(plain)).toMatchInlineSnapshot(`
    "console_app workflow run  Run a workflow app; streams workflow events  write sse

    Usage
      difyctl console_app workflow run [flags]
    ...
    Flags
      ...
      --inputs        map<string, json>  required  Variables declared by the app. The exact shape is per app: read \`input_schema\` from console_app.describe. A file variable takes a Dify file mapping (remote url or upload id) here, or a local file in \`files\`, not both.
    ...
    Examples
      # Required fields
      difyctl console_app workflow run --app-id <app_id> --inputs <inputs>
...
it('renders a static descriptor with arguments and flags', () => {
  expect(descriptorView(skillsInstall).text(plain)).toMatchInlineSnapshot(`
    "skills install  Write skills from the collection into a skills root  write
    ...
    Examples
      ...
      difyctl skills install ~/.claude/skills
      # Install one skill for Codex
      difyctl skills install ~/.codex/skills --skill difyctl"
  `)
})
...
it('example lines put positionals first, scalars as --flag value, non-scalars as json', () => {
  ...
  ).toBe(`difyctl console_app workflow run --app-id <app_id> --inputs '{"a":1}'`)
  expect(
    exampleLine(skillsInstall, {
      title: 't',
      input: { dir: '~/.claude/skills', skill: ['difyctl'] },
    }),
  ).toBe('difyctl skills install ~/.claude/skills --skill difyctl')
})
...
it('the map names each head, leaves by summary and groups by count', () => {
  const map = helpMap([
    ...
    { id: 'console_app list', summary: 'List apps', kind: 'list', deprecated: false },
    { id: 'console_app chat run', summary: 'Run a chat app', kind: 'sse', deprecated: false },
  ])
  expect(mapView(map).text(plain)).toMatchInlineSnapshot(`
    "console_app  2 commands: chat
...
it('a listing is one aligned row per entry', () => {
  ...
  expect(listingView(listing).text(plain)).toMatchInlineSnapshot(`
    "console_app list        List apps                    read list
    console_app legacy_run  Deprecated: use the mode op  write sse deprecated"
  `)
})
