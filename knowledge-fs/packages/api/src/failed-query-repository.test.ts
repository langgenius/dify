...
describe("createDatabaseFailedQueryRepository", () => {
  ...
  it.each(["workflow", "agent"] as const)(
    "persists %s Capability provenance and reuses an exact event across fresh grants",
    async (source) => {
      ...
      const executor = async (input: DatabaseExecuteInput): Promise<DatabaseExecuteResult> => {
        ...
        if (input.tableName === "failed_queries" && input.operation === "insert") {
          durable = {
            ...
            metadata: {
              source,
              [source === "agent" ? "agentCapture" : "workflowCapture"]: {
      ...
      const capture = (capabilityGrantId: string, query = "发票号码在哪里？") =>
        repository.captureFailedRetrieval({
          source,
          actorSubjectId: "dify-app:workflow-app",
      ...
      await expect(repository.completeFailedRetrievalTriage(triageInput)).resolves.toMatchObject({
