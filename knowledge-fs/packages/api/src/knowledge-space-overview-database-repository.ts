...
async function failedDocumentSignals(
  ...
  const result = await database.execute({
    ...
    operation: "select",
    params,
    // Derive failure from the latest bound task, even before the asynchronous product-state
    // reconciler catches up. Newer retries/successes suppress historical failed attempts.
    sql: `SELECT document.${q(database, "id")}, COALESCE(attempt.${q(database, "updated_at")}, document.${q(database, "updated_at")}) AS ${q(database, "updated_at")}, asset.${q(database, "metadata")} FROM ${q(database, "logical_documents")} document JOIN ${q(database, "document_revisions")} revision ON revision.${q(database, "tenant_id")} = document.${q(database, "tenant_id")} AND revision.${q(database, "knowledge_space_id")} = document.${q(database, "knowledge_space_id")} AND revision.${q(database, "document_id")} = document.${q(database, "id")} AND revision.${q(database, "revision")} = (SELECT MAX(candidate.${q(database, "revision")}) FROM ${q(database, "document_revisions")} candidate WHERE candidate.${q(database, "tenant_id")} = document.${q(database, "tenant_id")} AND candidate.${q(database, "knowledge_space_id")} = document.${q(database, "knowledge_space_id")} AND candidate.${q(database, "document_id")} = document.${q(database, "id")}) JOIN ${q(database, "document_assets")} asset ON asset.${q(database, "knowledge_space_id")} = revision.${q(database, "knowledge_space_id")} AND asset.${q(database, "id")} = revision.${q(database, "document_asset_id")} AND asset.${q(database, "version")} = revision.${q(database, "document_asset_version")} LEFT JOIN ${q(database, "document_compilation_attempts")} attempt ON attempt.${q(database, "tenant_id")} = revision.${q(database, "tenant_id")} AND attempt.${q(database, "knowledge_space_id")} = revision.${q(database, "knowledge_space_id")} AND attempt.${q(database, "id")} = (SELECT latest.${q(database, "id")} FROM ${q(database, "document_compilation_attempts")} latest WHERE latest.${q(database, "tenant_id")} = revision.${q(database, "tenant_id")} AND latest.${q(database, "knowledge_space_id")} = revision.${q(database, "knowledge_space_id")} AND latest.${q(database, "document_asset_id")} = revision.${q(database, "document_asset_id")} AND latest.${q(database, "document_version")} = revision.${q(database, "document_asset_version")} AND ${documentCompilationRevisionBindingSql(database, "latest", "revision")} ORDER BY latest.${q(database, "created_at")} DESC, latest.${q(database, "id")} DESC LIMIT 1) WHERE document.${q(database, "tenant_id")} = ${p(database, 1)} AND document.${q(database, "knowledge_space_id")} = ${p(database, 2)} AND document.${q(database, "status")} <> 'deleting' AND document.${q(database, "deletion_job_id")} IS NULL AND ${readableDocumentAssetPredicateSql(database, "asset", "attention_parent_source")} AND (attempt.${q(database, "run_state")} = 'failed' OR (attempt.${q(database, "id")} IS NULL AND document.${q(database, "status")} = 'failed')) AND ${assetPermissionSql(database, "asset", p(database, 3))}${resourceFilter} ORDER BY COALESCE(attempt.${q(database, "updated_at")}, document.${q(database, "updated_at")}) DESC, document.${q(database, "id")} ASC LIMIT ${p(database, params.length)};`,
    tableName: "logical_documents",
  });
  return result.rows.map((row) => {
    const id = stringColumn(row, "id");
