...
| RRV2.1-RRV2.3 | P0 | Done 2026-08-06 | Published PageIndex + profile models | Document shortlist, root-to-leaf layered LLM navigation, Value propagation, node queue, and bounded evidence opening. | Research selects documents before trees, inspects one visible sibling frontier per step, and safely falls back when navigation is degraded or quality-gated. | Multi-document, layered-depth/frontier/checkpoint, compatibility, queue, and selected-range tests pass. |
| RRV2.4-RRV2.6 | P0 | Done 2026-08-05 | RRV2.1-RRV2.3 + durable tasks | Interactive/durable policies, degraded/partial semantics, replay-safe checkpoints, and policy-derived planning/runtime budgets. | Interactive work is strictly bounded; durable tasks resume from immutable evidence; recoverable lane failures degrade while integrity failures remain terminal. | Policy, lifecycle, failure taxonomy, checkpoint, and planner/runtime budget tests pass. |
| RRV2.7 | P1 | Done 2026-08-06 | Stable RRV2 engine + existing manual Golden Questions | Exact-generation human-golden layered findability evaluation, durable routing, and bounded summary repair. No automatic question generation. | Sufficiently labelled trees receive reproducible findability metrics; absent labels are `not-evaluated` and never block publication. | Publication, persistence, routing, repair-runtime, manual-golden, and no-auto-generation tests pass. |
| RRV2.8 | P0 | Done 2026-08-06 | RRV2.1-RRV2.7 | Contract, regression, migration, coverage, and operational closure for book-like layered Research retrieval. | Backend-only iteration artifacts are reproducible and all required gates pass without automatic Golden Question generation. | Workspace tests/typecheck/lint/build, 90% API branch coverage, migration/OpenAPI/evaluation gates, and Dify contract lock pass. |
