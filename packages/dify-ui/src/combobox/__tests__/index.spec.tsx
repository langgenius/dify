...
describe('Combobox wrappers', () => {
  describe('Select-like trigger', () => {
    ...
    it('should apply small and large trigger size variants', async () => {
      ...
      await expect
        .element(smallScreen.getByRole('combobox', { name: 'Small resource type' }))
        .toHaveClass('rounded-md')
      await expect
        .element(smallScreen.getByRole('combobox', { name: 'Small resource type' }))
        .toHaveClass('system-xs-regular')
      ...
      await expect
        .element(largeScreen.getByRole('combobox', { name: 'Large resource type' }))
    ...
    it('should render default trigger icon and support hiding it', async () => {
      ...
      expect(
        withIcon.getByTestId('trigger').element().querySelector('.i-ri-arrow-down-s-line'),
      ...
      expect(
        withoutIcon
          .getByRole('combobox', { name: 'Resource type without icon' })
          .element()
  ...
  describe('Input group and controls', () => {
    ...
    it('should provide fallback aria labels and decorative icons for input controls', async () => {
      ...
      expect(
        screen
          .getByRole('button', { name: 'Clear combobox' })
          .element()
          .querySelector('.i-ri-close-line'),
      ).toHaveAttribute('aria-hidden', 'true')
      expect(
        screen
          .getByRole('button', { name: 'Open combobox options' })
          .element()
          .querySelector('.i-ri-arrow-down-s-line'),
      ).toHaveAttribute('aria-hidden', 'true')
    })
  ...
  describe('Content and options', () => {
    it('should use default overlay placement and Dify popup classes', async () => {
      ...
      await expect
        ...
        .toHaveClass('z-50')
      await expect
        .element(screen.getByRole('dialog', { name: 'combobox popup' }))
        .toHaveClass('rounded-xl')
      await expect
        .element(screen.getByRole('dialog', { name: 'combobox popup' }))
        .toHaveClass('w-(--anchor-width)')
      await expect
        .element(screen.getByRole('listbox', { name: 'combobox list' }))
    ...
    it('should render item text indicator status and empty wrappers with design classes', async () => {
      ...
      expect(screen.getByRole('option', { name: 'Workflow' }).element().className).not.toContain(
        'mx-1',
      )
      await expect
        .element(screen.getByTestId('list').getByText('Workflow'))
        .toHaveClass('system-sm-medium')
      ...
      expect(
        screen
          ...
          .getByText('Workflow')
          .element()
          .parentElement?.querySelector('.i-ri-check-line'),
      ).toHaveAttribute('aria-hidden', 'true')
    })
  ...
  describe('Multiple selection chips', () => {
    it('should render chip wrappers and default remove button label', async () => {
      ...
      await expect
        ...
        .toHaveAttribute('type', 'button')
      expect(
        screen.getByTestId('remove-chip').element().querySelector('.i-ri-close-line'),
      ).toHaveAttribute('aria-hidden', 'true')
    })
