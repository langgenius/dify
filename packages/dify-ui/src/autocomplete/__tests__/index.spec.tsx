...
describe('Autocomplete wrappers', () => {
  ...
  describe('Controls', () => {
    it('should provide fallback aria labels and decorative icons when labels are omitted', async () => {
      ...
      expect(
        screen
          .getByRole('button', { name: 'Clear autocomplete' })
          .element()
          .querySelector('.i-ri-close-line'),
      ).toHaveAttribute('aria-hidden', 'true')
      expect(
        screen
          .getByRole('button', { name: 'Open autocomplete suggestions' })
          .element()
  ...
  describe('Content and options', () => {
    it('should use default overlay placement and Dify popup classes', async () => {
      ...
      await expect
        ...
        .toHaveClass('z-50')
      await expect
        .element(screen.getByRole('dialog', { name: 'autocomplete popup' }))
        .toHaveClass('rounded-xl')
      await expect
        .element(screen.getByRole('dialog', { name: 'autocomplete popup' }))
        .toHaveClass('w-(--anchor-width)')
      await expect
        .element(screen.getByRole('listbox', { name: 'autocomplete list' }))
        .toHaveClass('scroll-py-1')
    })
    ...
    it('should render item text indicator status and empty wrappers with design classes', async () => {
      ...
      expect(
        screen
          .getByText('Workflow')
          .element()
          .parentElement?.querySelector('.i-ri-arrow-right-line'),
      ).toHaveAttribute('aria-hidden', 'true')
    })
