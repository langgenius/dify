...
function fileTreeRowClassName({ className }: { className?: string }) {
  return cn(
    'group/file-tree-row relative flex h-6 w-full min-w-0 cursor-pointer items-center rounded-md pr-1.5 pl-2 text-left outline-hidden select-none',
        ...
        {children ??
          (type === 'folder' ? (
            <React.Fragment>
              ...
              <span className="i-ri-folder-open-line hidden size-4 text-text-accent group-data-panel-open/file-tree-row:block" />
            </React.Fragment>
          ) : (
