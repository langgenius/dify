...
| Trigger-based `Select` fields                                                 | `FieldRoot name="..."` + `FieldLabel` + `Select` + `SelectTrigger`, or `Select` + `SelectLabel` + `SelectTrigger` when the label belongs inside the select; use `SelectGroupLabel` only for option groups inside `SelectContent` |
