...
from benchmarks.staging_public_schemas import STAGING_PUBLIC_SCENARIO_SEQUENCE, StagingPublicRunSample
...
def test_public_sample_success_requires_scenario_specific_evidence() -> None:
    ...
    file_sample = StagingPublicRunSample(
        scenario_id="file",
        benchmark_run_id="smoke.file",
        admitted=True,
        http_status_code=200,
        response_headers_ms=1,
        time_to_first_sse_ms=2,
        ...
        file_payload_sha256="341aacac661ccb210720bedaa9ead5d668fe5ea41a73532fc147c71e34040df1",
    ...
    assert not file_sample.model_copy(update={"file_payload_bytes": 16 * 1024 * 1024 - 1}).succeeded
    assert not file_sample.model_copy(update={"file_payload_sha256": "b" * 64}).succeeded
    assert not file_sample.model_copy(update={"file_integrity_valid": False}).succeeded
...
def test_public_smoke_sequence_remains_basic_shell_config() -> None:
    assert STAGING_PUBLIC_SCENARIO_SEQUENCE == ("basic", "shell", "config")
