...
class APIWorkflowRunRepository(WorkflowExecutionRepository, Protocol):
    ...
    def count_expired_runs_by_workflow_ids(
        ...
        Counts workflow runs created before the specified date for the given
        workflow IDs. Used by scheduled cleanup tasks to determine the scope
        of cleanup operations.

        Args:
            workflow_ids: List of workflow IDs to filter by
            before_date: Only count runs created before this date

        Returns:
            Total count of expired workflow runs
        """
        ...
    ...
    def get_expired_run_ids_by_workflow_ids(
        ...
        Retrieves workflow run IDs created before the specified date for the
        given workflow IDs. Used by scheduled cleanup tasks to process
        cleanup in batches.
        ...
        Returns:
            Sequence of workflow run IDs to be processed for cleanup
        """
        ...
