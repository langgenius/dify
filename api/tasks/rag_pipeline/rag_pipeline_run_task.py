...
def rag_pipeline_run_task(
    ...
    if dispatch_token is not None:
        ...
        if dispatch_outcome == TenantTaskDispatchClaimOutcome.DONE:
            logger.info("Skipping completed RAG pipeline dispatch %s", dispatch_token)
            return {"status": "already_completed", "dispatch_token": dispatch_token}
        if dispatch_outcome == TenantTaskDispatchClaimOutcome.BUSY:
            logger.info("RAG pipeline dispatch %s is owned by another worker; retrying", dispatch_token)
...
def run_single_rag_pipeline_task(
    ...
    *,
    ...
    queue_kind: RagPipelineQueueKind = RagPipelineQueueKind.REGULAR,
        ...
        try:
            ...
            with sessionmaker(db.engine, expire_on_commit=False).begin() as session:
                ...
                response = pipeline_generator._generate(  # type: ignore[attr-defined]
                    ...
                    # Celery batch workers have no response consumer. Blocking
                    # mode drains the queue, activates maintenance handoffs and
                    # joins the graph worker before this future can complete.
                    streaming=False,
...
def _release_legacy_tenant_slot(tenant_id: str, *, source_batch_id: str) -> None:
    ...
    has_next, next_file_id = tenant_isolated_task_queue.claim_task_once(
        claim_key=f"rag_pipeline_batch_release_claim:{source_batch_id}",
        ttl=7 * 24 * 60 * 60,
    )
    logger.info("rag pipeline tenant isolation queue %s next file: %s", tenant_id, next_file_id)
    if not has_next:
        redis_client.setex(released_key, 7 * 24 * 60 * 60, 1)
        return
    file_id = next_file_id.decode("utf-8") if isinstance(next_file_id, (bytes, bytearray)) else next_file_id
