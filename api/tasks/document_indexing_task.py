...
from core.rag.index_processor.constant.index_type import (
    IndexStructureType,
    IndexTechniqueType,
)
...
def _document_indexing(dataset_id: str, document_ids: Sequence[str]):
    ...
    with session_factory.create_session() as session, session.begin():
        ...
        for document in documents:
            if not document:
                logger.warning(
                    "Document not found before indexing started in dataset %s",
                    dataset_id,
                )
                continue
        ...
        for doc_id in document_ids:
            if doc_id not in found_ids:
                logger.warning(
                    "Document %s was deleted before indexing started in dataset %s",
                    doc_id,
                    dataset_id,
                )
            ...
            if dataset.indexing_technique == IndexTechniqueType.HIGH_QUALITY:
                ...
                if summary_index_setting and summary_index_setting.get("enable"):
                    ...
                    for document in documents:
                        if not document:
                            logger.warning(
                                "Document not found during summary-index scan in dataset %s",
                                dataset_id,
                            )
                            continue
                        logger.info(
...
def normal_document_indexing_task(tenant_id: str, dataset_id: str, document_ids: Sequence[str]):
    ...
    """
    logger.info(
        "normal document indexing task received: %s - %s - %s",
        tenant_id,
        dataset_id,
        document_ids,
    )
...
def priority_document_indexing_task(tenant_id: str, dataset_id: str, document_ids: Sequence[str]):
    ...
    """
    logger.info(
        "priority document indexing task received: %s - %s - %s",
        tenant_id,
        dataset_id,
        document_ids,
    )
