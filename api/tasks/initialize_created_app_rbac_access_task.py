...
from models.agent import Agent, AgentScope, AgentStatus
...
def _iter_resource_config_batches(
    ...
    while True:
        ...
        agent_ids = [str(agent_id) for agent_id in db.session().scalars(stmt).all()]
        if not agent_ids:
            break
        yield [
            enterprise_rbac_service.ResourceWhitelistConfigResource(
...
def initialize_created_app_rbac_access_task(
    ...
    try:
        ...
        logger.exception(
            "Failed to initialize app RBAC access; retrying: "
            "tenant_id=%s app_id=%s dataset_id=%s agent_id=%s attempt=%s",
            tenant_id,
            app_id,
            dataset_id,
            agent_id,
    ...
    try:
        ...
        for resources in _iter_resource_config_batches(tenant_id, APP_RBAC_RESOURCE_CONFIG_BATCH_SIZE):
            ...
            for config in configs.data:
                ...
                elif config.resource_type == enterprise_rbac_service.RBACResourceType.AGENT:
                    agent_ids.append(config.resource_id)
        ...
        for agent_id_batch in _chunks(agent_ids, APP_RBAC_MEMBER_APPEND_BATCH_SIZE):
            enterprise_rbac_service.RBACService.AgentAccess.append_whitelist_members_batch(
                tenant_id=tenant_id,
                account_id=actor_account_id,
                data=[
                    enterprise_rbac_service.AppendAgentWhitelistMembersBatchItem(
                        agent_id=agent_id,
                        account_ids=[member_account_id],
                        policy_id=APP_RBAC_DEFAULT_ACCESS_POLICY_ID,
                    )
                    for agent_id in agent_id_batch
                ],
            )
