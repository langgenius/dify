...
def clear_conversations_task(
    ...
    try:
        logger.info(
            "Starting conversation cleanup for app_id=%s, "
            "mode=%s, selective=%s, count=%s",
            app_id,
            conversation_mode,
        ...
        with Session() as session:
            ...
            if user_type == 'account' and user_id:
                conversation_query = conversation_query.filter(
                    ...
                    Conversation.from_account_id == user_id
                )
            ...
            while True:
                ...
                message_ids = [
                    row[0] for row in session.query(Message.id)
                    .filter(Message.conversation_id.in_(conversation_batch_ids))
                    .all()
                ]
                ...
                if message_ids:
                    ...
                    # Delete messages
                    total_deleted['messages'] += _delete_records_batch(
                        session, Message, Message.id.in_(message_ids)
                    )
                ...
                self.update_state(
                    state='PROGRESS',
                    meta={
                        'processed_batches': batch_offset // BATCH_SIZE,
                        'current_totals': total_deleted.copy()
                    }
                )
                ...
                logger.info(
                    "Processed batch %s, deleted %s conversations", 
                    batch_offset // BATCH_SIZE, len(conversations)
                )
            
        execution_time = time.time() - start_time
        logger.info(
            "Conversation cleanup completed in %.2fs. Deleted: %s",
            execution_time,
            total_deleted
        )
...
def _cleanup_message_files(session, message_ids: list[str]) -> list[str]:
    ...
    try:
        ...
        message_files = session.query(MessageFile).filter(
        ...
        if upload_file_ids:
            ...
            upload_files = session.query(UploadFile).filter(
            ...
            session.query(UploadFile).filter(
                UploadFile.id.in_(upload_file_ids)
            ).delete(synchronize_session=False)
            
        # Delete message file records
        session.query(MessageFile).filter(
            MessageFile.message_id.in_(message_ids)
        ).delete(synchronize_session=False)
...
def _delete_records_batch(session, model_class, filter_condition) -> int:
    ...
    try:
        result = session.query(model_class).filter(filter_condition).delete(synchronize_session=False)
        return result or 0
    except Exception as e:
