...
def upgrade():
    ...
    tool_providers = (
        op.get_bind().execute(sa.text("SELECT * FROM tool_providers WHERE tool_name = 'serpapi'")).fetchall()
    )
    ...
    for tool_provider in tool_providers:
        ...
        user_id = (
            op.get_bind()
            .execute(
                sa.text("SELECT account_id FROM tenant_account_joins WHERE tenant_id = :tenant_id AND role = 'owner'"),
                tenant_id=tenant_id,
            )
            .fetchone()["account_id"]
        )
        ...
        exists = (
            op.get_bind()
            .execute(
                sa.text("SELECT * FROM tool_builtin_providers WHERE tenant_id = :tenant_id AND provider = 'google'"),
                tenant_id=tenant_id,
            )
            .fetchone()
        )
