...
def upgrade():
    ...
    with op.batch_alter_table("tenant_default_models", schema=None) as batch_op:
        batch_op.alter_column(
            "model_name", existing_type=sa.VARCHAR(length=40), type_=sa.String(length=255), existing_nullable=False
        )
...
def downgrade():
    ...
    with op.batch_alter_table("tenant_default_models", schema=None) as batch_op:
        batch_op.alter_column(
            "model_name", existing_type=sa.String(length=255), type_=sa.VARCHAR(length=40), existing_nullable=False
        )
