...
For renewable credentials, implement `Config.runtime_settings_for_identity(runtime_settings)` to fingerprint their captured authentication source instead of an issued token. Keep destination settings and static credentials bound, and derive the source from the same owned resolution that produced the attempt's credentials. The default fingerprints every runtime value.
