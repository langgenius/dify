...
This directory holds **optional workspace packages** that plug into Dify’s API core. Today this means **vector database (VDB) backends** under `vdb/`: each is a small Python distribution with its own third-party dependencies and a standard entry point so the core can discover it at runtime.
