...
def rename_app_from_proposal(*, app_id: str, tenant_id: str, account_id: str, proposed: str) -> None:
    """Store ``proposed`` as the app's name, if it is usable. Never raises.

    Opens its own session: this runs in the Celery advance task, outside any
    Flask request.
    """
    ...
    try:
        ...
        with factory() as session:
            ...
            if app is None or app.name == name:
                return
            AppService().rename_app(app, name, account_id=account_id, session=session)
    except Exception:
        # Nothing downstream reads the name, so there is nothing to recover --
        # only a clumsier title to look at.
