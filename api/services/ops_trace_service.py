"""Tenant-checked OPS configuration and application entry points."""
...
from core.ops.provider_config import TracingProviderEnum, decrypt_provider_config, get_provider_config_fields
                ...
                if config is not None and config.tracing_config is not None:
                    destinations.append(
                        TraceProviderSettings(
                            ...
                            destination_settings_hash=_settings_hash(config.tracing_config),
                        )
                    )
    enterprise_config = _enterprise_config()
    if enterprise_config is not None:
        settings_hash = _settings_hash(enterprise_config)
        destinations.append(
            TraceProviderSettings(
                tenant_id=tenant_id,
                app_id=app_id,
...
def load_trace_provider_config(settings: TraceProviderSettings) -> dict[str, Any]:
    ...
    if settings.destination_type == "enterprise":
        ...
        with Session(db.engine) as session:
            ...
            if (
                ...
                raise ValueError("configuration_changed")
        enterprise_config = _enterprise_config()
        if enterprise_config is None or _settings_hash(enterprise_config) != settings.destination_settings_hash:
            raise ValueError("configuration_changed")
    ...
    with Session(db.engine) as session:
        ...
        if _settings_hash(encrypted) != settings.destination_settings_hash:
            raise ValueError("configuration_changed")
