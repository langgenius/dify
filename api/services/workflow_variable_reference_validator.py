...
def validate_variable_references(graph: Mapping[str, Any]) -> list[VariableReferenceIssue]:
    """Find variable references that read a node which may not run on the path reaching them.

    Such a reference (e.g. ``{{#1840.text#}}`` or a value selector) crashes the run
    mid-stream when the producer sat on a conditional branch that was not taken, leaving
    the chat stuck on "caching variables" (issue #34358); the graph still imports and
    saves because the existing structural validation never inspects data dependencies.
...
def format_variable_reference_errors(issues: Sequence[VariableReferenceIssue]) -> str:
    """Render a concise, toast-friendly error message for ``issues``.

    The reason is stated once and each offending pair is shown compactly as
    ``"reader" ← "producer"`` so the message stays short even with several issues.
    Toasts collapse newlines, so this returns a single paragraph rather than a list.
    """
    count = len(issues)
    ...
    return (
        f"{count} variable reference{'s' if count != 1 else ''} read from a node that "
        'may be skipped, so the run fails and the chat hangs on "caching variables". '
        f"Merge the branches with a Variable Aggregator or set a default value — {pairs}."
    )
...
def _nodes_runnable_without(
    ...
    *,
    ...
    """Nodes that can execute in some run where ``producer`` does not.
