...
class RBACResourceService:
    @staticmethod
    def get_app_agent_binding(session: Session | scoped_session, tenant_id: str, app_id: str) -> Agent | None:
        app_model = session.scalar(select(App).where(App.id == app_id, App.tenant_id == tenant_id))
        if app_model is None:
            return None
        return app_model.agent_app_binding_with_session(session=session, include_archived=True)

    @staticmethod
    def get_app_maintainer(session: Session | scoped_session, tenant_id: str, app_id: str) -> str | None:
        return session.scalar(
            select(App.maintainer).where(App.id == app_id, App.tenant_id == tenant_id, App.status == "normal")
        )

    @staticmethod
    def get_dataset_maintainer(session: Session | scoped_session, tenant_id: str, dataset_id: str) -> str | None:
        return session.scalar(
