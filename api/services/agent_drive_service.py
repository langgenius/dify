...
class AgentDriveService:
    ...
    def inspect_skill(self, *, tenant_id: str, agent_id: str, skill_path: str) -> AgentDriveSkillInspectInfo:
        ...
        Skill packages are normalized when committed to the drive, so this endpoint
        intentionally returns the standardized view: canonical ``SKILL.md`` as the
        source of truth plus the original package member list recorded in Agent
        Soul when available. It is not a raw archive echo endpoint.
        """
