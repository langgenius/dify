"""Pure JSON (de)serialization for the domain ``CopilotContext``.

``CopilotContext`` (see ``core.workflow_copilot.models``) is the per-session
working state persisted, one row per version, as the ``context`` JSONB
column on ``workflow_copilot_session_commits``. This module has no I/O and
...
def context_to_dict(fc: CopilotContext) -> dict[str, Any]:
    """Serialize a ``CopilotContext`` to a JSON-safe dict.
...
def context_from_dict(d: dict[str, Any]) -> CopilotContext:
    """Rebuild a ``CopilotContext`` from a dict produced by ``context_to_dict``.
