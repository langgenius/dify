"""Pure mapping from a Dify workflow-run result into the copilot domain.

``AppGenerateService.generate(..., streaming=False)`` returns a blocking
response dict; its ``["data"]`` sub-mapping plus the run's per-node
execution rows are all these functions need to build a copilot ``Run`` /
...
finish counts as a copilot-domain "succeeded" run. ``FAILED``,
