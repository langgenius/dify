...
class ModelProviderService:
    ...
    def get_provider_available_credentials(
        self, tenant_id: str, provider: str, user: "Account | None" = None, *, credential_query: CredentialQuery
    ) -> list[CredentialConfiguration]:
        return [
            CredentialConfiguration(credential_id=record.id, credential_name=record.name)
            for record in credential_query.list_models(
                workspace_id=tenant_id, provider=provider, actor_id=user.id if user else None
            )
        ]
