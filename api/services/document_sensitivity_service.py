...
class DocumentSensitivityService:
    ...
    def check_document_sensitivity(
        ...
        try:
            ...
            logger.warning(
                f"Failed to extract metadata from file {e}"
            )
            return None
            ...
            try:
                if hasattr(doc, 'custom_doc_props'):
                    for prop in doc.custom_doc_props.props:
                        ...
                        prop_value = (
                            str(prop.value).lower() if prop.value else ""
                        )
    ...
    def _extract_text_metadata(cls, file_path: str, extension: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                f"Failed to extract text metadata from {extension}: {e}"
            )
        return metadata
                ...
                if end_idx != -1:
                    ...
                    for line in frontmatter.split('\n'):
                        ...
                        if ':' in line and not line.startswith('#'):
                            ...
                            if key in ['category', 'classification', 'sensitivity', 
        ...
        try:
            ...
            if lines:
                ...
                sensitive_columns = [
                    ...
                    'confidential', 'sensitive', 'classified', 'restricted'
                ]
            ...
            for i, line in enumerate(lines[:10]):  # Check first 10 lines
                ...
                if ':' in line:
                    ...
                    if len(parts) == 2:
                        key = parts[0].strip()
                        value = parts[1].strip()
                        if key in ['classification', 'sensitivity', 'category',
                                 'confidential', 'restricted', 'secret', 'level']:
                            metadata[key] = value
        except Exception:
            pass
        return metadata
    ...
    def _check_metadata_sensitivity(
        ...
        for field in [
            'sensitivity', 'classification', 'mip_label', 'category'
        ]:
        ...
        for key, value in metadata.items():
            ...
            if ("msip_label" in key or "sensitivity" in key or 
                    "classification" in key or "confidential" in key):
                for blocked in blocked_levels:
