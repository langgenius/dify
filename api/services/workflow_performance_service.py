...
This service provides comprehensive performance tracking and analysis for workflows,
including execution metrics, node profiling, and optimization recommendations.
"""
...
class WorkflowPerformanceService:
    ...
    def record_workflow_execution(
        ...
        Args:
            ...
            cached_nodes: Number of nodes that used cached results
            ...
            execution_status: succeeded, failed, or partial
        ...
        logger.info(
            "Recorded workflow execution metrics: workflow_id=%s, run_id=%s, time=%.2fs, status=%s",
            workflow_id,
            workflow_run_id,
    ...
    def record_node_execution(
        ...
        logger.debug(
            "Recorded node execution: node_id=%s, type=%s, time=%.2fs, cached=%s",
            node_id,
            node_type,
            execution_time,
            is_cached,
    ...
    def create_optimization_recommendation(
        ...
        Args:
            ...
            estimated_improvement: Expected improvement (e.g., "30% faster")
            affected_nodes: List of affected node IDs
            recommendation_steps: Step-by-step guidance
            code_example: Example code implementation
            documentation_link: Link to relevant documentation
            supporting_metrics: Metrics supporting the recommendation
            sample_workflow_runs: Sample run IDs demonstrating the issue
            extra_info: Additional metadata
        ...
        recommendation = WorkflowOptimizationRecommendation(
            ...
            recommendation_steps=recommendation_steps or [],
        ...
        logger.info(
            "Created optimization recommendation: workflow_id=%s, category=%s, severity=%s, title=%s",
            workflow_id,
            category,
            severity,
            title,
    ...
    def dismiss_recommendation(
        ...
        Args:
            ...
            dismissed_by: User ID who dismissed it
        ...
        if not recommendation:
            logger.warning("Recommendation not found: %s", recommendation_id)
            return None
        ...
        logger.info("Dismissed recommendation: id=%s, by=%s", recommendation_id, dismissed_by)
    ...
    def get_active_recommendations(
        ...
        """
        stmt = select(WorkflowOptimizationRecommendation).where(
            and_(
        ...
        if category:
            stmt = stmt.where(WorkflowOptimizationRecommendation.category == category)

        if severity:
            stmt = stmt.where(WorkflowOptimizationRecommendation.severity == severity)
