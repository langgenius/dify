...
class WebsiteService:
    ...
    def __init__(self, *, providers: DatasourceProviderService) -> None:
        self._providers = providers

    def _get_credentials_and_config(self, tenant_id: str, provider: str) -> tuple[Any, Any]:
        ...
        credential = self._providers.get_datasource_credentials(
            tenant_id=tenant_id,
            provider=provider,
            plugin_id=plugin_id,
        )
    ...
    def crawl_url(self, api_request: WebsiteCrawlApiRequest) -> dict[str, Any]:
        """Crawl a URL using the specified provider with typed request."""
        request = api_request.to_crawl_request()

        api_key, config = self._get_credentials_and_config(current_user.current_tenant_id, request.provider)

        if request.provider == "firecrawl":
            return self._crawl_with_firecrawl(request=request, api_key=api_key, config=config)
        elif request.provider == "watercrawl":
            return self._crawl_with_watercrawl(request=request, api_key=api_key, config=config)
        elif request.provider == "jinareader":
            return self._crawl_with_jinareader(request=request, api_key=api_key)
        else:
            raise ValueError("Invalid provider")
    ...
    def get_crawl_status(self, job_id: str, provider: str) -> CrawlStatusDict:
        """Get crawl status using string parameters."""
        api_request = WebsiteCrawlStatusApiRequest(provider=provider, job_id=job_id)
        return self.get_crawl_status_typed(api_request)

    def get_crawl_status_typed(self, api_request: WebsiteCrawlStatusApiRequest) -> CrawlStatusDict:
        """Get crawl status using typed request."""
        api_key, config = self._get_credentials_and_config(current_user.current_tenant_id, api_request.provider)

        if api_request.provider == "firecrawl":
            return self._get_firecrawl_status(api_request.job_id, api_key, config)
        elif api_request.provider == "watercrawl":
            return self._get_watercrawl_status(api_request.job_id, api_key, config)
        elif api_request.provider == "jinareader":
            return self._get_jinareader_status(api_request.job_id, api_key)
        else:
            raise ValueError("Invalid provider")
    ...
    def get_crawl_url_data(self, job_id: str, provider: str, url: str, tenant_id: str) -> dict[str, Any] | None:
        api_key, config = self._get_credentials_and_config(tenant_id, provider)

        if provider == "firecrawl":
            return self._get_firecrawl_url_data(job_id, url, api_key, config)
        elif provider == "watercrawl":
            return self._get_watercrawl_url_data(job_id, url, api_key, config)
        elif provider == "jinareader":
            return self._get_jinareader_url_data(job_id, url, api_key)
        else:
            raise ValueError("Invalid provider")
    ...
    def get_scrape_url_data(self, provider: str, url: str, tenant_id: str, only_main_content: bool) -> dict[str, Any]:
        request = ScrapeRequest(provider=provider, url=url, tenant_id=tenant_id, only_main_content=only_main_content)

        api_key, config = self._get_credentials_and_config(tenant_id=request.tenant_id, provider=request.provider)

        if request.provider == "firecrawl":
            return self._scrape_with_firecrawl(request=request, api_key=api_key, config=config)
        elif request.provider == "watercrawl":
            return self._scrape_with_watercrawl(request=request, api_key=api_key, config=config)
        else:
            raise ValueError("Invalid provider")
