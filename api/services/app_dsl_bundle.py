...
class AppDslBundleService:
        ...
        try:
            with zipfile.ZipFile(io.BytesIO(content)) as archive:
                ...
                for marker, app in apps.items():
                    if app.file != f"apps/{marker}.yaml" or app.file not in names:
    ...
    def import_bundle(
        ...
        *,
        ...
        self._session.flush()
        drafts = []
        for imported_app in apps.values():
            if imported_app.mode in (AppMode.WORKFLOW, AppMode.ADVANCED_CHAT):
                draft = WorkflowService().get_draft_workflow(imported_app, session=self._session)
                if draft is not None:
                    drafts.append((imported_app, draft))
        ...
        def publish_events(_session: Session) -> None:
            ...
            for imported_app, draft_workflow in drafts:
                app_draft_workflow_was_synced.send(imported_app, synced_draft_workflow=draft_workflow)
