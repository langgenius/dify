...
def get_thread_messages_length_with_session(conversation_id: str, session: Session) -> int:
    """
    Get the number of thread messages based on the parent message id, using a provided session.

    This variant is used when calling from outside Flask application context (e.g., in a lazy resolver).
    """
    # Fetch all messages related to the conversation
    stmt = select(Message).where(Message.conversation_id == conversation_id).order_by(Message.created_at.desc())
