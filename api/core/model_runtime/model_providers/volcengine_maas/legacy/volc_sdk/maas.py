...
class MaasService(Service):
    ...
    def _call(self, endpoint_id, api, req_id, params, body, apikey=None, stream=False):
        ...
        if res.status_code != 200:
            ...
            if resp.error:
                raise MaasExceptionError(resp.error.code_n, resp.error.code, resp.error.message, req_id)
            else:
...
class MaasExceptionError(Exception):
...
def new_client_sdk_request_error(raw, req_id=""):
    return MaasExceptionError(1709701, "ClientSDKRequestError", "MaaS SDK request error: {}".format(raw), req_id)
...
class BinaryResponseContent:
    ...
    def stream_to_file(self, file: str) -> None:
        ...
        if len(error_bytes) > 0:
            ...
            raise MaasExceptionError(resp.error.code_n, resp.error.code, resp.error.message, self.request_id)
