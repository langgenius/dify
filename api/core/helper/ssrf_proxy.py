    ...
    while retries <= max_retries:
        try:
            ...
            if response.status_code in (401, 403):
                # Squid adds its Via header to ALL proxied responses, including
                # forwarded upstream 401/403s — so Via alone is not evidence of
                # an SSRF block. Only Server: squid/x.y identifies a response
                # generated by Squid itself (an ACL denial), as opposed to a
                # response merely forwarded through it. See #41434.
