...
type WorkflowToolEventListenerFactory = Callable[[WorkflowToolSource, str, str], Callable[[NodeEvent], None]]
...
class WorkflowToolContainerHandler:
    ...
    def __init__(
        ...
        frame_registry: FrameRegistry,
        *,
        sources: Mapping[str, WorkflowToolSource],
        hidden_event_listener: Callable[[NodeEvent], None] | None = None,
    ...
    def _create_frame(
        ...
        *,
        ...
        source = self._sources.get(payload.source_workflow_id)
        if source is None or source.app_id != payload.source_app_id:
        ...
        node_factory = DifyNodeFactory.from_graph_init_context(
            ...
            workflow_tools=parent_factory.workflow_tools if isinstance(parent_factory, DifyNodeFactory) else None,
        )
        graph = Graph.init(
            graph_config=graph_config,
        ...
        if self._event_listener_factory is not None:
            ...
            listener = self._event_listener_factory(source, parent_node.workflow_id, parent_execution_id)
