...
class HumanInputFormEventFilter:
    ...
    def __init__(self, *, form_repository: HumanInputFormSubmissionRepository) -> None:
        ...
        self._pending_traversals: list[GraphEdgeTakenEvent | GraphEdgeSkippedEvent] = []
    ...
    def initialize(self, context: GraphEventFilterContext) -> None:
        ...
        self._pending_traversals.clear()
    ...
    def on_event(self, event: GraphEngineEvent) -> Iterable[GraphEngineEvent]:
        if isinstance(event, GraphEdgeTakenEvent | GraphEdgeSkippedEvent):
            # Graphon emits traversal events immediately before the node result.
            # ResponseStreamFilter can turn them into Answer text, so hold this
            # batch until the callback's form notification can precede it.
            self._pending_traversals.append(event)
            return
    ...
    def flush(self) -> Iterable[GraphEngineEvent]:
        pending, self._pending_traversals = self._pending_traversals, []
        return pending
