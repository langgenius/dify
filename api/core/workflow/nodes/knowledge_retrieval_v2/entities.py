...
class KnowledgeRetrievalV2NodeData(BaseNodeData):
    ...
    def normalize_control_space_ids(cls, values: list[str]) -> list[str]:
        ...
        if any(not value for value in normalized):
            raise ValueError("Agentic Knowledge control-space ids must be non-empty")
    ...
    def normalize_query_variable_selector(cls, values: list[str] | None) -> list[str] | None:
        ...
        if not normalized:
            return None
        if len(normalized) < 2:
            raise ValueError("Agentic Knowledge query variable selector must contain at least two parts")
        if any(not value for value in normalized):
            raise ValueError("Agentic Knowledge query variable selector must be non-empty")
        return normalized
    ...
    def require_query_text_or_image(self) -> KnowledgeRetrievalV2NodeData:
        ...
        if not self.query_variable_selector and not self.query_attachment_selector:
            raise ValueError("Agentic Knowledge retrieval requires a query text or a query image variable")
        return self
    ...
    def normalize_query_attachment_selector(cls, values: list[str] | None) -> list[str] | None:
        ...
        if not normalized:
            return None
        if len(normalized) < 2:
            raise ValueError("Agentic Knowledge query attachment selector must contain at least two parts")
        if any(not value for value in normalized):
            raise ValueError("Agentic Knowledge query attachment selector must be non-empty")
        return normalized
