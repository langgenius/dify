...
def setup_tracer(arize_phoenix_config: ArizeConfig | PhoenixConfig) -> tuple[trace_sdk.Tracer, SimpleSpanProcessor]:
    ...
    try:
        ...
        if isinstance(arize_phoenix_config, ArizeConfig):
            ...
            import requests
            try:
                test_response = requests.get(f"{base_endpoint}/", timeout=5)
                logger.info("[Arize/Phoenix] Connectivity test successful, status: %s", test_response.status_code)
                timeout_setting = 30
            except Exception as connectivity_error:
                logger.warning("[Arize/Phoenix] Connectivity test failed: %s, using shorter timeout", str(connectivity_error))
                timeout_setting = 5  # Fail fast if connectivity is poor
...
class ArizePhoenixDataTrace(BaseTraceInstance):
    ...
    def workflow_trace(self, trace_info: WorkflowTraceInfo):
        ...
        if parent_trace_context:
            # Child workflow nests under parent tool in same trace - FOLLOW EXECUTION FLOW
            trace_id = parent_trace_context['trace_id']  # Use parent trace ID for single flow
            parent_span_id = parent_trace_context['parent_span_id']

            workflow_metadata["nested_workflow"] = "true"
            workflow_metadata["parent_trace_id"] = hex(parent_trace_context['trace_id'])
            workflow_metadata["parent_tool_span_id"] = hex(parent_trace_context['parent_span_id'])

            logger.info("[Arize/Phoenix] Child workflow nesting under parent tool in single execution flow")
        else:
            # Create new trace for root workflow - this becomes the main execution flow
            trace_id = string_to_trace_id128(trace_info.trace_id or trace_info.workflow_run_id)
            parent_span_id = None
            workflow_metadata["nested_workflow"] = "false"

        # Generate consistent workflow span ID from workflow_run_id
        span_id = string_to_span_id64(trace_info.workflow_run_id)
        context = SpanContext(
            trace_id=trace_id,
            span_id=span_id,
            is_remote=False,
        ...
        if parent_trace_context:
            ...
            parent_tool_name = parent_trace_context.get('workflow_tool_name', 'UnknownTool')
            if parent_tool_name and parent_tool_name not in ['workflow', 'unknown', 'UnknownTool', '']:
                ...
                workflow_span_name = "nested_{}_{}_{}".format(
                    app_name_clean,
                    parent_tool_clean,
                    workflow_id_short
                )
        ...
        logger.info("[Arize/Phoenix] Creating workflow span: {} ({}workflow)".format(
            workflow_span_name, "nested " if parent_trace_context else "main "))
        ...
        if parent_trace_context:
            # Child workflow nests under parent tool span - create proper parent context
            parent_span_context = SpanContext(
                trace_id=parent_trace_context['trace_id'],
                span_id=parent_trace_context['parent_span_id'],  # Parent TOOL span
                is_remote=False,
                trace_flags=TraceFlags(TraceFlags.SAMPLED),
                trace_state=TraceState(),
            )
            ...
            logger.info("[Arize/Phoenix] Child workflow will nest under parent tool span: {}".format(
                hex(parent_trace_context['parent_span_id'])))
        else:
            # Root workflow uses its own context
            workflow_context_parent = trace.set_span_in_context(trace.NonRecordingSpan(context))
        ...
        try:
            ...
            logger.info("[Arize/Phoenix] Processing {} workflow nodes".format(len(nodes)))
            ...
            if workflow_graph:
                hierarchy_map, node_mapping, execution_context = self._build_comprehensive_hierarchy(workflow_graph, nodes)
            else:
                hierarchy_map, node_mapping, execution_context = {}, {}, {}
            ...
            for i, node in enumerate(sorted_nodes[:10]):  # Log first 10
                logger.info("  {}: {} (type={}, status={}, index={})".format(
                    i+1, node.id[:8], node.node_type, getattr(node, 'status', 'unknown'),
                    getattr(node, 'index', 'none')))

            for node_execution in sorted_nodes:
                try:
                    logger.debug("[Arize/Phoenix] Processing node {} of type {}".format(node_execution.id, node_execution.node_type))
                    ...
                    try:
                        ...
                        process_data = {}
                        logger.warning("[Arize/Phoenix] Invalid process_data JSON for node {}".format(node_execution.id))
                    ...
                    if node_execution.node_type == 'tool':
                        ...
                        if child_workflow_id:
                            ...
                            if node_execution.inputs:
                                try:
                                    inputs = json.loads(node_execution.inputs) if isinstance(node_execution.inputs, str) else node_execution.inputs
                    ...
                    if node_execution.execution_metadata:
                        try:
                            ...
                            logger.warning("[Arize/Phoenix] Invalid execution_metadata JSON for node {}".format(node_execution.id))
                    ...
                    if node_execution.node_type == "llm":
                        ...
                        try:
                            ...
                            outputs_usage = {}
                            logger.warning("[Arize/Phoenix] Invalid outputs JSON for node {}".format(node_execution.id))
                    ...
                    # Create consistent node span ID from workflow_run_id + node_id
                    node_span_id = string_to_span_id64(f"{trace_info.workflow_run_id}_{node_execution.id}")
                    node_span_context = SpanContext(
                        trace_id=trace_id,
                    ...
                    logger.info("[Arize/Phoenix] Node debug - id: {}, node_id: {}, type: {}, index: {}".format(
                        node_execution.id, node_id, node_type, index_val))
                    ...
                    if node_execution.node_type == 'start':
                        ...
                        logger.info("[Arize/Phoenix] Node {} (start) is direct child of workflow".format(node_id))
                        ...
                        if last_non_end_span:
                            ...
                            logger.info("[Arize/Phoenix] Node {} (end) parented to last executed node".format(node_id))
                        else:
                            parent_context = workflow_context
                            logger.info("[Arize/Phoenix] Node {} (end) is direct child of workflow".format(node_id))
                        ...
                        logger.info("[Arize/Phoenix] Node {} ({}) is child of {} (from graph)".format(
                            node_id, node_type, parent_node_id))
                        ...
                        if logical_parent:
                            ...
                            logger.info("[Arize/Phoenix] Node {} ({}) found logical parent".format(node_id, node_type))
                        else:
                            parent_context = workflow_context
                            logger.info("[Arize/Phoenix] Node {} ({}) using workflow as parent (no logical parent)".format(
                                node_id, node_type))
                        ...
                        parent_context = workflow_context
                        if parent_node_id:
                            logger.debug("[Arize/Phoenix] Node {} ({}) parent {} not yet processed, using workflow".format(
                                node_id, node_type, parent_node_id))
                        else:
                            logger.debug("[Arize/Phoenix] Node {} ({}) is direct child of workflow (no parent in graph)".format(
                                node_id, node_type))
                    ...
                    logger.debug("[Arize/Phoenix] Node naming: type='{}', title='{}' -> span='{}'".format(
                        node_execution.node_type, getattr(node_execution, 'title', ''), base_name))
                    ...
                    if node_execution.node_type == 'question-classifier':
                        ...
                        if child_workflow_id:
                            ...
                            child_workflow_name = child_app_info.get("app_name", "UnknownWorkflow").replace(' ', '_').replace('-', '_')[:15]
                            ...
                            if tool_name and tool_name not in ['workflow', 'unknown', 'UnknownTool']:
                                ...
                                logger.info("[Arize/Phoenix] Tool (with name) triggers child workflow - naming: {} -> child: {}".format(
                                    span_name, child_workflow_id[:8]))
                            else:
                                ...
                                logger.info("[Arize/Phoenix] Tool (no name) triggers child workflow - naming: {} -> child: {}".format(
                                    span_name, child_workflow_id[:8]))
                        else:
                            # Regular tool
                        ...
                        if api_info and api_info.get('method') and api_info.get('url'):
                            ...
                            url_part = api_info['url'].split('/')[-1][:15] if '/' in api_info['url'] else api_info['url'][:15]
                    ...
                    logger.info("[Arize/Phoenix] Creating node span: {}".format(span_name))
                    ...
                    # For tool nodes that trigger child workflows, prepare for direct nesting
                    if node_execution.node_type == 'tool':
                        child_workflow_id = self._find_child_workflow_by_timing(node_execution)
                        if child_workflow_id:
                            # Mark tool as triggering a child workflow - child will nest under this tool
                            node_metadata["triggers_child_workflow"] = "true"
                            node_metadata["child_workflow_run_id"] = child_workflow_id

                            logger.info("[Arize/Phoenix] Tool {} will have child workflow {} nested under it".format(
                                node_execution.id[:8], child_workflow_id[:8]))
                    ...
                    try:
                        if node_execution.node_type == "llm":
                            ...
                            try:
                                ...
                                llm_outputs_usage = {}
                                logger.warning("[Arize/Phoenix] Invalid LLM outputs JSON for node {}".format(node_execution.id))

                            usage_data = (
                                process_data.get("usage", {}) if "usage" in process_data else llm_outputs_usage
                            )
                            if usage_data:
                    ...
                    logger.error(f"[Arize/Phoenix] Node data access error for {getattr(node_execution, 'id', 'unknown')}: {e}")
                    continue
                except (json.JSONDecodeError, TypeError) as e:
                    logger.error(f"[Arize/Phoenix] JSON parsing error for node {getattr(node_execution, 'id', 'unknown')}: {e}")
                    continue
                except Exception as e:
                    logger.error(f"[Arize/Phoenix] Unexpected error processing node {getattr(node_execution, 'id', 'unknown')}: {e}")
                    continue

            logger.info("[Arize/Phoenix] Completed workflow trace with {} nodes".format(len(nodes)))
    ...
    def _get_workflow_nodes(self, workflow_run_id: str):
        ...
        workflow_nodes = db.session.execute(
        ...
        logger.info("[Arize/Phoenix] Query returned {} nodes".format(len(workflow_nodes)))
        if workflow_nodes:
            first_node = workflow_nodes[0]
            logger.info("[Arize/Phoenix] First node type: {}".format(type(first_node)))
            logger.info("[Arize/Phoenix] First node value: {}".format(first_node))
    ...
    def _get_workflow_graph(self, workflow_run_id: str):
        ...
        try:
            ...
            logger.info("[Arize/Phoenix] Testing database access for workflow_run_id: {}".format(workflow_run_id))

            # Test basic database connection
            test_result = db.session.execute(select(text('1'))).scalar()
            logger.info("[Arize/Phoenix] Database connection test result: {}".format(test_result))
            ...
            if not workflow_run:
                ...
                if any_run:
                    logger.info("[Arize/Phoenix] WorkflowRun table accessible, but run {} not found".format(workflow_run_id))
                else:
                    logger.info("[Arize/Phoenix] WorkflowRun table appears empty or inaccessible")
                return None

            logger.info("[Arize/Phoenix] Found workflow run, workflow_id: {}".format(workflow_run.workflow_id))

            # Test Workflow table access
            logger.info("[Arize/Phoenix] Attempting to query workflows table...")
            workflow = db.session.execute(
                select(Workflow).where(Workflow.id == workflow_run.workflow_id)
            ).scalars().first()

            if not workflow:
                logger.warning("[Arize/Phoenix] Workflow not found for id: {}".format(workflow_run.workflow_id))
                return None

            if not workflow.graph:
                logger.warning("[Arize/Phoenix] Workflow graph is empty for workflow: {}".format(workflow_run.workflow_id))
                return None

            logger.info("[Arize/Phoenix] Found workflow graph, type: {}, length: {}".format(
                type(workflow.graph), len(str(workflow.graph))))

            import json
            graph = json.loads(workflow.graph) if isinstance(workflow.graph, str) else workflow.graph
            logger.info("[Arize/Phoenix] Parsed workflow graph with {} nodes, {} edges".format(
                len(graph.get('nodes', [])), len(graph.get('edges', []))))

            return graph

        except Exception as e:
            logger.error("[Arize/Phoenix] Failed to get workflow graph: {}".format(str(e)), exc_info=True)
            return None
    ...
    def _build_comprehensive_hierarchy(self, workflow_graph, execution_nodes):
        ...
        logger.info("  Graph relationships: {}".format(len(hierarchy_map)))
        logger.info("  Execution mappings: {}".format(len(node_mapping)))
        logger.info("  Decision paths: {}".format(len(decision_relationships)))
        logger.info("  Loop relationships: {}".format(len(loop_relationships)))

        return hierarchy_map, node_mapping, execution_context

    def _get_parent_workflow_context(self, trace_info):
        """Detect if this workflow is called as a tool from another workflow"""
        try:
            # Check if this workflow run has a parent workflow context
            # This could be stored in trace_info.metadata or determined by checking
            # if there's a recent workflow tool execution that matches this run

            # Look for parent context in metadata
            ...
            # Alternative: Check if there's a workflow tool node in another workflow
            # that references this workflow_run_id (more complex database query)
            parent_context = self._find_parent_workflow_tool(trace_info.workflow_run_id)
            if parent_context:
                return parent_context

        except Exception as e:
            logger.warning("[Arize/Phoenix] Could not determine parent workflow context: {}".format(str(e)))
    ...
    def _find_parent_workflow_tool(self, child_workflow_run_id):
        ...
        try:
            from models.workflow import WorkflowRun, Workflow, WorkflowNodeExecutionModel
            from models.tools import WorkflowToolProvider

            # Step 1: Get child workflow's app_id
            child_workflow_app_id = self._get_workflow_app_id(child_workflow_run_id)
            if not child_workflow_app_id:
                logger.warning("[Arize/Phoenix] Could not get app_id for workflow_run: {}".format(child_workflow_run_id[:8]))
                return None

            # Step 2: Check if this app is registered as a workflow tool
            workflow_tool = db.session.execute(
            ...
            if not workflow_tool:
                logger.debug("[Arize/Phoenix] App {} is not registered as a workflow tool".format(child_workflow_app_id[:8]))
                return None

            logger.info("[Arize/Phoenix] Found workflow tool registration: {} (app: {})".format(
                workflow_tool.name, child_workflow_app_id[:8]))

            # Step 3: Get child workflow timing for correlation
            child_run = db.session.execute(
                select(WorkflowRun).where(WorkflowRun.id == child_workflow_run_id)
            ).scalars().first()

            if not child_run or not child_run.created_at:
                return None

            # Step 4: Look for recent tool executions that might have called this workflow tool
            # Wider window for tools in loops/iterations that may have delays
            ...
            logger.debug("[Arize/Phoenix] Child workflow timing - start: {}, search window: {} to {}".format(
                child_run.created_at, start_window, end_window))

            # Search for tool nodes that executed this specific workflow tool
            potential_parent_tools = db.session.execute(
                select(WorkflowNodeExecutionModel).where(
            ...
            logger.info("[Arize/Phoenix] Found {} potential parent tool executions in time window for workflow '{}'".format(
                len(potential_parent_tools), workflow_tool.name))
            ...
            for i, tool in enumerate(potential_parent_tools[:5]):  # Log first 5 only
                try:
                    ...
                    logger.debug("[Arize/Phoenix] Potential parent {}: node_id={}, type={}, topic='{}', time={}".format(
                        i + 1, tool.id[:8], tool.node_type, topic, tool.created_at))
                except Exception:
                    logger.debug("[Arize/Phoenix] Potential parent {}: node_id={}, type={}, time={}".format(
                        i + 1, tool.id[:8], tool.node_type, tool.created_at))

            # Step 5: Direct SQL approach - find workflows created after tool execution
            logger.debug("[Arize/Phoenix] Trying direct SQL approach for tool->workflow mapping")
            direct_parent = self._find_parent_tool_by_sql(child_workflow_run_id, potential_parent_tools)
            if direct_parent:
                return direct_parent

            # Step 6: Check each tool execution to see if it called our workflow tool
            for tool_node in potential_parent_tools:
                if self._tool_called_workflow_tool(tool_node, workflow_tool, child_workflow_run_id):
                    parent_run_id = tool_node.workflow_run_id
                    parent_trace_id = string_to_trace_id128(parent_run_id)

                    logger.info("[Arize/Phoenix] Found parent workflow tool: {} -> child workflow: {} (tool: {})".format(
                        tool_node.id[:8], child_workflow_run_id[:8], workflow_tool.name))

                    # Get parent app name for additional context
            ...
            # Step 6: Fallback to old method if no specific tool match found
            logger.debug("[Arize/Phoenix] No specific tool match found, trying fallback timing correlation")
            return self._fallback_parent_tool_search(child_workflow_run_id)

        except Exception as e:
            logger.warning("[Arize/Phoenix] Error finding parent workflow tool: {}".format(str(e)))
    ...
    def _find_parent_tool_by_sql(self, child_workflow_run_id, potential_parent_tools):
        ...
        try:
            ...
            logger.debug("[Arize/Phoenix] Child workflow: app_id={}, start_time={}".format(
                child_run.app_id, child_run.created_at))
            ...
            for tool_node in potential_parent_tools:
                ...
                if 0 <= time_diff <= 30:
                    ...
                    if has_workflow_output:
                        ...
                        logger.info("[Arize/Phoenix] SQL Direct Match: Tool {} -> Child workflow {} ({}s gap)".format(
                            tool_node.id[:8], child_workflow_run_id[:8], time_diff))
            ...
            logger.warning("[Arize/Phoenix] Error in SQL parent tool search: {}".format(str(e)))
            return None
            ...
            if hasattr(tool_node, 'outputs') and tool_node.outputs:
                ...
                if isinstance(outputs, dict):
                    ...
                    if target_workflow_run_id in output_str:
                        logger.debug("[Arize/Phoenix] Tool {} output contains target workflow_run_id".format(tool_node.id[:8]))
                        return True
                    ...
                    if any(indicator in output_str.lower() for indicator in workflow_indicators):
                        logger.debug("[Arize/Phoenix] Tool {} output has workflow indicators".format(tool_node.id[:8]))
                        return True

            # Check process_data for workflow creation
            if hasattr(tool_node, 'process_data') and tool_node.process_data:
                import json
                process_data = json.loads(tool_node.process_data) if isinstance(tool_node.process_data, str) else tool_node.process_data

                if isinstance(process_data, dict):
                    process_str = str(process_data)
                    if target_workflow_run_id in process_str:
                        logger.debug("[Arize/Phoenix] Tool {} process_data contains target workflow_run_id".format(tool_node.id[:8]))
                        return True

            return False

        except Exception as e:
            logger.debug("[Arize/Phoenix] Error checking tool workflow output: {}".format(str(e)))
            return False
    ...
    def _get_workflow_app_id(self, workflow_run_id):
        ...
        try:
            ...
            logger.warning("[Arize/Phoenix] Error getting app_id for workflow_run {}: {}".format(
                workflow_run_id[:8], str(e)))
            return None
        ...
        try:
            ...
            if hasattr(tool_node, 'process_data') and tool_node.process_data:
                ...
                if isinstance(process_data, dict):
                    ...
                    if (workflow_tool.name in tool_name or
                        workflow_tool.app_id == app_id_ref or
                        'workflow' in tool_provider.lower()):
                        logger.debug("[Arize/Phoenix] Tool {} matches workflow tool by process_data".format(tool_node.id[:8]))
                        return True
            ...
            if hasattr(tool_node, 'inputs') and tool_node.inputs:
                ...
                if isinstance(inputs, dict):
                    ...
                    if workflow_tool.app_id in str(inputs):
                        logger.debug("[Arize/Phoenix] Tool {} matches workflow tool by app_id in inputs".format(tool_node.id[:8]))
                        return True

                    # Log for debugging
                    logger.debug("[Arize/Phoenix] Tool {} inputs checked, no app_id match".format(tool_node.id[:8]))
            ...
            if hasattr(tool_node, 'outputs') and tool_node.outputs:
                ...
                if isinstance(outputs, dict):
                    ...
                    if child_workflow_run_id in str(outputs):
                        logger.info("[Arize/Phoenix] FOUND: Tool {} created child workflow {} (in outputs)".format(
                            tool_node.id[:8], child_workflow_run_id[:8]))
                        return True
                    ...
                    if any(key in output_str for key in ['workflow_run_id', 'execution_id', 'run_id']):
                        logger.debug("[Arize/Phoenix] Tool {} has workflow references in outputs".format(tool_node.id[:8]))
                        # Additional logging for debugging
                        logger.debug("[Arize/Phoenix] Tool outputs keys: {}".format(list(outputs.keys())))
                ...
                try:
                    ...
                    if isinstance(process_data, dict):
                        ...
                        if child_workflow_run_id in str(process_data):
                            logger.info("[Arize/Phoenix] FOUND: Tool {} created child workflow {} (in process_data)".format(
                                tool_node.id[:8], child_workflow_run_id[:8]))
                            return True

                except Exception as e:
                    logger.debug("[Arize/Phoenix] Could not parse process_data for tool {}: {}".format(tool_node.id[:8], e))
            ...
            if hasattr(tool_node, 'created_at') and tool_node.created_at:
                ...
                if child_run and child_run.created_at:
                    ...
                    if 0 <= time_diff <= 15:  # Child started within 15 seconds after tool
                        logger.debug("[Arize/Phoenix] Tool {} matches by timing correlation ({:.1f}s)".format(
                            tool_node.id[:8], time_diff))
                        return True

        except Exception as e:
            logger.debug("[Arize/Phoenix] Error checking tool-workflow correlation: {}".format(str(e)))
    ...
    def _fallback_parent_tool_search(self, child_workflow_run_id):
        ...
        try:
            ...
            recent_workflow_tools = db.session.execute(
                select(WorkflowNodeExecutionModel).where(
                    WorkflowNodeExecutionModel.node_type.in_(['workflow', 'tool'])
                ).order_by(WorkflowNodeExecutionModel.created_at.desc()).limit(20)
            ).scalars().all()
            ...
            for tool_node in recent_workflow_tools:
                ...
                if self._tool_created_workflow(tool_node, child_workflow_run_id):
                    ...
                    logger.info("[Arize/Phoenix] Fallback: Found parent workflow tool: {} -> child: {}".format(
                        tool_node.id[:8], child_workflow_run_id[:8]))
            ...
            logger.warning("[Arize/Phoenix] Error in fallback parent tool search: {}".format(str(e)))
    ...
    def _tool_created_workflow(self, tool_node, child_workflow_run_id):
        ...
        try:
            ...
            logger.debug("[Arize/Phoenix] Error checking tool-workflow relationship: {}".format(str(e)))
    ...
    def _check_timing_relationship(self, tool_node, child_workflow_run_id):
        ...
        try:
            ...
            if child_run and tool_node.created_at and child_run.created_at:
                ...
                if 0 <= time_diff <= 15:
                    logger.info("[Arize/Phoenix] Timing match: tool {} -> workflow {} ({:.1f}s delay)".format(
                        tool_node.id[:8], child_workflow_run_id[:8], time_diff))
                    return True
    ...
    def _find_child_workflow_by_timing(self, tool_node):
        """Find child workflow triggered by this tool using enhanced correlation"""
        try:
            from models.workflow import WorkflowRun
                    ...
                    for key, value in outputs.items():
                        if 'workflow_run_id' in str(key).lower() or 'run_id' in str(key).lower():
                            ...
                            if len(potential_workflow_id) > 30:  # Workflow IDs are long UUIDs
                                logger.info("[Arize/Phoenix] Found direct workflow reference in tool {} outputs: {}".format(
                                    tool_node_id[:8], potential_workflow_id[:8]))
                    ...
                    for key, value in process_data.items():
                        if 'workflow' in str(key).lower() and 'run' in str(key).lower():
                            ...
                            if len(potential_workflow_id) > 30:
                                logger.info("[Arize/Phoenix] Found workflow reference in tool {} process_data: {}".format(
                                    tool_node_id[:8], potential_workflow_id[:8]))
                                return potential_workflow_id

            # Third, enhanced timing correlation with uniqueness check
            potential_children = db.session.execute(
            ...
            from models.workflow import WorkflowNodeExecutionModel
            other_tools = db.session.execute(
                select(WorkflowNodeExecutionModel).where(
                ...
                if other_tool.created_at and other_tool.created_at <= tool_node.created_at:
                    ...
                    for child_run in potential_children:
                        ...
                        if 0 <= time_diff <= 15:  # This child might belong to the earlier tool
                            ...
                            logger.debug("[Arize/Phoenix] Child workflow {} already associated with earlier tool {}".format(
                                child_run.id[:8], other_tool.id[:8]))
            ...
            for child_run in potential_children:
                if child_run.id not in already_associated:  # Only consider unassociated workflows
                    time_diff = (child_run.created_at - tool_node.created_at).total_seconds()
            ...
            if best_match and best_time_diff <= 15:
                logger.info("[Arize/Phoenix] Found unique child workflow: tool {} -> {} ({:.1f}s)".format(
                    tool_node.id[:8], best_match.id[:8], best_time_diff))
                return best_match.id

            if potential_children:
                logger.debug("[Arize/Phoenix] Tool {} - all potential children already associated with other tools".format(
                    tool_node_id[:8]))

        except Exception as e:
            logger.debug("[Arize/Phoenix] Error finding child workflow: {}".format(str(e)))
    ...
    def _find_parent_tools_by_timing(self, child_workflow_run_id):
        ...
        try:
            from models.workflow import WorkflowRun, WorkflowNodeExecutionModel
            ...
            logger.debug("[Arize/Phoenix] Error finding parent tools: {}".format(str(e)))
            return []
    ...
    def _find_logical_parent_span(self, node_execution, node_spans, execution_context):
        ...
        try:
            ...
            logger.debug("[Arize/Phoenix] Error finding logical parent: {}".format(str(e)))
    ...
    def _get_workflow_tool_name(self, node_execution):
        ...
        try:
            if hasattr(node_execution, 'process_data') and node_execution.process_data:
                process_data = json.loads(node_execution.process_data) if isinstance(node_execution.process_data, str) else node_execution.process_data
            ...
            logger.debug("[Arize/Phoenix] Could not extract workflow tool name: {}".format(str(e)))
    ...
    def _is_workflow_tool(self, node_execution):
        ...
        try:
            if hasattr(node_execution, 'process_data') and node_execution.process_data:
                ...
                if isinstance(process_data, dict):
                    tool_type = process_data.get('tool_type') or process_data.get('provider') or process_data.get('tool_name', '')
        ...
        try:
            ...
            if hasattr(node_execution, 'process_data') and node_execution.process_data:
                ...
                if isinstance(process_data, dict):
                    ...
                    tool_name = (process_data.get('tool_name') or
                               process_data.get('provider') or
                               process_data.get('tool_type') or
            ...
            if hasattr(node_execution, 'execution_metadata') and node_execution.execution_metadata:
                metadata = json.loads(node_execution.execution_metadata) if isinstance(node_execution.execution_metadata, str) else node_execution.execution_metadata
            ...
            logger.debug("[Arize/Phoenix] Error extracting tool name: {}".format(str(e)))
    ...
    def _ensure_child_workflows_processed(self, main_trace_info):
        ...
        try:
            ...
            for tool_node in tool_nodes:
                ...
                if child_workflow_id:
                    ...
                    if not self._child_workflow_trace_exists(child_workflow_id):
                        logger.info("[Arize/Phoenix] Child workflow {} not yet traced, should be processed first".format(
                            child_workflow_id[:8]))
            ...
            logger.debug("[Arize/Phoenix] Error ensuring child workflows processed: {}".format(str(e)))
    ...
    def _get_child_workflow_references(self, main_workflow_run_id):
        ...
        try:
            ...
            logger.debug("[Arize/Phoenix] Error getting child workflow references: {}".format(str(e)))
            return []
    ...
    def _get_child_workflow_trace_info(self, child_workflow_run_id):
        ...
        try:
            ...
            logger.debug("[Arize/Phoenix] Error getting child workflow trace info: {}".format(str(e)))
            return None
    ...
    def _get_tool_name_from_db(self, app_id):
        ...
        try:
            ...
            tool_provider = db.session.execute(
                select(WorkflowToolProvider).where(WorkflowToolProvider.app_id == app_id)
            ).scalars().first()
            ...
            logger.debug("[Arize/Phoenix] Error getting tool name from DB by app_id: {}".format(str(e)))
    ...
    def _get_tool_name_by_provider_id(self, provider_id):
        ...
        try:
            ...
            from models.tools import BuiltinToolProvider, ApiToolProvider, WorkflowToolProvider

            # Try workflow tool provider first
            tool = db.session.execute(
                select(WorkflowToolProvider).where(WorkflowToolProvider.id == provider_id)
            ).scalars().first()
            ...
            tool = db.session.execute(
                select(BuiltinToolProvider).where(BuiltinToolProvider.id == provider_id)
            ).scalars().first()
            ...
            logger.debug("[Arize/Phoenix] Error getting tool name from DB by provider_id: {}".format(str(e)))
            ...
            if node.node_type in ['question-classifier', 'if-else']:
                ...
                if node.node_type == 'question-classifier' and decision_output:
                    ...
                    if class_id and next_nodes:
                        ...
                        logger.info("[Arize/Phoenix] Decision path: {} -> {} (class: {})".format(
                            decision_node_id, next_node_id, decision_output.get('class_name', '')[:50]))
                ...
                # For if-else, determine which branch was taken based on subsequent execution
                    ...
                    logger.info("[Arize/Phoenix] IF/ELSE path: {} -> {}".format(
                        decision_node_id, next_node_id))
    ...
    def _extract_decision_output(self, node):
        ...
        try:
            ...
            logger.warning("[Arize/Phoenix] Could not parse decision output for node {}: {}".format(node.id, str(e)))
        return None
    ...
    def _resolve_loop_hierarchy(self, execution_nodes, workflow_graph):
        ...
        for loop_node in loop_nodes:
            ...
            logger.info("[Arize/Phoenix] Processing loop node: {} (index: {})".format(loop_node_id, loop_index))
            ...
            for node in execution_nodes:
                if node != loop_node and node.node_type not in ['start', 'end']:
                    ...
                    if self._is_node_in_loop_execution(node, loop_node, execution_nodes, workflow_graph):
                        ...
                        logger.info("[Arize/Phoenix] Loop child: {} -> loop: {}".format(node_id, loop_node_id))
        ...
        for iteration_node in iteration_nodes:
            ...
            logger.info("[Arize/Phoenix] Processing iteration node: {} (index: {})".format(iteration_node_id, iteration_index))
            ...
            for node in execution_nodes:
                if node != iteration_node and node.node_type not in ['start', 'end']:
                    ...
                    if self._is_node_in_iteration_execution(node, iteration_node, execution_nodes):
                        ...
                        logger.info("[Arize/Phoenix] Iteration child: {} -> iteration: {}".format(node_id, iteration_node_id))
    ...
    def _get_app_info_from_workflow_run_id(self, workflow_run_id: str) -> dict:
        ...
        try:
            ...
            app = db.session.execute(
                select(App).where(App.id == workflow.app_id)
            ).scalars().first()
            ...
            logger.info("[Arize/Phoenix] Found app: %s (%s) for workflow_run: %s",
                       app.name, app.id, workflow_run_id[:8])

            return {"app_id": app.id, "app_name": app.name}

        except Exception as e:
            logger.error("[Arize/Phoenix] Error getting app info for workflow_run %s: %s",
                        workflow_run_id[:8], str(e))
