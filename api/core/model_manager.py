...
class ModelManager:
    def __init__(self, enable_credentials_cache: bool = False, provider_manager: ProviderManager | None = None):
        # ``provider_manager`` can be injected so callers that also hold a ProviderManager
        # (e.g. ``DifyCredentialsProvider``) can share its per-instance configurations cache
        # and avoid rebuilding the tenant provider graph more than once per request.
