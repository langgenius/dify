...
class SQLAlchemyWorkflowNodeExecutionRepository(WorkflowNodeExecutionRepository):
    ...
    def save_synchronously(self, execution: WorkflowNodeExecution) -> None:
        """Persist immediately before an Agent v2 participant is materialized."""

        use_async_persistence = self._use_async_persistence
        self._use_async_persistence = False
        try:
            self.save(execution)
        finally:
            self._use_async_persistence = use_async_persistence
    ...
    def save_execution_data(self, execution: WorkflowNodeExecution) -> None:
        ...
        with self._session_factory() as session:
            ...
            if existing_model is not None and existing_model.finished_at is not None:
                if domain_model.finished_at is None or domain_model.finished_at < existing_model.finished_at:
                    return
            existing_process_data = existing_model.process_data_dict if existing_model is not None else None
        ...
        if domain_model.process_data is not None:
            process_data = preserve_workflow_agent_binding_id(existing_process_data, domain_model.process_data)
            if process_data is None:
                raise ValueError("process data is unavailable")
            result = self._truncate_and_upload(
                process_data,
                domain_model.id,
            ...
            if result is not None:
                ...
                process_data = truncated_process_data
        ...
        with self._session_factory() as session, session.begin():
            ...
            if domain_model.process_data is not None:
                process_data = preserve_workflow_agent_binding_id(db_model.process_data_dict, process_data)
                db_model.process_data = self._json_encode(process_data) if process_data is not None else None
                db_model.offload_data = _replace_offload(
                    db_model.offload_data,
                    ExecutionOffLoadType.PROCESS_DATA,
