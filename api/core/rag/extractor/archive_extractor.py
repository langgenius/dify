                ...
                for member in zip_ref.infolist():
                    ...
                    if self._is_safe_path(member.filename):
                        ...
                        if total_size[0] > self._max_total_size:
                            print("Warning: Total extraction size limit exceeded, stopping extraction")
                            break
                        ...
                        if not member.is_dir():
                            ...
                            extracted_files.append(
                                {"path": extracted_path, "relative_path": member.filename, "size": member.file_size}
                            )
            ...
            with tarfile.open(self._file_path, mode) as tar_ref:
                for member in tar_ref.getmembers():
                    ...
                    if self._is_safe_path(member.name) and member.isfile():
                        ...
                        extracted_files.append(
                            {"path": extracted_path, "relative_path": member.name, "size": member.size}
                        )
            ...
            with py7zr.SevenZipFile(self._file_path, mode="r") as archive:
                ...
                for root, dirs, files in os.walk(self._temp_dir):
                    for file in files:
                        ...
                        extracted_files.append({"path": full_path, "relative_path": relative_path, "size": file_size})
        except Exception as e:
