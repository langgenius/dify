...
class FeishuWikiExtractor(BaseExtractor):
    ...
    def get_document_markdown_content(self, document_id: str):
        ...
        data = {"app_id": self._app_id, "app_secret": self._app_secret, "document_id": document_id, "doc_type": "docx"}
        response = requests.post(
            url=FEISHU_PLUGIN_DOMAIN + "/document/docx2md", headers=headers, timeout=(60, 120), json=data
        )
    ...
    def _get_app_info(cls, tenant_id: str, feishu_workspace_id: str) -> (str, str):
        ...
        feishuwiki_oauth = FeishuWikiOAuth(app_id=app_id, app_secret=app_secret)
