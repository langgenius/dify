...
import grpc
from flexport.customs.productlibrary.productclassification.v1beta1 import (
    product_classification_api_pb2 as flexport_dot_customs_dot_productlibrary_dot_productclassification_dot_v1beta1_dot_product__classification__api__pb2,
)


class ProductClassificationAPIStub:
...
class ProductClassificationAPIServicer:
    """Service to CREATE/UPDATE/DELETE product classifications.
    API to update product classification (hs_code) in PPC and Classification State
    note: This API is a more general API which is not needed as of now
    rpc UpdateProductClassificationAndState(UpdateProductClassificationAndStateRequest) returns (UpdateProductClassificationAndStateResponse);
...
class ProductClassificationAPI:
    """Service to CREATE/UPDATE/DELETE product classifications.
    API to update product classification (hs_code) in PPC and Classification State
    note: This API is a more general API which is not needed as of now
    rpc UpdateProductClassificationAndState(UpdateProductClassificationAndStateRequest) returns (UpdateProductClassificationAndStateResponse);
    ...
    @staticmethod
    def UpdateClassificationWithClassificationAdapterResult(
        request,
        target,
        options=(),
