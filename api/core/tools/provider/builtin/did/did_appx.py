...
class DIDApp:
    ...
    def talks(
        ...
        id: str = response['id']
        if wait:
            return self._monitor_job_status(id=id, poll_interval=poll_interval)
        return id

    def check_talks_status(self, job: str):
        endpoint = f'{self.base_url}/talks/{job}'
    ...
    def _monitor_job_status(self, id: str, poll_interval: int):
        while True:
            status = self.check_talks_status(id)
