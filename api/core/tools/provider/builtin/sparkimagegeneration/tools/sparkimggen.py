...
class SparkTool(BuiltinTool):
    def _invoke(self,
        ...
        result = []
        for image in res:
            result.append(self.create_blob_message(blob=b64decode(image['base64_image']),
                                                   meta={'mime_type': 'image/png'},
                                                   save_as=self.VARIABLE_KEY.IMAGE.value))
        return result

    def img_generation(self, prompt):
        response = spark_response(text=prompt, appid=self.runtime.credentials.get('APPID'),
        ...
        if code != 0:
            return self.create_text_message(f'请求错误: {code}, {data}')
        else:
            text = data["payload"]["choices"]["text"]
            image_content = text[0]
            image_base = image_content["content"]
            json_data = {
                "base64_image": image_base
            }
        return [json_data]
