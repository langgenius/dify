...
class ApiTool(Tool):
    ...
    def assembling_request(self, parameters: dict[str, Any]) -> dict[str, Any]:
        ...
        if credentials["auth_type"] == "api_key":
            ...
            if "access_token" not in credentials:
                # Implement token acquisition logic here if needed, or raise an error
                raise ToolProviderCredentialValidationError(
                    "Missing access_token for OAuth"
                )
            # Add Authorization header for OAuth
            headers["Authorization"] = f"Bearer {credentials['access_token']}"
        else:
            raise ToolProviderCredentialValidationError(
                f"Unsupported auth_type '{credentials['auth_type']}'"
            )
    ...
    def validate_and_parse_response(
        self, response: httpx.Response | requests.Response
    ) -> str:
        """
        ...
        if (
            ...
            if "content" in self.api_bundle.openapi["requestBody"]:
                for content_type in self.api_bundle.openapi["requestBody"]["content"]:
                    ...
                    required = (
                        body_schema["required"] if "required" in body_schema else []
                    )
                    properties = (
                        body_schema["properties"] if "properties" in body_schema else {}
                    )
