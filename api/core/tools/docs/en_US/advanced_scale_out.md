...
Before starting the advanced integration, please make sure you have read the [Quick Integration](./tool_scale_out.md) and have a basic understanding of the tool integration process in Dify.

## Tool Interface

We have defined a series of shortcut methods in the `Tool` class to help developers quickly build more complex tools.

### Message Return

Dify supports various message types such as `text`, `link`, `image`, `file BLOB`, etc. You can return different types of messages to the LLM and users through the following interfaces.
...
This tool needs to pass in the user_id and the text to be summarized, and returns a summarized text. Dify will use the default model of the current workspace to summarize the long text.
...
This tool needs to pass in the web page link to be crawled and a user_agent (which can be empty), and returns a string containing the information of the web page. The `user_agent` is an optional parameter that can be used to identify the tool. If not passed, Dify will use the default `user_agent`.
...
We can notice that we used `self.VARIABLE_KEY.IMAGE.value` as the variable name of the image. In order for developers' tools to cooperate with each other, we defined this `KEY`. You can use it freely, or you can not use this `KEY`. Passing a custom KEY is also acceptable.
