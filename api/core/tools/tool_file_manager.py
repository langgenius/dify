...
class ToolFileManager:
    ...
    def create_file_by_key(
        user_id: str, id: Optional[str], tenant_id: str, conversation_id: str, file_key: str, mimetype: str, name: str,
        size: int
    ) -> ToolFile:
        """
        create file
        ...
        if not tool_file:
            tool_file = ToolFile(id=id, user_id=user_id, tenant_id=tenant_id,
                        conversation_id=conversation_id, file_key=file_key, mimetype=mimetype, name=name, size=size)
            db.session.add(tool_file)
            db.session.commit()
        return tool_file
