"""Entities describing the durable pause state of a workflow run.

``WorkflowResumptionContext`` is the serialized snapshot required to resume a
paused workflow. It is written by the workflow persistence layer as part of the
single pause transaction (see ``WorkflowPersistenceLayer.persist_pause``)
and read by every resume-side consumer.
"""
...
class PauseStateConfig:
    """Configuration container for persisting workflow pause state.

    The `state_owner_user_id` is used when creating state file for pause.
    It generally should be the id of the creator of the workflow.
    """
    ...
    def session_maker(self) -> sessionmaker[Session]:
        if isinstance(self.session_factory, Engine):
            return sessionmaker(self.session_factory)
