...
Detect a canvas hand-edit made while a copilot session waited, classify the
...
def _append_notice(fc: CopilotContext, text: str) -> ConversationItem:
...
def _reset_working_fields(fc: CopilotContext) -> None:
...
def apply_recovery_action(
    dify: DifyPort, turn: Turn, s: Session, fc: CopilotContext
