...
from dify_graph.entities.pause_reason import ToolCallPending
...
class LLMNode(Node[LLMNodeData]):
    ...
    def _run(self) -> Generator:
        ...
        try:
            ...
            from core.app.llm.model_access import fetch_model_config

            model_instance, model_config = fetch_model_config(
    ...
    def _sanitize_orphaned_tool_calls(
        ...
        """Strip orphaned assistant tool calls and matching tool messages from history."""
        ...
        for index, message in enumerate(prompt_messages):
            if isinstance(message, AssistantPromptMessage) and message.tool_calls:
                has_match = any(tool_call.id in valid_tc_ids for tool_call in message.tool_calls)
                if not has_match:
                    orphaned_tc_ids.update(tool_call.id for tool_call in message.tool_calls if tool_call.id)
                    prompt_messages[index] = AssistantPromptMessage(content=message.content, tool_calls=[])

        if not orphaned_tc_ids:
            return prompt_messages

        return [
            message
            for message in prompt_messages
            if not (isinstance(message, ToolPromptMessage) and message.tool_call_id in orphaned_tc_ids)
        ]
    ...
    def invoke_llm(
        *,
        ...
        if relevant_results:
            result_ids: set[str] = {
                tc_id for result in relevant_results if (tc_id := getattr(result, "tool_call_id", None)) is not None
            }
            current_prompt_messages = LLMNode._sanitize_orphaned_tool_calls(
                current_prompt_messages,
                valid_tc_ids=result_ids,
            )
