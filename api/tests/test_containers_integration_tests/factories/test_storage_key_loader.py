...
class TestStorageKeyLoader:
    ...
    # ------------------------------------------------------------------
    # Per-test helpers (use container_session as parameter)
    # ------------------------------------------------------------------
    ...
    def test_load_storage_keys_local_file(self, container_session: Session):
        """Test loading storage keys for LOCAL_FILE transfer method."""
        tenant_id = str(uuid4())
        user_id = str(uuid4())

        upload_file = self._create_upload_file(container_session, tenant_id, user_id)
        file = self._create_file(tenant_id, related_id=upload_file.id, transfer_method=FileTransferMethod.LOCAL_FILE)

        loader = StorageKeyLoader(container_session, tenant_id, access_controller=DatabaseFileAccessController())
        loader.load_storage_keys([file])

        assert file._storage_key == upload_file.key

    def test_load_storage_keys_remote_url(self, container_session: Session):
    ...
    def test_load_storage_keys_tool_file(self, container_session: Session):
        """Test loading storage keys for TOOL_FILE transfer method."""
        tenant_id = str(uuid4())
        user_id = str(uuid4())
        conversation_id = str(uuid4())

        tool_file = self._create_tool_file(container_session, tenant_id, user_id, conversation_id)
    ...
    def test_load_storage_keys_mixed_methods(self, container_session: Session):
        """Test batch loading with mixed transfer methods."""
        tenant_id = str(uuid4())
        user_id = str(uuid4())
        conversation_id = str(uuid4())

        upload_file1 = self._create_upload_file(container_session, tenant_id, user_id)
        upload_file2 = self._create_upload_file(container_session, tenant_id, user_id)
        tool_file = self._create_tool_file(container_session, tenant_id, user_id, conversation_id)
    ...
    def test_load_storage_keys_empty_list(self, container_session: Session):
        """Test with empty file list — should not raise."""
        tenant_id = str(uuid4())
        loader = StorageKeyLoader(container_session, tenant_id, access_controller=DatabaseFileAccessController())
        loader.load_storage_keys([])

    def test_load_storage_keys_ignores_legacy_file_tenant_id(self, container_session: Session):
    ...
    def test_load_storage_keys_missing_file_id(self, container_session: Session):
    ...
    def test_load_storage_keys_nonexistent_upload_file_records(self, container_session: Session):
    ...
    def test_load_storage_keys_nonexistent_tool_file_records(self, container_session: Session):
    ...
    def test_load_storage_keys_invalid_uuid(self, container_session: Session):
    ...
    def test_load_storage_keys_batch_efficiency(self, container_session: Session):
        """Batched operations should issue exactly 2 queries for mixed file types."""
        tenant_id = str(uuid4())
        user_id = str(uuid4())
        conversation_id = str(uuid4())

        upload_files = [self._create_upload_file(container_session, tenant_id, user_id) for _ in range(3)]
        tool_files = [self._create_tool_file(container_session, tenant_id, user_id, conversation_id) for _ in range(2)]
        ...
        loader = StorageKeyLoader(container_session, tenant_id, access_controller=DatabaseFileAccessController())
        with patch.object(container_session, "scalars", wraps=container_session.scalars) as mock_scalars:
    ...
    def test_load_storage_keys_tenant_isolation(self, container_session: Session):
        """Loader should not surface records belonging to a different tenant."""
        tenant_id = str(uuid4())
        other_tenant_id = str(uuid4())
        user_id = str(uuid4())

        upload_file_current = self._create_upload_file(container_session, tenant_id, user_id)
    ...
    def test_load_storage_keys_mixed_tenant_batch(self, container_session: Session):
    ...
    def test_load_storage_keys_duplicate_file_ids(self, container_session: Session):
    ...
    def test_load_storage_keys_session_isolation(self, container_session: Session):
        ...
        # A loader with a fresh, separate session cannot see uncommitted rows from container_session
