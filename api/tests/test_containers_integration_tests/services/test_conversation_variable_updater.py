...
class TestConversationVariableUpdater:
    ...
    def test_should_update_conversation_variable_data_and_commit(self, container_session: Session):
        conversation_id = str(uuid4())
        variable = StringVariable(id=str(uuid4()), name="topic", value="old value")
        self._create_conversation_variable(container_session, conversation_id=conversation_id, variable=variable)
        ...
        row = container_session.get(ConversationVariable, (variable.id, conversation_id))
        assert row is not None
        assert row.data == updated_variable.model_dump_json()

    def test_should_raise_not_found_when_variable_missing(self, container_session: Session):
        conversation_id = str(uuid4())
    ...
    def test_should_do_nothing_when_flush_is_called(self, container_session: Session):
        updater = ConversationVariableUpdater(sessionmaker(bind=db.engine))
