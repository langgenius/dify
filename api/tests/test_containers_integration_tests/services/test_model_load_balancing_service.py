...
class TestModelLoadBalancingService:
    ...
    def test_get_load_balancing_configs_success(
        ...
        is_enabled, configs = service.get_load_balancing_configs(
            tenant_id=tenant.id, provider="openai", model="gpt-3.5-turbo", model_type="llm", session=db_session_with_containers
        )
    ...
    def test_get_load_balancing_configs_provider_not_found(
        ...
        with pytest.raises(ValueError) as exc_info:
            service.get_load_balancing_configs(
                tenant_id=tenant.id, provider="nonexistent_provider", model="gpt-3.5-turbo", model_type="llm",session=db_session_with_containers
            )
