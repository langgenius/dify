...
class TestDatasetPermissionServiceGetPartialMemberList:
    ...
    def test_get_dataset_partial_member_list_with_members(self, container_session: Session):
        """
    ...
    def test_get_dataset_partial_member_list_with_single_member(self, container_session: Session):
        """
    ...
    def test_get_dataset_partial_member_list_empty(self, container_session: Session):
        """
...
class TestDatasetPermissionServiceUpdatePartialMemberList:
    ...
    def test_update_partial_member_list_add_new_members(self, container_session: Session):
        """
        ...
        # Act
        DatasetPermissionService.update_partial_member_list(tenant.id, dataset.id, user_list, session=container_session)

        # Assert
        result = DatasetPermissionService.get_dataset_partial_member_list(dataset.id, session=container_session)
        assert set(result) == {member_1.id, member_2.id}

    def test_update_partial_member_list_replace_existing(self, container_session: Session):
        """
        ...
        DatasetPermissionService.update_partial_member_list(tenant.id, dataset.id, old_users, session=container_session)

        new_users = DatasetPermissionTestDataFactory.build_user_list_payload([new_member_1.id, new_member_2.id])

        # Act
        DatasetPermissionService.update_partial_member_list(tenant.id, dataset.id, new_users, session=container_session)

        # Assert
        result = DatasetPermissionService.get_dataset_partial_member_list(dataset.id, session=container_session)
        assert set(result) == {new_member_1.id, new_member_2.id}

    def test_update_partial_member_list_empty_list(self, container_session: Session):
        """
        ...
        DatasetPermissionService.update_partial_member_list(tenant.id, dataset.id, users, session=container_session)

        # Act
        DatasetPermissionService.update_partial_member_list(tenant.id, dataset.id, [], session=container_session)

        # Assert
        result = DatasetPermissionService.get_dataset_partial_member_list(dataset.id, session=container_session)
        assert result == []

    def test_update_partial_member_list_database_error_requires_caller_rollback(self, container_session: Session):
        ...
        original_flush = container_session.flush
        ...
        with pytest.MonkeyPatch.context() as mp:
            ...
            mp.setattr(container_session, "flush", _raise_flush)
            with pytest.raises(Exception, match="Database connection error"):
                DatasetPermissionService.update_partial_member_list(
                    tenant.id, dataset.id, user_list, session=container_session
                )
            mp.setattr(container_session, "flush", original_flush)

        # Assert
        container_session.rollback()
        result = DatasetPermissionService.get_dataset_partial_member_list(dataset.id, session=container_session)
        assert result == [existing_member.id]
        assert container_session.query(DatasetPermission).filter_by(dataset_id=dataset.id).count() == 1
...
class TestDatasetPermissionServiceClearPartialMemberList:
    ...
    def test_clear_partial_member_list_success(self, container_session: Session):
        """
    ...
    def test_clear_partial_member_list_empty_list(self, container_session: Session):
        """
    ...
    def test_clear_partial_member_list_database_error_requires_caller_rollback(self, container_session: Session):
        ...
        result = DatasetPermissionService.get_dataset_partial_member_list(dataset.id, session=container_session)
        assert set(result) == {member_1.id, member_2.id}
        assert container_session.query(DatasetPermission).filter_by(dataset_id=dataset.id).count() == 2


class TestDatasetServiceCheckDatasetPermission:
    """Verify dataset access checks against persisted partial-member permissions."""

    def test_check_dataset_permission_different_tenant_should_fail(self, container_session: Session):
        ...
        with pytest.raises(NoPermissionError):
            DatasetService.check_dataset_permission(dataset, other_user, container_session)

    def test_check_dataset_permission_owner_can_access_any_dataset(self, container_session: Session):
        ...
        DatasetService.check_dataset_permission(dataset, owner, container_session)

    def test_check_dataset_permission_only_me_creator_can_access(self, container_session: Session):
    ...
    def test_check_dataset_permission_only_me_others_cannot_access(self, container_session: Session):
        ...
        with pytest.raises(NoPermissionError):
            DatasetService.check_dataset_permission(dataset, other, container_session)

    def test_check_dataset_permission_all_team_allows_access(self, container_session: Session):
    ...
    def test_check_dataset_permission_partial_members_with_permission_success(self, container_session: Session):
        """
        ...
        DatasetService.check_dataset_permission(dataset, user, container_session)

        # Assert
        permissions = DatasetPermissionService.get_dataset_partial_member_list(dataset.id, session=container_session)
        assert user.id in permissions

    def test_check_dataset_permission_partial_members_without_permission_error(self, container_session: Session):
        """
    ...
    def test_check_dataset_permission_partial_team_creator_can_access(self, container_session: Session):
...
class TestDatasetServiceCheckDatasetOperatorPermission:
    ...
    def test_check_dataset_operator_permission_partial_members_with_permission_success(self, container_session):
        """
        Test that user with explicit permission can access partial_members dataset.
        """
        # Arrange
        ...
        # Act (should not raise)
        DatasetService.check_dataset_operator_permission(user=user, dataset=dataset, session=container_session)

        # Assert
        permissions = DatasetPermissionService.get_dataset_partial_member_list(dataset.id, session=container_session)
        assert user.id in permissions

    def test_check_dataset_operator_permission_partial_members_without_permission_error(self, container_session):
        """
        Test error when user without permission tries to access partial_members dataset.
        """
        # Arrange
