...
_OTHER_WORKSPACE_ID = WorkspaceId("00000000-0000-0000-0000-000000000901")
...
def test_workspace_member_contact_contract_uses_guarded_postgresql_writes(
    ...
    snapshot = repository.load_snapshot(workspace_id)
    ...
    with pytest.raises(ValueError, match="scope does not match"):
        repository.save_workspace_member_contact(
            contact,
            organization_scope=WorkspaceScope(_OTHER_WORKSPACE_ID),
        )
...
def test_deployment_external_and_platform_contracts_use_postgresql(
    ...
    with pytest.raises(ContactDirectoryError) as wrong_workspace:
        repository.hard_delete_external(_OTHER_WORKSPACE_ID, external.id)
    ...
    assert repository.load_snapshot(workspace_id).platform_contact_ids == frozenset()
