...
class TestWorkspacesList:
    def test_lists_only_members_workspaces_with_role(self, app: Flask, make_account: Callable[..., Account]) -> None:
        ...
        assert [(role.id, role.name) for role in only.roles] == [
...
class TestWorkspaceDetail:
    def test_member_can_read_detail(self, app: Flask, make_account: Callable[..., Account]) -> None:
        account = make_account()
...
class TestWorkspaceSwitch:
    ...
    def test_switch_to_non_member_workspace_is_404(self, app: Flask, make_account: Callable[..., Account]) -> None:
        account = make_account()
