...
class TestConversationListApi:
    ...
    def test_non_chat_mode_raises(self, app: Flask) -> None:
        with app.test_request_context("/conversations"), pytest.raises(NotChatAppError):
...
class TestConversationApi:
    ...
    def test_non_chat_mode_raises(self, app: Flask) -> None:
        with app.test_request_context(f"/conversations/{uuid4()}"), pytest.raises(NotChatAppError):
    ...
    def test_delete_not_found(self, mock_delete: MagicMock, app: Flask) -> None:
        c_id = uuid4()
        with (
            app.test_request_context(f"/conversations/{c_id}"),
            pytest.raises(NotFound, match="Conversation Not Exists"),
        ):
...
class TestConversationRenameApi:
    ...
    def test_non_chat_mode_raises(self, app: Flask) -> None:
        with (
            app.test_request_context(f"/conversations/{uuid4()}/name", method="POST", json={"name": "x"}),
    ...
    def test_rename_not_found(self, mock_ns: MagicMock, mock_rename: MagicMock, app: Flask) -> None:
        ...
        with (
            app.test_request_context(f"/conversations/{c_id}/name", method="POST", json={"name": "X"}),
            pytest.raises(NotFound, match="Conversation Not Exists"),
        ):
...
class TestConversationPinApi:
    ...
    def test_non_chat_mode_raises(self, app: Flask) -> None:
        with app.test_request_context(f"/conversations/{uuid4()}/pin", method="PATCH"), pytest.raises(NotChatAppError):
    ...
    def test_pin_not_found(self, mock_pin: MagicMock, app: Flask) -> None:
        c_id = uuid4()
        with app.test_request_context(f"/conversations/{c_id}/pin", method="PATCH"), pytest.raises(NotFound):
...
class TestConversationUnPinApi:
    ...
    def test_non_chat_mode_raises(self, app: Flask) -> None:
        with (
            app.test_request_context(f"/conversations/{uuid4()}/unpin", method="PATCH"),
            pytest.raises(NotChatAppError),
        ):
