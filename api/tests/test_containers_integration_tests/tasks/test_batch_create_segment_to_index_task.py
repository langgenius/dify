...
class TestBatchCreateSegmentToIndexTask:
    ...
    def test_batch_create_segment_to_index_task_success_qa_model(
        self, db_session_with_containers, mock_external_service_dependencies
    ):
        """
        Test successful batch creation of segments for QA model documents.

        This test verifies that the task can successfully:
        1. Process a CSV file with question-answer pairs
        2. Create document segments with both content and answers
        3. Calculate proper word counts including answers
        4. Handle QA model specific logic
        """
        # Create test data
        account, tenant = self._create_test_account_and_tenant(db_session_with_containers)
        dataset = self._create_test_dataset(db_session_with_containers, account, tenant)

        # Create document with QA model format
        document = Document(
            tenant_id=tenant.id,
            dataset_id=dataset.id,
            position=1,
            data_source_type="upload_file",
            batch="test_batch",
            name="test_qa_document",
            created_from="upload_file",
            created_by=account.id,
            indexing_status="completed",
            enabled=True,
            archived=False,
            doc_form="qa_model",  # QA model format
            word_count=0,
        )
        ...
        # Create CSV content with QA format
        csv_content = self._create_test_csv_content("qa_model")
        ...
        # Verify segment content, answers, and metadata
        for i, segment in enumerate(segments):
            assert segment.tenant_id == tenant.id
            assert segment.dataset_id == dataset.id
            assert segment.document_id == document.id
            assert segment.position == i + 1
            assert segment.status == "completed"
            assert segment.answer is not None  # QA model should have answers
            assert segment.word_count > 0  # Should include both content and answer word counts
    ...
    def test_batch_create_segment_to_index_task_economy_indexing_technique(
        self, db_session_with_containers, mock_external_service_dependencies
    ):
        """
        Test task execution with economy indexing technique.

        This test verifies that the task can handle:
        1. Economy indexing technique (no embedding model required)
        2. Proper token calculation (set to 0 for economy)
        3. Vector index creation without embedding model
        4. Successful completion with economy settings
        """
        # Create test data
        account, tenant = self._create_test_account_and_tenant(db_session_with_containers)

        # Create dataset with economy indexing technique
        fake = Faker()
        dataset = Dataset(
            tenant_id=tenant.id,
            name=fake.company(),
            description=fake.text(),
            data_source_type="upload_file",
            indexing_technique="economy",  # Economy indexing
            embedding_model=None,  # No embedding model for economy
        ...
        for i, segment in enumerate(segments):
            ...
            assert segment.position == i + 1
            assert segment.status == "completed"
            assert segment.tokens == 0  # Economy indexing sets tokens to 0
