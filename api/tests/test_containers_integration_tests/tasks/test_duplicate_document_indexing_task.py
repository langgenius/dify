...
class TestDuplicateDocumentIndexingTask:
    ...
    def test_duplicate_document_indexing_performance_timing(
        self, db_session_with_containers, mock_external_service_dependencies
    ):
        """
        Test performance timing and logging.
        ...
        # Create segments for documents
        for document in documents:
            self._create_test_segments(db_session_with_containers, document, dataset)

        document_ids = [doc.id for doc in documents]

        # Act: Execute the task with timing
        import time
    ...
    def test_duplicate_document_indexing_database_transaction_consistency(
        self, db_session_with_containers, mock_external_service_dependencies
    ):
        """
        Test database transaction consistency and rollback scenarios.
        ...
        # All documents should be in parsing status
        for document in documents:
            db.session.refresh(document)
            assert document.indexing_status == "parsing"
            assert document.processing_started_at is not None
        ...
        # Verify IndexingRunner was called with all documents
        mock_runner_instance = mock_external_service_dependencies["indexing_runner_instance"]
        mock_runner_instance.run.assert_called_once()
        run_args = mock_runner_instance.run.call_args[0][0]
        assert len(run_args) == 2

    def test_duplicate_document_indexing_comprehensive_error_scenarios(
        self, db_session_with_containers, mock_external_service_dependencies
    ):
        """
        ...
        - Task completes without unhandled exceptions
        """
        # Arrange: Create test data
        dataset, documents = self._create_test_dataset_and_documents(
            db_session_with_containers, mock_external_service_dependencies, document_count=1
        ...
        for error_name, exception in test_exceptions:
            ...
            documents[0].indexing_status = "completed"
            documents[0].error = None
            documents[0].stopped_at = None
            db.session.commit()
            ...
            # Verify document was prepared for re-indexing before the error
            db.session.refresh(documents[0])
            assert documents[0].indexing_status == "parsing"
