...
class TestSendEmailCodeLoginMailTask:
    ...
    def test_send_email_code_login_mail_task_performance_metrics(
        self, db_session_with_containers, mock_external_service_dependencies
    ):
        """
        Test email code login mail task performance metrics and logging.

        This test verifies that the task can properly:
        1. Measure and log execution time
        2. Log start and completion messages
        3. Provide performance insights
        4. Handle timing measurements accurately
        """
        # Arrange: Setup test data
        fake = Faker()
        test_email = fake.email()
        ...
        # Assert: Verify expected outcomes
        # Verify start log message (with click.style formatting)
        start_calls = [
            call
            for call in mock_logger.info.call_args_list
            if f"Start email code login mail to {test_email}" in str(call)
        ]
        assert len(start_calls) == 1

        # Verify success log message with performance metrics
        success_calls = [
            call
            for call in mock_logger.info.call_args_list
            if f"Send email code login mail to {test_email} succeeded" in str(call)
        ]
        assert len(success_calls) == 1

        # Verify performance logging contains latency information
        success_call = success_calls[0]
        assert "latency:" in str(success_call)

        # Verify execution time is reasonable (should be very fast with mocks)
        execution_time = end_time - start_time
        assert execution_time < 1.0, f"Task execution took too long: {execution_time:.3f} seconds"

    def test_send_email_code_login_mail_task_concurrent_execution(
        self, db_session_with_containers, mock_external_service_dependencies
    ):
        """
        Test email code login mail task with concurrent execution.

        This test verifies that the task can properly:
        1. Handle multiple concurrent executions
        2. Maintain proper isolation between executions
        3. Process all requests without conflicts
        4. Complete all executions successfully
        """
        # Arrange: Setup test data
        fake = Faker()
        test_language = "en-US"
        ...
        test_emails = [fake.email() for _ in range(num_concurrent_tasks)]
        test_codes = [fake.numerify("######") for _ in range(num_concurrent_tasks)]
        ...
        for i in range(num_concurrent_tasks):
            thread = threading.Thread(target=run_task, args=(test_emails[i], test_codes[i]))
        ...
        # Assert: Verify expected outcomes
        assert len(results) == num_concurrent_tasks, "All tasks should complete successfully"
        assert len(errors) == 0, "No tasks should fail"

        # Verify email service was called for each task
        mock_email_service_instance = mock_external_service_dependencies["email_service_instance"]
        assert mock_email_service_instance.send_email.call_count == num_concurrent_tasks
