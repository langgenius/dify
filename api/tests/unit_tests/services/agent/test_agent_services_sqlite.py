...
def test_preview_conversation_lookup_is_account_and_tenant_scoped(agent_session: Session) -> None:
    ...
    assert (
        service.get_current_preview_conversation(tenant_id="tenant-1", agent_id="agent-1", account_id="account-1")
        == conversation.id
    )
    assert (
        service.get_current_preview_conversation(tenant_id="tenant-2", agent_id="agent-1", account_id="account-1")
        is None
    )
    assert (
        service.get_current_preview_conversation(tenant_id="tenant-1", agent_id="agent-1", account_id="missing") is None
    )
