...
def test_import_warnings_cover_runtime_setup_removed_from_package(monkeypatch, unbound_session: Session) -> None:
    soul = AgentSoulConfig.model_validate(
        {
            "tools": {
                "dify_tools": [
...
def test_clone_inline_binding_copies_soul_and_drive_rows(monkeypatch, unbound_session: Session) -> None:
...
def test_create_workflow_only_agent_sets_backing_app_and_snapshot(monkeypatch, sqlite_session: Session) -> None:
    service = AgentDslService(sqlite_session)
    roster_service = Mock()
