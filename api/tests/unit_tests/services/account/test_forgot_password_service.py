...
def test_web_send_rejects_unknown_account_before_issuing_a_token() -> None:
    dependencies = Dependencies()

    with pytest.raises(account_errors.AccountNotFoundError):
        dependencies.service.send_code(
            email="missing@example.com", language="en-US", ip_address="127.0.0.1", require_account=True
        )

    assert dependencies.tokens.issued == []
    assert dependencies.notifications.sent == []
    assert dependencies.send_limits.recorded == []
