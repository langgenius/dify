...
def test_publish_sends_json_encoded_event_to_the_session_topic() -> None:
    ...
    with patch("services.workflow_copilot.progress_bus.get_pubsub_broadcast_channel", return_value=fake_channel):
        progress_bus.publish(SESSION_ID, event)

    fake_channel.topic.assert_called_once_with(f"workflow_copilot:{SESSION_ID}")
