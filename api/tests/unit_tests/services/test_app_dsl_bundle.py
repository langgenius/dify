...
@pytest.mark.parametrize("mode", APP_MODES)
def test_invalid_nested_deployment_rolls_back_all_app_state(
    sqlite_session: Session, monkeypatch: pytest.MonkeyPatch, mode: AppMode
) -> None:
    account, _, _ = _seed_bundle(sqlite_session, monkeypatch)
    source_provider = sqlite_session.scalar(select(WorkflowToolProvider).where(WorkflowToolProvider.name == "nested"))
...
def test_bundle_emits_webhook_draft_sync_only_after_commit(
    sqlite_session: Session, monkeypatch: pytest.MonkeyPatch
) -> None:
    account, _, _ = _seed_bundle(sqlite_session, monkeypatch)
    bundle = _single_app_bundle()
    workflow = cast(WorkflowContentDict, bundle.documents["app_1"]["workflow"])
    workflow["graph"]["nodes"][0] = {
        "id": "webhook",
        "data": {"type": "trigger-webhook", "title": "Webhook", "method": "POST"},
    }
    dsl_service = AppDslService(sqlite_session)
    receiver = Mock()
    with app_draft_workflow_was_synced.connected_to(receiver):
        normal = dsl_service.import_app(
            account=account,
            import_mode=ImportMode.YAML_CONTENT,
            yaml_content=yaml.safe_dump(bundle.documents["app_1"]),
        )
        assert normal.status == ImportStatus.COMPLETED
        assert receiver.call_count == 1
        normal_draft = receiver.call_args.kwargs["synced_draft_workflow"]
        receiver.reset_mock()

        imported = AppDslBundleService(sqlite_session).import_bundle(bundle, account=account, dsl_service=dsl_service)
        receiver.assert_not_called()
        sqlite_session.commit()
        receiver.assert_called_once()
        imported_draft = receiver.call_args.kwargs["synced_draft_workflow"]
        assert receiver.call_args.args == (imported,)
        assert imported_draft.app_id == imported.id
        assert imported_draft.version == Workflow.VERSION_DRAFT
        assert imported_draft.graph_dict == normal_draft.graph_dict

        sqlite_session.commit()
        receiver.assert_called_once()


def test_rollback_discards_draft_sync_callbacks(sqlite_session: Session, monkeypatch: pytest.MonkeyPatch) -> None:
    account, _, _ = _seed_bundle(sqlite_session, monkeypatch)
    receiver = Mock()
    with app_draft_workflow_was_synced.connected_to(receiver):
        AppDslBundleService(sqlite_session).import_bundle(
            _single_app_bundle(), account=account, dsl_service=AppDslService(sqlite_session)
        )
        receiver.assert_not_called()
        sqlite_session.rollback()
        sqlite_session.commit()
...
@pytest.mark.parametrize(
    "corruption",
    ["kind", "version", "resource_kind", "relationship_kind", "duplicate_relationship", "incompatible_target"],
)
