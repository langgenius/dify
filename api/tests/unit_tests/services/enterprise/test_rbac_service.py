...
class TestResourcePermissions:
    ...
    def test_knowledge_fs_permissions_batch_get_returns_empty_without_remote_call(
        self, mock_send: MagicMock, sqlite_session: Session
    ):
        out = svc.RBACService.KnowledgeFSPermissions.batch_get("tenant-1", "acct-1", [], session=sqlite_session)
    ...
    def test_knowledge_fs_permissions_batch_get_uses_full_legacy_permissions_when_rbac_disabled(
        ...
        out = svc.RBACService.KnowledgeFSPermissions.batch_get(
            "tenant-1", "acct-1", ["control-1", "control-2"], session=sqlite_session
        )
    ...
    def test_knowledge_fs_permissions_batch_get_uses_control_space_contract(
        self, mock_send: MagicMock, sqlite_session: Session
    ):
        mock_send.return_value = {
            "data": [
