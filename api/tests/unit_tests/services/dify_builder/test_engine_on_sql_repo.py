...
(``tests/unit_tests/core/dify_builder/test_fix_flow.py``) exercises --
``Runner`` + ``fix_registry()`` + ``PlaceholderAgent`` + ``FakeDifyPort`` --
but swaps the in-memory fake ``Repository`` for ``SqlDifyBuilderRepository``
...
def _seed_fix_session(repo: SqlDifyBuilderRepository) -> Session:
    ...
    DifyBuilderContext pointing at a pre-existing failed run -- mirrors the P1
...
def test_full_fix_flow_request_fix_to_publish_success_on_sql_repo(repo: SqlDifyBuilderRepository) -> None:
...
def _seed_checklist_session(repo: SqlDifyBuilderRepository, errors: list[ChecklistError]) -> Session:
