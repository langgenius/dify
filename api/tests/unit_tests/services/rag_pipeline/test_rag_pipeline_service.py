...
def test_update_customized_pipeline_template_success(mocker) -> None:
    ...
    mocker.patch(
        "services.rag_pipeline.rag_pipeline.db.session.query", side_effect=[query_mock_1, query_mock_2]
    )
    ...
    info = SimpleNamespace(
        ...
        icon_info=SimpleNamespace(model_dump=lambda: {"icon": "🔥"}),
    )
...
def test_update_customized_pipeline_template_not_found(mocker) -> None:
    ...
    info = SimpleNamespace(name="x", description="d", icon_info=SimpleNamespace(model_dump=lambda: {}))
...
def test_update_customized_pipeline_template_duplicate_name(mocker) -> None:
    ...
    info = SimpleNamespace(name="dup", description="d", icon_info=SimpleNamespace(model_dump=lambda: {}))
...
def test_handle_node_run_result_success(mocker, rag_pipeline_service) -> None:
    ...
    result = rag_pipeline_service._handle_node_run_result(
        getter=lambda: (node_instance, mock_getter()),
...
def test_get_first_step_parameters_success(mocker, rag_pipeline_service) -> None:
    ...
    workflow.rag_pipeline_variables = [
        {"variable": "url", "label": "URL", "type": "string"}
    ]
    mocker.patch.object(rag_pipeline_service, "get_published_workflow", return_value=workflow)
    
    # 2. Run test
    result = rag_pipeline_service.get_first_step_parameters(
        pipeline=pipeline,
        node_id="node-1",
        is_draft=False
    )
...
def test_get_second_step_parameters_success(mocker, rag_pipeline_service) -> None:
    ...
    workflow.rag_pipeline_variables = [
        {"variable": "var1", "label": "Var 1"}
    ]
    mocker.patch.object(rag_pipeline_service, "get_published_workflow", return_value=workflow)
    
    # 2. Run test
    result = rag_pipeline_service.get_second_step_parameters(
        pipeline=pipeline,
        node_id="node-1",
        is_draft=False
    )
