...
def test_publish_customized_pipeline_template_success(
    ...
    session_factory.return_value.begin.return_value.__enter__.return_value = mocker.Mock()
    ...
    rag_pipeline_service.publish_customized_pipeline_template("p1", args, account, "t1", session=mock_db.session)
...
def test_get_draft_workflow_success(mocker: MockerFixture, rag_pipeline_service: RagPipelineService) -> None:
    ...
    rag_pipeline_service._session = mock_db.session
...
def test_publish_customized_pipeline_template_raises_for_missing_pipeline(
    ...
    with pytest.raises(ValueError, match="Pipeline not found"):
        rag_pipeline_service.publish_customized_pipeline_template("p1", {}, _make_account(), "t1", session=session)
...
def test_publish_customized_pipeline_template_raises_when_dataset_missing(
    ...
    with pytest.raises(ValueError, match="Dataset not found"):
        rag_pipeline_service.publish_customized_pipeline_template(
            "p1", {}, _make_account(), "t1", session=mock_db.session
        )
