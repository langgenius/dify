...
class TestAppModelValidation:
    ...
    def test_app_desc_or_prompt_without_description(self, unbound_session: Session):
        ...
        with patch.object(App, "app_model_config_with_session", return_value=None) as get_model_config:
            # Act
            result = app.desc_or_prompt_with_session(session=unbound_session)
    ...
    def test_app_is_agent_with_session_updates_legacy_agent_mode(self, sqlite_session: Session):
        app = App(
            tenant_id=str(uuid4()),
            name="Test App",
            mode=AppMode.CHAT,
    ...
    def test_app_mode_compatible_with_agent(self, unbound_session: Session):
        ...
        with patch.object(App, "is_agent_with_session", return_value=False) as is_agent:
            # Act
            result = app.mode_compatible_with_agent_with_session(session=unbound_session)
    ...
    def test_deleted_tools_checks_plugin_builtin_providers_through_core_plugin_service(self, unbound_session: Session):
        ...
        with (
            ...
            result = app.deleted_tools_with_session(session=unbound_session)
...
class TestAnnotationReplyConfigLoader:
    def test_load_annotation_reply_config_returns_disabled_when_setting_missing(self, sqlite_session: Session):
        result = load_annotation_reply_config(sqlite_session, str(uuid4()))
    ...
    def test_load_annotation_reply_config_returns_embedding_model(self, sqlite_session: Session):
        ...
        result = load_annotation_reply_config(sqlite_session, app_id)
    ...
    def test_load_annotation_reply_config_raises_when_binding_missing(self, sqlite_session: Session):
        ...
        with pytest.raises(ValueError, match="Collection binding detail not found"):
            load_annotation_reply_config(sqlite_session, app_id)
...
class TestConversationModel:
    ...
    def test_conversation_summary_or_query_without_summary(self, unbound_session: Session):
        ...
        with patch.object(Conversation, "first_message_with_session", return_value=mock_message) as get_first_message:
            # Act
            result = conversation.summary_or_query_with_session(session=unbound_session)

            # Assert
            assert result == "First message query"
            get_first_message.assert_called_once_with(session=unbound_session)

    def test_model_config_uses_caller_session_for_annotation_reply(self, sqlite_session: Session):
        app_id = str(uuid4())
        app_model_config = AppModelConfig(app_id=app_id)
        sqlite_session.add(app_model_config)
        sqlite_session.commit()
        conversation = Conversation(
            app_id=app_id,
        ...
        load_config.assert_called_once_with(sqlite_session, app_id)
    ...
    def test_override_model_config_uses_caller_session_for_annotation_reply(self, unbound_session: Session):
        app_id = str(uuid4())
        conversation = Conversation(
            app_id=app_id,
            mode=AppMode.CHAT,
        ...
        with (
            ...
            conversation.model_config_with_session(session=unbound_session)

        load_config.assert_called_once_with(unbound_session, app_id)
        to_dict.assert_called_once_with(annotation_reply=annotation_reply)
