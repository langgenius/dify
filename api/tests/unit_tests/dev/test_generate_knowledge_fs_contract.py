...
def test_committed_routes_preserve_the_upstream_trace_header_contract() -> None:
    routes_without_request_trace = {
        (method, path)
        for method, path, _, _, _, _, request_headers, _, _ in KNOWLEDGE_FS_CONTRACT_OPERATIONS
        if "x-trace-id" not in request_headers
    }
    routes_without_response_trace = {
        (method, path)
        for method, path, _, _, _, _, _, response_headers, _ in KNOWLEDGE_FS_CONTRACT_OPERATIONS
...
@pytest.mark.parametrize(
    ("method", "path", "scope", "media_type", "expected_permission"),
    [
        ("get", "/assets/{id}", "knowledge-spaces:read", "application/octet-stream", "dataset_document_download"),
        (
            "post",
        ...
        ("post", "/knowledge-spaces/{id}/documents", "knowledge-spaces:write", "application/json", "dataset_edit"),
        ("post", "/queries", "knowledge-spaces:read", "application/json", "dataset_readonly"),
        ("post", "/imports", "knowledge-spaces:write", "application/json", "dataset_create_and_management"),
    ],
)
def test_render_routes_assigns_proxy_permissions(
    method: str, path: str, scope: str, media_type: str, expected_permission: str
) -> None:
    route = operation(scope)
    route["responses"] = {"200": {"content": {media_type: {}}}}

    rendered = render_routes({"paths": {path: {method: route}}})

    assert f'"{expected_permission}"' in rendered
