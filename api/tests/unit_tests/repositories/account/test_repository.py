...
def _persist_account(session: Session) -> Account:
    account = Account(
        name="Original", email="account@example.com", normalized_email=normalize_email("account@example.com")
    )
...
def test_account_repository_finds_mixed_case_sso_email_from_lowercase_input(
    sqlite_session: Session,
    sqlite_session_factory: sessionmaker[Session],
) -> None:
    """Workspace invitations must resolve the original casing of an SSO identity (CUS-1658)."""
...
@pytest.mark.parametrize(
    ("email", "expected_name"),
    [("USER@EXAMPLE.COM", "Older"), ("user@example.com", "Newer")],
)
def test_account_repository_prefers_exact_email_then_oldest_normalized_match(
    sqlite_session: Session,
    sqlite_session_factory: sessionmaker[Session],
    email: str,
    expected_name: str,
