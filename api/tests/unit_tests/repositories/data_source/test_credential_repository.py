...
def test_provider_reads_return_scoped_snapshots_and_preserve_default_selection(
    sqlite_session_factory: sessionmaker[Session],
) -> None:
    repository = SQLAlchemyDatasourceCredentialRepository(session_factory=sqlite_session_factory)
    with sqlite_session_factory.begin() as session:
        selected = _provider(is_default=True)
        other = _provider()
        other.name = "Other credential"
        foreign = _provider(workspace_id="workspace-2", is_default=True)
        wrong_provider = _provider(provider="other-provider", is_default=True)
        wrong_plugin = _provider(plugin_id="other-plugin", is_default=True)
        session.add_all([selected, other, foreign, wrong_provider, wrong_plugin])
        selected_id, other_id = selected.id, other.id
        forbidden_ids = [row.id for row in (foreign, wrong_provider, wrong_plugin)]

    records = repository.list_provider_credentials(
        workspace_id="workspace-1", provider="notion_datasource", plugin_id="langgenius/notion_datasource"
    )
    assert [record.id for record in records] == [selected_id, other_id]
    assert all(isinstance(record, DatasourceCredentialRecord) for record in records)
    assert records[0].encrypted_credentials == {"integration_secret": "encrypted"}
    default = repository.get_provider_credential(
    ...
    assert default.id == selected_id
    for credential_id in forbidden_ids:
        assert (
