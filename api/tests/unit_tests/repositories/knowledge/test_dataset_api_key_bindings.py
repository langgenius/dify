...
class TestGetKeyScope:
    def test_unbound_key_is_unrestricted(self, sqlite_session: Session) -> None:
        ...
        scope = dataset_api_key_bindings.get_key_scope(sqlite_session, token.id)
    ...
    def test_other_keys_bindings_are_ignored(self, sqlite_session: Session) -> None:
        ...
        assert not dataset_api_key_bindings.get_key_scope(sqlite_session, token.id).restricted
    ...
    def test_bound_dataset_ids_helper_only_reports_legacy_bindings(self, sqlite_session: Session) -> None:
        ...
        assert dataset_api_key_bindings.get_bound_dataset_ids(sqlite_session, token.id) == {dataset_id}
...
class TestListBindingsByToken:
    def test_groups_each_kind_separately(self, sqlite_session: Session) -> None:
        ...
        bindings = dataset_api_key_bindings.list_bindings_by_token(sqlite_session, [token.id, unbound.id])
    ...
    def test_empty_input(self, sqlite_session: Session) -> None:
        assert dataset_api_key_bindings.list_bindings_by_token(sqlite_session, []) == {}
...
class TestFindUnknownKnowledgeSpaceIds:
    def test_accepts_active_spaces_of_the_tenant_only(self, sqlite_session: Session) -> None:
        ...
        unknown = dataset_api_key_bindings.find_unknown_knowledge_space_ids(
    ...
    def test_empty_input(self, sqlite_session: Session) -> None:
        assert dataset_api_key_bindings.find_unknown_knowledge_space_ids(sqlite_session, [], "tenant") == []
...
class TestBindKnowledgeSpaces:
    def test_persists_typed_rows(self, sqlite_session: Session) -> None:
        ...
        dataset_api_key_bindings.bind_knowledge_spaces(sqlite_session, token.id, [space_id])
        sqlite_session.commit()
...
class TestDeleteKeysScopedOnlyToKnowledgeSpace:
    def test_deletes_key_bound_only_to_the_space(self, sqlite_session: Session) -> None:
        ...
        deleted = dataset_api_key_bindings.delete_keys_scoped_only_to_knowledge_space(sqlite_session, space_id)
        sqlite_session.commit()
    ...
    def test_keeps_key_bound_elsewhere_but_drops_the_space_binding(self, sqlite_session: Session) -> None:
        ...
        assert dataset_api_key_bindings.get_key_scope(sqlite_session, token.id).knowledge_space_ids == frozenset()
        assert dataset_api_key_bindings.get_key_scope(sqlite_session, token.id).dataset_ids == {dataset_id}

    def test_ignores_unrestricted_keys(self, sqlite_session: Session) -> None:
        unbound = _dataset_token(sqlite_session)
        sqlite_session.commit()

        deleted = dataset_api_key_bindings.delete_keys_scoped_only_to_knowledge_space(sqlite_session, str(uuid.uuid4()))
        sqlite_session.commit()
...
def test_dataset_management_excludes_mixed_scope_keys(sqlite_session: Session) -> None:
    token = _dataset_token(sqlite_session)
    assert token.tenant_id is not None
    space = _control_space(sqlite_session, token.tenant_id)
    sqlite_session.flush()
    sqlite_session.add_all(
        [
            DatasetApiTokenBinding(api_token_id=token.id, dataset_id="dataset"),
            _space_binding(token.id, space.id),
        ]
    )
    sqlite_session.commit()
    assert list(sqlite_session.scalars(dataset_api_key_bindings.token_ids_scoped_only_to("dataset"))) == []
