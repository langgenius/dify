...
def _run_one_turn(runner: FunctionCallAgentRunner, mocker: MockerFixture, tool_calls, responses):
    ...
    mocker.patch(
        "core.agent.fc_agent_runner.ToolEngine.agent_invoke",
        side_effect=[(response, [], tool_invoke_meta) for response in responses],
    )
