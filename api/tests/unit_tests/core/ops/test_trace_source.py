...
def test_disable_tracing_does_not_load_provider(
    ...
    assert ops_trace_service.get_app_trace_settings(tenant_id=tenant.id, app_id=app.id) == {
        "enabled": False,
...
def test_enable_tracing_requires_provider_dependencies(
    ...
    with pytest.raises(TraceProviderNotInstalledError):
        ops_trace_service.update_app_trace_settings(
            tenant_id=tenant.id, app_id=app.id, enabled=True, tracing_provider="weave"
        )

    sqlite_session.refresh(app)
    assert app.tracing == previous
