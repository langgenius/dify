...
def test_persist_to_database_updates_existing_and_inserts_new(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    existing = SimpleNamespace(tenant_id=RESOURCE_TENANT_ID, process_data=None, process_data_dict=None)
...
def test_save_execution_data_handles_existing_db_model_and_truncation(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert json.loads(existing.process_data) == {
        "c": 3,
        "workflow_agent_binding_id": "workflow-binding-1",
    }
    assert existing.agent_workspace_binding_id == "authoritative-participant"
...
def test_save_synchronously_bypasses_async_persistence(mock_task, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "core.repositories.sqlalchemy_workflow_node_execution_repository.FileService",
        lambda *_: SimpleNamespace(upload_file=Mock()),
    )
    ...
    db_model = SimpleNamespace(node_execution_id="node-exec")
    persist = Mock()
    monkeypatch.setattr(repo, "_to_db_model", lambda _: db_model)
    monkeypatch.setattr(repo, "_persist_to_database", persist)
    repo.set_async_persistence(True)

    repo.save_synchronously(_execution())

    persist.assert_called_once_with(db_model)
