...
def test_enterprise_traces_use_owned_event_and_respect_enablement(kind, enabled):
    ...
    with (
        ...
        patch("enterprise.telemetry.operation_trace.record_enterprise_operation") as record,
    ):
        emit(event)
    if enabled:
...
def test_trace_recording_error_does_not_interrupt_application(caplog):
    with (
        ...
        patch(
            "enterprise.telemetry.operation_trace.record_enterprise_operation", side_effect=ValueError("private data")
        ),
    ):
