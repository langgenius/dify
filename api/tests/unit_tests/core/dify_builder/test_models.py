...
how dify-enterprise/server/pkg/enterprise/biz/dify_builder/*_test.go construct
...
FixDiagnose}``, ``&DifyBuilderContext{}``, ``&DifyBuilderContext{FailedRunID: "TR-1"}``).
...
def test_fix_context_round_trip_defaults_empty():
    # Mirrors runner_test.go: repo.CreateSession(..., &DifyBuilderContext{}, nil)
...
def test_fix_context_partial_construction():
    # Mirrors handlers_fix_test.go: &DifyBuilderContext{FailedRunID: "TR-1"}
    fc = DifyBuilderContext(failed_run_id="TR-1")
    assert fc.failed_run_id == "TR-1"
    assert fc.source == ""

    # Mirrors handlers_fix_test.go: &DifyBuilderContext{Source: "checklist", ChecklistErrors: errs}
    errs = [ChecklistError(node_id="n1", unconnected=True)]
    fc2 = DifyBuilderContext(source="checklist", checklist_errors=errs)
...
def test_dify_builder_context_recovery_fields_default():
