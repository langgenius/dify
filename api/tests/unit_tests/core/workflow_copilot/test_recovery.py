...
from core.workflow_copilot.state import PcState, is_waiting, is_working
...
def test_entry_state_for_all_modes():
    ...
    mapping below is exactly what services.workflow_copilot.service assigns
...
def test_target_node_ids_fix_guard_branches():
    ...
    assert recovery.target_node_ids(CopilotContext(), EntryMode.FIX) == []

    # a diagnosis with no resolved culprit node.
    fc = CopilotContext(diagnosis=Diagnosis(culprit_node_id=""))
...
def test_check_recovery_classifies_unchanged_via_runner():
    ...
    fc = CopilotContext(last_snapshot_hash=dify.hash, last_structure_fingerprint="stale-fp", next_seq=1)
    s = _seed(repo, EntryMode.BUILD, PcState.BUILD_REVIEW, fc)
    runner = Runner(Env(dify=dify, agent=StubAgent(), repo=repo, now=lambda: datetime.min), {})
...
def test_recovery_restart_in_fix_mode_drives_working_entry_state_to_waiting():
    ...
    fc = CopilotContext(failed_run_id="TR-1", recovery_class="structural_invalidating", next_seq=1)
