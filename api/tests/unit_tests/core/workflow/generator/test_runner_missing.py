...
def test_collect_container_errors():
    ...
    nodes = [
        {"id": "n1", "data": {"type": "llm"}},
        {"id": "n2", "data": {"type": "code", "parentId": "n1"}}
    ]
...
def test_collect_unresolved_refs():
    ...
    # Missing node ref
    nodes = [
        {"id": "n1", "data": {"type": "llm", "prompt_template": [{"text": "{#unknown.var#}"}]}}
    ]
...
def test_postprocess_graph_edges():
    ...
    graph = {
        "nodes": [
            {"id": "sys", "data": {"type": "start"}},
            {"id": "bad id", "data": {"type": "llm"}}
        ],
        "edges": [
            {"source": "sys", "target": "bad id", "id": "edge_1"}
        ]
    }
...
def test_document_extractor_start_vars():
    nodes = [
        {"id": "n1", "data": {"type": "document-extractor", "variable_selector": ["start", "doc"]}}
    ]
