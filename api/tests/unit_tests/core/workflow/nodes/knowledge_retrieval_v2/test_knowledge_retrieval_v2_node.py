...
def test_automatic_metadata_filter_fails_closed_on_missing_model_or_catalog_rejection() -> None:
    ...
    assert (
        missing_model.error == "Agent Knowledge Base automatic metadata filtering requires a metadata filtering model"
    )
...
def test_node_fails_closed_for_binding_rejection_and_invalid_query_type() -> None:
    ...
    assert rejected.error == (
        "[knowledge_fs_binding_not_enabled] Agent Knowledge Base space-a is not bound to this workflow"
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "[knowledge_fs_workflow_access_disabled] Workflow access is disabled for Agent Knowledge Base space-a; "
        ...
        (
            ...
            "[knowledge_fs_space_unavailable] Agent Knowledge Base space-a is not ready for workflow retrieval; "
        ...
        (
            ...
            "[knowledge_fs_authorization_not_ready] Agent Knowledge Base space-a permissions are not ready; "
