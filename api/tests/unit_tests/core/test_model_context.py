...
def test_credit_usage_metadata_is_scoped_and_propagated_to_worker_threads() -> None:
    with use_credit_usage_metadata({"created_by": "workflow"}):
        assert get_credit_usage_metadata() == {"created_by": "workflow"}

        with ThreadPoolExecutor(max_workers=1) as executor:
            read_context = propagate_context(get_credit_usage_metadata)
            assert executor.submit(read_context).result() == {"created_by": "workflow"}

    assert get_credit_usage_metadata() is None


def test_nested_credit_usage_metadata_keeps_the_outer_business_feature() -> None:
    with use_credit_usage_metadata({"created_by": "workflow"}):
        with use_credit_usage_metadata({"created_by": "knowledge_retrieval"}):
            assert get_credit_usage_metadata() == {"created_by": "workflow"}


def test_credit_usage_created_by_decorator_sets_feature() -> None:
    @with_credit_usage_created_by("knowledge_retrieval")
    def read_context() -> object:
        return get_credit_usage_metadata()

    assert read_context() == {"created_by": "knowledge_retrieval"}
