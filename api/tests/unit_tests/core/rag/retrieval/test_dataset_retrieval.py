...
class TestDatasetRetrievalAdditionalHelpers:
    ...
    def test_automatic_metadata_filter_func_passes_field_types_to_prompt(
        ...
        sqlite_session.add_all(
            [
                DatasetMetadata(
                    tenant_id=tenant_id, dataset_id=dataset_id, type=field_type, name=name, created_by=user_id
                )
                for name, field_type in (("author", "string"), ("published_at", "time"))
            ]
        )
        ...
        prompt_kwargs = mock_get_prompt_template.call_args.kwargs
        assert prompt_kwargs["query"] == "published after 2024-01-01"
        assert sorted(prompt_kwargs["metadata_fields"], key=operator.itemgetter("name")) == [
