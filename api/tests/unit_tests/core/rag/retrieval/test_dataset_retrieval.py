...
class TestInternalHooksCoverage:
    ...
    def test_multiple_retrieve_thread_branches(self, retrieval: DatasetRetrieval) -> None:
        flask_app = SimpleNamespace(app_context=lambda: nullcontext())
        dataset = SimpleNamespace(
            id="d1",
            provider="dify",
        ...
        all_documents: list[Document] = []
        cancel_event = SimpleNamespace(is_set=lambda: True, set=Mock())
        ...
        with patch("core.rag.retrieval.dataset_retrieval.threading.Thread", _JoinDrivenThread):
            retrieval._multiple_retrieve_thread(
                flask_app=flask_app,  # type: ignore[arg-type]
                available_datasets=[dataset],
                metadata_condition=SimpleNamespace(),
                metadata_filter_document_ids={"other": []},
                all_documents=all_documents,
                tenant_id="tenant-1",
                reranking_enable=False,
                reranking_mode="reranking_model",
                ...
                attachment_id=None,
                dataset_count=1,
                cancel_event=SimpleNamespace(is_set=lambda: False, set=Mock()),  # type: ignore[arg-type]
                thread_exceptions=[],
            )

        dataset_unknown = SimpleNamespace(
            id="d2",
            provider="dify",
            indexing_technique="other",
            embedding_model="m",
            embedding_model_provider="p",
        )
        with patch.object(retrieval, "_retriever", side_effect=lambda **kwargs: kwargs["all_documents"].append(_doc())):
            retrieval._multiple_retrieve_thread(
                flask_app=flask_app,  # type: ignore[arg-type]
                available_datasets=[dataset_unknown],
                metadata_condition=None,
                metadata_filter_document_ids=None,
                all_documents=all_documents,
                tenant_id="tenant-1",
        ...
        with patch("core.rag.retrieval.dataset_retrieval.threading.Thread", _ImmediateThread):
            with patch("core.rag.retrieval.dataset_retrieval.DataPostProcessor") as mock_post:
                mock_post.return_value.invoke.side_effect = lambda **kwargs: kwargs["documents"]
                with patch.object(
                    retrieval, "_retriever", side_effect=lambda **kwargs: kwargs["all_documents"].append(_doc())
                ):
                    retrieval._multiple_retrieve_thread(
                        flask_app=flask_app,  # type: ignore[arg-type]
                        available_datasets=[dataset_unknown, dataset_unknown],
                        metadata_condition=None,
                        metadata_filter_document_ids=None,
                        all_documents=all_documents,
                        tenant_id="tenant-1",
        ...
        assert mock_post.return_value.invoke.call_count >= 2

        failing_flask = SimpleNamespace(app_context=lambda: (_ for _ in ()).throw(RuntimeError("ctx failed")))
        exceptions: list[Exception] = []
        cancel = SimpleNamespace(set=Mock())
        retrieval._multiple_retrieve_thread(
            flask_app=failing_flask,  # type: ignore[arg-type]
            available_datasets=[dataset],
            metadata_condition=None,
            metadata_filter_document_ids=None,
            all_documents=[],
