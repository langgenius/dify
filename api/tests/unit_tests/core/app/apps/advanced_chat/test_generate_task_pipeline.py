...
def test_save_message_enqueues_message_trace() -> None:
    """When trace_manager is provided, _save_message must enqueue a MESSAGE_TRACE task."""
...
def test_save_message_skips_trace_when_no_trace_manager() -> None:
    """When trace_manager is None, _save_message must not fail."""
    app_config = SimpleNamespace(app_id="app-1", tenant_id="tenant-1", sensitive_word_avoidance=None)
    application_generate_entity = SimpleNamespace(
        app_config=app_config,
        files=[],
