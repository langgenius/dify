...
class DocumentProcessingTestDataFactory:
    ...
    @staticmethod
    def create_mock_process_rule(rule_id: str = "rule-001", mode: str = "automatic", rules: dict | None = None) -> Mock:
...
class TestDocumentParsing:
    ...
    def test_document_parsing_multiple_documents(self, mock_indexing_runner, mock_db_session):
        ...
        documents = [DocumentProcessingTestDataFactory.create_mock_document(document_id=f"doc-{i}") for i in range(3)]
...
class TestChunkingStrategy:
    ...
    def test_get_splitter_validates_token_limits(self):
        ...
        with pytest.raises(ValueError, match="should be between 50"):
            IndexingRunner._get_splitter(processing_rule_mode, 30, chunk_overlap, separator, mock_embedding_instance)
...
class TestFailureRecovery:
    ...
    def test_handles_sandbox_plan_batch_upload_restriction(self, mock_feature_service, mock_db_session):
        ...
        # Arrange
        dataset = DocumentProcessingTestDataFactory.create_mock_dataset()
        documents = [DocumentProcessingTestDataFactory.create_mock_document(document_id=f"doc-{i}") for i in range(2)]

        # Create features with sandbox plan
        features = DocumentProcessingTestDataFactory.create_mock_features(billing_enabled=True, plan=CloudPlan.SANDBOX)
    ...
    def test_handles_batch_upload_limit_exceeded(self, mock_feature_service, mock_db_session):
        ...
        # Arrange
        dataset = DocumentProcessingTestDataFactory.create_mock_dataset()
        documents = [DocumentProcessingTestDataFactory.create_mock_document(document_id=f"doc-{i}") for i in range(15)]
...
class TestTaskIntegration:
    ...
    def test_tenant_queue_with_multiple_waiting_tasks(self, mock_db_session, mock_queue):
        ...
        next_tasks = [
            {"tenant_id": "tenant-456", "dataset_id": "dataset-789", "document_ids": [f"doc-{i}"]} for i in range(3)
        ]
