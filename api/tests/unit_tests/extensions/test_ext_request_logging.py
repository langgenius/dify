...
class TestLoggingLevel:
    ...
    def test_logging_should_be_skipped_if_level_is_above_debug(self, enable_request_logging, caplog):
...
class TestRequestReceiverLogging:
    @pytest.mark.usefixtures("enable_request_logging")
    def test_non_json_request(self, enable_request_logging, caplog, mock_response_receiver):
    ...
    @pytest.mark.usefixtures("enable_request_logging")
    def test_json_request(self, enable_request_logging, caplog, mock_response_receiver):
        caplog.set_level(logging.DEBUG)
        app = _get_test_app()
        init_app(app)
    ...
    @pytest.mark.usefixtures("enable_request_logging")
    def test_json_request_with_empty_body(self, enable_request_logging, caplog, mock_response_receiver):
        caplog.set_level(logging.DEBUG)
        app = _get_test_app()
        init_app(app)
    ...
    @pytest.mark.usefixtures("enable_request_logging")
    def test_json_request_with_invalid_json_as_body(self, enable_request_logging, caplog, mock_response_receiver):
        caplog.set_level(logging.DEBUG)
        app = _get_test_app()
        init_app(app)
        ...
        assert "Failed to parse JSON request" in error_records[0].getMessage()


class TestResponseReceiverLogging:
    @pytest.mark.usefixtures("enable_request_logging")
    def test_non_json_response(self, enable_request_logging, caplog):
        caplog.set_level(logging.DEBUG)
        app = _get_test_app()
        response = Response(
            "OK",
    ...
    @pytest.mark.usefixtures("enable_request_logging")
    def test_json_response(self, enable_request_logging, caplog, mock_response_receiver):
        caplog.set_level(logging.DEBUG)
        app = _get_test_app()
        response = Response(
    ...
    def test_json_request_with_invalid_json_as_body(self, enable_request_logging, caplog, mock_response_receiver):
        ...
        error_records = [r for r in caplog.records if r.levelname == "ERROR"]
        assert len(error_records) == 1
        assert "Failed to parse JSON response" in error_records[0].getMessage()
