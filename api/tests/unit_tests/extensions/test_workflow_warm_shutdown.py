...
def test_warm_shutdown_sets_abort_command() -> None:
    with active_workflow_task("task-a"):
        ...
        run_count = workflow_warm_shutdown.get_active_workflow_task_count()
...
def test_warm_shutdown_abort_command_stays_available_for_late_workflow_runs() -> None:
    workflow_warm_shutdown.set_celery_warm_shutdown_abort_command(reason="worker shutdown")
    assert workflow_warm_shutdown.get_active_workflow_task_count() == 0
