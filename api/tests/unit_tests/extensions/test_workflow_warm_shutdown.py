...
def test_worker_shutting_down_pushes_abort_commands_for_warm_shutdown(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_worker_shutdown_logs_when_all_command_channels_ended(
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
) -> None:
    caplog.set_level(logging.INFO, logger=workflow_warm_shutdown.logger.name)
    monkeypatch.setattr(workflow_warm_shutdown, "get_celery_signal_command_channel_count", lambda: 0)

    workflow_warm_shutdown._on_worker_shutdown()

    assert "after all tracked workflow command channels ended" in caplog.text


def test_worker_shutdown_logs_remaining_command_channels(
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
) -> None:
    caplog.set_level(logging.INFO, logger=workflow_warm_shutdown.logger.name)
    monkeypatch.setattr(workflow_warm_shutdown, "get_celery_signal_command_channel_count", lambda: 2)

    workflow_warm_shutdown._on_worker_shutdown()

    assert "with 2 workflow command channel(s) still active after warm shutdown wait" in caplog.text
