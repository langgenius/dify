...
def _patch_auth_and_router(temp_router):
    ...
    with patch("controllers.fastopenapi.console_router", temp_router), \
        ...
        try:
            ...
            # Reload again in finally block to restore original state if needed
            # (though patch context manager handles the mocking part, reloading ensures clean state)
            importlib.reload(extensions.ext_fastopenapi)
...
def _force_reload_module(target_module: str, alias_module: str):
    ...
    return controllers.console.feature
