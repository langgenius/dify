...
def test_system_features_unauthenticated(app):
    """
    ...
    # Define mocks for this specific test scenario
    def noop(f):
        return f
    ...
    # We deliberately DO NOT patch current_user to be authenticated,
    # or we patch it to raise Unauthorized to simulate the real behavior.
    # However, simpler is to mock it to return False for is_authenticated
    # or let the code handle the Unauthorized exception if we can simulate it.

    # Since the code catches Unauthorized from current_user.is_authenticated,
    # let's simulate exactly that or simply an unauthenticated user.
    ...
    try:
        with (
            ...
            patch("controllers.console.wraps.account_initialization_required", side_effect=noop),
            patch("controllers.console.wraps.cloud_utm_record", side_effect=noop),
            patch("libs.login.current_user", mock_user),
            patch("controllers.console.feature.FeatureService.get_system_features") as mock_service,
        ):
