...
def _patch_snippet_service_factory(
    ...
    monkeypatch.setattr(
        snippets_module,
        "_snippet_service",
        lambda session=None: snippets_module.SnippetService(session=session or sqlite_session),
    )
...
def _persist_snippet(session: Session, snippet: CustomizedSnippet) -> None:
    session.add(snippet)
    session.commit()
...
def test_create_snippet_defaults_unknown_type_and_returns_created(
    ...
    req_data = snippets_module.CreateSnippetPayload.model_construct(
        name="Snippet",
        type="unknown",
        description="Description",
        graph=None,
...
def test_create_snippet_rejects_forbidden_nodes(app: Flask, monkeypatch: pytest.MonkeyPatch, unbound_session: Session):
    user = _account("account-1")
    create_snippet = Mock()
    monkeypatch.setattr(snippets_module.SnippetService, "create_snippet", create_snippet)
    ...
    with app.test_request_context(
        ...
        response, status_code = handler(api, req_data, unbound_session, "tenant-1", user)
...
def test_patch_snippet_updates_persisted_model(app: Flask, monkeypatch: pytest.MonkeyPatch, sqlite_session: Session):
    user = _account("account-1")
    snippet = _snippet()
    _persist_snippet(sqlite_session, snippet)
    ...
    assert snippet.icon_info == {"icon": "star", "icon_background": None, "icon_type": None, "icon_url": None}
    assert snippet.updated_by == user.id


def test_delete_snippet_deletes_persisted_model(app: Flask, monkeypatch: pytest.MonkeyPatch, sqlite_session: Session):
    snippet = _snippet()
    user = _account()
    _persist_snippet(sqlite_session, snippet)
    ...
    assert sqlite_session.get(CustomizedSnippet, snippet.id) is None
...
def test_export_snippet_returns_yaml_attachment(app: Flask, monkeypatch: pytest.MonkeyPatch, sqlite_session: Session):
    ...
    monkeypatch.setattr(
        snippets_module,
        "SnippetDslService",
        Mock(return_value=Mock(export_snippet_dsl=export_snippet_dsl)),
    )
...
def test_import_snippet_returns_202_for_pending_confirmation(
    ...
    monkeypatch.setattr(
        snippets_module,
        "SnippetDslService",
        Mock(return_value=Mock(import_snippet=import_snippet)),
    )
    ...
    with app.test_request_context(
        ...
        response, status_code = handler(api, req_data, unbound_session, user)
...
def test_import_confirm_returns_200_for_completed_import(
    ...
    monkeypatch.setattr(
        snippets_module,
        "SnippetDslService",
        Mock(return_value=Mock(confirm_import=confirm_import)),
    )
    ...
    with app.test_request_context(
        ...
        response, status_code = handler(api, unbound_session, user, import_id="import-1")
...
def test_check_dependencies_returns_dependency_result(
    ...
    snippet = _snippet()
    check_dependencies = Mock(return_value=CheckDependenciesResult(leaked_dependencies=[]))

    monkeypatch.setattr(snippets_module.SnippetService, "get_snippet_by_id", Mock(return_value=snippet))
    monkeypatch.setattr(
        snippets_module,
        "SnippetDslService",
        Mock(return_value=Mock(check_dependencies=check_dependencies)),
    )
...
def test_increment_use_count_returns_persisted_count(
    app: Flask, monkeypatch: pytest.MonkeyPatch, sqlite_session: Session
):
    ...
    assert sqlite_session.scalar(select(CustomizedSnippet.use_count).where(CustomizedSnippet.id == snippet.id)) == 3
