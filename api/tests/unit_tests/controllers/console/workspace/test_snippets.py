...
def test_list_snippets_returns_pagination(app: Flask, monkeypatch: pytest.MonkeyPatch, sqlite_session: Session):
    ...
    with app.test_request_context(
        ...
        response, status_code = handler(api, session, "tenant-1")
...
def test_create_snippet_defaults_unknown_type_and_returns_created(
    ...
    with app.test_request_context(
        ...
        response, status_code = handler(api, req_data, session, "tenant-1", user)
...
def test_get_snippet_detail_raises_when_missing(app: Flask, monkeypatch: pytest.MonkeyPatch, sqlite_session: Session):
    ...
    with app.test_request_context("/workspaces/current/customized-snippets/snippet-1"):
        with pytest.raises(NotFound, match="Snippet not found"):
            handler(api, session, "tenant-1", snippet_id="snippet-1")
...
def test_get_snippet_detail_returns_snippet(app: Flask, monkeypatch: pytest.MonkeyPatch, sqlite_session: Session):
    ...
    with app.test_request_context("/workspaces/current/customized-snippets/snippet-1"):
        response, status_code = handler(api, session, "tenant-1", snippet_id="snippet-1")
...
def test_patch_snippet_returns_400_for_empty_payload(
    ...
    with app.test_request_context(
        ...
        response, status_code = handler(api, req_data, session, "tenant-1", user, snippet_id="snippet-1")

    assert status_code == 400
    assert response == {"message": "No valid fields to update"}


def test_patch_snippet_updates(app: Flask, monkeypatch: pytest.MonkeyPatch, sqlite_session: Session):
    user = _account("account-1")
    snippet = _snippet()
...
def test_delete_snippet_deletes(app: Flask, monkeypatch: pytest.MonkeyPatch, sqlite_session: Session):
    snippet = _snippet()
    user = _account()
    session = sqlite_session
    delete_snippet = Mock()
    ...
    with app.test_request_context("/workspaces/current/customized-snippets/snippet-1", method="DELETE"):
        response, status_code = handler(api, session, "tenant-1", user, snippet_id="snippet-1")
...
def test_export_snippet_returns_yaml_attachment(app: Flask, monkeypatch: pytest.MonkeyPatch, sqlite_session: Session):
    ...
    with app.test_request_context(
        ...
        response = handler(api, session, "tenant-1", snippet_id="snippet-1")
...
def test_increment_use_count_returns_refreshed_count(
    ...
    def increment_use_count(*, session, snippet):
    ...
    increment_use_count_mock.assert_called_once_with(session=session, snippet=snippet)
