...
def test_list_snippets_returns_pagination(app, monkeypatch: pytest.MonkeyPatch):
    snippets = [_snippet()]
...
def test_create_snippet_defaults_unknown_type_and_returns_created(app, monkeypatch: pytest.MonkeyPatch):
...
def test_create_snippet_rejects_forbidden_nodes(app, monkeypatch: pytest.MonkeyPatch):
    user = _account("account-1")
    create_snippet = Mock()
    monkeypatch.setattr(snippets_module.SnippetService, "create_snippet", create_snippet)
...
def test_get_snippet_detail_raises_when_missing(app, monkeypatch: pytest.MonkeyPatch):
...
def test_get_snippet_detail_returns_snippet(app, monkeypatch: pytest.MonkeyPatch):
    snippet = _snippet()
...
def test_patch_snippet_returns_400_for_empty_payload(app, monkeypatch: pytest.MonkeyPatch):
    snippet = _snippet()
...
def test_patch_snippet_updates_and_commits(app, monkeypatch: pytest.MonkeyPatch):
    user = _account("account-1")
    snippet = _snippet()
...
def test_delete_snippet_deletes_and_commits(app, monkeypatch: pytest.MonkeyPatch):
    snippet = _snippet()
    session = SimpleNamespace(merge=Mock(return_value=snippet), commit=Mock())
    delete_snippet = Mock()
...
def test_export_snippet_returns_yaml_attachment(app, monkeypatch: pytest.MonkeyPatch):
...
def test_import_snippet_returns_202_for_pending_confirmation(app, monkeypatch: pytest.MonkeyPatch):
    user = _account("account-1")
...
def test_import_snippet_returns_400_for_failed_import(app, monkeypatch: pytest.MonkeyPatch):
    user = _account("account-1")
...
def test_import_confirm_returns_200_for_completed_import(app, monkeypatch: pytest.MonkeyPatch):
    user = _account("account-1")
...
def test_check_dependencies_raises_when_snippet_missing(app, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(snippets_module.SnippetService, "get_snippet_by_id", Mock(return_value=None))
...
def test_check_dependencies_returns_dependency_result(app, monkeypatch: pytest.MonkeyPatch):
    snippet = _snippet()
...
def test_increment_use_count_raises_when_snippet_missing(app, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(snippets_module.SnippetService, "get_snippet_by_id", Mock(return_value=None))
