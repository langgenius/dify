...
class _Runtime:
    ...
    asr_calls: list[tuple[AudioAppRef, bytes | None, str | None]] = field(default_factory=list)
    tts_calls: list[tuple[AudioAppRef, str, str | None, str | None, str | None]] = field(default_factory=list)

    def transcript_asr(self, *, app: AudioAppRef, audio: AudioUpload | None) -> dict[str, str]:
        self._assert_reference(app)
        self.asr_calls.append(
            (app, audio.stream.read() if audio is not None else None, audio.mime_type if audio else None)
        )
    ...
    def transcript_tts(
        ...
        *,
        ...
        self._assert_reference(app)
        self.tts_calls.append((app, account_id, text, voice, message_id))
        if self.error is not None:
            raise self.error
        return self.output

    def _assert_reference(self, app: AudioAppRef) -> None:
        assert inspect(app, raiseerr=False) is None
        assert app.app_id == self.harness.target_app.id
        assert app.tenant_id == self.harness.target_app.tenant_id
        assert app.tenant_id != self.harness.installed_app.tenant_id
...
def runtime(
    ...
    services = _Services(app_audio=state)
...
def test_asr_preserves_transcript_and_upload_stream_and_mime(runtime: _Runtime, text: str) -> None:
    ...
    app, data, mime_type = runtime.asr_calls[0]
    assert app.app_mode == "completion"
    assert data == _WAV
...
def test_tts_keeps_payload_values_and_authenticated_account(
    ...
    app, account_id, text, voice, message_id = runtime.tts_calls[0]
    assert app.app_mode == "completion"
...
def test_closing_http_response_releases_provider_without_reading_remaining_audio(
    runtime: _Runtime, consume: bool
) -> None:
    stream = _Stream()
    runtime.output = AudioOutput(data=stream, mime_type="audio/mpeg")
    url = f"/installed-apps/{runtime.harness.installed_app.id}/text-to-audio"
    # Dispatch before the WSGI client reads a chunk to cover an unstarted response.
    with runtime.harness.app.test_request_context(url, method="POST", json={"text": "hello"}):
...
@pytest.mark.parametrize("operation", _OPERATIONS)
def test_installed_audio_does_not_record_trial_usage(runtime: _Runtime, operation: _Operation) -> None:
    assert runtime.request(operation).status_code == 200

    with runtime.factory() as session:
        assert session.scalar(select(AccountTrialAppRecord)) is None
        installation = session.get(InstalledApp, runtime.harness.installed_app.id)
        assert installation is not None
        assert installation.last_used_at is None


@pytest.mark.parametrize("operation", _OPERATIONS)
def test_shared_runtime_routes_audio_to_actual_owner_despite_stale_installation_metadata(
    runtime: _Runtime, monkeypatch: pytest.MonkeyPatch, operation: _Operation
) -> None:
    with runtime.factory.begin() as session:
        app = session.get(App, runtime.harness.target_app.id)
        installation = session.get(InstalledApp, runtime.harness.installed_app.id)
        assert app is not None
        assert installation is not None
        assert app.tenant_id != installation.tenant_id
        installation.app_owner_tenant_id = installation.tenant_id
    ...
    services = _Services(app_audio=AppAudioRuntime(session_factory=runtime.factory))
    monkeypatch.setattr(module, "application_services", lambda: services)

    response = runtime.request(operation)

    if operation == "audio-to-text":
        _assert_json_response(response, status=200, body={"text": "识别完成"})
    else:
        assert response.status_code == 200
        assert response.data == _WAV
        assert response.headers["Content-Type"] == "audio/wav"
    assert calls == [(AudioAppRef(app.id, app.tenant_id, app.mode), None)]


@pytest.mark.parametrize("operation", _OPERATIONS)
@pytest.mark.parametrize("change", ["removed", "owner", "mode"])
def test_shared_runtime_rejects_app_changes_after_admission(
    runtime: _Runtime, monkeypatch: pytest.MonkeyPatch, operation: _Operation, change: str
) -> None:
    def change_app() -> None:
        with runtime.factory.begin() as session:
            app = session.get(App, runtime.harness.target_app.id)
    ...
    runtime.harness.state.permission_action = change_app
    services = _Services(app_audio=AppAudioRuntime(session_factory=runtime.factory))
    monkeypatch.setattr(module, "application_services", lambda: services)

    _error(runtime.request(operation), status=400, code="app_unavailable")
    assert runtime.harness.state.permission_calls == [(runtime.harness.account.id, runtime.harness.target_app.id)]
...
@pytest.mark.parametrize(
    ...
    [
        (AppDefinitionUnavailableError("app removed"), 400, "app_unavailable", None),
