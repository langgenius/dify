"""Unit tests for the workflow copilot console JSON routes.

These test the UNDECORATED module-level route-logic functions
(``_create``/``_view``/``_action``/``_message``) and the ``workflow_copilot_required``
...
# --- workflow_copilot_required gate -----------------------------------------
#
...
# `workflow_copilot_required` sits directly below `@with_current_tenant_id`, so
...
def test_workflow_copilot_required_blocks_when_feature_disabled(monkeypatch):
    def dummy(_self, _current_tenant_id, *_args, **_kwargs):
        return {"ok": True}, 200

    wrapped = mod.workflow_copilot_required(dummy)

    features = MagicMock()
    features.workflow_copilot_enabled = False
...
def test_workflow_copilot_required_passes_through_when_enabled(monkeypatch):
    ...
    features = MagicMock()
    features.workflow_copilot_enabled = True
    monkeypatch.setattr(mod.FeatureService, "get_features", lambda _tenant_id: features)
