...
def test_get_app_model_with_trial_injects_model(monkeypatch: pytest.MonkeyPatch) -> None:
    app_model = SimpleNamespace(id="app-1", mode=AppMode.CHAT.value, status="normal", tenant_id="t1")
    query = SimpleNamespace(where=lambda *_args, **_kwargs: query, first=lambda: app_model)
...
def test_get_app_model_with_trial_rejects_wrong_mode(monkeypatch: pytest.MonkeyPatch) -> None:
    app_model = SimpleNamespace(id="app-1", mode=AppMode.CHAT.value, status="normal", tenant_id="t1")
    query = SimpleNamespace(where=lambda *_args, **_kwargs: query, first=lambda: app_model)

    monkeypatch.setattr(wraps_module.db, "session", SimpleNamespace(query=lambda *_args, **_kwargs: query))

    @wraps_module.get_app_model_with_trial(mode=[AppMode.COMPLETION])
    def handler(app_model):
        return app_model.id
...
def test_get_app_model_with_trial_requires_app_id() -> None:
    @wraps_module.get_app_model_with_trial
    def handler(app_model):
        return app_model.id
