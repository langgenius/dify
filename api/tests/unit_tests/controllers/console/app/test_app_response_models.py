...
def test_app_import_response_rejects_unknown_app_mode(app_module):
    with pytest.raises(ValidationError):
        app_module.AppImportResponse.model_validate(
            {
                "id": "import-1",
                "status": "completed",
                "app_id": APP_ID,
...
def test_app_partial_serialization_uses_aliases(app_models, sqlite_session: Session):
    ...
    for invalid_field in (
        {"mode": "unknown"},
        {"access_mode": "unknown"},
        {"model_config": {**serialized["model_config"], "model": ["not", "a", "model"]}},
    ):
        with pytest.raises(ValidationError):
            AppPartial.model_validate({**serialized, **invalid_field})
...
def test_app_detail_with_site_includes_nested_serialization(
    ...
    monkeypatch.setattr(
        "services.app_service.load_annotation_reply_config",
        lambda _session, _app_id: {"enabled": False},
    )
    ...
    for invalid_config in (
        ...
        {"file_upload": {"image": {"enabled": True, "transfer_methods": ["unknown"]}}},
        {
            "dataset_configs": {
                ...
                "metadata_filtering_conditions": {"conditions": [{"name": "source", "comparison_operator": "unknown"}]},
            }
        },
        {"user_input_form": [{"unsupported": {"label": "Name", "variable": "name"}}]},
        {"external_data_tools": [{"enabled": True}]},
    ):
        with pytest.raises(ValidationError):
            app_models.AppDetailWithSite.model_validate({**detail, "model_config": {**model_config, **invalid_config}})

    supported_config = app_models.AppDetailWithSite.model_validate(
        {
            **detail,
            "model_config": {
                **model_config,
                    ...
                    "tools": [
                        {
                            ...
                            "provider_id": "search",
                            "tool_name": "lookup",
                            "tool_parameters": {"key": "[masked]"},
                            "enabled": True,
                        },
                        {"dataset": {"enabled": True, "id": "dataset-id"}},
                    ],
                    "prompt": {"first_prompt": "Find the answer", "next_iteration": "Continue"},
                    "max_iteration": 5,
                },
            },
        }
    ).model_dump(mode="json")
    assert supported_config["model_config"]["external_data_tools"][0]["label"] == "Customer lookup"
    assert supported_config["model_config"]["external_data_tools"][1] == {"enabled": False}
    assert supported_config["model_config"]["agent_mode"]["tools"][0]["tool_parameters"] == {"key": "[masked]"}
...
def test_app_detail_update_preserves_access_mode_and_permissions(
    app, app_module, sqlite_session: Session, monkeypatch: pytest.MonkeyPatch
):
    app_obj = _persist_response_graph(sqlite_session)
    monkeypatch.setattr(
        app_module,
        ...
        lambda: SimpleNamespace(
            update_app=lambda *_args, **_kwargs: app_obj,
            get_app=lambda app, *, session: app,
        ),
    )
    monkeypatch.setattr(app_module.SystemFeatureService, "is_webapp_auth_enabled", lambda: True)
    monkeypatch.setattr(
        app_module.EnterpriseService.WebAppAuth,
        "get_app_access_mode_by_id",
        lambda *, app_id: SimpleNamespace(access_mode="sso_verified"),
    )
    monkeypatch.setattr(
        app_module.enterprise_rbac_service.RBACService.MyPermissions,
        "get",
        lambda *_args, **_kwargs: app_module.enterprise_rbac_service.MyPermissionsResponse(
            app=app_module.enterprise_rbac_service.ResourcePermissionSnapshot(default_permission_keys=["app.acl.edit"])
        ),
    )

    with app.test_request_context("/apps/app-1", method="PUT"):
        response = _unwrap(app_module.AppApi.put)(
            app_module.AppApi(),
            app_module.UpdateAppPayload(name="Updated"),
            sqlite_session,
            TENANT_ID,
            _account(),
            app_obj,
    ...
    assert response["access_mode"] == "sso_verified"
    assert response["permission_keys"] == ["app.acl.edit"]


def test_app_detail_update_masks_agent_tool_parameters(
    app, app_module, sqlite_session: Session, monkeypatch: pytest.MonkeyPatch
):
    ...
    model_config = app_obj.app_model_config_with_session(session=sqlite_session)
    assert model_config is not None
    model_config.agent_mode = json.dumps(
        {
            "enabled": True,
            "strategy": None,
            "tools": [
    ...
    monkeypatch.setattr(app_service.AppService, "update_app", lambda self, *_args, **_kwargs: app_obj)
    account = _account()
    account._current_tenant = Tenant(name="Tenant")
    account._current_tenant.id = TENANT_ID
    monkeypatch.setattr(app_service, "current_user", account)
    monkeypatch.setattr(
        app_service,
        "mask_agent_tool_parameters",
        lambda **kwargs: {
            **kwargs["agent_mode"],
            "tools": [{**kwargs["agent_mode"]["tools"][0], "tool_parameters": {"key": "[masked]"}}],
        },
    )
    ...
    assert response["model_config"]["agent_mode"]["tools"][0]["tool_parameters"] == {"key": "[masked]"}
