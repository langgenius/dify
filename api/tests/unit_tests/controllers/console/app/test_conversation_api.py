...
def _captured_where_clauses(monkeypatch: pytest.MonkeyPatch, app: Flask, endpoint_class, query_class, app_mode):
    """
...
def test_completion_conversation_end_time_filter_is_inclusive(app: Flask, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    end_clauses = [c for c in clauses if "created_at" in c and "14:30:59" in c]
...
def test_chat_conversation_end_time_filter_is_inclusive(app: Flask, monkeypatch: pytest.MonkeyPatch) -> None:
    """ChatConversationApi must also use <= for the end-time boundary (control)."""
    clauses = _captured_where_clauses(
        monkeypatch,
        app,
