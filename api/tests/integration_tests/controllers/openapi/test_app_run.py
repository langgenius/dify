...
def test_run_chat_dispatches_to_chat_handler(flask_app: Flask, account_token, app_in_workspace, monkeypatch: pytest.MonkeyPatch):
    captured = {}
...
def test_run_chat_without_query_returns_422(flask_app: Flask, account_token, app_in_workspace, monkeypatch: pytest.MonkeyPatch):
    client = flask_app.test_client()
    res = client.post(
        f"/openapi/v1/apps/{app_in_workspace.id}/run",
...
def test_run_completion_dispatches_to_completion_handler(flask_app: Flask, account_token, app_with_mode, monkeypatch: pytest.MonkeyPatch):
...
def test_run_workflow_with_query_returns_422(flask_app: Flask, account_token, app_with_mode, monkeypatch: pytest.MonkeyPatch):
...
def test_run_workflow_no_query_dispatches_to_workflow_handler(flask_app: Flask, account_token, app_with_mode, monkeypatch: pytest.MonkeyPatch):
    app = app_with_mode("workflow")
...
def test_run_unsupported_mode_returns_422(flask_app: Flask, account_token, app_with_mode, monkeypatch: pytest.MonkeyPatch):
...
def test_run_with_insufficient_scope_returns_403(flask_app: Flask, account_token, app_in_workspace, monkeypatch: pytest.MonkeyPatch):
...
def test_run_streaming_returns_event_stream(flask_app: Flask, account_token, app_in_workspace, monkeypatch: pytest.MonkeyPatch):
