...
2. Navigate to: `GET /console/api/github/oauth/authorize?app_id=<optional>`
3. Complete OAuth flow on GitHub
4. Verify callback creates connection

#### Test Connection Management
1. `GET /console/api/github/connections` - List connections
2. `POST /console/api/github/connections` - Create connection (after OAuth)
3. `GET /console/api/github/connections/<id>` - Get connection
4. `PATCH /console/api/github/connections/<id>` - Update connection
5. `DELETE /console/api/github/connections/<id>` - Delete connection

#### Test Repository Operations
1. `GET /console/api/github/connections/<id>/repositories` - List repositories
2. `GET /console/api/github/connections/<id>/branches` - List branches

## Known Issues Fixed

1. ✅ **Dataclass field ordering** - Fixed by adding default values to required fields
2. ✅ **SSRF proxy import** - Changed from `get_ssrf_httpx_client()` to `make_request()`
3. ✅ **Indentation error** - Fixed in `_exchange_code_for_token` method
