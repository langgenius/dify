...
class Dataset(Base):
    ...
    def doc_metadata(self):
        dataset_metadatas = db.session.scalars(select(DatasetMetadata).where(DatasetMetadata.dataset_id == self.id)).all()
...
class ExternalKnowledgeApis(Base):
    ...
    def dataset_bindings(self):
        external_knowledge_bindings = (
            db.session.scalars(select(ExternalKnowledgeBindings).where(ExternalKnowledgeBindings.external_knowledge_api_id == self.id)).all()
        )
