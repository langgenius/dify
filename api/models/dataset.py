...
class Dataset(Base):
    ...
    keyword_number = mapped_column(db.Integer, nullable=True, server_default=db.text("10"))
    ...
    enable_api = mapped_column(db.Boolean, nullable=False, server_default=db.text("true"))
...
class PipelineBuiltInTemplate(Base):  # type: ignore[name-defined]
    ...
    icon = mapped_column(db.JSON, nullable=False)
    yaml_content = mapped_column(db.Text, nullable=False)
...
class Pipeline(Base):  # type: ignore[name-defined]
    ...
    is_published = mapped_column(db.Boolean, nullable=False, server_default=db.text("false"))
...
class DocumentPipelineExecutionLog(Base):
    ...
    datasource_info = mapped_column(db.Text, nullable=False)
    datasource_node_id = mapped_column(db.String(255), nullable=False)
    input_data = mapped_column(db.JSON, nullable=False)
    created_by = mapped_column(StringUUID, nullable=True)
    created_at = mapped_column(db.DateTime, nullable=False, server_default=func.current_timestamp())
...
class PipelineRecommendedPlugin(Base):
    ...
    id = mapped_column(StringUUID, server_default=db.text("uuidv7()"))
    plugin_id = mapped_column(db.Text, nullable=False)
    provider_name = mapped_column(db.Text, nullable=False)
