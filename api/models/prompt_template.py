...
class PromptTemplate(db.Model):  # type: ignore[name-defined]
    ...
    versions = db.relationship(
        "PromptVersion", back_populates="prompt_template", cascade="all, delete-orphan", lazy="dynamic"
    )
    ...
    def get_latest_version(self):
        ...
        """
        return self.versions.order_by(desc(PromptVersion.created_at)).first()  # type: ignore

    def to_dict(self):
        latest_version = self.get_latest_version()
        model_settings = None
        if latest_version:
            model_settings = {"model_name": latest_version.model_name, "parameters": latest_version.model_parameters}
...
class PromptVersion(db.Model):  # type: ignore[name-defined]
