...
from models.workflow import WorkflowRun, WorkflowAppLog, WorkflowNodeExecutionModel,ConversationVariable
...
def clean_workflow_runlogs_precise():
    ...
    try:
        total_workflow_runs = db.session.query(WorkflowRun).filter(
            WorkflowRun.created_at < cutoff_date
        ).count()
        if total_workflow_runs == 0:
            _logger.info("No expired workflow run logs found")
            return
        _logger.info(f"Found {total_workflow_runs} expired workflow run logs to clean")
        ...
        while True:
            workflow_runs = db.session.query(WorkflowRun.id).filter(
                WorkflowRun.created_at < cutoff_date
            ...
            if success:
                ...
                if failed_batches >= MAX_RETRIES:
                    _logger.error(f"Failed to delete batch after {MAX_RETRIES} retries, aborting cleanup for today")
                    break
                else:
                    # Calculate incremental delay times: 5, 10, 15 minutes
                    retry_delay_minutes = failed_batches * 5
                    _logger.warning(f"Batch deletion failed, retrying in {retry_delay_minutes} minutes...")
                    time.sleep(retry_delay_minutes * 60)
                    continue
        
        _logger.info(f"Cleanup completed: {total_deleted} expired workflow run logs deleted")
...
def _delete_batch_with_retry(workflow_run_ids: List[str], attempt_count: int) -> bool:
    ...
    try:
        with db.session.begin_nested():
            message_data = db.session.query(Message.id, Message.conversation_id).filter(
                Message.workflow_run_id.in_(workflow_run_ids)
            ).all()
            message_id_list = [msg.id for msg in message_data]
            conversation_id_list = list(set([msg.conversation_id for msg in message_data if msg.conversation_id]))
            if message_id_list:
                ...
                db.session.query(MessageChain).filter(
                    MessageChain.message_id.in_(message_id_list)
                ).delete(synchronize_session=False)
                
                db.session.query(MessageFile).filter(
                    MessageFile.message_id.in_(message_id_list)
                ).delete(synchronize_session=False)
                
                db.session.query(MessageAnnotation).filter(
                    MessageAnnotation.message_id.in_(message_id_list)
                ).delete(synchronize_session=False)
                
                db.session.query(MessageFeedback).filter(
                    MessageFeedback.message_id.in_(message_id_list)
                ).delete(synchronize_session=False)
            ...
            db.session.query(WorkflowAppLog).filter(
                WorkflowAppLog.workflow_run_id.in_(workflow_run_ids)
            ).delete(synchronize_session=False)
            ...
            if conversation_id_list:
                db.session.query(ConversationVariable).filter(
                    ConversationVariable.conversation_id.in_(conversation_id_list)).delete(synchronize_session=False)
                
                db.session.query(Conversation).filter(
                    Conversation.id.in_(conversation_id_list)
                ).delete(synchronize_session=False)
        ...
        db.session.rollback()
        _logger.error(f"Batch deletion failed (attempt {attempt_count + 1}): {str(e)}")
        return False
