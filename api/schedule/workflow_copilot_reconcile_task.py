"""Beat sweeper for interrupted workflow-copilot sessions (P3b Task 5).
...
live advance lock past ``WORKFLOW_COPILOT_MAX_ADVANCE_SECONDS`` of
...
``schedule/*`` beat tasks -- NOT the ``workflow_copilot`` queue used by
``tasks.workflow_copilot_advance_task``. The ``FlaskTask`` base
...
from core.workflow_copilot.state import PcState, is_working
from extensions.ext_database import db
from libs.datetime_utils import naive_utc_now
from models.workflow_copilot import CopilotSession
...
def reconcile_interrupted_sessions() -> int:
    ...
    stale_before = naive_utc_now() - timedelta(seconds=dify_config.WORKFLOW_COPILOT_MAX_ADVANCE_SECONDS)
    factory = _session_factory()
    with factory() as db_session:
        rows = db_session.execute(
            select(CopilotSession.id, CopilotSession.current_state)
            .where(CopilotSession.current_state.in_(_WORKING_STATES))
            .where(CopilotSession.updated_at < stale_before)
        ).all()
    ...
    for sid, state in rows:
        ...
        logger.warning("workflow copilot session %s interrupted (state=%s, no live lock)", sid, state)
        flagged += 1
    return flagged
