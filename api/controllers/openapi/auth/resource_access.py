...
class CheckResourceAccess(Requirement):
    # CheckSubject rejects unsupported routes first, then bindings are loaded
    # before any app or workspace requirements can read their context.
    rank = Rank.FIRST

    @override
    def run(self, subject: Subject, ctx: Context, session: Session) -> None:
        row = session.scalar(select(ResourceAccessToken).where(ResourceAccessToken.id == str(subject.token_id)))
        if row is None:
            raise Unauthorized("invalid_bearer")
