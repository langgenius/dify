...
# API Response Models
performance_summary_fields = console_ns.model(
...
node_performance_fields = console_ns.model(
    "NodePerformance",
    {
...
bottleneck_fields = console_ns.model(
    "Bottleneck",
    {
...
optimization_recommendation_fields = console_ns.model(
...
cache_statistics_fields = console_ns.model(
    "CacheStatistics",
    {
...
class WorkflowOptimizationRecommendationsAPI(Resource):
    ...
    def get(self, app_model, workflow_id):
        ...
        parser = reqparse.RequestParser()
        parser.add_argument(
            "severity", type=str, location="args", choices=["info", "low", "medium", "high", "critical"]
        )
        parser.add_argument(
            "category",
            type=str,
            location="args",
            choices=["performance", "cost", "reliability", "scalability", "best_practice"],
        )
        args = parser.parse_args()
...
class InvalidateCacheAPI(Resource):
    ...
    def post(self, app_model, workflow_id):
        ...
        parser.add_argument(
            "older_than_hours", type=int, location="json", help="Invalidate entries older than specified hours"
        )
        args = parser.parse_args()
        ...
        return {"invalidated_count": count, "message": f"Successfully invalidated {count} cache entries"}
...
class CleanupCacheAPI(Resource):
    ...
    def post(self, app_model, workflow_id):
        ...
        return {"cleaned_count": count, "message": f"Successfully cleaned up {count} expired cache entries"}
