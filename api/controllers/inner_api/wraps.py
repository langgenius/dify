...
def billing_inner_api_only(view: Callable[P, R]):
    ...
    def decorated(*args: P.args, **kwargs: P.kwargs):
        ...
        if not inner_api_key or not hmac.compare_digest(inner_api_key, dify_config.INNER_API_KEY):
            abort(401)
...
def plugin_inner_api_only(view: Callable[P, R]):
    ...
    def decorated(*args: P.args, **kwargs: P.kwargs):
        ...
        # get header 'X-Inner-Api-Key'
        inner_api_key = request.headers.get("X-Inner-Api-Key")
        if not inner_api_key or not hmac.compare_digest(inner_api_key, dify_config.INNER_API_KEY_FOR_PLUGIN):
            abort(404)
