...
def _dict_to_workflow_node_execution_model(data: dict[str, Any]) -> WorkflowNodeExecutionModel:
    ...
    model.triggered_from_workflow_id = data.get("triggered_from_workflow_id") or None
    model.triggered_from_node_execution_id = data.get("triggered_from_node_execution_id") or None
...
class LogstoreAPIWorkflowNodeExecutionRepository(DifyAPIWorkflowNodeExecutionRepository):
    ...
    def get_workflow_tool_executions(
        ...
        parents = self.logstore_client.execute_sql(
            sql=f"""
                SELECT id, node_execution_id, workflow_id FROM "{AliyunLogStore.workflow_node_execution_logstore}"
        ...
        parent_workflow_id = escape_identifier(parents[0]["workflow_id"])
                ...
                sql=f"""
                    SELECT * FROM (
                        ...
                        WHERE {sql_filters}
                          ...
                          AND triggered_from_node_execution_id = '{parent_execution_id}'
                          AND triggered_from_workflow_id = '{parent_workflow_id}'
                    ) AS executions WHERE rn = 1
