...
- **Agent Workspace Binding**：Agent 在某个 Workspace 中的一次具体参与，同时拥有独立的 Materialized Home 和 session 状态。
...
| Product caller 到 Binding 的准确指针 | 对应 conversation、build draft 或 workflow node execution row |
...
flowchart TD
    ...
    Caller["Product Caller"] -->|stores Binding ID| Binding
...
Binding 表达“某个 Agent 以某个配置和基础 Home Snapshot 参与某个 Workspace”的具体 participant。`AgentWorkspaceBinding.id` 同时是 participant、Materialized Home 和持久 Agenton session 的逻辑身份。
...
同一个 Agent 也可以在同一个 Workspace 中拥有多个 ACTIVE Bindings。每个 Binding 的 Materialized Home、session snapshot 和 HITL pending state 相互独立。

participant 的业务调用方在自己的 row 上保存准确的 `AgentWorkspaceBinding.id`。后续运行、文件访问、Build Apply 和 retire 都只解析该 ID，不通过 Agent、Workspace、配置 generation、候选数量或时间顺序反向选择 Binding。
...
| Binding | 保存 Binding ID 的 Product caller | caller 结束、显式 reset 或上层资源批量退出 |
...
| Agent request | 保留 | caller pointer 为空时创建，否则准确复用 | 创建或复用 | acquire / release |
| request 结束 | 保留 | 保留 | 保留 | release |
| request 后浏览文件 | 保留 | 按 caller pointer 准确复用 | 复用当前 Workspace | acquire / release |
| Build Apply | 从 build Binding 创建新 Snapshot | retire build Binding | 最后一个 Binding 退出时 retire | checkpoint 操作内 acquire / release |
| Build Discard | 保留 | retire draft 指向的 Binding | 最后一个 Binding 退出时 retire | 无 |
| Conversation 删除 | 保留 | retire conversation 指向的 Binding | 最后一个 Binding 退出时 retire | 无 |
...
| 字段 | 含义 |
|---|---|
| `id` | participant、Materialized Home 和持久 session 的逻辑身份 |
| `tenant_id`, `app_id` | 租户与应用边界 |
...
| `status`, `retired_at` | 生命周期 |
| `created_at`, `updated_at` | 时间信息 |

Binding 不再包含 `active_guard`，也不再使用 `(tenant_id, workspace_id, agent_id, active_guard)` 唯一约束。主键 `id` 是 participant 的唯一身份；同一 Agent、Workspace、Home Snapshot 和配置 generation 可以对应多个 ACTIVE Bindings。

### 6.4 Product caller pointers

实际拥有 participant 的三个业务表保存 nullable logical pointer：
...
pointer 为空表示该 caller 尚未创建 participant。首次创建时，Binding row 与 caller pointer 在同一个数据库事务中写入；pointer 非空时只加载该 Binding。Binding missing、已 RETIRED、owner 不匹配或配置 generation 不匹配时 fail fast，不搜索替代 Binding，也不自动重建。

这些字段不建立外键、唯一索引或 lookup index。业务历史 row 可以比已完成物理 collection 的 Binding row 保留更久。

Workflow 中，`workflow_agent_binding_id` 仍表示 `WorkflowAgentNodeBinding.id` 的配置身份；`agent_workspace_binding_id` 才表示 runtime participant，两者不能混用。
...
3. 后端返回不透明 ref；
4. Dify API 在同一事务中写入 ACTIVE Workspace/Binding 账本和 Product caller pointer，并提交。
...
1. **同步 retire**：在当前产品事务中把 ACTIVE 改为 RETIRED 并写入 `retired_at`；Workspace retirement 同时清除 Workspace 的 `active_guard`。
...
get_active_binding(session, tenant_id, binding_id, expected_owner_scope) -> Binding?
...
- 每次显式 materialization 创建新的 Binding；
- 按 Product caller 保存的 Binding ID 准确加载并校验 owner/config/Home generation；
...
`AgentAppWorkspaceStore` 和 `WorkflowAgentWorkspaceStore` 把具体业务上下文转换为统一的 `WorkspaceOwnerScope`，并通过 Product caller row 上的 `agent_workspace_binding_id` 解析或创建 Binding。

它们不创建新的资源抽象，只负责：

- conversation、DEBUG_BUILD draft 和 workflow node execution caller 的映射；
- 首次创建时在同一事务中写入 Binding 与 caller pointer；
- 后续调用按 pointer exact resolve，不选择候选 Binding；
- Binding session snapshot 的读取和保存；
- workflow terminal 时的 Workspace retirement。

### 8.4 文件访问服务

产品文件接口接受保存 participant pointer 的准确 caller：normal Agent App 使用 conversation，build 使用 DEBUG_BUILD draft，workflow 使用 node execution。Dify API 先完成 tenant、app、account 和 owner 校验，再从 caller pointer 解析准确的 ACTIVE Binding，最后把 `backend_binding_ref` 发送给 Dify Agent。
...
其中 `binding_id` 已经是 participant identity。每次 create 都必须物化新的 Materialized Home，backend 不按 Agent、Workspace、Home Snapshot 或配置 generation 去重。
...
- 同一个 Agent 的多个 Bindings 共享 Workspace，但各自拥有独立 Materialized Home；
...
因此 current E2B 在第二个 Binding 尝试接入现有 Workspace 时明确失败，不创建 fallback Workspace，也不切换 backend。
...
sequenceDiagram
    ...
    Product->>API: exact caller + Agent/config
    API->>DB: load caller and Binding pointer
    alt pointer 为空
        ...
        API->>DB: commit ACTIVE Workspace/Binding + caller pointer
    else pointer 非空
        API->>DB: exact load Binding and validate owner/generation
    end
...
1. 客户端提交 conversation、DEBUG_BUILD draft 或 workflow node execution 等准确 caller locator；
2. Dify API 校验当前用户对该产品对象的访问权；
3. API 从 caller pointer 解析准确的 ACTIVE Binding；
...
1. 从 DEBUG_BUILD draft 的 `agent_workspace_binding_id` 加载并校验准确的 ACTIVE Binding；
...
| 目录/文件 | 角色 |
|---|---|
| `api/models/agent.py` | Home Snapshot、Workspace、Binding 和 Build Draft pointer |
| `api/models/model.py` | Conversation caller pointer |
| `api/models/workflow.py` | Workflow node execution caller pointer |
...
| `api/core/app/apps/agent_app/session_store.py` | conversation/build caller pointer 到 Binding 的映射 |
| `api/core/workflow/nodes/agent_v2/session_store.py` | workflow node execution pointer 到 Binding 的映射和终态 retire |
...
- caller pointer 指向 missing、RETIRED 或错误 owner 的 Binding；
