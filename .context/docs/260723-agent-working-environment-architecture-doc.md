...
产品流程保存准确的 `AgentWorkspaceBinding.id`。后续运行、文件访问、Build Apply 和 retire 都只解析该 ID，不通过 Agent、Workspace、配置 generation、候选数量或时间顺序反向选择 Binding。
...
| Binding | Agent 与 Workspace 的具体参与关系 | 对应产品流程结束、显式 reset 或上层资源批量退出 |
...
| Agent request | 保留 | 尚未关联 Binding 时创建，否则准确复用 | 创建或复用 | acquire / release |
| request 结束 | 保留 | 保留 | 保留 | release |
| request 后浏览文件 | 保留 | 准确复用 | 复用当前 Workspace | acquire / release |
...
### 6.4 业务对象的 Binding 引用

三个业务表保存 nullable Binding ID：
...
字段为空表示对应业务对象尚未创建 participant。首次创建时，Binding row 与该字段在同一个数据库事务中写入；字段非空时只加载该 Binding。Binding missing、已 RETIRED、owner 不匹配或配置 generation 不匹配时 fail fast，不搜索替代 Binding，也不自动重建。
...
3. 后端返回不透明 ref；
4. Dify API 在同一事务中写入 ACTIVE Workspace/Binding 账本和业务对象的 Binding ID，并提交。
...
- 按业务对象保存的 Binding ID 准确加载并校验 owner/config/Home generation；
...
`AgentAppWorkspaceStore` 和 `WorkflowAgentWorkspaceStore` 把具体业务上下文转换为统一的 `WorkspaceOwnerScope`，并通过业务 row 上的 `agent_workspace_binding_id` 解析或创建 Binding。

它们不创建新的资源抽象，只负责：

- conversation、DEBUG_BUILD draft 和 workflow node execution 的映射；
- 首次创建时在同一事务中写入 Binding 与业务 row 的 Binding ID；
- 后续调用按 Binding ID exact resolve，不选择候选 Binding；
- Binding session snapshot 的读取和保存；
- workflow terminal 时的 Workspace retirement。

### 8.4 文件访问服务

产品文件接口接受准确的业务 locator：normal Agent App 使用 conversation，build 使用 DEBUG_BUILD draft，workflow 使用 node execution。Dify API 先完成 tenant、app、account 和 owner 校验，再解析该业务对象关联的 ACTIVE Binding，最后把 `backend_binding_ref` 发送给 Dify Agent。
...
sequenceDiagram
    ...
    Product->>API: product locator + Agent/config
    API->>DB: load business row and Binding ID
    alt 尚未关联 Binding
        ...
        API->>DB: commit ACTIVE Workspace/Binding + business reference
    else 已有关联 Binding
...
1. 客户端提交 conversation、DEBUG_BUILD draft 或 workflow node execution 等准确产品 locator；
2. Dify API 校验当前用户对该产品对象的访问权；
3. API 解析该业务对象关联的 ACTIVE Binding；
...
| 目录/文件 | 角色 |
|---|---|
| `api/models/agent.py` | Home Snapshot、Workspace、Binding 和 Build Draft 的 Binding ID |
| `api/models/model.py` | Conversation 的 Binding ID |
| `api/models/workflow.py` | Workflow node execution 的 Binding ID |
...
| `api/core/app/apps/agent_app/session_store.py` | conversation/build 业务对象到 Binding 的映射 |
| `api/core/workflow/nodes/agent_v2/session_store.py` | workflow node execution 到 Binding 的映射和终态 retire |
...
- 业务对象引用 missing、RETIRED 或错误 owner 的 Binding；
