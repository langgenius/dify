...
2. **已验证** - ESLint 和 TypeScript 检查均未发现模块缺失错误
3. **可回滚** - 所有更改可通过 Git 轻松回滚
...
2. 添加 Git pre-commit hook 阻止提交未使用代码
3. 在 `package.json` 中添加 `knip` 脚本：
   ```json
   {
     "scripts": {
...
2. ✅ 删除未使用的图标组件（已确认使用动态 URL）
3. ✅ 删除旧实现的配置页面（已被新版替代）
4. ⏸️ 保留有疑问的 service 层导出
5. ⏸️ 保留可能的动态引用
