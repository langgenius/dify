...
function Confirm({
  ...
  return createPortal(
    <div
      className={
        'fixed inset-0 z-[10000000] flex items-center justify-center bg-background-overlay'
      }
      onClick={(e) => {
        e.preventDefault()
        e.stopPropagation()
    ...
    >
      <div
        ref={dialogRef}
        className={'relative w-full max-w-[481px] overflow-hidden'}
      >
        <div className="shadows-shadow-lg flex max-w-full flex-col items-start rounded-2xl border-[0.5px] border-solid border-components-panel-border bg-components-panel-bg">
          <div className="flex flex-col items-start gap-2 self-stretch pb-4 pl-6 pr-6 pt-6">
            ...
            <div className="system-md-regular w-full text-text-tertiary">
              {content}
            </div>
          </div>
          {email && (
            <div className="w-full space-y-1 px-6 py-3">
              <div className="system-sm-semibold py-1 text-text-secondary">
                {t('education.emailLabel')}
              </div>
              <div className="system-sm-regular rounded-lg bg-components-input-bg-disabled px-3 py-2 text-components-input-text-filled-disabled">
                {email}
              </div>
            </div>
          )}
          <div className="flex items-center justify-between gap-2 self-stretch p-6">
            ...
            <Button variant="primary" className="!w-20" onClick={onConfirm}>
