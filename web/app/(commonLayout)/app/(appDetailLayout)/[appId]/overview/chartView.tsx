...
import { AvgRAGDelayTime, AvgResponseTime, AvgSessionInteractions, AvgUserInteractions, ConversationsChart, CostChart, EndUsersChart, TokenPerSecond, UserSatisfactionRate, WorkflowCostChart, WorkflowDailyTerminalsChart, WorkflowMessagesChart } from '@/app/components/app/overview/appChart'
...
export default function ChartView({ appId }: IChartViewProps) {
  ...
  return (
    <div>
      ...
      {!isWorkflow && (
        <div className='grid gap-6 grid-cols-1 xl:grid-cols-2 w-full mb-6'>
          <AvgRAGDelayTime period={period} id={appId}/>
        </div>
      )}
    </div>
  )
