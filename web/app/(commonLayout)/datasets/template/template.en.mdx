...
<div>
  ...
  Service API of iEchor authenticates using an `API-Key`.
    ...
    <Properties>
      <Property name='data' type='multipart/form-data json string' key='data'>
        - original_document_id Source document ID (optional)
          - Used to re-upload the document or moiechor the document cleaning and segmentation configuration. The missing information is copied from the source document
