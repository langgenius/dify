...
export default function RoleRouteGuard({ children }: { children: ReactNode }) {
  const currentWorkspaceRoleQuery = useQuery(
    consoleQuery.workspaces.current.post.queryOptions({
      select: (workspace) => workspace.role,
    }),
  )
  const pathname = usePathname()
  const shouldGuardRoute = datasetOperatorRedirectRoutes.some((route) =>
    isPathUnderRoute(pathname, route),
  )
  ...
  if (shouldGuardRoute && currentWorkspaceRoleQuery.isPending) return <Loading type="app" />

  if (shouldRedirect) redirect('/datasets')
