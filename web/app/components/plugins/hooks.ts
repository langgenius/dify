  ...
  const categories = useMemo(() => {
    return categoryKeys.map((category) => {
      ...
      return {
        name: category,
        label: isSingle
          ? t(`categorySingle.${category}`, { ns: 'plugin' })
          : t(`category.${category}s`, { ns: 'plugin' }),
      }
    })
  }, [t, isSingle])
...
export function usePluginsWithLatestVersion(
  plugins: PluginDetail[] = EMPTY_PLUGINS,
): PluginDetail[] {
  const marketplacePluginIds = useMemo(
    () => plugins.filter((p) => p.source === PluginSource.marketplace).map((p) => p.plugin_id),
    [plugins],
  )

  const { data: latestVersionData } = useQuery(
    consoleQuery.plugins.latestVersions.queryOptions({
