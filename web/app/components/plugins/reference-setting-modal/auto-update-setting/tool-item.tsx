...
const ToolItem: FC<Props> = ({ payload, isChecked, onCheckChange }) => {
  ...
  return (
    <div className="p-1">
      <div className="flex w-full items-center rounded-lg pr-2 select-none hover:bg-state-base-hover">
        <div className="flex h-8 grow items-center space-x-2 pr-2 pl-3">
          ...
          <div className="max-w-[150px] shrink-0 truncate system-sm-medium text-text-primary">
            {renderI18nObject(label, language)}
          </div>
          <div className="max-w-[150px] shrink-0 truncate system-xs-regular text-text-quaternary">
            {org}
          </div>
        </div>
        <Checkbox
