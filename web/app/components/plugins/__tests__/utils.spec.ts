...
const createPlugin = (
  overrides: Partial<Pick<Plugin, 'from' | 'name' | 'org' | 'type'>> = {},
...
describe('plugins/utils', () => {
  describe('getPluginCardIconUrl', () => {
    ...
    it('returns absolute urls and root-relative urls as-is', () => {
      expect(getPluginCardIconUrl(createPlugin(), 'https://example.com/icon.png', 'tenant-1')).toBe(
        'https://example.com/icon.png',
      )
      expect(getPluginCardIconUrl(createPlugin(), '/icons/demo.png', 'tenant-1')).toBe(
        '/icons/demo.png',
      )
    })
    ...
    it('builds the workspace icon url for tenant-scoped plugins', () => {
      expect(getPluginCardIconUrl(createPlugin(), 'icon.png', 'tenant-1')).toBe(
        `${API_PREFIX}/workspaces/current/plugin/icon?tenant_id=tenant-1&filename=icon.png`,
      )
    })
  })
})
