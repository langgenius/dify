...
vi.mock('@/app/components/plugins/plugin-detail-panel/operation-dropdown', () => ({
  default: ({ detailUrl }: { detailUrl: string }) => (
    <div data-testid="operation-dropdown">{detailUrl}</div>
  ),
}))
...
describe('DetailHeader', () => {
  ...
  it('renders the plugin summary, source badge, auth section, and modal container', () => {
    ...
    expect(screen.getByTestId('operation-dropdown')).toHaveTextContent(
      'https://marketplace.example.com/plugins/acme/provider-a',
    )
