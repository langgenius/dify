...
const ModelList = ({
  ...
  return (
    <div className='px-4 py-2'>
      <div className='text-text-secondary system-sm-semibold-uppercase mb-1 flex h-6 items-center'>{t('plugin.detailPanel.modelNum', { num: res.data.length })}</div>
      <div className='flex flex-col'>
        {res.data.map(model => (
          <div key={model.model} className='flex h-6 items-center py-1'>
            ...
            />
            <ModelName
              className='text-text-secondary system-md-regular grow'
              modelItem={model}
              showModelType
              showMode
              showContextSize
