...
const InstallFromGitHub: React.FC<InstallFromGitHubProps> = ({
  ...
  const getTitle = useCallback(() => {
    ...
    return updatePayload
      ? t(($) => $[`${i18nPrefix}.updatePlugin`], { ns: 'plugin' })
      : t(($) => $[`${i18nPrefix}.installPlugin`], { ns: 'plugin' })
  ...
  const handleError = (e: any, isInstall: boolean) => {
    const message =
      e?.response?.message || t(($) => $['installModal.installFailedDesc'], { ns: 'plugin' })
    setErrorMsg(message)
    setState((prevState) => ({
      ...prevState,
    ...
    >
      <DialogContent
        ...
        className={cn(
          ...
          cn(
            modalClassName,
            'flex max-h-[calc(100dvh-48px)] flex-col items-start rounded-2xl border-[0.5px] border-components-panel-border bg-components-panel-bg p-0 shadow-xl',
          ),
        )}
      >
        <DialogClose
        ...
        {[
          ...
          <div
            className={`flex min-h-0 flex-1 flex-col items-start justify-center self-stretch overflow-y-auto px-6 py-3 ${state.step === InstallStepFromGitHub.installed ? 'gap-2' : 'gap-4'}`}
          >
            {state.step === InstallStepFromGitHub.setUrl && (
              ...
              >
                ...
                <div className="mt-4 flex items-center justify-end gap-2 self-stretch">
                  <Button variant="secondary" className="min-w-18" onClick={onClose}>
            ...
            {state.step === InstallStepFromGitHub.selectPackage && (
              <SelectPackage
                ...
                onSelectVersion={(item) =>
                  setState((prevState) => ({ ...prevState, selectedVersion: String(item.value) }))
                }
                selectedPackage={state.selectedPackage}
                packages={packages}
                onSelectPackage={(item) =>
                  setState((prevState) => ({ ...prevState, selectedPackage: String(item.value) }))
                }
