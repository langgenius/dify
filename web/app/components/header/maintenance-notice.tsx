...
import { NOTICE_I18N, getLanguage } from '@/i18n/language'
...
const MaintenanceNotice = () => {
  ...
  return (
    <div className='shrink-0 flex items-center px-4 h-[38px] bg-[#FFFAEB] border-b border-[0.5px] border-b-[#FEF0C7] z-20'>
      <div className='shrink-0 flex items-center mr-2 px-2 h-[22px] bg-[#F79009] text-white text-[11px] font-medium rounded-xl'>{titleByLocale[getLanguage(locale)]}</div>
      {
        (NOTICE_I18N.href && NOTICE_I18N.href !== '#')
          ? <div className='grow text-xs font-medium text-gray-700 cursor-pointer' onClick={handleJumpNotice}>{descByLocale[getLanguage(locale)]}</div>
          : <div className='grow text-xs font-medium text-gray-700'>{descByLocale[getLanguage(locale)]}</div>
      }
