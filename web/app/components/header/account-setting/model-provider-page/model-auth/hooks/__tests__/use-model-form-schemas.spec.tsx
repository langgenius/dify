...
describe('useModelFormSchemas', () => {
  ...
  it('selects correct form schemas based on providerFormSchemaPredefined', () => {
    ...
    const { result: emptyResult } = renderHook(() =>
      useModelFormSchemas({} as unknown as ModelProvider, true),
    )
  ...
  it('computes form values correctly for credentials and models', () => {
    ...
    } as unknown as CustomModelCredential
    const { result } = renderHook(() =>
      useModelFormSchemas(mockProvider, true, { api_key: 'val' }, mockCredential, mockModel),
    )
    expect((result.current.formValues as Record<string, unknown>).api_key).toBe('val')
    expect((result.current.formValues as Record<string, unknown>).__authorization_name__).toBe(
      'Test',
    )
    ...
    const { result: emptyCredsRes } = renderHook(() =>
      useModelFormSchemas(mockProvider, true, undefined, mockCredential),
    )
    expect(
      (emptyCredsRes.current.formValues as Record<string, unknown>).__authorization_name__,
    ).toBe('Test')
  })
  ...
  it('handles model name and type schemas for custom models', () => {
    ...
    const { result: customWithVal } = renderHook(() =>
      useModelFormSchemas(mockProvider, false, undefined, undefined, mockModel),
    )
    expect(
      (customWithVal.current.modelNameAndTypeFormValues as Record<string, unknown>).__model_name,
    ).toBe('custom')
  })
})
