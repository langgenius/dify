...
describe('useCredentialData', () => {
  ...
  it('determines correct config source and parameters', () => {
    vi.mocked(useGetCredential).mockReturnValue({
      isLoading: false,
      data: {},
    } as unknown as ReturnType<typeof useGetCredential>)
    const mockProvider = { provider: 'openai' } as unknown as ModelProvider

    // Predefined source
    renderHook(() => useCredentialData(mockProvider, true), { wrapper })
    expect(useGetCredential).toHaveBeenCalledWith(
      'openai',
      undefined,
      undefined,
      undefined,
  ...
  it('returns appropriate loading and data states', () => {
    ...
    const { result: loadingRes } = renderHook(() => useCredentialData(mockProvider, true), {
      wrapper,
    })
  ...
  it('passes credential and model identifier correctly', () => {
    ...
    renderHook(() => useCredentialData(mockProvider, true, true, mockCredential, mockModel), {
      wrapper,
    })
    expect(useGetCredential).toHaveBeenCalledWith(
      'openai',
