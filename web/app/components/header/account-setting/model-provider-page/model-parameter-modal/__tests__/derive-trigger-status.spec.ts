...
describe('deriveTriggerStatus', () => {
  it('returns empty when modelId is missing', () => {
    expect(
      deriveTriggerStatus(undefined, 'openai', mockProvider, mockModel, baseCredentialState),
    ).toBe('empty')
  })

  it('returns empty when providerName is missing', () => {
    expect(
      deriveTriggerStatus('gpt-4', undefined, mockProvider, mockModel, baseCredentialState),
    ).toBe('empty')
  })

  it('returns incompatible when provider plugin is not installed', () => {
    expect(deriveTriggerStatus('gpt-4', 'openai', undefined, mockModel, baseCredentialState)).toBe(
      'incompatible',
    )
  })
  ...
  it('returns credits-exhausted when credits priority and exhausted', () => {
    ...
    expect(deriveTriggerStatus('gpt-4', 'openai', mockProvider, mockModel, state)).toBe(
      'credits-exhausted',
    )
  })
  ...
  it('returns incompatible when currentModel is missing (deprecated)', () => {
    expect(
      deriveTriggerStatus('gpt-4', 'openai', mockProvider, undefined, baseCredentialState),
    ).toBe('incompatible')
  })
  ...
  it('returns credits-exhausted when currentModel is missing and AI credits are exhausted without api key', () => {
    ...
    expect(deriveTriggerStatus('gpt-4', 'openai', mockProvider, undefined, state)).toBe(
      'credits-exhausted',
    )
  })
  ...
  it('returns configure-required when model status is no-configure', () => {
    ...
    expect(deriveTriggerStatus('gpt-4', 'openai', mockProvider, model, baseCredentialState)).toBe(
      'configure-required',
    )
  })
  ...
  it('returns active when all conditions are satisfied', () => {
    expect(
      deriveTriggerStatus('gpt-4', 'openai', mockProvider, mockModel, baseCredentialState),
    ).toBe('active')
  })
