...
const Trigger: FC<TriggerProps> = ({
  ...
  const status = deriveTriggerStatus(
    modelId,
    providerName,
  ...
  const iconProvider =
    currentProvider || modelProviders.find((item) => item.provider === providerName)
  ...
  if (status === 'empty') {
    return (
      ...
      >
        ...
        <span
          className={cn(
            'i-ri-arrow-down-s-line size-4 shrink-0 text-text-tertiary',
            isInWorkflow && 'absolute top-[9px] right-2 h-3.5 w-3.5',
          )}
        />
      </div>
    )
  ...
  return (
    <div className="flex h-8 min-w-[296px] cursor-pointer items-center gap-px overflow-hidden rounded-lg">
      <div
        className={cn(
          'flex flex-1 items-center gap-0.5 rounded-l-lg p-1',
          isInWorkflow
            ? 'border border-workflow-block-parma-bg bg-workflow-block-parma-bg'
      ...
      >
        ...
        <div className="flex flex-1 items-center truncate px-1 py-[3px]">
          {currentModel ? (
            ...
            <div className="truncate text-[13px] font-normal text-components-input-text-filled">
              {modelId}
            </div>
          )}
        </div>
      ...
      <div
        ref={settingsRef}
        className={cn(
          'flex shrink-0 items-center justify-center rounded-r-lg p-2',
          isInWorkflow
            ? 'border border-workflow-block-parma-bg bg-workflow-block-parma-bg'
