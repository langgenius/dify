...
const Card = ({ item, disabled }: CardProps) => {
  ...
  const handleAction = useCallback(
    (action: string, credentialItem: DataSourceCredential, renamePayload?: Record<string, any>) => {
      if (action === 'edit') {
      ...
      if (action === 'rename') handleRename(renamePayload as any)
    ...
    [openConfirm, handleEdit, handleSetDefault, handleRename],
  )

  return (
    <div className="rounded-xl bg-background-section-burn">
      <div className="flex items-center p-3 pb-2">
        <img src={icon} className="mr-3 flex size-10 shrink-0 items-center justify-center" />
        <div className="grow">
          <div className="system-md-semibold text-text-primary">{renderI18nObject(label)}</div>
        ...
        <Configure pluginPayload={pluginPayload} item={item} onUpdate={handleAuthUpdate} />
      </div>
      ...
      {!!credentials_list.length && (
        <div className="space-y-1 p-3 pt-2">
          {credentials_list.map((credentialItem) => (
            <Item key={credentialItem.id} credentialItem={credentialItem} onAction={handleAction} />
          ))}
        </div>
      )}
      {!credentials_list.length && (
