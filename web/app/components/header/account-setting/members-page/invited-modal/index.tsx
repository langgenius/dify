...
const InvitedModal = ({ invitationResults, onCancel }: IInvitedModalProps) => {
  ...
  const successInvitationResults = useMemo<SuccessInvitationResult[]>(
    () =>
      invitationResults?.filter((item) => item.status === 'success') as SuccessInvitationResult[],
    [invitationResults],
  )
  const failedInvitationResults = useMemo<FailedInvitationResult[]>(
    () =>
      invitationResults?.filter((item) => item.status !== 'success') as FailedInvitationResult[],
    [invitationResults],
  )
  ...
  return (
    ...
    >
      <DialogContent backdropProps={{ forceRender: true }} className="w-[480px] p-8">
        ...
        {!IS_CE_EDITION && (
          <div className="mb-10 text-sm text-text-tertiary">
