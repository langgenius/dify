...
import type { ScrapflyConfig } from '@/models/common'
...
const I18N_PREFIX = 'datasetCreation.scrapfly'

const DEFAULT_BASE_URL = 'https://api.scrapfly.io'

const ConfigScrapflyModal: FC<Props> = ({
  onCancel,
  onSaved,
}) => {
  const { t } = useTranslation()
  const [isSaving, setIsSaving] = useState(false)
  const [config, setConfig] = useState<ScrapflyConfig>({
    api_key: '',
    base_url: '',
  })
          ...
          <div className='mx-2 max-h-[calc(100vh-120px)] w-[640px] overflow-y-auto rounded-2xl bg-components-panel-bg shadow-xl'>
            <div className='px-8 pt-8'>
              <div className='mb-4 flex items-center justify-between'>
                <div className='system-xl-semibold text-text-primary'>{t(`${I18N_PREFIX}.configScrapfly`)}</div>
              </div>
              ...
              <div className='my-8 flex h-8 items-center justify-between'>
                <a className='flex items-center space-x-1 text-xs font-normal leading-[18px] text-text-accent' target='_blank' href='https://scrapfly.io/dashboard'>
...
export default React.memo(ConfigScrapflyModal)
