...
} from '@/service/workspace-api-key'
...
const WorkspaceApiKeyModal: FC<WorkspaceApiKeyModalProps> = ({
  ...
  const getExpiresInDays = (data: Partial<WorkspaceApiKey>) => {
    if (typeof data.expires_in_days === 'number') return data.expires_in_days
  ...
  const handleSave = async () => {
    if (!name.trim() && !isRegenerate && !isEdit) {
      notify({
        type: 'error',
        message: t('common.workspaceApiKey.modal.nameError'),
      })
      return
    }
    ...
    if (scopes.length === 0 && !isRegenerate && !isEdit) {
      notify({
        type: 'error',
        message: t('common.workspaceApiKey.modal.scopeRequired'),
      })
      return
    }
    ...
    try {
      ...
      if (isRegenerate && data.id) {
        ...
        notify({
          type: 'success',
          message: t('common.workspaceApiKey.updateSuccess'),
        })
        if (onSave) onSave(res)
      }
 else {
        res = await createWorkspaceApiKey({
          name,
          scopes,
          expiresInDays: validExpiresInDays,
        })
    ...
    return (
      ...
      >
        <div className="mb-2 text-xl font-semibold text-text-primary">
          {isRegenerate
            ? t('common.workspaceApiKey.regeneratedApiKey')
            : t('common.workspaceApiKey.newApiKey')}
        </div>
  ...
  return (
    <Modal isShow onClose={noop} className="!w-[520px] !max-w-none !p-8 !pb-6">
      <div className="mb-2 text-xl font-semibold text-text-primary">
        {(() => {
          if (isRegenerate)
            return t('common.workspaceApiKey.modal.regenerateTitle')
          if (isEdit) return t('common.workspaceApiKey.modal.editTitle')
          return t('common.workspaceApiKey.modal.addTitle')
        })()}
      </div>
      <div className="mb-5 text-sm text-text-secondary">
        {(() => {
          if (isRegenerate)
            return t('common.workspaceApiKey.modal.regenerateDescription')
          if (isEdit) return t('common.workspaceApiKey.modal.editDescription')
          return t('common.workspaceApiKey.modal.addDescription')
        })()}
      </div>
      ...
      {(!isRegenerate || isEdit) && (
        <div className="py-2">
          ...
          <input
            ...
            placeholder={
              t('common.workspaceApiKey.modal.namePlaceholder') || ''
            }
          />
          <div className="text-sm font-medium leading-9 text-text-primary">
          ...
          <ScopeSelector selectedScopes={scopes} onScopesChange={setScopes} />
        </div>
      )}
      ...
      <div className="mt-6">
        <div className="flex items-center justify-end">
          ...
          <Button
            variant="primary"
            disabled={
              (isRegenerate ? false : !name.trim() || scopes.length === 0)
              || loading
            }
            onClick={handleSave}
          >
            {(() => {
              if (isRegenerate) return t('common.workspaceApiKey.regenerate')
              if (isEdit) return t('common.operation.save')
              return t('common.workspaceApiKey.add')
            })()}
          </Button>
        </div>

        {/* ボタンが無効化されている理由の説明 */}
        {!isRegenerate && (!name.trim() || scopes.length === 0) && (
          <div className="mt-2 text-xs text-red-600 dark:text-red-400">
            {!name.trim() && scopes.length === 0
              ? t('common.workspaceApiKey.modal.validationHint.nameAndScope')
              : !name.trim()
              ? t('common.workspaceApiKey.modal.validationHint.nameOnly')
              : t('common.workspaceApiKey.modal.validationHint.scopeOnly')}
          </div>
        )}
      </div>
    </Modal>
