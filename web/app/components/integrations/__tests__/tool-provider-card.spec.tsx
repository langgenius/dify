...
vi.mock('react-i18next', async () => {
  ...
  return {
    useTranslation: () => ({
      i18n: { language: 'en-US' },
      t: withSelectorKey((key: string, options?: { count?: number; ns?: string }) =>
        options?.ns ? `${options.ns}.${key}${options.count ? `:${options.count}` : ''}` : key,
      ),
    }),
  }
})
...
describe('IntegrationsToolProviderCard', () => {
  it('shows a built-in corner badge for builtin tools', () => {
    ...
    expect(screen.getByTestId('corner-mark')).toHaveTextContent(
  ...
  it('does not show a built-in corner badge for marketplace plugin tools', () => {
    render(
      <IntegrationsToolProviderCard
        collection={createCollection({ plugin_id: 'author/provider' })}
        showBuiltInBadge
      />,
    )
    ...
    render(
      <IntegrationsToolProviderCard
        collection={createCollection({
          tools: [{ name: 'tool-a' }, { name: 'tool-b' }] as Collection['tools'],
        })}
      />,
    )
  ...
  it('shows author and labels in the labeled card style', () => {
    ...
    expect(screen.getByTestId('card-builtin-provider')).toHaveClass(
      'shadow-xs',
