...
export const useSegmentListData = (
  options: UseSegmentListDataOptions,
): UseSegmentListDataReturn => {
  const {
    searchValue,
    selectedStatus,
    selectedSegmentIds,
  ...
  const currentQueryKey = useMemo(
    () => [...useSegmentListKey, datasetId, documentId, queryParams],
    [datasetId, documentId, queryParams],
  )
  // Fetch segment list
  ...
  useEffect(() => {
    if (segmentListRef.current && needScrollToBottom.current) {
      segmentListRef.current.scrollTo({
        top: segmentListRef.current.scrollHeight,
        behavior: 'smooth',
      })
  ...
  const updateSegmentInCache = useCallback(
    (segmentId: string, updater: (seg: SegmentDetailModel) => SegmentDetailModel) => {
  ...
  const updateSegmentsInCache = useCallback(
    (segmentIds: string[], updater: (seg: SegmentDetailModel) => SegmentDetailModel) => {
      queryClient.setQueryData<SegmentsResponse>(currentQueryKey, (old) => {
        if (!old) return old
        return {
          ...old,
  ...
  const onChangeSwitch = useCallback(
    async (enable: boolean, segId?: string) => {
  ...
  const handleUpdateSegment = useCallback(
    async (
      ...
      if (needRegenerate) params.regenerate_child_chunks = needRegenerate
      eventEmitter?.emit('update-segment')
      await updateSegment(
        { datasetId, documentId, segmentId, body: params },
        {
          onSuccess(res) {
            toast.success(t(($) => $['actionMsg.modifiedSuccessfully'], { ns: 'common' }))
  ...
  const totalText = useMemo(() => {
    ...
    const total =
      typeof segmentListData?.total === 'number' ? formatNumber(segmentListData.total) : 0
