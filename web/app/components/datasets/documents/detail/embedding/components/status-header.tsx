...
const StatusHeader: FC<StatusHeaderProps> = React.memo(
  ({
    ...
    const getStatusText = () => {
      ...
      if (isCompleted) return t(($) => $['embedding.completed'], { ns: 'datasetDocuments' })
      if (isPaused) return t(($) => $['embedding.paused'], { ns: 'datasetDocuments' })
      if (isError) return t(($) => $['embedding.error'], { ns: 'datasetDocuments' })
      return ''
    }
