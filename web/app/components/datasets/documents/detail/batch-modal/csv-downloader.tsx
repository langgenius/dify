...
const CSVDownload: FC<{ docForm: ChunkingMode }> = ({ docForm }) => {
  ...
  const getTemplate = () => {
    if (locale === LanguagesSupported[1]) {
      if (docForm === ChunkingMode.qa) return CSV_TEMPLATE_QA_CN
      return CSV_TEMPLATE_CN
    }
    if (docForm === ChunkingMode.qa) return CSV_TEMPLATE_QA_EN
            ...
            <tbody className="text-text-tertiary">
              <tr>
                <td className="h-9 border-b border-divider-subtle pr-2 pl-3 text-[13px]">
                  {t(($) => $['list.batchModal.question'], { ns: 'datasetDocuments' })} 1
                </td>
                <td className="h-9 border-b border-divider-subtle pr-2 pl-3 text-[13px]">
                  {t(($) => $['list.batchModal.answer'], { ns: 'datasetDocuments' })} 1
                </td>
              </tr>
              <tr>
                <td className="h-9 pr-2 pl-3 text-[13px]">
                  {t(($) => $['list.batchModal.question'], { ns: 'datasetDocuments' })} 2
                </td>
                <td className="h-9 pr-2 pl-3 text-[13px]">
                  {t(($) => $['list.batchModal.answer'], { ns: 'datasetDocuments' })} 2
                </td>
              </tr>
            </tbody>
          </table>
        ...
        {docForm === ChunkingMode.text && (
          <table className="w-full table-fixed border-separate border-spacing-0 rounded-lg border border-divider-subtle text-xs">
            ...
            <tbody className="text-text-tertiary">
              <tr>
                <td className="h-9 border-b border-divider-subtle pr-2 pl-3 text-[13px]">
                  {t(($) => $['list.batchModal.content'], { ns: 'datasetDocuments' })} 1
                </td>
              </tr>
              <tr>
                <td className="h-9 pr-2 pl-3 text-[13px]">
                  {t(($) => $['list.batchModal.content'], { ns: 'datasetDocuments' })} 2
                </td>
              </tr>
            </tbody>
          </table>
