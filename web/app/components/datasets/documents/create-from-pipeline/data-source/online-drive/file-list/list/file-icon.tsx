...
import BucketsBlue from '@/app/components/base/icons/src/public/knowledge/online-drive/BucketsBlue'
import Folder from '@/app/components/base/icons/src/public/knowledge/online-drive/Folder'
...
const FileIcon = ({ type, fileName, size = 'md', className }: FileIconProps) => {
  ...
  if (type === OnlineDriveFileType.bucket) {
    return <BucketsBlue className={cn('size-4.5', className)} />
  }

  if (type === OnlineDriveFileType.folder) {
    return <Folder className={cn('size-4.5', className)} />
  }
