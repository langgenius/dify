...
export const useDatasourceUIState = ({
  ...
  const isShowVectorSpaceFull = useMemo(() => {
    if (!datasource || !datasourceType) return false
  ...
  const nextBtnDisabled = useMemo(() => {
    if (!datasource || !datasourceType) return true
    ...
    const disabledConditions: Record<string, boolean> = {
      ...
      [DatasourceType.onlineDocument]:
        isCheckingVectorSpace || isShowVectorSpaceFull || onlineDocumentsLength === 0,
      [DatasourceType.websiteCrawl]:
        isCheckingVectorSpace || isShowVectorSpaceFull || websitePagesLength === 0,
      [DatasourceType.onlineDrive]:
        isCheckingVectorSpace || isShowVectorSpaceFull || selectedFileIdsLength === 0,
    }
  ...
  const showSelect = useMemo(() => {
    if (datasourceType === DatasourceType.onlineDocument) return currentWorkspacePagesLength > 0
  ...
  const totalOptions = useMemo(() => {
    if (datasourceType === DatasourceType.onlineDocument) return currentWorkspacePagesLength
  ...
  const selectedOptions = useMemo(() => {
    if (datasourceType === DatasourceType.onlineDocument) return onlineDocumentsLength
  ...
  const tip = useMemo(() => {
    if (datasourceType === DatasourceType.onlineDocument)
      return t(($) => $['addDocuments.selectOnlineDocumentTip'], {
        ns: 'datasetPipeline',
        count: 50,
      })
