...
const PreviewItem: FC<IPreviewItemProps> = ({ type = PreviewType.TEXT, index, content, qa }) => {
    ...
    <div className="rounded-xl bg-gray-50 p-4">
      <div className="flex h-5 items-center justify-between text-xs text-gray-500">
        ...
        <div className="flex items-center space-x-1">
          {textIcon}
          <span>
            {charNums} {t('stepTwo.characters', { ns: 'datasetCreation' })}
          </span>
        </div>
      </div>
      <div className="mt-2 line-clamp-6 max-h-[120px] overflow-hidden text-sm text-gray-800">
        {type === PreviewType.TEXT && <div style={{ whiteSpace: 'pre-line' }}>{content}</div>}
        {type === PreviewType.QA && (
