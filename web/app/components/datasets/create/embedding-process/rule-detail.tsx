...
const RuleDetail: FC<RuleDetailProps> = ({ sourceData, indexingType, retrievalMethod }) => {
  ...
  const getTextCleaningValue = useCallback((): string => {
    ...
    const enabledRules =
      sourceData.rules?.pre_processing_rules?.filter((rule) => rule.enabled) || []
  ...
  const indexMethodIconSrc = isEconomical
    ? indexMethodIcon.economical
    : indexMethodIcon.high_quality
  const indexModeLabel = t(($) => $[`stepTwo.${isEconomical ? 'economical' : 'qualified'}`], {
    ns: 'datasetCreation',
  })

  const effectiveRetrievalMethod = isEconomical
    ? 'keyword_search'
    : (retrievalMethod ?? 'semantic_search')
  const retrievalLabel = t(($) => $[`retrieval.${effectiveRetrievalMethod}.title`], {
    ns: 'dataset',
  })
  const retrievalIconSrc =
    RETRIEVAL_ICON_MAP[retrievalMethod as keyof typeof RETRIEVAL_ICON_MAP] ?? retrievalIcon.vector
