...
const StatusAction: FC<Props> = ({
  ...
  return (
    <div className='border-components-panel-border bg-components-panel-bg-blur shadow-xs relative flex h-[34px] items-center rounded-lg border pl-2 pr-3'>
      ...
      <div className='relative z-10 flex h-full items-center space-x-2'>
        ...
        <div className='text-text-secondary text-[13px] font-normal'>{description}</div>
        <Divider type='vertical' className='!h-4' />
        <div onClick={onAction} className={cn('text-text-accent cursor-pointer text-[13px] font-semibold', disabled && 'text-text-disabled cursor-not-allowed')}>{actionText}</div>
      </div>
    </div>
  )
}
