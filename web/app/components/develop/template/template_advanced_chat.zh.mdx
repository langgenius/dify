...
<Row>
  <Col>
    ...
    <Properties>
      ...
      （选填）用于可观测性分组的链路会话 ID。该值会覆盖 Arize/Phoenix 追踪导出的 OpenInference session.id，不会改变 conversation_id、workflow_run_id、trace_id 或 span 关系。支持 Header <code>X-Trace-Session-Id</code>、查询参数 <code>trace_session_id</code> 和请求体字段 <code>trace_session_id</code>。优先级：Header &gt; Query 参数 &gt; Request Body。
      </Property>
    </Properties>
