  ...
  <Col>
    ...
    <Properties>
      ...
      <Property name='tool_call_mode' type='string' key='tool_call_mode'>
        ...
        - `openclaw_text` 从 `answer` 文本解析 OpenClaw 风格工具调用并触发回调。
        该模式会在消息结束事件时解析代码块或形如 `tool(args)` 的行，生成 `event: tool_call` 事件并继续推理；适用于 OpenClaw 网关/客户端在提示中注入工具语法、模型把工具调用以文本输出的场景。
      </Property>
