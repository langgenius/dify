  ...
  <Col>
    ...
    ### Request Body
      ...
      - `trace_session_id` (string) Optional
        用于可观测性分组的链路会话 ID。该值会覆盖 Arize/Phoenix 追踪导出的 OpenInference session.id，不会改变 conversation_id、workflow_run_id、trace_id 或 span 关系。支持 Header `X-Trace-Session-Id`、查询参数 `trace_session_id` 和请求体字段 `trace_session_id`。优先级：Header > Query 参数 > Request Body。
