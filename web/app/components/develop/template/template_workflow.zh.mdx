...
<Heading
  url='/workflows/tasks/:task_id'
  method='GET'
  title='获取workflow任务执行情况'
...
<Row>
  ...
  <Col sticky>
    ### Request Example
    <CodeGroup title="Request" tag="GET" label="/workflows/tasks/:task_id" targetCode={`curl -X GET '${props.appDetail.api_base_url}/workflows/tasks/:task_id/stop' \\\n-H 'Authorization: Bearer {api_key}' \\\n-H 'Content-Type: application/json'`}>
      ```bash {{ title: 'cURL' }}
      curl -X POST '${props.appDetail.api_base_url}/workflows/tasks/:task_id' \
      -H 'Authorization: Bearer {api_key}' \
      -H 'Content-Type: application/json'
      ```
    </CodeGroup>
