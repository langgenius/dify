...
<Row>
  <Col>
    The application returns content moderation. It is recommended that the returned content be called in groups of 50 characters.
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="POST" label="/moderation" targetCode={`curl --location --request POST '${props.appDetail.api_base_url}/moderation' \\\n--header 'Authorization: Bearer ENTER-YOUR-SECRET-KEY' \\\n--data-raw '{ \n "app_id": "", \n "text": ""\n}'`}>
    ...
    ```json {{ title: 'Response' }}
    {
      "flagged": true, // Whether it violates the verification rules
      "text": ""
    }
    ```
    </CodeGroup>
