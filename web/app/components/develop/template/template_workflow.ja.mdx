  ...
  <Col>
    ...
    ### リクエストボディ
      ...
      - `trace_session_id` (string) オプション
        可観測性のグルーピングに使用するトレースセッションID。この値は Arize/Phoenix tracing がエクスポートする OpenInference session.id を上書きし、conversation_id、workflow_run_id、trace_id、または span の関係は変更しません。Header `X-Trace-Session-Id`、クエリパラメータ `trace_session_id`、リクエストボディフィールド `trace_session_id` をサポートします。優先順位：Header > クエリパラメータ > リクエストボディ。
