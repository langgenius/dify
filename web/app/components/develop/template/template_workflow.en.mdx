  ...
  <Col>
    ...
    ### Request Body
      ...
      - `trace_session_id` (string) Optional
        Trace session ID for observability grouping. This value overrides the OpenInference session.id exported by Arize/Phoenix tracing and does not change conversation_id, workflow_run_id, trace_id, or span relationships. Supports Header `X-Trace-Session-Id`, query parameter `trace_session_id`, and request body field `trace_session_id`. Priority: Header > Query parameter > Request Body.
