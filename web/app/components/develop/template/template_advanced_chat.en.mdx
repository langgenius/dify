...
<Row>
  <Col>
    Get detailed information of a message by message ID, including workflow run ID and other key information.

    ### Path Parameters
    <Properties>
      <Property name='message_id' type='string' key='message_id'>
        Message ID, can be obtained from chat interface response or event stream
      </Property>
    </Properties>

    ### Response
    Returns detailed information of the message, including the following key fields:
    - `id` (string) Message ID
    - `conversation_id` (string) Conversation ID
    - `workflow_run_id` (string) Workflow run ID (if message comes from workflow execution)
    - `inputs` (object) Message input content
    - `query` (string) User query content
    - `answer` (string) Assistant reply content
  ...
  <Col sticky>
    <CodeGroup
      ...
      targetCode={`curl --location --request GET '${props.apiBaseUrl}/messages/{message_id}' \\\n--header 'Authorization: Bearer {api_key}'`}
    >
    ```bash {{ title: 'cURL' }}
    curl --location --request GET '${props.apiBaseUrl}/messages/{message_id}' \
    --header 'Authorization: Bearer {api_key}'
    ```
    </CodeGroup>
...
<Heading
  url='/node-executions'
  method='GET'
  title='Get Node Execution List'
  name='#node-executions'
/>
<Row>
  <Col>
    Get the node execution list based on workflow run ID, used to view the execution status and results of each node in the workflow.

    ### Query Parameters
    <Properties>
      <Property name='workflow_run_id' type='string' key='workflow_run_id'>
        Workflow run ID (required). Can be obtained through the following methods:
        <br />• `workflow_run_id` field in the `workflow_started` event when workflow execution starts
        <br />• `workflow_run_id` field in the `workflow_finished` event when workflow execution completes  
        <br />• `workflow_run_id` field in the workflow run interface response
        <br />• `sys.workflow_run_id` field in node execution output
        <br />• Through message detail interface: `GET /messages/{message_id}` to get the message's `workflow_run_id` field
      </Property>
    </Properties>

    ### Response
    Returns a list of node executions. Each node execution includes the following fields:
    - `id` (string) Node execution ID
    - `node_id` (string) Node ID
    - `node_type` (string) Node type
    - `title` (string) Node name
    - `index` (int) Execution sequence number
    - `predecessor_node_id` (string) Predecessor node ID
    - `inputs` (object) Node input content, same as input content
    - `process_data` (object) Node process data
    - `outputs` (object) Node output content, same as input content
    - `status` (string) Execution status: `running` / `succeeded` / `failed` / `stopped`
    - `error` (string) Error reason (if execution failed)
    - `elapsed_time` (float) Node execution time (seconds)
    - `execution_metadata` (object) Execution metadata
    - `created_at` (timestamp) Node execution start time

    ### Errors
    - 400, `invalid_param`, Invalid parameter input
    - 401, `unauthorized`, Authentication failed
    - 404, `workflow_run_not_found`, Workflow run not found
    - 500, Internal server error
