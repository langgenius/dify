...
const AddApiKeyModal = ({ isShow, onClose }: AddApiKeyModalProps) => {
  ...
  const { data: datasetsPages } = useInfiniteDatasets({ keyword }, { enabled: isShow && pickerOpen })
  ...
  const toggleKb = (id: string, name: string) => {
    setSelected(prev => (prev.some(kb => kb.id === id) ? prev.filter(kb => kb.id !== id) : [...prev, { id, name }]))
  }
  ...
  return (
    <>
          ...
          <div className="flex flex-col gap-y-3 px-6 py-4">
            ...
            <RadioGroup<Scope> value={scope} onValueChange={setScope}>
              <div className="flex flex-col gap-y-3">
                <label htmlFor="api-key-scope-all" className="flex cursor-pointer items-start gap-x-2">
                ...
                <label htmlFor="api-key-scope-specific" className="flex cursor-pointer items-start gap-x-2">
                    ...
                    : (
                        <div className="flex flex-wrap gap-1.5">
                          {selected.map(kb => (
                            ...
                            >
                              <span className="max-w-40 truncate system-xs-medium text-text-secondary">{kb.name}</span>
                              <button
                                type="button"
                  ...
                  <Popover open={pickerOpen} onOpenChange={setPickerOpen}>
                    ...
                    <PopoverContent placement="bottom" sideOffset={4} className="w-80 p-2">
                      <Input
                        ...
                        placeholder={t(($) => $['apiKeyModal.searchKnowledgeBases'], { ns: 'appApi' })}
                      />
                        ...
                        <div className="flex flex-col">
                          {datasets.map(ds => (
                            ...
                            >
                              ...
                              />
                              <span className="min-w-0 grow truncate system-sm-regular text-text-secondary">{ds.name}</span>
                            </label>
                          ))}
                        </div>
                      </ScrollArea>
      ...
      <SecretKeyGenerateModal isShow={!!generatedKey} newKey={generatedKey} onClose={() => setGeneratedKey(undefined)} />
    </>
  )
}
