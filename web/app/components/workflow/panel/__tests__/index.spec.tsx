...
const createResizeEntry = (mode: MockResizeMode): ResizeObserverEntry =>
  ({
    borderBoxSize:
      mode === 'borderBox' ? ([{ inlineSize: 720, blockSize: 0 }] as ResizeObserverSize[]) : [],
    contentBoxSize: [],
...
class MockResizeObserver {
  ...
  observe = vi.fn(() => {
    if (!mockResizeModes.length) return

    this.callback(mockResizeModes.map(createResizeEntry), this as unknown as ResizeObserver)
  })
...
vi.mock('reactflow', () => ({
  useStore: (selector: (state: { getNodes: () => MockNode[] }) => unknown) =>
    selector({
      getNodes: () => mockNodes,
    }),
  useStoreApi: () => ({
