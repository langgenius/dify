  ...
  return (
    <div className="flex flex-col gap-y-3 px-4 py-3">
      ...
      <div className="rounded-lg border border-components-panel-border bg-components-panel-bg px-3 py-2">
        ...
        {!isRunHistoryLoading && !latestFailedRun && (
          <div className="system-sm-regular mt-1 text-text-quaternary">
            {t(($) => $['copilot.runToFixAutoEmpty'], { ns: 'workflow' })}
          </div>
        )}
      </div>
      <div>
        <div className="mb-1 system-xs-medium text-text-tertiary">
          {t(($) => $['copilot.runToFixManualLabel'], { ns: 'workflow' })}
        </div>
        <Input
          size="small"
          value={manualRunId}
          onChange={(e) => onManualRunIdChange(e.target.value)}
          placeholder={t(($) => $['copilot.runToFixManualPlaceholder'], { ns: 'workflow' })}
        />
      </div>
      <Button
        variant="primary"
      ...
      >
        {isStarting
          ? t(($) => $['copilot.starting'], { ns: 'workflow' })
          : usesAutoDetectedRun
            ? t(($) => $['copilot.useThisRun'], { ns: 'workflow' })
            : t(($) => $['copilot.startFix'], { ns: 'workflow' })}
      </Button>
    </div>
  )
}
