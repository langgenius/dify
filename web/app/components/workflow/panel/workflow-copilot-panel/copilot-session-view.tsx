import type { CopilotActionKind, ProgressEntry, SessionView } from '@/app/components/workflow-copilot/types'
...
function SessionStatus({ view }: { view: SessionView }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-x-2 gap-y-1 rounded-lg border border-components-panel-border bg-components-panel-bg px-3 py-2 system-xs-regular text-text-secondary">
      <div className="text-text-tertiary">{t(($) => $['copilot.sessionIdLabel'], { ns: 'workflow' })}</div>
      <div className="truncate text-right">{view.SessionID}</div>
      <div className="text-text-tertiary">{t(($) => $['copilot.sessionStateLabel'], { ns: 'workflow' })}</div>
      <div className="truncate text-right">{view.State}</div>
      <div className="text-text-tertiary">{t(($) => $['copilot.sessionVersionLabel'], { ns: 'workflow' })}</div>
      <div className="truncate text-right">{view.Version}</div>
      <div className="text-text-tertiary">{t(($) => $['copilot.sessionRunStatusLabel'], { ns: 'workflow' })}</div>
