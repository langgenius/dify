import type {
  DifyBuilderActionKind,
  ProgressEntry,
  SessionView,
} from '@/app/components/dify-builder/types'
import type { ChecklistItem } from '@/app/components/workflow/hooks/use-checklist'
import { Button } from '@langgenius/dify-ui/button'
import { Input } from '@langgenius/dify-ui/input'
import { memo } from 'react'
import { useTranslation } from 'react-i18next'
import {
  CHECKLIST_AWAIT_RECHECK_STATE,
  DIFY_BUILDER_MANUAL_ACTION_KINDS,
} from '@/app/components/dify-builder/types'

function useActionLabels(): Record<DifyBuilderActionKind, string> {
  const { t } = useTranslation()

  return {
    approve_repair: t(($) => $['difyBuilder.actionApproveRepair'], { ns: 'workflow' }),
    run_verify: t(($) => $['difyBuilder.actionRunVerify'], { ns: 'workflow' }),
    provide_testdata: t(($) => $['difyBuilder.actionProvideTestdata'], { ns: 'workflow' }),
    publish: t(($) => $['difyBuilder.actionPublish'], { ns: 'workflow' }),
    undo: t(($) => $['difyBuilder.actionUndo'], { ns: 'workflow' }),
    re_fix: t(($) => $['difyBuilder.actionReFix'], { ns: 'workflow' }),
    recheck: t(($) => $['difyBuilder.checklistRecheckAction'], { ns: 'workflow' }),
  }
}

function SessionStatus({ view }: { view: SessionView }) {
  const { t } = useTranslation()

  return (
    <div className="grid grid-cols-2 gap-x-2 gap-y-1 rounded-lg border border-components-panel-border bg-components-panel-bg px-3 py-2 system-xs-regular text-text-secondary">
      <div className="text-text-tertiary">
        {t(($) => $['difyBuilder.sessionIdLabel'], { ns: 'workflow' })}
      </div>
      <div className="truncate text-right">{view.session_id}</div>
      <div className="text-text-tertiary">
        {t(($) => $['difyBuilder.sessionStateLabel'], { ns: 'workflow' })}
      </div>
      <div className="truncate text-right">{view.state}</div>
      <div className="text-text-tertiary">
        {t(($) => $['difyBuilder.sessionVersionLabel'], { ns: 'workflow' })}
      </div>
      <div className="truncate text-right">{view.version}</div>
      <div className="text-text-tertiary">
        {t(($) => $['difyBuilder.sessionRunStatusLabel'], { ns: 'workflow' })}
      </div>
      <div className="truncate text-right">{view.run_status}</div>
      <div className="text-text-tertiary">
        {t(($) => $['difyBuilder.sessionCanvasReadOnlyLabel'], { ns: 'workflow' })}
      </div>
      <div className="truncate text-right">{String(view.canvas_read_only)}</div>
      <div className="text-text-tertiary">
        {t(($) => $['difyBuilder.sessionInterruptedLabel'], { ns: 'workflow' })}
      </div>
      <div className="truncate text-right">{String(view.interrupted)}</div>
    </div>
  )
}

function ProgressLog({ entries }: { entries: ProgressEntry[] }) {
  const { t } = useTranslation()

  if (!entries.length) {
    return (
      <div className="system-xs-regular text-text-quaternary">
        {t(($) => $['difyBuilder.progressEmpty'], { ns: 'workflow' })}
      </div>
    )
  }

  return (
    <div className="flex max-h-60 flex-col gap-y-1 overflow-y-auto rounded-lg border border-components-panel-border bg-components-panel-bg px-3 py-2">
      {entries.map((entry) => (
        <div key={entry.id} className="system-xs-regular text-text-secondary">
          <span className="text-text-accent">{entry.event}</span>
          {' · '}
          <span className="wrap-break-word">{JSON.stringify(entry.data)}</span>
        </div>
      ))}
    </div>
  )
}

function ChecklistRecheck({
  fixableItems,
  unfixableItems,
  onRecheck,
  isRefreshingDraft,
}: {
  fixableItems: ChecklistItem[]
  unfixableItems: ChecklistItem[]
  onRecheck: () => void
  isRefreshingDraft: boolean
}) {
  const { t } = useTranslation()

  return (
    <div className="flex flex-col gap-y-2 rounded-lg border border-components-panel-border bg-components-panel-bg px-3 py-2">
      <div className="system-sm-semibold-uppercase text-text-secondary">
        {t(($) => $['difyBuilder.checklistRecheckTitle'], { ns: 'workflow' })}
      </div>
      <div className="system-xs-regular text-text-tertiary">
        {t(($) => $['difyBuilder.checklistRecheckDescription'], { ns: 'workflow' })}
      </div>
      {unfixableItems.length > 0 && (
        <div className="system-xs-regular text-text-quaternary">
          {t(($) => $['difyBuilder.checklistUnfixableNote'], {
            ns: 'workflow',
            count: unfixableItems.length,
          })}
        </div>
      )}
      <Button
        variant="secondary"
        size="small"
        disabled={isRefreshingDraft}
        loading={isRefreshingDraft}
        onClick={onRecheck}
      >
        {t(($) => $['difyBuilder.checklistRecheckAction'], { ns: 'workflow' })}
        {fixableItems.length > 0 ? ` (${fixableItems.length})` : ''}
      </Button>
    </div>
  )
}

export type DifyBuilderSessionViewProps = {
  view: SessionView
  progressLog: ProgressEntry[]
  onRunAction: (kind: DifyBuilderActionKind) => void
  messageText: string
  onMessageTextChange: (value: string) => void
  onSendMessage: () => void
  fixableChecklistItems: ChecklistItem[]
  unfixableChecklistItems: ChecklistItem[]
  onRecheck: () => void
  isRefreshingDraftForRecheck: boolean
}

function DifyBuilderSessionView({
  view,
  progressLog,
  onRunAction,
  messageText,
  onMessageTextChange,
  onSendMessage,
  fixableChecklistItems,
  unfixableChecklistItems,
  onRecheck,
  isRefreshingDraftForRecheck,
}: DifyBuilderSessionViewProps) {
  const { t } = useTranslation()
  const actionLabels = useActionLabels()

  return (
    <div className="flex flex-col gap-y-3 px-4 py-3">
      <SessionStatus view={view} />

      {view.state === CHECKLIST_AWAIT_RECHECK_STATE && (
        <ChecklistRecheck
          fixableItems={fixableChecklistItems}
          unfixableItems={unfixableChecklistItems}
          onRecheck={onRecheck}
          isRefreshingDraft={isRefreshingDraftForRecheck}
        />
      )}

      <div className="flex flex-col gap-y-2">
        <div className="system-sm-semibold-uppercase text-text-secondary">
          {t(($) => $['difyBuilder.progressTitle'], { ns: 'workflow' })}
        </div>
        <ProgressLog entries={progressLog} />
      </div>

      <div className="flex flex-col gap-y-2">
        <div className="system-sm-semibold-uppercase text-text-secondary">
          {t(($) => $['difyBuilder.actionsTitle'], { ns: 'workflow' })}
        </div>
        <div className="flex flex-wrap gap-2">
          {DIFY_BUILDER_MANUAL_ACTION_KINDS.map((kind) => (
            <Button key={kind} size="small" onClick={() => onRunAction(kind)}>
              {actionLabels[kind]}
            </Button>
          ))}
        </div>
      </div>

      <div className="flex items-center gap-x-2">
        <Input
          size="small"
          value={messageText}
          onChange={(e) => onMessageTextChange(e.target.value)}
          placeholder={t(($) => $['difyBuilder.messagePlaceholder'], { ns: 'workflow' })}
          onKeyDown={(e) => {
            if (e.key === 'Enter' && messageText.trim()) onSendMessage()
          }}
        />
        <Button size="small" disabled={!messageText.trim()} onClick={onSendMessage}>
          {t(($) => $['difyBuilder.messageSend'], { ns: 'workflow' })}
        </Button>
      </div>
    </div>
  )
}

export default memo(DifyBuilderSessionView)
