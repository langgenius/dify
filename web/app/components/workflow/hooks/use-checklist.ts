    ...
    isRequiredNodesType.forEach((type: string) => {
      if (!filteredNodes.find(node => node.data.type === type)) {
        list.push({
          id: `${type}-need-added`,
          type,
          title: t(`workflow.blocks.${type}` as any),
          errorMessage: t('workflow.common.needAdd', { node: t(`workflow.blocks.${type}` as any) }),
          canNavigate: false,
        })
      }
    })
...
export const useChecklistBeforePublish = () => {
  ...
  const handleCheckBeforePublish = useCallback(async () => {
    ...
    for (let i = 0; i < isRequiredNodesType.length; i++) {
      ...
      if (!filteredNodes.find(node => node.data.type === type)) {
        notify({ type: 'error', message: t('workflow.common.needAdd', { node: t(`workflow.blocks.${type}` as any) }) })
        return false
      }
    }
