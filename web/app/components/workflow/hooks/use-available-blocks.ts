...
const availableBlocksFilter = (nodeType: BlockEnum, inContainer?: boolean) => {
  if (nodeType === BlockEnum.StartPlaceholder) return false
...
export const useAvailableBlocks = (nodeType?: BlockEnum, inContainer?: boolean) => {
  const { nodes: availableNodes } = useNodesMetaData()
  const availableNodesType = useMemo(
    () => availableNodes.map((node) => node.metaData.type),
    [availableNodes],
  )
  const availablePrevBlocks = useMemo(() => {
    if (
      ...
      nodeType === BlockEnum.StartPlaceholder ||
      nodeType === BlockEnum.DataSource ||
  ...
  const getAvailableBlocks = useCallback(
    (nodeType?: BlockEnum, inContainer?: boolean) => {
      ...
      return {
        availablePrevBlocks: availablePrevBlocks.filter((nType) =>
          availableBlocksFilter(nType, inContainer),
        ),
