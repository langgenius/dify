...
export const updateEdgeHoverState = (edges: Edge[], edgeId: string, hovering: boolean) =>
  produce(edges, (draft) => {
    const currentEdge = draft.find((edge) => edge.id === edgeId)
    if (currentEdge) currentEdge.data._hovering = hovering
  })

export const updateEdgeSelectionState = (edges: Edge[], changes: EdgeChange[]) =>
  produce(edges, (draft) => {
    changes.forEach((change) => {
      if (change.type === 'select') {
        const currentEdge = draft.find((edge) => edge.id === change.id)
        if (currentEdge) currentEdge.selected = change.selected
      }
    })
  })

export const buildContextMenuEdges = (edges: Edge[], edgeId: string) =>
  produce(edges, (draft) => {
    draft.forEach((item) => {
      item.selected = item.id === edgeId
      if (item.data._isBundled) item.data._isBundled = false
    })
  })

export const clearNodeSelectionState = (nodes: Node[]) =>
