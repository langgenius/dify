...
export const useDynamicTestRunOptions = (): TestRunOptions => {
  ...
  return useMemo(() => {
    ...
    for (const node of nodes) {
      ...
      if (nodeData.type === BlockEnum.Start) {
        userInput = {
          ...
          icon: <BlockIcon type={BlockEnum.Start} size="md" />,
          nodeId: node.id,
          enabled: true,
        }
        ...
        allTriggers.push({
          ...
          icon: <BlockIcon type={BlockEnum.TriggerSchedule} size="md" />,
          nodeId: node.id,
          enabled: true,
        })
        ...
        allTriggers.push({
          ...
          icon: <BlockIcon type={BlockEnum.TriggerWebhook} size="md" />,
          nodeId: node.id,
          enabled: true,
        })
        ...
        if (nodeData.provider_id) {
          ...
          triggerIcon = targetTriggers.find(
            (toolWithProvider) => toolWithProvider.name === nodeData.provider_id,
          )?.icon
        }

        const icon = <BlockIcon type={BlockEnum.TriggerPlugin} size="md" toolIcon={triggerIcon} />
        ...
        allTriggers.push({
          ...
          name:
            nodeData.title ||
            (nodeData as any).plugin_name ||
            t(($) => $['blocks.trigger-plugin'], { ns: 'workflow' }),
          icon,
          nodeId: node.id,
          enabled: true,
        })
    ...
    if (!userInput) {
      ...
      if (startNode && startNode.data?.type === BlockEnum.Start) {
        userInput = {
          ...
          type: TriggerType.UserInput,
          name:
            (startNode.data as CommonNodeType)?.title ||
            t(($) => $['blocks.start'], { ns: 'workflow' }),
          icon: <BlockIcon type={BlockEnum.Start} size="md" />,
          nodeId: startNode.id,
          enabled: true,
        }
    ...
    const runAll: TriggerOption | undefined =
      triggerNodeIds.length > 1
        ? {
            id: 'run-all',
