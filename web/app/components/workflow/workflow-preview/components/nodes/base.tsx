...
const BaseCard = ({ id, data, children }: NodeCardProps) => {
  ...
  return (
    <div
      className={cn('flex rounded-2xl border-2 border-transparent')}
      style={{
        width:
          data.type === BlockEnum.Iteration || data.type === BlockEnum.Loop ? data.width : 'auto',
        height:
          data.type === BlockEnum.Iteration || data.type === BlockEnum.Loop ? data.height : 'auto',
      }}
    >
      <div
        className={cn(
