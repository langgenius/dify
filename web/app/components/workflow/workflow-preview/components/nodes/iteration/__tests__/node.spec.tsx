...
vi.mock('reactflow', () => ({
  Background: (props: { id: string; gap: [number, number]; size: number; color: string }) => (
    <div
...
describe('workflow preview iteration node', () => {
  it('scales the dotted background with the current viewport zoom', () => {
    render(<IterationNode id="iteration-1" data={{} as never} />)

    expect(screen.getByTestId('background')).toHaveAttribute(
      'data-id',
      'iteration-background-iteration-1',
    )
