...
const RunningTitle = () => {
  ...
  return (
    <div className='flex items-center h-[18px] text-xs text-gray-500'>
      <ClockPlay className='mr-1 w-4 h-4 text-gray-500' />
      <span>{isChatMode ? `Test Chat#${historyWorkflowData?.sequence_number}` : `Test Run#${historyWorkflowData?.sequence_number}`}</span>
      <span className='mx-1'>·</span>
      <span className='ml-1 uppercase inline-block px-1 h-[18px] leading-[18px] rounded-[5px] border border-indigo-300 bg-white/[0.48] text-[10px] font-semibold text-indigo-600'>
