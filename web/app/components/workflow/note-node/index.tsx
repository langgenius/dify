...
import { NoteEditor, NoteEditorContextProvider, NoteEditorToolbar } from './note-editor'
...
const NoteNode = ({ id, data }: NodeProps<NoteNodeType>) => {
  ...
  const { handleNodesCopy, handleNodesDuplicate, handleNodeDelete } = useNodesInteractions()
      ...
      >
        <>
          ...
          <div className="grow overflow-y-auto px-3 py-2.5">
            <div className={cn(data.selected && 'nodrag nopan nowheel cursor-text')}>
              <NoteEditor
          ...
          {data.showAuthor && (
            <div className="p-3 pt-0 text-xs text-text-tertiary">{data.author}</div>
          )}
        </>
      </NoteEditorContextProvider>
    </div>
