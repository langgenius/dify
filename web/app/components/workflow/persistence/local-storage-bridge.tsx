...
export const WorkflowLocalStorageBridge = () => {
  const [storedNodePanelWidth] = useLocalStorage<number>(
    WORKFLOW_NODE_PANEL_WIDTH_KEY,
    undefined,
    numberStorageOptions,
  )
  const [storedPreviewPanelWidth] = useLocalStorage<number>(
    WORKFLOW_PREVIEW_PANEL_WIDTH_KEY,
    undefined,
    numberStorageOptions,
  )
  const [storedVariableInspectPanelHeight] = useLocalStorage<number>(
    WORKFLOW_VARIABLE_INSPECT_PANEL_HEIGHT_KEY,
    undefined,
    numberStorageOptions,
  )
  const [storedControlMode] = useLocalStorage<string>(
    WORKFLOW_OPERATION_MODE_KEY,
    undefined,
    rawStorageOptions,
  )
  ...
  const setControlModeStorage = useSetLocalStorage<string>(
    WORKFLOW_OPERATION_MODE_KEY,
    rawStorageOptions,
  )
