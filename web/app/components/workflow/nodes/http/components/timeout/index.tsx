...
const Timeout: FC<Props> = ({ readonly, payload, onChange }) => {
  ...
  return (
    <FieldCollapse
      title={t(($) => $[`${i18nPrefix}.timeout.title`], { ns: 'workflowIntegrations' })}
    >
      <div className="mt-2 space-y-1">
        <div className="space-y-3">
          <InputField
            title={t(($) => $['nodes.http.timeout.connectLabel'], { ns: 'workflowIntegrations' })!}
            description={t(($) => $['nodes.http.timeout.connectPlaceholder'], {
              ns: 'workflowIntegrations',
            })!}
            placeholder={t(($) => $['nodes.http.timeout.connectPlaceholder'], {
              ns: 'workflowIntegrations',
            })!}
            readOnly={readonly}
            value={connect}
          ...
          />
          <InputField
            title={t(($) => $['nodes.http.timeout.readLabel'], { ns: 'workflowIntegrations' })!}
            description={t(($) => $['nodes.http.timeout.readPlaceholder'], {
              ns: 'workflowIntegrations',
            })!}
            placeholder={t(($) => $['nodes.http.timeout.readPlaceholder'], {
              ns: 'workflowIntegrations',
            })!}
            readOnly={readonly}
            value={read}
          ...
          />
          <InputField
            title={t(($) => $['nodes.http.timeout.writeLabel'], { ns: 'workflowIntegrations' })!}
            description={t(($) => $['nodes.http.timeout.writePlaceholder'], {
              ns: 'workflowIntegrations',
            })!}
            placeholder={t(($) => $['nodes.http.timeout.writePlaceholder'], {
              ns: 'workflowIntegrations',
            })!}
            readOnly={readonly}
            value={write}
