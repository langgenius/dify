...
const useKnowledgeMetadataConfig = ({ id, inputRef, setInputs }: Params) => {
  ...
  const handleToggleConditionLogicalOperator =
    useCallback<HandleToggleConditionLogicalOperator>(() => {
      const nextInputs = produce(inputRef.current, (draft) => {
        ...
        draft.metadata_filtering_conditions!.logical_operator =
          currentLogicalOperator === LogicalOperator.and ? LogicalOperator.or : LogicalOperator.and
      })
      setInputs(nextInputs)
    }, [inputRef, setInputs])
