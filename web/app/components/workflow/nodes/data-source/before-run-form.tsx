...
import {
  ...
  useOnlineDrive,
  useWebsiteCrawl,
} from '@/app/components/rag-pipeline/components/panel/test-run/preparation/hooks'
...
const BeforeRunForm: FC<CustomRunFormProps> = (props) => {
  ...
  const clearDataSourceData = useCallback(() => {
    if (datasourceType === DatasourceType.onlineDocument) clearOnlineDocumentData()
    else if (datasourceType === DatasourceType.websiteCrawl) clearWebsiteCrawlData()
    else if (datasourceType === DatasourceType.onlineDrive) clearOnlineDriveData()
  ...
  return (
    <PanelWrap nodeName={payload.title} onHide={onCancel}>
