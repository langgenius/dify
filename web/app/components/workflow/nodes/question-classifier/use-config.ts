...
const useConfig = (id: string, payload: QuestionClassifierNodeType) => {
  ...
  const setInputs = useCallback(
    (newInputs: QuestionClassifierNodeType) => {
  ...
  const handleVisionChange = useCallback(
    (newPayload: QuestionClassifierNodeType['vision']) => {
  ...
  useEffect(() => {
    if (!isHandlingModelChangeRef.current) return
  ...
  useEffect(() => {
    ...
    const nextInputs = produce(currentInputs, (draft) => {
      if (!draft.model) draft.model = defaultConfig.model

      if (!draft.classes) draft.classes = defaultConfig.classes

      if (!draft._targetBranches) draft._targetBranches = defaultConfig._targetBranches

      if (!draft.vision) draft.vision = defaultConfig.vision

      if (
        !isSnippetFlow &&
        draft.query_variable_selector.length === 0 &&
        isChatMode &&
        startNodeId
      ) {
      ...
      if (!currentInputs.model && defaultConfig.model) shouldUpdate = true

      if (!currentInputs.classes && defaultConfig.classes) shouldUpdate = true

      if (!currentInputs._targetBranches && defaultConfig._targetBranches) shouldUpdate = true

      if (!currentInputs.vision && defaultConfig.vision) shouldUpdate = true
    })
  ...
  const handleSortTopic = useCallback(
    (newTopics: (Topic & { id: string })[]) => {
      const newInputs = produce(inputs, (draft) => {
