...
const extractSchemaType = (
  schema: any,
  _schemaTypeDefinitions?: SchemaTypeDefinition[],
): string | undefined => {
  if (!schema) return undefined
...
const toFieldType = (normalizedType: string | undefined, schemaType?: string): Type => {
  if (schemaType === 'file') return normalizedType === 'array' ? Type.array : Type.file
...
const convertJsonSchemaToField = (
  ...
  if (Array.isArray(schema?.enum)) field.enum = schema.enum

  if (fieldType === Type.object) {
    const properties = schema?.properties || {}
    field.properties = Object.entries(properties).reduce(
      (acc, [key, value]) => {
  ...
  return Object.entries(properties).map(([name, propertySchema]) => {
    ...
    if (normalizedType === 'object') {
      const childProperties = propertySchema?.properties
        ? Object.entries(propertySchema.properties).reduce(
            (acc, [key, value]) => {
              acc[key] = convertJsonSchemaToField(value, schemaTypeDefinitions)
              return acc
            },
    ...
    if (!errorMessage && isReadyForCheckValid) {
      triggerInputsSchema
        ...
        .forEach((field) => {
          ...
          const rawParam =
            payload.event_parameters?.[field.variable] ??
          ...
          const targetParam =
            typeof rawParam === 'object' && rawParam !== null && 'type' in rawParam
