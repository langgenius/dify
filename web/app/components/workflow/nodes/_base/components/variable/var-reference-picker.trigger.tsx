...
import Line3 from '@/app/components/base/icons/src/public/common/Line3'
...
const VarReferencePickerTrigger: FC<Props> = ({
  ...
  const pill = (
    <div
      className={cn(
        'h-full items-center rounded-[5px] px-1.5',
    ...
    >
      {hasValue ? (
        <>
            ...
            >
              <div className="h-3 px-px">
                {'type' in (outputVarNode || {}) && outputVarNode?.type && (
                  <VarBlockIcon type={outputVarNode.type} className="text-text-primary" />
                )}
              </div>
              <div
          ...
          {showErrorIcon && (
            <RiErrorWarningFill
              data-testid="var-reference-picker-error-icon"
              className="ml-0.5 size-3 text-text-destructive"
            />
          )}
        </>
      ) : (
        <div
          className={`overflow-hidden ${readonly ? 'text-components-input-text-disabled' : 'text-components-input-text-placeholder'} system-sm-regular text-ellipsis`}
        >
          {isLoading ? (
            <div className="flex items-center">
  ...
  const variablePicker = (
    <div className="h-full grow">
      <div
        ...
        className={cn(
          'h-full',
          isSupportConstantValue && 'flex items-center rounded-lg bg-components-panel-bg py-1 pl-1',
        )}
      >
        {hoveredPill}
      </div>
  ...
  const triggerContent = (
    <div
      className={cn(
        className,
        'group/picker-trigger-wrap relative flex!',
        !readonly && 'cursor-pointer',
      )}
      data-testid="var-reference-picker-trigger"
      onClick={() => {
        if (!isConstant || readonly) return
    ...
    >
      <>
        {isAddBtnTrigger ? (
          ...
          <div
            ...
            className={cn(
              ...
              'group/wrap relative flex h-8 w-full items-center',
              !isSupportConstantValue && 'rounded-lg bg-components-input-bg-normal p-1',
              isInTable && 'border-none bg-transparent',
              readonly && 'bg-components-input-bg-disabled',
                ...
                <TypeSelector
                  ...
                  trigger={
                    <div className="flex h-8 items-center rounded-lg bg-components-input-bg-normal px-2">
                      <div className="mr-1 system-sm-regular text-components-input-text-filled">
        ...
        {!hasValue && typePlaceHolder && (
          <Badge className="absolute top-1.5 right-2" text={typePlaceHolder} uppercase={false} />
        )}
      </>
