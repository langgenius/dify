...
export function formatValue(value: unknown, type: InputVarType) {
  ...
  if (type === InputVarType.number) return Number.parseFloat(String(value))
  if (type === InputVarType.json) return JSON.parse(String(value))
  if (type === InputVarType.contexts) return (value as string[]).map((item) => JSON.parse(item))
  if (type === InputVarType.multiFiles) return getProcessedFiles(value as FileEntity[])

  if (type === InputVarType.singleFile) {
    if (Array.isArray(value)) return getProcessedFiles(value as FileEntity[])
...
export const isFilesLoaded = (forms: FormProps[]) => {
  ...
  const files = filesForm.values['#files#'] as unknown as
    | Array<{ transfer_method?: TransferMethod; upload_file_id?: string }>
    | undefined
  return !files?.some(
  ...
  forms.forEach((form, index) => {
    ...
    form.inputs.forEach((input) => {
      ...
      if (!errMsg && missingRequired) {
        errMsg = t('errorMsg.fieldRequired', {
          ns: 'workflow',
      ...
      if (
        !errMsg &&
        (input.type === InputVarType.singleFile || input.type === InputVarType.multiFiles) &&
        value
      ) {
        const fileIsUploading = Array.isArray(value)
          ? value.find(
              (item: { transferMethod?: TransferMethod; uploadedId?: string }) =>
                item.transferMethod === TransferMethod.local_file && !item.uploadedId,
            )
          : (value as { transferMethod?: TransferMethod; uploadedId?: string }).transferMethod ===
              TransferMethod.local_file &&
            !(value as { transferMethod?: TransferMethod; uploadedId?: string }).uploadedId

        if (fileIsUploading) errMsg = t('errorMessage.waitForFileUpload', { ns: 'appDebug' })
      }
    })
  })
...
export const shouldAutoRunBeforeRunForm = (
...
export const shouldAutoShowGeneratedForm = (
  filteredExistVarForms: FormProps[],
  isHumanInput: boolean,
) => {
  return filteredExistVarForms.length === 0 && isHumanInput
