...
const ConditionValue = ({
  ...
  const operatorName = isComparisonOperatorNeedTranslate(operator) ? t(`workflow.nodes.ifElse.comparisonOperator.${operator}` as any) : operator
  ...
  const selectName = useCallback((c: Condition) => {
    ...
    if (isSelect) {
      ...
      return name
        ? t(`workflow.nodes.ifElse.optionName.${name.i18nKey}` as any).replace(/\{\{#([^#]*)#\}\}/g, (a: string, b: string) => {
            const arr: string[] = b.split('.')
            if (isSystemVar(arr))
              return `{{${b}}}`
      ...
      <div className="ml-[10px] border-l border-divider-regular pl-[10px]">
        {
          sub_variable_condition?.conditions.map((c: Condition, index) => (
            <div className="relative flex h-6 items-center space-x-1" key={c.id}>
              ...
              <div className="system-xs-medium text-text-primary">{isComparisonOperatorNeedTranslate(c.comparison_operator) ? t(`workflow.nodes.ifElse.comparisonOperator.${c.comparison_operator}` as any) : c.comparison_operator}</div>
              ...
              {index !== sub_variable_condition.conditions.length - 1 && (<div className="absolute bottom-[-10px] right-1 z-10 text-[10px] font-medium uppercase leading-4 text-text-accent">{t(`${i18nPrefix}.${sub_variable_condition.logical_operator}` as any)}</div>)}
            </div>
          ))
        }
      </div>
