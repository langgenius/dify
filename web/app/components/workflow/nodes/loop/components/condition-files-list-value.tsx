...
const ConditionValue = ({ condition }: ConditionValueProps) => {
  ...
  const isSelect = useCallback((c: Condition) => {
    return (
      c.comparison_operator === ComparisonOperator.in ||
      c.comparison_operator === ComparisonOperator.notIn
    )
  }, [])
        ...
        {sub_variable_condition?.conditions.map((c: Condition, index) => (
          <div className="relative flex h-6 items-center space-x-1" key={c.id}>
            ...
            <div className="system-xs-medium text-text-primary">
              {isComparisonOperatorNeedTranslate(c.comparison_operator)
                ? t(`nodes.ifElse.comparisonOperator.${c.comparison_operator}`, { ns: 'workflow' })
