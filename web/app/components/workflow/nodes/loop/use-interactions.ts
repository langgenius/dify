...
export const useNodeLoopInteractions = () => {
  ...
  const handleNodeLoopChildDrag = useCallback(
    (node: Node) => {
      ...
      return {
        restrictPosition: getRestrictedLoopPosition(
          node,
  ...
  const handleNodeLoopChildSizeChange = useCallback(
    (nodeId: string) => {
      const { nodes } = collaborativeWorkflow.getState()
      const currentNode = nodes.find((n) => n.id === nodeId)!
      const parentId = currentNode.parentId

      if (parentId) handleNodeLoopRerender(parentId)
    },
    [collaborativeWorkflow, handleNodeLoopRerender],
  )

  const handleNodeLoopChildrenCopy = useCallback(
    (nodeId: string, newNodeId: string, idMapping: Record<string, string>) => {
      const { nodes } = collaborativeWorkflow.getState()
