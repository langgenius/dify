...
import { LOOP_PADDING, NESTED_ELEMENT_Z_INDEX } from '../../constants'
...
export const getRestrictedLoopPosition = (node: Node, parentNode?: Node) => {
  ...
  if (!node.data.isInLoop || !parentNode) return restrictPosition
...
export const buildLoopChildCopy = ({
  ...
  const params = {
    ...
    data: {
      ...
      _dimmed: false,
      title:
        nodesWithSameTypeCount > 0
          ? `${defaultValue.title} ${nodesWithSameTypeCount + 1}`
          : defaultValue.title,
      isInLoop: true,
      loop_id: newNodeId,
      type: childNodeType,
