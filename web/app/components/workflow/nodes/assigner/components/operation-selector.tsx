      ...
      >
        ...
        >
          <div className="flex items-center p-1">
            ...
            >
              {selectedItem?.name ? t(`${i18nPrefix}.operations.${selectedItem?.name}` as any) : t(`${i18nPrefix}.operations.title` as any)}
            </span>
          </div>
        ...
        <div className="flex w-[140px] flex-col items-start rounded-xl border-[0.5px] border-components-panel-border bg-components-panel-bg-blur shadow-lg">
          <div className="flex flex-col items-start self-stretch p-1">
            <div className="flex items-start self-stretch px-3 pb-0.5 pt-1">
              <div className="system-xs-medium-uppercase flex grow text-text-tertiary">{t(`${i18nPrefix}.operations.title` as any)}</div>
            </div>
            {items.map(item => (
              item.value === 'divider'
                ? (
                ...
                : (
                    ...
                    >
                      <div className="flex min-h-5 grow items-center gap-1 px-1">
                        <span className="system-sm-medium flex grow text-text-secondary">{t(`${i18nPrefix}.operations.${item.name}` as any)}</span>
                      </div>
                      {item.value === value && (
