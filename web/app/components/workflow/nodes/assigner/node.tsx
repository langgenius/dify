...
const NodeComponent: FC<NodeProps<AssignerNodeType>> = ({ data }) => {
  ...
  if (data.version === '2') {
    ...
    const validOperationItems =
      operationItems?.filter(
        (item) => item.variable_selector && item.variable_selector.length > 0,
      ) || []
              ...
              rightSlot={
                !!value.operation && (
                  <Badge
                    ...
                    text={t(`${i18nPrefix}.operations.${value.operation}`, { ns: 'workflow' })}
                  />
                )
              }
            />
  ...
  const { assigned_variable_selector: variable, write_mode: writeMode } =
    data as unknown as LegacyAssignerNodeType
      ...
      <VariableLabelInNode
        ...
        rightSlot={
          writeMode && (
            <Badge
              className="ml-auto! shrink-0"
              text={t(`nodes.assigner.operations.${writeMode}`, { ns: 'workflow' })}
            />
          )
        }
      />
