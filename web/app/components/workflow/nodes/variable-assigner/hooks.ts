...
export const useVariableAssigner = () => {
  ...
  const handleAssignVariableValueChange = useCallback(
    (nodeId: string, value: ValueSelector, varDetail: Var, groupId?: string) => {
      ...
      if (groupId && groupId !== 'target') {
        ...
        if (
          node.data.variables.some((item) => item.join('.') === (value as ValueSelector).join('.'))
        )
          return
        payload = {
...
export const useGetAvailableVars = () => {
  ...
  const getAvailableVars = useCallback(
    (nodeId: string, handleId: string, filterVar: (v: Var) => boolean, hideEnv = false) => {
      ...
      if (hideEnv) {
        const availableVars = getNodeAvailableVars({
          ...
          .map((node) => ({
            ...node,
            vars: node.isStartNode
              ? node.vars.filter((v) => !v.variable.startsWith('sys.'))
              : node.vars,
          }))
      ...
      return filterSnippetSystemVars(
