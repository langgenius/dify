...
const useConfig = (id: string, payload: EventSourceNodeType) => {
  const { nodesReadOnly: readOnly } = useNodesReadOnly()
  const defaultConfig = useStore(s => s.nodesDefaultConfigs)[payload.type]

  const { inputs, setInputs: doSetInputs } = useNodeCrud<EventSourceNodeType>(id, payload)
  const inputsRef = useRef(inputs)
  const setInputs = useCallback((newPayload: EventSourceNodeType) => {
  ...
  const { handleAddVariable: handleAddEmptyVariable } = useVarList<EventSourceNodeType>({
    inputs,
    setInputs,
  })
  ...
  const {
    ...
    setRunInputData,
    runResult,
  } = useOneStepRun<EventSourceNodeType>({
    id,
    data: inputs,
    defaultRunInputData: {},
  })
