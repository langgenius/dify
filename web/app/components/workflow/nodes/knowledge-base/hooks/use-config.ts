import type { KnowledgeBaseNodeType, RerankingModel, SummaryIndexSetting } from '../types'
...
export const useConfig = (id: string) => {
  ...
  const handleNodeDataUpdate = useCallback(
    (data: Partial<KnowledgeBaseNodeType>) => {
  ...
  const handleChunkStructureChange = useCallback(
    (chunkStructure: ChunkStructureEnum) => {
      ...
      handleNodeDataUpdate({
        ...
        indexing_technique:
          ...
          chunkStructure === ChunkStructureEnum.question_answer
            ? IndexMethodEnum.QUALIFIED
            : indexing_technique,
        retrieval_model: {
          ...retrieval_model,
          search_method:
            (chunkStructure === ChunkStructureEnum.parent_child ||
              chunkStructure === ChunkStructureEnum.question_answer) &&
            !isHighQualitySearchMethod(search_method)
              ? RetrievalSearchMethodEnum.keywordSearch
              : search_method,
        },
        index_chunk_variable_selector:
          chunkStructure === chunk_structure ? index_chunk_variable_selector : [],
      })
    },
  ...
  const handleIndexMethodChange = useCallback(
    (indexMethod: IndexMethodEnum) => {
      ...
      handleNodeDataUpdate(
        produce(nodeData?.data as KnowledgeBaseNodeType, (draft) => {
  ...
  const handleKeywordNumberChange = useCallback(
  ...
  const handleRetrievalSearchMethodChange = useCallback(
    (searchMethod: RetrievalSearchMethodEnum) => {
      ...
      const changeData = {
        retrieval_model: {
          ...
          reranking_mode:
            nodeData?.data.retrieval_model.reranking_mode || RerankingModeEnum.RerankingModel,
        },
      }
      if (searchMethod === RetrievalSearchMethodEnum.hybrid) {
        changeData.retrieval_model = {
          ...changeData.retrieval_model,
          reranking_enable:
            changeData.retrieval_model.reranking_mode === RerankingModeEnum.RerankingModel,
        }
      }
      handleNodeDataUpdate(changeData)
    },
  ...
  const handleHybridSearchModeChange = useCallback(
  ...
  const handleRerankingModelEnabledChange = useCallback(
  ...
  const handleWeighedScoreChange = useCallback(
    (weightedScore: { value: number[] }) => {
      const nodeData = getNodeData()
      handleNodeDataUpdate({
        retrieval_model: {
          ...nodeData?.data.retrieval_model,
  ...
  const handleScoreThresholdEnabledChange = useCallback(
    (isEnabled: boolean) => {
      const nodeData = getNodeData()
      handleNodeDataUpdate({
        retrieval_model: {
  ...
  const handleInputVariableChange = useCallback(
    (inputVariable: string | ValueSelector) => {
      handleNodeDataUpdate({
  ...
  const handleSummaryIndexSettingChange = useCallback(
