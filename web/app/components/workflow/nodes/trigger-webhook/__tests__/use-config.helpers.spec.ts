...
describe('trigger webhook config helpers', () => {
  it('syncs variables, updates existing ones and validates names', () => {
    ...
    expect(draft.variables).toContainEqual(
  ...
  it('updates content, source fields and webhook urls', () => {
    ...
    ).toEqual([{ name: 'page', type: VarType.number, required: true }])
    ...
    ).toEqual([{ name: 'payload', type: VarType.string, required: true }])
    ...
    ).toEqual([{ name: 'x-request-id', required: true }])
    ...
    expect(updateWebhookUrls(createInputs(), 'https://hook', 'https://debug')).toEqual(
      expect.objectContaining({
