...
export const checkJsonDepth = (json: any) => {
  ...
  if (getTypeOf(json) === "array") {
    ...
    const propertyDepths = Object.values(json).map((value) =>
...
export const convertBooleanToString = (schema: any) => {
  if (schema.type === Type.boolean) schema.type = Type.string;
  if (
    schema.type === Type.array &&
    schema.items &&
    schema.items.type === Type.boolean
  )
  ...
  if (
    ...
    schema.items.properties = Object.entries(schema.items.properties).reduce(
      (acc, [key, value]) => {
        acc[key] = convertBooleanToString(value);
        return acc;
      },
...
const fieldSchema: z.ZodTypeAny = z.lazy(() =>
  z
    .object({
    ...
    .superRefine((val, ctx) => {
      ...
      if (
        ...
        (val.properties === undefined || typeof val.properties !== "object")
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "Object fields must have a properties object.",
        });
      }
      ...
      if (val.type === Type.array && !val.items) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "Array fields must define an items schema.",
        });
      }
    }),
);
