...
describe('use-llm-prompt-config', () => {
  it('initializes prompt config storage when it is missing or incomplete', () => {
    ...
    const { result: missingConfigResult } = renderHook(() =>
      useLLMPromptConfig({
    ...
    const { result: missingVariablesResult } = renderHook(() =>
      useLLMPromptConfig({
    ...
    expect(handleMissingVariablesInputs).toHaveBeenCalledWith(
      expect.objectContaining({
        prompt_config: {
          jinja2_variables: [
            {
  ...
  it('updates variables, context, prompt, and memory for chat prompts', () => {
    ...
    expect(
      (
        handleSetInputs.mock.calls[3]![0].prompt_template as Array<{
          jinja2_text?: string
        }>
      )[0]!.jinja2_text,
    ).toBe('{{ new_name }}')
  ...
  it('handles completion prompt branches, including the non-jinja early return and jinja rename flow', () => {
    ...
    const { result: jinjaResult } = renderHook(() =>
      useLLMPromptConfig({
    ...
    expect(handleJinjaInputs).toHaveBeenCalledWith(
      expect.objectContaining({
        prompt_template: expect.objectContaining({
          jinja2_text: '{{ budget }}',
        }),
