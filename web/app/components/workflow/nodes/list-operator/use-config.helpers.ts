...
export const updateFilterEnabled = (inputs: ListFilterNodeType, enabled: boolean) =>
  produce(inputs, (draft) => {
    draft.filter_by.enabled = enabled
    if (enabled && !draft.filter_by.conditions) draft.filter_by.conditions = []
  })

export const updateFilterCondition = (inputs: ListFilterNodeType, condition: Condition) =>
  produce(inputs, (draft) => {
    draft.filter_by.conditions[0] = condition
  })

export const updateLimit = (inputs: ListFilterNodeType, limit: Limit) =>
  produce(inputs, (draft) => {
    draft.limit = limit
  })

export const updateExtractEnabled = (inputs: ListFilterNodeType, enabled: boolean) =>
  produce(inputs, (draft) => {
    draft.extract_by.enabled = enabled
    if (enabled) draft.extract_by.serial = '1'
  })

export const updateExtractSerial = (inputs: ListFilterNodeType, value: string) =>
  produce(inputs, (draft) => {
...
export const updateOrderByKey = (inputs: ListFilterNodeType, key: string) =>
  produce(inputs, (draft) => {
    draft.order_by.key = key
  })

export const updateOrderByType = (inputs: ListFilterNodeType, type: OrderBy) =>
  produce(inputs, (draft) => {
