...
const useConfig = (id: string, payload: IterationNodeType) => {
  ...
  const handleInputChange = useCallback(
    (input: ValueSelector | string, _varKindType: VarKindType, varInfo?: Var) => {
  ...
  const childrenNodeVars = toNodeOutputVars(
  ...
  const handleOutputVarChange = useCallback(
    (output: ValueSelector | string, _varKindType: VarKindType, varInfo?: Var) => {
      if (isEqual(inputs.output_selector, output as ValueSelector)) return
  ...
  const changeFlattenOutput = useCallback(
    (value: boolean) => {
      const newInputs = produce(inputs, (draft) => {
