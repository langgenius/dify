...
export const useNodeIterationInteractions = () => {
  ...
  const handleNodeIterationRerender = useCallback(
    (nodeId: string) => {
      ...
      const resize = getIterationContainerResize(
        currentNode,
        getIterationContainerBounds(childrenNodes),
      )
  ...
  const handleNodeIterationChildDrag = useCallback(
    (node: Node) => {
      ...
      return {
        restrictPosition: getRestrictedIterationPosition(
          node,
  ...
  const handleNodeIterationChildrenCopy = useCallback(
    (nodeId: string, newNodeId: string, idMapping: Record<string, string>) => {
      ...
      const copyChildren = childrenNodes.map((child, index) => {
        ...
        const nextCount = getNextChildNodeTypeCount(
          childNodeTypeCount,
          childNodeType,
          nodesWithSameType.length,
        )
        const title =
