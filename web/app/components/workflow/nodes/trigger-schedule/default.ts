    ...
    if (!errorMessages) {
      if (payload.mode === 'cron') {
        if (!payload.cron_expression || payload.cron_expression.trim() === '')
          ...
          errorMessages = t(($) => $['nodes.triggerSchedule.invalidCronExpression'], {
            ns: 'workflow',
          })
        ...
        if (!payload.frequency)
          errorMessages = t(($) => $[`${i18nPrefix}.fieldRequired`], {
            ns: 'workflow',
            field: t(($) => $['nodes.triggerSchedule.frequencyLabel'], { ns: 'workflow' }),
          })
    ...
    if (!errorMessages) {
      try {
        ...
        if (nextTimes.length === 0)
          errorMessages = t(($) => $['nodes.triggerSchedule.noValidExecutionTime'], {
            ns: 'workflow',
          })
      } catch {
        errorMessages = t(($) => $['nodes.triggerSchedule.executionTimeCalculationError'], {
          ns: 'workflow',
        })
      }
    }
