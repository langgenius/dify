...
describe('iteration format log', () => {
  ...
  it('should keep missing iteration index items in the current record when the node has not restarted', () => {
    ...
    const result = addChildrenToIterationNode(parent, [firstRunChild, secondRunChild, streamingChild])

    expect(result.details).toEqual([
      [firstRunChild],
      [secondRunChild, streamingChild],
    ])
  })
  ...
  it('should not move an earlier missing iteration index item into the latest record', () => {
    ...
    const result = addChildrenToIterationNode(parent, [firstRunChild, streamingChild, secondRunChild])

    expect(result.details).toEqual([
      [firstRunChild, streamingChild],
      [secondRunChild],
    ])
  })
})
