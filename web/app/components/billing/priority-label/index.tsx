...
const PriorityLabel = ({ className }: PriorityLabelProps) => {
  ...
  return (
    <Tooltip popupContent={(
      <div>
        <div className="mb-1 text-xs font-semibold text-text-primary">{`${t('billing.plansCommon.documentProcessingPriority')}: ${t(`billing.plansCommon.priority.${priority}` as any)}`}</div>
        {
    ...
    >
      ...
      >
        ...
        <span>{t(`billing.plansCommon.priority.${priority}` as any)}</span>
      </div>
    </Tooltip>
  )
}
