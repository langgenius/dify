...
const PriorityLabel = ({ className }: PriorityLabelProps) => {
  ...
  return (
    <Tooltip popupContent={(
      <div>
        <div className="mb-1 text-xs font-semibold text-text-primary">{`${t('plansCommon.documentProcessingPriority', { ns: 'billing' })}: ${t(`billing.plansCommon.priority.${priority}` as any) as string}`}</div>
        {
