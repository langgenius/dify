...
const style = {
  ...
  [Plan.team]: {
    ...
    title: "text-[#3538CD]",
    hoverAndActive:
      "hover:shadow-lg hover:!text-white hover:!bg-[#444CE7] hover:!border-[#3538CD] active:!text-white active:!bg-[#3538CD] active:!border-[#3538CD]",
  },
  [Plan.enterprise]: {
    bg: "bg-[#FFEED3]",
    title: "text-[#DC6803]",
...
const PlanItem: FC<Props> = ({
  ...
  const isPlanDisabled =
    planInfo.level <= ALL_PLANS[currentPlan].level ||
    (!canPay && plan !== Plan.enterprise);
  const { isCurrentWorkspaceManager } = useAppContext();
  const messagesRequest = (() => {
    const value = planInfo.messageRequest[isZh ? "zh" : "en"];
    if (value === contractSales) return t("billing.plansCommon.contractSales");
  ...
  const btnText = (() => {
    ...
    if (isCurrent) return t("billing.plansCommon.currentPlan");

    return {
      [Plan.sandbox]: t("billing.plansCommon.startForFree"),
      [Plan.professional]: (
        <>
          {t("billing.plansCommon.getStartedWith")}
  ...
  const comingSoon = (
    <div className="leading-[12px] text-[9px] font-semibold text-[#3538CD] uppercase">
              ...
              <TooltipPlus
                popupContent={
                  <div className="w-[200px]">
                    {t(
                      "billing.plansCommon.supportItems.llmLoadingBalancingTooltip"
                    )}
                  </div>
                }
              >
  ...
  return (
    <div
      className={cn(
        isMostPopularPlan ? "bg-[#0086C9] p-0.5" : "pt-7",
        "flex flex-col min-w-[290px] w-[290px] rounded-xl"
      )}
    >
      {isMostPopularPlan && (
        <div className="flex items-center h-7 justify-center leading-[12px] text-xs font-medium text-[#F5F8FF]">
      ...
      <div className={cn(style[plan].bg, "grow px-6 py-6 rounded-[10px]")}>
        ...
        <div
          className={cn(
            isFreePlan ? "mb-5 text-[#FB6514]" : "mb-4 text-gray-500",
        ...
        {!isFreePlan && !isEnterprisePlan && (
          <div className="flex items-end h-9">
            ...
            <div className="ml-1">
              {isYear && (
                <div className="leading-[18px] text-xs font-medium text-[#F26725]">
                  {t("billing.plansCommon.save")}${planInfo.price * 2}
                </div>
              )}
              <div className="leading-[18px] text-[15px] font-normal text-gray-500">
        ...
        <div
          className={cn(
            isMostPopularPlan &&
              !isCurrent &&
              "!bg-[#444CE7] !text-white !border !border-[#3538CD] shadow-sm",
            isPlanDisabled
              ? "opacity-30"
              : `${style[plan].hoverAndActive} cursor-pointer`,
            "mt-4 flex h-11 items-center justify-center border-[2px] border-gray-900 rounded-3xl text-sm font-semibold text-gray-900"
          )}
        ...
        <KeyValue
          ...
          value={
            planInfo.teamMembers === NUM_INFINITE
              ? (t("billing.plansCommon.unlimited") as string)
        ...
        <KeyValue
          ...
          value={
            planInfo.buildApps === NUM_INFINITE
              ? (t("billing.plansCommon.unlimited") as string)
              : planInfo.buildApps
          }
        />
        <KeyValue
          ...
          value={
            planInfo.vectorSpace === NUM_INFINITE
              ? (t("billing.plansCommon.unlimited") as string)
              : planInfo.vectorSpace >= 1000
              ? `${planInfo.vectorSpace / 1000}G`
              : `${planInfo.vectorSpace}MB`
          }
        ...
        <KeyValue
          ...
          value={
            planInfo.annotatedResponse === NUM_INFINITE
              ? (t("billing.plansCommon.unlimited") as string)
        ...
        <KeyValue
          ...
          value={
            planInfo.customTools === NUM_INFINITE
              ? (t("billing.plansCommon.unlimited") as string)
              : planInfo.customTools === unAvailable
              ? (t("billing.plansCommon.unavailable") as string)
