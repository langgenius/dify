...
export const useWorkflowTemplate = () => {
  ...
  const { newNode: startNode } = generateNewNode({
    data: {
      ...
      title: t(`workflow.blocks.${startDefault.metaData.type}` as any),
    },
  ...
  if (isChatMode) {
    const { newNode: llmNode } = generateNewNode({
      ...
      data: {
        ...
        title: t(`workflow.blocks.${llmDefault.metaData.type}` as any),
      },
      position: {
    ...
    const { newNode: answerNode } = generateNewNode({
      ...
      data: {
        ...
        title: t(`workflow.blocks.${answerDefault.metaData.type}` as any),
      },
      position: {
