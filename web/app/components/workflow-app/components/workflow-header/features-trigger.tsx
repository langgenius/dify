...
import { useChecklist, useChecklistBeforePublish, useIsChatMode, useNodesReadOnly, useNodesSyncDraft,
...
const FeaturesTrigger = () => {
  ...
  const startNodeIds = useMemo(() => nodes.filter(node => node.data.type === BlockEnum.Start).map(node => node.id), [nodes])
  ...
  const hasTriggerNode = useMemo(() => (nodes.some(node => isTriggerNode(node.data.type as BlockEnum))), [nodes])
  ...
  const onPublish = useCallback(async (params?: ModelAndParameter | PublishWorkflowParams) => {
    ...
    if (await handleCheckBeforePublish()) {
      ...
      if (res) {
        ...
        toast.success(publishSuccessMessage)
  ...
  }, [needWarningNodes, handleCheckBeforePublish, publishWorkflow, appID, t, updatePublishedWorkflow, updateAppDetail, workflowStore, resetWorkflowVersionHistory, invalidateAppTriggers, hasUserInputNode, isChatMode])
  ...
  return (
    <>
      ...
      {isChatMode && (
        <Button className={cn('rounded-lg border border-transparent text-components-button-secondary-text', theme === 'dark' && 'border-black/5 bg-white/10 backdrop-blur-sm')} onClick={handleShowFeatures}>
