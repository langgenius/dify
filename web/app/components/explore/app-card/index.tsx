...
const AppCard = ({ app, canCreate, onCreate, isExplore }: AppCardProps) => {
  ...
  return (
    <Card isPressable className="min-h-[160px] col-span-1 group">
      <div className="h-full w-full  bg-white border-2 border-solid border-transparent rounded-lg shadow-sm  flex flex-col transition-all duration-200 ease-in-out cursor-pointer hover:shadow-lg">
        <div className="flex pt-[14px] text-start px-[14px] pb-3 h-[66px] items-center gap-3 grow-0 shrink-0">
        ...
        <div className="mb-1 px-[14px] text-xs text-start leading-normal text-gray-500 line-clamp-4 group-hover:line-clamp-2 group-hover:h-9">
          {app.description}
        </div>
        {isExplore && canCreate && (
          <div
            className={cn(
              "hidden items-center flex-wrap min-h-[42px] px-[14px] pt-2 pb-[10px] group-hover:flex"
            )}
          >
