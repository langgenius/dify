...
export default function AppNavItem({
  ...
  const folded = !isMainNav && isFolded
      ...
      className={cn(
        isMainNav
          ...
          : cn(
              ...
              'w-full justify-start px-2 data-[folded=true]:justify-center data-[folded=true]:px-1',
            ),
      )}
    >
      <Link
        ...
        aria-label={name}
        title={name}
        className={cn(isMainNav ? 'flex min-w-0 flex-1 items-center gap-2 outline-hidden' : 'flex w-0 grow items-center space-x-2 outline-hidden group-data-[folded=true]:w-auto group-data-[folded=true]:justify-center group-data-[folded=true]:space-x-0')}
      >
        ...
        <div className={cn(isMainNav ? 'min-w-0 flex-1 truncate py-1 pr-1 system-sm-regular' : 'truncate system-sm-regular text-components-menu-item-text group-data-[folded=true]:hidden')} title={name}>{name}</div>
      </Link>
      <div className={cn(isMainNav ? 'h-6 shrink-0' : 'h-6 shrink-0 group-data-[folded=true]:hidden')}>
        <ItemOperation
          isPinned={isPinned}
          togglePin={togglePin}
