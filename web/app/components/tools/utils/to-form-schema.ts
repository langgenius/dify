...
export const toolCredentialToFormSchemas = (
  parameters: ToolCredential[],
): ToolCredentialFormSchema[] => {
  if (!parameters) return []
...
export const addDefaultValue = (
  ...
  formSchemas: { variable: string; type: string; default?: unknown }[],
) => {
    ...
    if (
      ...
      if (typeof itemValue === 'string')
        newValues[formSchema.variable] =
          itemValue === 'true' || itemValue === '1' || itemValue === 'True'
      else if (typeof itemValue === 'number') newValues[formSchema.variable] = itemValue === 1
      else if (typeof itemValue === 'boolean') newValues[formSchema.variable] = itemValue
    }
  })
  return newValues
}

const correctInitialData = (
  type: string,
  target: FormValueInput,
  ...
  if (type === 'boolean') {
    ...
    if (typeof defaultValue === 'boolean') target.value = defaultValue

    if (typeof defaultValue === 'number') target.value = defaultValue === 1
  }
...
export const generateFormValue = (
  value: Record<string, unknown>,
  formSchemas: { variable: string; default?: unknown; type: string }[],
  isReasoning = false,
) => {
...
export const getStructureValue = (
  value: Record<string, unknown>,
  ...
  formSchemas.forEach((formSchema) => {
    ...
    if (
      ...
      const initialValue: FormValueInput = {
        type: 'constant',
        value:
          typeof formSchema.default === 'string'
            ? formSchema.default.replace(/\n/g, '\\n')
            : formSchema.default,
      }
...
export const generateAgentToolValue = (
  value: Record<string, { value?: unknown; auto?: 0 | 1 }>,
  formSchemas: { variable: string; default?: unknown; type: string }[],
  isReasoning = false,
) => {
  ...
  if (!isReasoning) {
    formSchemas.forEach((formSchema) => {
      ...
      newValues[formSchema.variable]!.value = correctInitialData(
        formSchema.type,
