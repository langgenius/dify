...
const WorkflowToolDrawerFrame = ({
  ...
  onHide,
  children,
}: WorkflowToolDrawerFrameProps) => {
  const handleOpenChange = React.useCallback<NonNullable<DrawerRootProps['onOpenChange']>>(
    (open) => {
      if (!open) onHide()
    },
    [onHide],
...
export function WorkflowToolDrawer({
  ...
  const onConfirm = () => {
    ...
    if (!name)
      errorMessage = t('errorMsg.fieldRequired', {
        ns: 'common',
        field: t('createTool.nameForToolCall', { ns: 'tools' }),
      })
          ...
          <div className="h-0 grow space-y-4 overflow-y-auto px-6 py-3">
            ...
            <div>
              <div className="py-2 system-sm-medium text-text-primary">
                {t('createTool.name', { ns: 'tools' })} <span className="ml-1 text-red-500">*</span>
              </div>
                      ...
                      <tr key={index} className="border-b border-divider-regular last:border-0">
                        ...
                        <td>
                          ...
                          {item.name !== '__image' && (
                            <MethodSelector
                              value={item.form}
                              onChange={(value) => handleParameterChange('form', value, index)}
                            />
                          )}
                        </td>
                        <td className="w-[236px] p-2 pl-3 text-text-tertiary">
                          <input
                            ...
                            placeholder={
                              t('createTool.toolInput.descriptionPlaceholder', { ns: 'tools' })!
                            }
                                ...
                                <Tooltip>
                                  ...
                                  <TooltipContent>
                                    <div className="w-[180px]">
                                      {t('createTool.toolOutput.reservedParameterDuplicateTip', {
                                        ns: 'tools',
                                      })}
                                    </div>
                                  </TooltipContent>
