...
const Empty = ({
  ...
  const hasTitle = t(`tools.addToolModal.${renderType}.title` as any) !== `tools.addToolModal.${renderType}.title`
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      ...
      <div className="mb-1 mt-2 text-[13px] font-medium leading-[18px] text-text-primary">
        {hasTitle ? t(`tools.addToolModal.${renderType}.title` as any) : 'No tools available'}
      </div>
      {(!isAgent && hasTitle) && (
        <Comp className={cn('flex items-center text-[13px] leading-[18px] text-text-tertiary', hasLink && 'cursor-pointer hover:text-text-accent')} {...linkProps}>
          {t(`tools.addToolModal.${renderType}.tip` as any)}
          {' '}
