...
const ProviderCard = ({ active, collection, onSelect }: Props) => {
  ...
  return (
    <Card
      isPressable
      className={cn(
        "group col-span-1 bg-white border-2 border-solid border-transparent rounded-xl shadow-sm min-h-[160px] flex flex-col transition-all duration-200 ease-in-out cursor-pointer hover:shadow-lg",
    ...
    >
      <div className="flex pt-[14px] px-[14px] pb-3 h-[66px] items-center gap-3 grow-0 shrink-0">
        <div className="relative shrink-0">
          {typeof collection.icon === "string" && (
            <div
              className="w-10 h-10 bg-center bg-cover bg-no-repeat rounded-md"
      ...
      {collection.labels?.length > 0 && (
        <div className="flex items-center shrink-0 mt-1 pt-1 pl-[14px] pr-[6px] pb-[6px] h-[42px]">
          <div
            className="relative w-full flex items-center gap-1 py-[7px] rounded-md text-gray-500"
            title={labelContent}
          >
