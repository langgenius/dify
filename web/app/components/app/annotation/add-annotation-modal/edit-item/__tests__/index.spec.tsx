...
describe('AddAnnotationModal/EditItem', () => {
  it('should render query inputs with user avatar and placeholder strings', () => {
    render(<EditItem type={EditItemType.Query} content="Why?" onChange={vi.fn()} />)

    expect(screen.getByText('appAnnotation.addModal.queryName')).toBeInTheDocument()
    expect(
      screen.getByPlaceholderText('appAnnotation.addModal.queryPlaceholder'),
    ).toBeInTheDocument()
    expect(screen.getByText('Why?')).toBeInTheDocument()
  })

  it('should render answer name and placeholder text', () => {
    render(<EditItem type={EditItemType.Answer} content="Existing answer" onChange={vi.fn()} />)

    expect(screen.getByText('appAnnotation.addModal.answerName')).toBeInTheDocument()
    expect(
      screen.getByPlaceholderText('appAnnotation.addModal.answerPlaceholder'),
    ).toBeInTheDocument()
  ...
  it('should propagate changes when answer content updates', () => {
    ...
    render(<EditItem type={EditItemType.Answer} content="" onChange={handleChange} />)
