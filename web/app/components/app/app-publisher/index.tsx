...
const AppPublisher = ({
  ...
  const isAppAccessSet = useMemo(
    () => isPublisherAccessConfigured(appDetail, appAccessSubjects),
    [appAccessSubjects, appDetail],
  )

  const noAccessPermission = useMemo(
    () =>
      Boolean(
        systemFeatures.webapp_auth.enabled &&
        appDetail &&
  ...
  const disabledFunctionButton = useMemo(
    () => !publishedAt || missingStartNode || noAccessPermission,
    [publishedAt, missingStartNode, noAccessPermission],
  )
  ...
  useEffect(() => {
    if (systemFeatures.webapp_auth.enabled && open && appDetail) refetch()
