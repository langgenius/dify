...
describe('log list utils', () => {
  ...
  it('should update annotation state helpers', () => {
    ...
    expect(
      applyAnnotationAdded(items, 'annotation-1', 'Dify', 'question', 'answer', 1)[1]!.annotation,
    ).toEqual(
      expect.objectContaining({
        id: 'annotation-1',
        authorName: 'Dify',
  ...
  it('should derive urls, scroll thresholds, row values, and detail metadata', () => {
    expect(buildConversationUrl('/apps/app-1/logs', 'page=2', 'conversation-1')).toBe(
      '/apps/app-1/logs?page=2&conversation_id=conversation-1',
    )
    expect(
      isNearTopLoadMore({
        clientHeight: 200,
  ...
  it('should remove conversation ids from urls, handle default inputs, and detect conversation feedback', () => {
    expect(buildConversationUrl('/apps/app-1/logs', 'page=2&conversation_id=conversation-1')).toBe(
      '/apps/app-1/logs?page=2',
    )
