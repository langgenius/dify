...
function DetailPanel({ detail, onFeedback }: IDetailPanel) {
  ...
  return (
    <div ref={ref} className='border-components-panel-border flex h-full flex-col rounded-xl border-[0.5px]'>
      {/* Panel Header */}
      <div className='bg-components-panel-bg flex shrink-0 items-center gap-2 rounded-t-xl pb-2 pl-4 pr-3 pt-3'>
        <div className='shrink-0'>
          <div className='text-text-primary system-xs-semibold-uppercase mb-0.5'>{isChatMode ? t('appLog.detail.conversationId') : t('appLog.detail.time')}</div>
          {isChatMode && (
            <div className='text-text-secondary system-2xs-regular-uppercase flex items-center'>
              <Tooltip
      ...
      <div className='shrink-0 px-1 pt-1'>
        <div className='bg-background-section-burn rounded-t-xl p-3 pb-2'>
      ...
      <div className='bg-background-section-burn mx-1 mb-1 grow overflow-auto rounded-b-xl'>
        {!isChatMode
          ...
          : threadChatItems.length < 8
            ...
            : <div
              ...
              <InfiniteScroll
                ...
                next={fetchData}
                hasMore={hasMore}
                loader={<div className='text-text-tertiary system-xs-regular text-center'>{t('appLog.detail.loading')}...</div>}
...
const ConversationList: FC<IConversationList> = ({ logs, appDetail, onRefresh }) => {
  ...
  const renderTdValue = (value: string | number | null, isEmptyStyle: boolean, isHighlight = false, annotation?: LogAnnotation) => {
    return (
      <Tooltip
        popupContent={
          <span className='text-text-tertiary inline-flex items-center text-xs'>
    ...
    <div className='overflow-x-auto'>
      <table className={cn('mt-2 w-full min-w-[440px] border-collapse border-0')}>
        <thead className='system-xs-medium-uppercase text-text-tertiary'>
          <tr>
            <td className='bg-background-section-burn w-5 whitespace-nowrap rounded-l-lg pl-2 pr-1'></td>
            <td className='bg-background-section-burn whitespace-nowrap py-1.5 pl-3'>{isChatMode ? t('appLog.table.header.summary') : t('appLog.table.header.input')}</td>
            <td className='bg-background-section-burn whitespace-nowrap py-1.5 pl-3'>{t('appLog.table.header.endUser')}</td>
            {isChatflow && <td className='bg-background-section-burn whitespace-nowrap py-1.5 pl-3'>{t('appLog.table.header.status')}</td>}
            <td className='bg-background-section-burn whitespace-nowrap py-1.5 pl-3'>{isChatMode ? t('appLog.table.header.messageCount') : t('appLog.table.header.output')}</td>
            <td className='bg-background-section-burn whitespace-nowrap py-1.5 pl-3'>{t('appLog.table.header.userRate')}</td>
            <td className='bg-background-section-burn whitespace-nowrap py-1.5 pl-3'>{t('appLog.table.header.adminRate')}</td>
            <td className='bg-background-section-burn whitespace-nowrap py-1.5 pl-3'>{t('appLog.table.header.updatedTime')}</td>
            <td className='bg-background-section-burn whitespace-nowrap rounded-r-lg py-1.5 pl-3'>{t('appLog.table.header.time')}</td>
          </tr>
        </thead>
        <tbody className="text-text-secondary system-sm-regular">
          {logs.data.map((log: any) => {
            ...
            return <tr
              key={log.id}
              className={cn('border-divider-subtle hover:bg-background-default-hover cursor-pointer border-b', currentConversation?.id !== log.id ? '' : 'bg-background-default-hover')}
              onClick={() => {
                setShowDrawer(true)
              ...
              <td className='h-4'>
                {!log.read_at && (
                  <div className='flex items-center p-3 pr-0.5'>
                    <span className='bg-util-colors-blue-blue-500 inline-block h-1.5 w-1.5 rounded'></span>
                  </div>
                )}
              </td>
