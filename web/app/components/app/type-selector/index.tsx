      ...
      <div className="relative">
        ...
        {value.length > 0 && (
          <button
            ...
            onClick={() => onChange([])}
          >
            <RiCloseCircleFill className="size-3.5 text-text-quaternary group-hover:text-text-tertiary" />
          </button>
        )}
        <PopoverContent
...
export const AppTypeIcon = React.memo(
  ({ type, className, wrapperClassName, style }: AppTypeIconProps) => {
    const wrapperClassNames = cn(
      'inline-flex size-5 items-center justify-center rounded-md border border-divider-regular',
      wrapperClassName,
    )
...
function AppTypeSelectTrigger({ values }: { readonly values: AppSelectorProps['value'] }) {
  ...
  return (
    <div className={cn('relative flex h-8 items-center justify-between -space-x-2')}>
