...
const TagItemEditor: FC<TagItemEditorProps> = ({
  ...
  return (
    <>
      <div className={cn('flex shrink-0 items-center gap-0.5 rounded-lg border border-gray-200 py-1 pl-2 pr-1 text-sm leading-5 text-gray-700')}>
        {!isEditing && (
          <>
            <div className='text-sm leading-5 text-gray-700'>
              {tag.name}
            </div>
            <div className='leading-4.5 shrink-0 px-1 text-sm font-medium text-gray-500'>{tag.binding_count}</div>
            <div className='group/edit shrink-0 cursor-pointer rounded-md p-1 hover:bg-black/5' onClick={() => setIsEditing(true)}>
              <RiEditLine className='h-3 w-3 text-gray-500 group-hover/edit:text-gray-800' />
            </div>
            <div className='group/remove shrink-0 cursor-pointer rounded-md p-1 hover:bg-black/5' onClick={() => {
