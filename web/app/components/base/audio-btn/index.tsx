...
const AudioBtn = ({
  ...
  return (
    <div className={`${(isPlaying && !hasEnded) ? 'mr-1' : className}`}>
      ...
      >
        <div
          className={'box-border p-0.5 flex items-center justify-center cursor-pointer'}
          onClick={togglePlayPause}>
          <div className={`w-6 h-6 rounded-md hover:bg-gray-200 ${!isPause ? ((isPlaying && !hasEnded) ? s.playIcon : s.stopIcon) : s.pauseIcon}`}></div>
        </div>
      </Tooltip>
    </div>
  )
