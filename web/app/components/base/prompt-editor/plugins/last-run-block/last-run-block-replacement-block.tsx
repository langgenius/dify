...
const LastRunReplacementBlock = ({ onInsert }: LastRunBlockType) => {
  const [editor] = useLexicalComposerContext()

  useEffect(() => {
    if (!editor.hasNodes([LastRunBlockNode]))
      throw new Error(
        'LastRunMessageBlockNodePlugin: LastRunMessageBlockNode not registered on editor',
      )
  }, [editor])
  ...
  useEffect(() => {
    ...
    return mergeRegister(
      editor.registerNodeTransform(CustomTextNode, (textNode) =>
        decoratorTransform(textNode, getMatch, createLastRunBlockNode),
      ),
    )
  }, [])
