...
const createWorkflowNodesMap = (title = 'Node One'): NonNullable<WorkflowNodesMap> => ({
  'node-1': {
    title,
    type: BlockEnum.LLM,
    height: 100,
  ...
  describe('Workflow map updates', () => {
    it('should update local workflow node map when UPDATE_WORKFLOW_NODES_MAP command is dispatched', async () => {
      ...
      act(() => {
        ...
        handled = editor!.dispatchCommand(UPDATE_WORKFLOW_NODES_MAP, { workflowNodesMap: createWorkflowNodesMap(), nodeOutputVars: [] })
      })
