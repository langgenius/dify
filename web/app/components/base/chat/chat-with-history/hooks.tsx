...
export const useChatWithHistory = (installedAppInfo?: InstalledApp) => {
  ...
  const handleNewConversationInputsChange = useCallback((newInputs: Record<string, any>) => {
    ...
    // eslint-disable-next-line eslint-react/set-state-in-effect -- This handler intentionally syncs derived input defaults when called from the reset effect below.
  ...
  useEffect(() => {
    if (appConversationData?.data && !appConversationDataLoading)
      // eslint-disable-next-line eslint-react/set-state-in-effect -- Conversation query results intentionally replace the local editable list.
  ...
  useEffect(() => {
    if (newConversation) {
      // eslint-disable-next-line eslint-react/set-state-in-effect -- Newly resolved conversation names intentionally patch the local list cache.
  ...
  useEffect(() => {
    if (currentConversationItem)
      // eslint-disable-next-line eslint-react/set-state-in-effect -- Selected conversation changes intentionally resync the editable input snapshot.
