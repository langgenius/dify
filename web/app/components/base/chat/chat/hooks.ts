...
export const useChat = (
  ...
  const handleResume = useCallback(
    async (
      ...
      { onGetSuggestedQuestions, onConversationComplete, isPublicAPI }: SendCallback,
    ) => {
          ...
          updateChatTreeNode(messageId, (responseItem) => {
            if (responseItem.workflowProcess)
              responseItem.workflowProcess.status =
                workflowFinishedData.status as WorkflowRunningStatus
          })
        },
        ...
        async onCompleted(hasError?: boolean) {
          ...
          if (
            ...
            const isUseAgentThought =
              ...
              newResponseItem.agent_thoughts[newResponseItem.agent_thoughts?.length - 1].thought ===
                newResponseItem.answer
