...
const Answer: FC<AnswerProps> = ({
  ...
  return (
    <div
      className={
        isMobile
          ? 'flex flex-col items-start mb-2 last:mb-0'
          : 'flex mb-2 last:mb-0'
      }
    >
      ...
      <div
        className={`chat-answer-container group grow ${
          isMobile ? 'max-w-full mt-4' : 'w-0 ml-4'
        }`}
        ref={containerRef}
      >
        <div
          className={cn(
            `group relative ${!isMobile && 'pr-10'}`,
            chatAnswerContainerInner,
          )}
        >
          <AnswerTriangle
            className={
              isMobile
                ? 'absolute left-0 -top-2 w-2 h-3 text-gray-100 rotate-180'
