...
function ToastCard({
  ...
  return (
    <BaseToast.Root
      toast={toastItem}
      className={cn(
        'pointer-events-auto absolute right-0 top-0 w-[360px] max-w-[calc(100vw-2rem)] origin-top cursor-default select-none rounded-xl focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-components-input-border-hover',
        ...
        'h-(--toast-current-height) z-[calc(100-var(--toast-index))]',
        ...
        'data-expanded:translate-x-(--toast-swipe-movement-x) data-expanded:translate-y-[calc(var(--toast-offset-y)+var(--toast-swipe-movement-y)+(var(--toast-index)*8px))] data-expanded:scale-100 data-expanded:h-(--toast-height)',
        ...
        <BaseToast.Content className="relative flex items-start gap-1 overflow-hidden p-3 transition-opacity duration-200 data-behind:opacity-0 data-expanded:opacity-100">
          ...
          <div className="min-w-0 flex-1 p-1">
            <div className="flex w-full items-center gap-1">
              {toastItem.title != null && (
                <BaseToast.Title className="wrap-break-word text-text-primary system-sm-semibold">
                  {toastItem.title}
                </BaseToast.Title>
              )}
            </div>
            {toastItem.description != null && (
              <BaseToast.Description className="mt-1 wrap-break-word text-text-secondary system-xs-regular">
              ...
              <div className="flex w-full items-start gap-1 pb-1 pt-2">
                <BaseToast.Action
                  className={cn(
                    ...
                    'hover:bg-state-base-hover focus-visible:bg-state-base-hover focus-visible:outline-hidden focus-visible:ring-1 focus-visible:ring-components-input-border-hover',
                  )}
                />
              </div>
            )}
          ...
          <div className="flex shrink-0 items-center justify-center rounded-md p-0.5">
            <BaseToast.Close
              ...
              className={cn(
                'flex h-5 w-5 items-center justify-center rounded-md hover:bg-state-base-hover focus-visible:bg-state-base-hover focus-visible:outline-hidden focus-visible:ring-1 focus-visible:ring-components-input-border-hover disabled:cursor-not-allowed disabled:opacity-50',
              )}
            >
