...
import type { DataSourceFeishuPage, DataSourceFeishuPageMap } from '@/models/common'

type PageSelectorProps = {
  value: Set<string>
  disabledValue: Set<string>
  searchValue: string
  pagesMap: DataSourceFeishuPageMap
  list: DataSourceFeishuPage[]
  onSelect: (selectedPagesId: Set<string>) => void
  canPreview?: boolean
  previewPageId?: string
  onPreview?: (selectedPageId: string) => void
}
type FeishuPageTreeItem = {
  children: Set<string>
  descendants: Set<string>
  deepth: number
  ancestors: string[]
} & DataSourceFeishuPage
type FeishuPageTreeMap = Record<string, FeishuPageTreeItem>
type FeishuPageItem = {
  expand: boolean
  deepth: number
} & DataSourceFeishuPage
...
const recursivePushInParentDescendants = (
  pagesMap: DataSourceFeishuPageMap,
  listTreeMap: FeishuPageTreeMap,
  current: FeishuPageTreeItem,
  leafItem: FeishuPageTreeItem,
) => {
...
const ItemComponent = ({ index, style, data }: ListChildComponentProps<{
  ...
  listMapWithChildrenAndDescendants: FeishuPageTreeMap
  searchValue: string
  previewPageId: string
  pagesMap: DataSourceFeishuPageMap
}>) => {
      ...
      </div>
      {
        canPreview && (
          <div
            ...
            {t('common.dataSource.feishu.selector.preview')}
          </div>
        )
      }
      {
...
const PageSelector = ({
  ...
  const [dataList, setDataList] = useState<FeishuPageItem[]>([])
  ...
  const listMapWithChildrenAndDescendants = useMemo(() => {
    return list.reduce((prev: FeishuPageTreeMap, next: DataSourceFeishuPage) => {
  ...
  if (!currentDataList.length) {
    return (
      <div className='flex items-center justify-center h-[296px] text-[13px] text-gray-500'>
        {t('common.dataSource.feishu.selector.noSearchResult')}
      </div>
    )
  }
