...
export const transformFormSchemasSecretInput = (
  isPristineSecretInputNames: string[],
...
export const getTransformedValuesWhenSecretInputPristine = (
  ...
  for (let i = 0; i < formSchemas.length; i++) {
    ...
    if (schema!.type === FormTypeEnum.secretInput) {
      ...
      if (fieldMeta?.isPristine) isPristineSecretInputNames.push(schema!.name)
    }
  }
