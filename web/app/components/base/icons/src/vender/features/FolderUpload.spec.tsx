...
describe('FolderUpload Icon Component', () => {
  it('renders correctly with default attributes from JSON', () => {
    const { container } = render(<FolderUpload />)
    const svg = container.querySelector('svg')

    expect(svg).toBeInTheDocument()
    expect(svg).toHaveAttribute('data-icon', 'FolderUpload')
  ...
  it('applies custom className and style props', () => {
    const { container } = render(
      <FolderUpload className="custom-icon" style={{ opacity: '0.8' }} />,
    )
  ...
  it('handles click events', () => {
    const onClick = vi.fn()
    const { container } = render(<FolderUpload onClick={onClick} />)
    const svg = container.querySelector('svg')
