...
describe('LoveMessage Icon Component', () => {
  it('renders correctly with default attributes from JSON', () => {
    const { container } = render(<LoveMessage />)
    const svg = container.querySelector('svg')

    expect(svg).toBeInTheDocument()
    expect(svg).toHaveAttribute('data-icon', 'LoveMessage')
  ...
  it('applies custom className and style props', () => {
    const { container } = render(
      <LoveMessage className="custom-icon" style={{ opacity: '0.8' }} />,
    )
  ...
  it('handles click events', () => {
    const onClick = vi.fn()
    const { container } = render(<LoveMessage onClick={onClick} />)
    const svg = container.querySelector('svg')
