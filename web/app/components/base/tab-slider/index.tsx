...
const TabSlider: FC<TabSliderProps> = ({
  ...
  return (
    <div
      ...
      className={cn(className, 'relative inline-flex items-center justify-center rounded-lg bg-components-segmented-control-bg-normal p-0.5')}
    >
      <div
        data-testid="tab-slider-bg"
        className="shadows-shadow-xs absolute top-0.5 right-0 bottom-0.5 left-0 rounded-lg bg-components-panel-bg transition-transform duration-300 ease-in-out"
        style={sliderStyle}
      />
      {options.map((option, index) => (
        <div
          ...
          className={cn(
            'relative z-10 flex cursor-pointer items-center justify-center gap-1 rounded-lg px-2.5 py-1.5 transition-colors duration-300 ease-in-out',
            'system-md-semibold',
            index === activeIndex
              ? 'text-text-primary'
              : 'text-text-tertiary',
