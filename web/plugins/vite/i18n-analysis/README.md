...
and boundaries present in the build graph are reported. Translation loading is
unchanged, and this report does not introduce additional build failures.
...
The report is written before the unused-key check, so it remains available if that
check fails. `build.write: false` only prints the summary and writes no report.
