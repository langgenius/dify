...
2. ✅ **测试工具文件** - 仅用于测试
3. ✅ **注释标记为废弃的文件** - 有明确的废弃注释

### 第二优先级：95% 安全（较低风险）
4. ✅ **自动生成的组件** - 有 "GENERATE BY script" 注释
5. ✅ **旧实现文件** - 已被新版本替代（需人工确认）

### 第三优先级：80% 安全（中等风险）
6. ⚠️ **未使用的工具函数** - 确认无动态引用
7. ⚠️ **未使用的基础组件** - 确认无动态加载

### 第四优先级：需要谨慎（高风险）
8. 🔴 **Store 文件** - **已被配置保护，不应删除**
9. 🔴 **Context 文件** - **已被配置保护，不应删除**
10. 🔴 **Provider 文件** - **已被配置保护，不应删除**
11. 🔴 **初始化文件** - **已被配置保护，不应删除**
...
2. 🔒 **所有 public/ 目录下的文件** - URL 引用
3. 🔒 **所有 context.tsx 文件** - React Context
4. 🔒 **所有 store.ts 文件** - Zustand 状态管理
5. 🔒 **所有 provider.tsx 文件** - React Provider
6. 🔒 **根布局引用的文件** - 全局初始化
