...
**`--fix`**: Automatically fixes auto-fixable lint violations.
Use `vp fmt` for formatting concerns such as whitespace, package.json key order, and Tailwind class order.
...
As a fallback, the commit hook runs `vp staged`, which formats staged files with `vp fmt` before applying autofixable ESLint changes.
