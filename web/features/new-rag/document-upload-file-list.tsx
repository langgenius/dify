  ...
  return (
    <ul className={cn('space-y-1', className)} aria-label={ariaLabel}>
      {items.map((item) => {
        ...
        const extension = documentUploadFileExtension(item.file.name).toLocaleUpperCase() || 'FILE'
            ...
            <span className="flex shrink-0 items-center gap-1 py-2 pr-3">
              {fileUploading ? (
                ...
                canPreviewLocalFile(item.file) && (
                  <Button
                    ...
                    size="small"
                    type="button"
                    onClick={() => openLocalFilePreview(item.file)}
                  >
              ...
              >
                <span
                  aria-hidden
                  className={
                    variant === 'compact' ? 'i-ri-close-line size-4' : 'i-ri-delete-bin-line size-4'
                  }
                />
              </button>
            </span>
