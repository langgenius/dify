...
export class InMemoryContactImPlatformRepository implements ContactImPlatformRepository {
  ...
  private assertProviderReplacement(provider: ContactImProvider, replaceActiveProvider: boolean) {
    if (provider === ContactImProvider.Email) return

    const activeProvider = this.seed.integrations.find(
  ...
  private removeReplacedProvider(provider: ContactImProvider, replaceActiveProvider: boolean) {
    if (provider === ContactImProvider.Email || !replaceActiveProvider) return

    this.seed.integrations = this.seed.integrations.filter(
      (integration) =>
        integration.provider === ContactImProvider.Email || integration.provider === provider,
    )
  }
  ...
  async saveCredentials(command: SaveContactImCredentialsCommand) {
    ...
    this.removeReplacedProvider(command.provider, command.replaceActiveProvider)
