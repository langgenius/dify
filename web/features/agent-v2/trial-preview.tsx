...
function PublishedConfiguration({ preview }: { preview: TrialAgentPreviewResponse }) {
  const { t } = useTranslation('agentV2')
  const { t: tCommon } = useTranslation('common')
  const id = useId()
  const sections = [
    { key: 'tools', label: t(($) => $['agentDetail.configure.tools.label']), items: preview.tools },
    {
      key: 'knowledge',
      label: t(($) => $['agentDetail.configure.knowledgeRetrieval.label']),
      items: preview.knowledge,
    },
    {
      key: 'skills',
    ...
    { key: 'files', label: t(($) => $['agentDetail.configure.files.label']), items: preview.files },
  ]
  ...
  return (
    <div className="h-full overflow-y-auto rounded-lg bg-components-panel-bg p-4">
      <AgentOrchestrateReadOnlyContext value>
        <AgentModelField
          currentModel={preview.model ?? undefined}
      ...
      >
        <p className="pb-4 system-sm-regular wrap-break-word whitespace-pre-wrap text-text-primary">
          {preview.system_prompt || tCommon(($) => $.noData)}
        </p>
      </ConfigureSection>
...
export function AgentTrialPreview({ appId }: { appId: string }) {
  const { data, isPending, isError } = useQuery(
    consoleQuery.trialApps.byAppId.agentPreview.get.queryOptions({
