...
function CredentialStatus({
  ...
  const handleAuthorizationItemClick = useCallback(
    (id: string) => {
      onCredentialChange(
        id === '__workspace_default__' ? undefined : id || undefined,
        tool.credentialType,
      )
    },
  ...
  const handleDefaultCredentialChange = useCallback(
    (id?: string) => {
      if (!tool.credentialId && id) onCredentialChange(id, tool.credentialType)
    },
      ...
      <div className="group relative flex min-h-7 items-center gap-1 rounded-md py-px pr-0 pl-1 hover:bg-state-base-hover">
        ...
        {!readOnly && (
          <div className="hidden shrink-0 items-center gap-1 px-0.5 group-focus-within:flex group-hover:flex">
            <button
              type="button"
              aria-label={t(($) => $['agentDetail.configure.tools.editAction'], {
                name: action.name,
              })}
            ...
            <button
              type="button"
              aria-label={t(($) => $['agentDetail.configure.tools.removeAction'], {
                name: action.name,
              })}
