...
describe('AgentOrchestrateHeader', () => {
  it('should render configure title without build mode copy by default', () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'agentV2.agentDetail.configure.title' }),
    ).toBeInTheDocument()
    expect(
  ...
  it('should render build mode copy when build draft is active', () => {
    ...
    expect(
      screen.getByText('agentV2.agentDetail.configure.buildDraft.modeBadge'),
    ).toBeInTheDocument()
    expect(
      screen.getByText('agentV2.agentDetail.configure.buildDraft.modeDescription'),
    ).toBeInTheDocument()
  })
