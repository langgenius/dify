    ...
    default: () => function MockChat(props: {
      ...
      return (
        <div
          ...
          data-speech-agent-id={props.speechToTextTarget?.type === 'agent' ? props.speechToTextTarget.agentId : ''}
          data-speech-draft-type={props.speechToTextTarget?.type === 'agent' ? props.speechToTextTarget.draftType : ''}
        >
          {props.chatNode}
...
vi.mock('@/app/components/base/chat/chat/chat-input-area', () => ({
  default: ({ footerNotice, speechToTextTarget }: { footerNotice?: ReactNode, speechToTextTarget?: SpeechToTextTarget }) => (
    <div
      data-testid="agent-preview-chat-input"
      data-speech-agent-id={speechToTextTarget?.type === 'agent' ? speechToTextTarget.agentId : ''}
      data-speech-draft-type={speechToTextTarget?.type === 'agent' ? speechToTextTarget.draftType : ''}
    >
      {footerNotice}
    </div>
  ),
...
describe('AgentPreviewChat', () => {
  ...
  it('should bind Agent preview voice input to the normal Agent draft', () => {
    ...
    expect(screen.getByTestId('agent-preview-chat-input')).toHaveAttribute('data-speech-agent-id', 'agent-1')
    expect(screen.getByTestId('agent-preview-chat-input')).toHaveAttribute('data-speech-draft-type', 'draft')
  ...
  it('should bind Agent build voice input to the account build draft', () => {
    ...
    expect(screen.getByTestId('agent-preview-chat-input')).toHaveAttribute('data-speech-draft-type', 'debug_build')
