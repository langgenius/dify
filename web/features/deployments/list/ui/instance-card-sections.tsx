...
export function ReleaseMetaTooltip({
  release,
  deployed,
  children,
}: {
  ...
  const rows = [
    ...
    {
      label: t(($) => $['card.tooltip.deploymentStatus']),
      value: deployed
        ? t(($) => $['card.tooltip.deployed'])
        : t(($) => $['card.tooltip.notDeployedShort']),
    },
...
function EnvironmentChip({ row }: { row: EnvironmentDeployment }) {
  const { t } = useTranslation('deployments')
      ...
      <PopoverContent popupClassName="px-3 py-2">
        <div className="flex min-w-40 flex-col gap-1">
          {rows.map((row) => {
            ...
            return (
              <span key={row.environment.id} className="whitespace-nowrap text-text-secondary">
                {summary}
              </span>
            )
          })}
...
export function DeploymentStatusContent({
  ...
  if (emptyAction) return <div className="flex min-w-0 items-center">{emptyAction}</div>

  return null
}

export function DeploymentAccessLinks({
  appInstanceId,
  access,
  isLoading,
}: {
  ...
  if (isLoading) {
    return (
      <div
        role="group"
        aria-label={t(($) => $['overview.accessStatus'])}
  ...
  if (links.length === 0) {
    return (
      <div
        role="group"
        aria-label={t(($) => $['overview.accessStatus'])}
        className="flex min-w-0 grow items-center gap-1.5 text-text-quaternary"
      >
