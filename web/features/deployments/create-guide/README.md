...
| `../../components/empty-state`                        | Reuses deployment empty/error state presentation for the source app list.                                     |
| `../../components/env-var-bindings`                   | Renders target environment variable bindings from computed deployment options.                                |
| `../../components/env-var-bindings-utils`             | Converts contract env var slots and normalizes env var value-type metadata for target readiness and payloads. |
| `../../components/runtime-credential-bindings`        | Renders target runtime credential binding choices from computed deployment options.                           |
| `../../components/runtime-credential-bindings-utils`  | Reuses credential slot keys, selected credential mapping, and required-binding validation.                    |
| `../../components/title-tooltip`                      | Shows full labels and descriptions when step text is visually truncated.                                      |
| `../../components/unsupported-dsl-nodes-alert`        | Presents unsupported DSL node feedback from precheck and submission errors.                                   |
