const translation = {
  ...
  inputFieldPanel: {
    ...
    globalInputs: {
      ...
      tooltip: 'グローバル入力はすべてのノードで共有されます。ユーザーは任意のデータソースを選択する際にこれらを入力する必要があります。区切り文字や最大チャンク長などのフィールドは複数のデータソースに統一適用できます。データソース変数で参照される入力フィールドのみが最初のステップ（データソース）に表示され、他のフィールドは第二のステップ（ドキュメント処理）に表示されます。',
    },
    preview: {
      stepOneTitle: 'データソース',
