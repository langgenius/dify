const translation = {
  ...
  settings: {
    ...
    generalGroup: '一般',
    workspaceApiKey: 'ワークスペース API キー',
  },
  account: {
    avatar: 'アバター',
    name: '名前',
  ...
  workspaceApiKey: {
    title: 'ワークスペースAPIキー',
    description: 'ワークスペースメンバーがDify APIで認証するためのAPIキーを管理します。APIキーは個々のワークスペースメンバーに紐付けられています。',
    add: 'APIキーを追加',
    manage: 'APIキーを管理',
    regenerate: 'APIキーを再生成',
    updateSuccess: 'APIキーが正常に更新されました',
    updateFailed: 'APIキーの更新に失敗しました',
    selectorPlaceholder: 'APIキーを選択してください',
    createdAt: '作成日時',
    expiresAt: '有効期限',
    lastUsedAt: '最終使用日時',
    type: 'タイプ',
    ...
    deleteFailed: 'APIキーの削除に失敗しました',
    newApiKey: '新しいAPIキー',
    regeneratedApiKey: '再生成されたAPIキー',
    newApiKeyDescription: 'APIキーを今すぐコピーしてください。この画面を閉じると二度と表示されません！',
    securityWarning: 'APIキーは安全に保管してください！公開したり、バージョン管理にコミットしたりしないでください。',
    yourApiKey: 'あなたのAPIキー',

    modal: {
      addTitle: 'APIキーを追加',
      addDescription: 'APIキーは、Difyワークスペース全体のアプリケーションでの管理と使用を簡素化するために使用されます。',
      editTitle: 'APIキーを編集',
      editDescription: 'このAPIキーの名前、権限、有効期限設定を更新します。',
      deleteTitle: 'APIキーを削除',
      deleteDescription: 'APIキーを削除すると、そのキーは無効になり、アプリケーションで使用できなくなります。',
      deleteFailed: 'APIキーの削除に失敗しました',
      deleteSuccess: 'APIキーが正常に削除されました',
      deleteConfirm: '本当にこのAPIキーを削除しますか？',
      nameLabel: 'APIキー名',
      namePlaceholder: 'APIキー名を入力してください',
      nameError: '名前は必須です',
      expiresLabel: '有効期限（日）',
      expiresPlaceholder: '有効期限（日）を入力（デフォルト：30日）',
      expiresDescription: 'APIキーの有効期限を設定します。デフォルトは30日です。',
      ...
      addFailed: 'APIキーの追加に失敗しました',
      regenerateTitle: 'APIキーを再生成',
      regenerateDescription: 'APIキーを再生成すると、現在のキーは無効になり、新しいキーが発行されます。新しいキーをアプリケーションで設定する必要があります。',
