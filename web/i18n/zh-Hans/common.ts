const translation = {
  ...
  account: {
    ...
    langGeniusAccount: 'FinRegHub 账号',
    langGeniusAccountTip: '您的 FinRegHub 账号和相关的用户数据。',
    editName: '编辑名字',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      onTrial: '体验',
      exhausted: '超出限额',
      desc: '托管 OpenAI 由 FinRegHub 提供的托管 OpenAI 服务，你可以使用 GPT-3.5 等模型，在体验额度消耗完毕前你需要设置其它模型供应商。',
      callTimes: '调用次数',
  ...
  apiBasedExtension: {
    title: 'API 扩展提供了一个集中式的 API 管理，在此统一添加 API 配置后，方便在 FinRegHub 上的各类应用中直接使用。',
  ...
  about: {
    changeLog: '更新日志',
    updateNow: '现在更新',
    nowAvailable: 'FinRegHub 0.1 现已可用。',
    latestAvailable: 'FinRegHub 0.1 已是最新版本。',
  },
  appMenus: {
    overview: '概览',
    promptEng: '编排',
