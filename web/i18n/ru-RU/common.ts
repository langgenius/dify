const translation = {
  ...
  settings: {
    ...
    generalGroup: 'ОБЩЕЕ',
    workspaceApiKey: 'Ключи API рабочего пространства',
  },
  account: {
    avatar: 'Аватар',
    name: 'Имя',
  ...
  you: 'Ты',
  workspaceApiKey: {
    title: 'Ключи API рабочего пространства',
    description: 'Управляйте ключами API, которые позволяют участникам рабочего пространства аутентифицироваться с API Dify. Ключи API привязаны к отдельным участникам рабочего пространства.',
    add: 'Добавить ключ API',
    manage: 'Управление ключами API',
    regenerate: 'Сгенерировать ключ API повторно',
    updateSuccess: 'Ключ API успешно обновлен',
    updateFailed: 'Не удалось обновить ключ API',
    selectorPlaceholder: 'Пожалуйста, выберите ключ API',
    createdAt: 'Создано',
    expiresAt: 'Истекает',
    lastUsedAt: 'Последнее использование',
    type: 'Тип',
    none: 'Нет',
    deleteSuccess: 'Ключ API успешно удален',
    deleteFailed: 'Не удалось удалить ключ API',
    newApiKey: 'Ваш новый ключ API',
    regeneratedApiKey: 'Ваш повторно сгенерированный ключ API',
    newApiKeyDescription: 'Скопируйте свой ключ API сейчас. Вы больше не сможете его увидеть!',
    securityWarning: 'Храните свой ключ API в безопасности! Не делитесь им публично и не коммитьте его в систему контроля версий.',
    yourApiKey: 'Ваш ключ API',

    modal: {
      addTitle: 'Добавить ключ API',
      addDescription: 'Ключи API используются для упрощения управления и использования в приложениях по всему рабочему пространству Dify.',
      editTitle: 'Редактировать ключ API',
      editDescription: 'Обновите имя, разрешения и настройки срока действия для этого ключа API.',
      deleteTitle: 'Удалить ключ API',
      deleteDescription: 'Удаление ключа API сделает его недействительным и непригодным для использования в приложениях.',
      deleteFailed: 'Не удалось удалить ключ API',
      deleteSuccess: 'Ключ API успешно удален',
      deleteConfirm: 'Вы уверены, что хотите удалить этот ключ API?',
      nameLabel: 'Имя ключа API',
      namePlaceholder: 'Введите имя ключа API',
      nameError: 'Имя обязательно',
      expiresLabel: 'Срок действия (дни)',
      expiresPlaceholder: 'Введите количество дней до истечения срока действия (по умолчанию: 30 дней)',
      expiresDescription: 'Установите срок действия для ключа API. По умолчанию 30 дней.',
      ...
      addFailed: 'Не удалось добавить ключ API',
      regenerateTitle: 'Сгенерировать ключ API повторно',
      regenerateDescription: 'Повторная генерация ключа API сделает текущий ключ недействительным и выдаст новый. Вам нужно будет настроить новый ключ в своих приложениях.',
      regenerateFailed: 'Не удалось повторно сгенерировать ключ API',
    },
  },
}
