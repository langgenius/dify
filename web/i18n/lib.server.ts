...
async function getI18nConfig<const T extends readonly [Namespace, ...Namespace[]]>(ns: T) {
  const lang = await getLocaleOnServer()
  return getTranslation(lang, ns)
}

export function useTranslation<const T extends readonly [Namespace, ...Namespace[]]>(ns: T) {
