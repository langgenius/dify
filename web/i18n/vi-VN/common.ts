const translation = {
  ...
  settings: {
    ...
    workspaceApiKey: 'Khóa API Không gian làm việc',
  },
  account: {
    avatar: 'Ảnh đại diện',
    name: 'Tên',
  ...
  you: 'Bạn',
  workspaceApiKey: {
    title: 'Khóa API Không gian làm việc',
    description: 'Quản lý các khóa API cho phép thành viên không gian làm việc xác thực với API Dify. Các khóa API được liên kết với từng thành viên không gian làm việc.',
    add: 'Thêm Khóa API',
    manage: 'Quản lý Khóa API',
    regenerate: 'Tạo lại Khóa API',
    updateSuccess: 'Cập nhật khóa API thành công',
    updateFailed: 'Cập nhật khóa API thất bại',
    selectorPlaceholder: 'Vui lòng chọn một khóa API',
    createdAt: 'Được tạo vào',
    expiresAt: 'Hết hạn vào',
    lastUsedAt: 'Sử dụng lần cuối',
    type: 'Loại',
    none: 'Không có',
    deleteSuccess: 'Xóa khóa API thành công',
    deleteFailed: 'Xóa khóa API thất bại',
    newApiKey: 'Khóa API mới của bạn',
    regeneratedApiKey: 'Khóa API đã tạo lại của bạn',
    newApiKeyDescription: 'Sao chép khóa API của bạn ngay bây giờ. Bạn sẽ không thể nhìn thấy nó nữa!',
    securityWarning: 'Giữ khóa API của bạn an toàn! Không chia sẻ công khai hoặc cam kết nó vào kiểm soát phiên bản.',
    yourApiKey: 'Khóa API của bạn',

    modal: {
      addTitle: 'Thêm Khóa API',
      addDescription: 'Các khóa API được sử dụng để đơn giản hóa việc quản lý và sử dụng trong các ứng dụng trên toàn không gian làm việc Dify.',
      editTitle: 'Chỉnh sửa Khóa API',
      editDescription: 'Cập nhật tên, quyền và cài đặt hết hạn cho khóa API này.',
      deleteTitle: 'Xóa Khóa API',
      deleteDescription: 'Việc xóa một khóa API sẽ làm cho nó không hợp lệ và không thể sử dụng được trong các ứng dụng.',
      deleteFailed: 'Xóa khóa API thất bại',
      deleteSuccess: 'Xóa khóa API thành công',
      deleteConfirm: 'Bạn có chắc chắn muốn xóa khóa API này không?',
      nameLabel: 'Tên Khóa API',
      namePlaceholder: 'Nhập tên khóa API',
      nameError: 'Tên là bắt buộc',
      expiresLabel: 'Hết hạn (ngày)',
      expiresPlaceholder: 'Nhập số ngày hết hạn (mặc định: 30 ngày)',
      expiresDescription: 'Đặt thời gian hết hạn cho khóa API. Mặc định là 30 ngày.',
      ...
      regenerateTitle: 'Tạo lại Khóa API',
      regenerateDescription: 'Việc tạo lại khóa API sẽ làm cho khóa hiện tại không hợp lệ và cấp một khóa mới. Bạn sẽ cần cấu hình khóa mới trong các ứng dụng của mình.',
      regenerateFailed: 'Tạo lại khóa API thất bại',
    },
  },
}
