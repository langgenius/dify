const translation = {
  ...
  workspaceApiKey: {
    title: '워크스페이스 API 키',
    description: '워크스페이스 멤버가 Dify API로 인증할 수 있도록 하는 API 키를 관리합니다. API 키는 개별 워크스페이스 멤버에게 연결됩니다.',
    add: 'API 키 추가',
    manage: 'API 키 관리',
    regenerate: 'API 키 재생성',
    updateSuccess: 'API 키가 성공적으로 업데이트되었습니다',
    updateFailed: 'API 키 업데이트 실패',
    selectorPlaceholder: 'API 키를 선택하세요',
    createdAt: '생성일',
    expiresAt: '만료일',
    lastUsedAt: '마지막 사용',
    type: '유형',
    ...
    deleteFailed: 'API 키 삭제 실패',
    newApiKey: '새 API 키',
    regeneratedApiKey: '재생성된 API 키',
    newApiKeyDescription: '지금 API 키를 복사하세요. 다시 볼 수 없습니다!',
    securityWarning: 'API 키를 안전하게 보관하세요! 공개적으로 공유하거나 버전 제어에 커밋하지 마세요.',
    yourApiKey: 'API 키',

    modal: {
      addTitle: 'API 키 추가',
      addDescription: 'API 키는 Dify 워크스페이스 전체의 애플리케이션에서 관리 및 사용을 간소화하는 데 사용됩니다.',
      editTitle: 'API 키 편집',
      editDescription: '이 API 키의 이름, 권한 및 만료 설정을 업데이트합니다.',
      deleteTitle: 'API 키 삭제',
      deleteDescription: 'API 키를 삭제하면 해당 키가 무효화되고 애플리케이션에서 사용할 수 없게 됩니다.',
      deleteFailed: 'API 키 삭제 실패',
      deleteSuccess: 'API 키가 성공적으로 삭제되었습니다',
      deleteConfirm: '이 API 키를 삭제하시겠습니까?',
      nameLabel: 'API 키 이름',
      namePlaceholder: 'API 키 이름을 입력하세요',
      nameError: '이름은 필수입니다',
      expiresLabel: '만료 (일)',
      expiresPlaceholder: '만료일 (기본값: 30일)을 입력하세요',
      expiresDescription: 'API 키의 만료 기간을 설정합니다. 기본값은 30일입니다.',
      ...
      addFailed: 'API 키 추가 실패',
      regenerateTitle: 'API 키 재생성',
      regenerateDescription: 'API 키를 재생성하면 현재 키가 무효화되고 새 키가 발급됩니다. 애플리케이션에서 새 키를 구성해야 합니다.',
      regenerateFailed: 'API 키 재생성 실패',
    },
  },
}
