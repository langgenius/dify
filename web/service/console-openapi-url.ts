...
const repeatedQueryArrayRules: readonly QueryArrayCompatibilityRule[] = [
  ...
  { path: /\/workspaces\/current\/human-input\/contact-options\/batch$/, fields: ['contact_ids'] },
  { path: /\/workspaces\/current\/human-input\/contacts\/batch$/, fields: ['contact_ids'] },
...
export function normalizeConsoleOpenAPIURL(url: string | URL) {
  ...
  if (!repeatedQueryArrayFields.length) return normalizedUrl.href
