...
export const fetchAccountIntegrates = ({
  ...
  url: string
  params: Record<string, any>
}): Promise<{ data: AccountIntegrate[] | null }> => {
...
export const switchWorkspace = ({
  ...
  url: string
  body: Record<string, any>
}): Promise<CommonResponse & { new_tenant: IWorkspace }> => {
...
export const verifyForgotPasswordToken = ({
  ...
  url: string
  body: { token: string }
}): Promise<CommonResponse & { is_valid: boolean; email: string }> => {
...
export const verifyWebAppForgotPasswordToken = ({
  ...
  return post<CommonResponse & { is_valid: boolean; email: string }>(
    url,
    { body },
    { isPublicAPI: true },
  )
