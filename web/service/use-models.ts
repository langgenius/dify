...
export const useModelProviderModelList = (provider: string) => {
  return useQuery({
    ...
    queryFn: () =>
      get<{ data: ModelItem[] }>(`/workspaces/current/model-providers/${provider}/models`),
  })
}

export const useGetProviderCredential = (
  enabled: boolean,
  provider: string,
  credentialId?: string,
) => {
...
export const useAddProviderCredential = (provider: string) => {
  return useMutation({
    mutationFn: (data: ProviderCredential) =>
      post<{ result: string }>(`/workspaces/current/model-providers/${provider}/credentials`, {
        body: data,
      }),
  })
}
...
export const useEditProviderCredential = (provider: string) => {
  return useMutation({
    mutationFn: (data: ProviderCredential) =>
      put<{ result: string }>(`/workspaces/current/model-providers/${provider}/credentials`, {
        body: data,
      }),
  })
}
...
export const useActiveProviderCredential = (provider: string) => {
  return useMutation({
    mutationFn: (data: { credential_id: string; model?: string; model_type?: ModelTypeEnum }) =>
      post<{ result: string }>(
...
export const useGetModelCredential = (
  ...
  return useQuery({
    ...
    queryFn: () =>
      get<ModelCredential>(
        `/workspaces/current/model-providers/${provider}/models/credentials?model=${model}&model_type=${modelType}&config_from=${configFrom}${credentialId ? `&credential_id=${credentialId}` : ''}`,
      ),
    staleTime: 0,
    gcTime: 0,
  })
...
export const useAddModelCredential = (provider: string) => {
  return useMutation({
    mutationFn: (data: ModelCredentialPayload) =>
      post<{ result: string }>(
