...
Given('my console session requires token refresh', async function (this: DifyWorld) {
  ...
  expect(
    hasAccessToken,
    'Expected the authenticated E2E session to include a console access token.',
  ).toBe(true)
  expect(
    hasRefreshToken,
    'Expected the authenticated E2E session to include a console refresh token.',
  ).toBe(true)
...
When(
  'I open the default console entry after the access token expires',
