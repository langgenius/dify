...
Then(
  'I should see the unpublished Agent v2 Access Point overview',
  async function (this: DifyWorld) {
    ...
    await expect(accessRegion.getByLabel('Toggle Web app access')).toBeDisabled()
    await expect(accessRegion.getByRole('button', { name: 'Launch' })).toBeDisabled()
    ...
    await expect(accessRegion.getByLabel('Toggle Backend service API access')).toBeDisabled()
    await expect(accessRegion.getByRole('button', { name: /^API Key\b/ })).toBeDisabled()
