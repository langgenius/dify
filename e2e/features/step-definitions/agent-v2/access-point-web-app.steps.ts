...
Then('the Agent v2 Web app should open in a new tab', async function (this: DifyWorld) {
  const webAppPage = this.agentBuilder.accessPoint.webAppPage
  const webAppURL = this.agentBuilder.accessPoint.webAppURL
  if (!webAppPage || !webAppURL) throw new Error('No Agent v2 Web app page was opened.')
...
Then(
  ...
  async function (this: DifyWorld) {
    ...
    await expect(webAppPage.getByText(agentBuilderExpectedTokens.updatedAgentReply)).toBeVisible({
      timeout: 120_000,
    })
  },
)
...
Then(
  ...
  async function (this: DifyWorld) {
    ...
    await expect(webAppPage.getByText(agentBuilderExpectedTokens.agentReply)).toBeVisible({
      timeout: 120_000,
    })
  },
)
...
Then(
  ...
  async function (this: DifyWorld) {
    ...
    await expect(
      webAppPage.getByText(agentBuilderExpectedTokens.updatedAgentReply),
    ).not.toBeVisible()
  },
)
