...
Workflow `knowledge-retrieval-v2` 的 39 个已执行场景、15 个环境阻塞项、3 个规格/实现阻塞项及完整证据见：[Workflow Knowledge Retrieval v2 / KnowledgeFS 功能补测报告](./knowledge-retrieval-v2-functional-test-report.md)。
...
| TC-RE-022 | P1  | 省略 mode，通过 App/Workflow Knowledge Retrieval v2 跟随空间配置运行 | 使用各空间 Settings；草稿、发布 Web 与 Service API 一致 | 初测 FAIL/ENV-BLOCKED；配置、保存与发布通过，但所有运行入口均误报未绑定。KF-BUG-021 / WTA-1953 已完成代码修复和专项回归，待部署后验证真实草稿、Web 与 Service 正向结果；字面 `auto` 按合同应拒绝 |
...
| TC-AU-013 | P0  | 撤销 Service API Key 后重试访问                        | 立即失效，不会继续签发空间 token                         | PASS；临时 Workflow App key 删除后同 key 立即返回 401                                                                                                                                                                           |
| TC-AU-014 | P0  | Agent/Agent Chat 绑定 Agent channel                    | 只允许支持的 app 类型且 space active/channel enabled     | NOT-RUN                                                                                                                                                                                                                         |
| TC-AU-015 | P0  | Workflow/Advanced Chat 绑定 Workflow channel           | 最多 10 个 space，第 11 个有明确错误                     | 初测 PARTIAL/FAIL；真实发布后 binding 精确建立，1/6 空间通过、10/11 由自动化合同覆盖。KF-BUG-021 / WTA-1953 已增加独立 Workflow Access 开关并细分 admission 错误，专项回归通过；尚未部署，真实 channel 正向运行仍为 ENV-BLOCKED |
| TC-AU-016 | P0  | 解除 app binding 后运行已发布 app                      | 旧 binding 不再可用，运行给出明确错误                    | PARTIAL/ENV-BLOCKED；发布图从 `ceshi` 切到 QA 时旧 binding 由 1→0，切回恢复 1；KF-BUG-021 已补齐 binding/channel/space/auth 的可区分错误，但部署前仍无法验证真实旧 capability 撤权                                              |
...
## N. Workflow Knowledge Retrieval v2 补测

本轮新增独立补测矩阵，共执行 39 个浏览器/核心合同场景：31 个通过或通过负向合同、5 个失败、1 个部分失败、2 个被同一运行缺陷阻断；这些是初测原始结果，继续保留。另有 15 个真实链路断言标记为 `ENV-BLOCKED`，3 个取消/error-strategy/retry 断言标记为规格或实现阻塞，未使用 mock 结果冒充端到端通过。

已确认问题：

1. `KF-BUG-020` / [WTA-1954](https://linear.app/dify/issue/WTA-1954)（High，Done）：初测从节点选择器新增 v2 节点后主编辑区空白并出现未处理 `AbortError`。根因是通用 `WorkflowHistoryStoreInitializer` 在约 500 ms 后接收到新的 history 对象便卸载整棵 Workflow 子树；`AbortError` 是卸载后查询取消的结果。代码已修复，更宽的 Workflow 组合回归 9 个 spec、27 个测试通过，待部署后真实 Chrome 复测。
2. `KF-BUG-021` / [WTA-1953](https://linear.app/dify/issue/WTA-1953)（High，Done）：初测时 binding 已建立，但 channel/permissions 未就绪的所有运行表面统一显示未绑定。代码已增加独立 `workflow_enabled` 开关，细分 binding/channel/space/auth admission 状态，并以稳定 marker 映射为 24 个 locale 的本地化消息；020/021 最终 Web 组合 7 个 spec、70 个测试通过，API 18 个测试通过，待部署后真实正向运行。
3. `KF-BUG-022` / [WTA-1956](https://linear.app/dify/issue/WTA-1956)（High，Backlog）：`created_after` / `created_before` 输入在刷新和 DSL 导出后静默丢失。
4. `KF-BUG-023` / [WTA-1955](https://linear.app/dify/issue/WTA-1955)（Medium，Backlog）：最后一个 node-kind filter 无法取消。
5. `KF-BUG-024` / [WTA-1957](https://linear.app/dify/issue/WTA-1957)（Medium，Backlog）：未解析空间在草稿检查清单中无持久警告，发布时才显示未本地化内部 ID。

以上五个 issue 均属于 Linear 项目 `Make RAG Great Again`、Milestone `测试联调`；020/021 已指派给 `jyong` 并完成，022～024 保持 Backlog。修复后的自动化结果不改写 WF2 原始浏览器 FAIL，也不代表尚未部署的真实 E2E 已通过。

另有一个已静态确认但未按浏览器缺陷编号的合同缺口：`KNOWLEDGE_FS_BASE_URL` 未配置时 composition root 会在 I/O 前 fail closed，但 v2 节点解析 runtime 的位置可能绕过其受控 `RetrievalUnavailable` 映射；详见补测报告 `WF2-CONTRACT-001`。

完整逐项结果、运行表面、自动化证据、测试对象和环境恢复记录见：[Workflow Knowledge Retrieval v2 / KnowledgeFS 功能补测报告](./knowledge-retrieval-v2-functional-test-report.md)。

## 测试执行建议顺序

1. 部署已修复的 KF-BUG-020/021，在真实 Chrome 中先验证添加任意节点后等待超过 history 保存窗口仍不白屏，再通过独立 Workflow Access 开关启用 `workflow_enabled=true`；同时准备 permissions ready、含唯一 marker 的固定空间。
2. 修复 KF-BUG-022，并用刷新、DSL 导出与真实 filtered retrieval 三层验证时间过滤；随后处理 KF-BUG-023/024。
3. 经用户确认后执行 TC-DO-003–014，等索引 ready。
4. 紧接执行 TC-DD-004–012、TC-RE-009–011、TC-QA-009–017 及补测报告 WF2-LIVE-001～015；先完成规格/实现后再执行 WF2-LIVE-016～018。
5. 使用 owner/editor/viewer 三账号执行 TC-AU-001–009，并在正在运行的 SSE 中撤权。
6. 最后执行删除和清理，核对列表、FS、检索、概览和 App binding 全部无残留。
