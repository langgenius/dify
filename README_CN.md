...
**2. 全面的模型支持**:
与数百种专有/开源 LLMs 以及数十种推理提供商和自托管解决方案无缝集成，涵盖 GPT、Mistral、Llama3 以及任何与 OpenAI API 兼容的模型。完整的支持模型提供商列表可在[此处](https://docs.dify.ai/cn/documentation/pages/workspace/model-providers)找到。
...
- **自托管 Dify 社区版</br>**
  ...
  使用我们的[使用指南](https://docs.dify.ai/cn/documentation/pages/getting-started/introduction)进行进一步的参考和更深入的说明。
...
如果您需要自定义配置，请参考 [.env.example](docker/.env.example) 文件中的注释，并更新 `.env` 文件中对应的值。此外，您可能需要根据您的具体部署环境和需求对 `docker-compose.yaml` 文件本身进行调整，例如更改镜像版本、端口映射或卷挂载。完成任何更改后，请重新运行 `docker-compose up -d`。您可以在[此处](https://docs.dify.ai/cn/self-hosting/configuration/environments)找到可用环境变量的完整列表。
