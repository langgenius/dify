...
workflow tools. Without that option, each endpoint retains its existing format default. Normal browser
exports remain `.ifpkg` packages and include app assets. Bundles contain app configuration and tool
definitions; they do not replace asset-bearing packages. Import accepts the ZIP through the existing
file uploader, CLI, or `bundle-content` API mode with base64 content in `yaml_content`.
