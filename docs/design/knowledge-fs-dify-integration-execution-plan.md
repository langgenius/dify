...
- Dify 是用户身份、Workspace、成员、角色、知识库可见范围、产品权限和 API Access 的唯一事实来源。
- Dify 向前端提供知识库产品 API，并在每次操作前完成资源级授权。
...
flowchart LR
    ...
    DifyAPI --> Authz["Dify 身份、Workspace、成员、RBAC、Dataset 权限"]
    DifyAPI --> Binding["Dataset ↔ KnowledgeSpace 绑定"]
...
2. **资源先绑定再调用**：除创建操作外，Dify 必须先由 `dataset_id` 解析出唯一的 `knowledge_space_id`，再签发 Capability。
3. **列表先授权再聚合**：Dify 必须先计算用户可见 Dataset，再只向 KnowledgeFS 批量查询这些空间的技术摘要。
4. **不信任客户端资源 ID**：路径、查询参数和请求体中的 Dataset、Space、Document、Task ID 必须由服务端绑定关系交叉验证。
...
10. **失败关闭**：绑定缺失、授权服务异常、签名密钥未知、模型 capability 不一致、空间归属不一致时均拒绝操作，不猜测、不自动放宽。
...
13. **内容范围显式传递**：若 Source/Document 存在比 Space 更细的产品权限，策略仍由 Dify 计算。Capability 只携带不可伪造的 opaque scope IDs 和 policy revision，KFS 仅据此在 LIMIT 前过滤；若产品没有该层权限，`permission_scope` 只能表示技术归属，不得称为成员 ACL。
14. **运行时双向隔离**：KnowledgeFS runtime Dataset 不能进入 Dify 原生 indexing/retrieval/delete pipeline，普通 Dataset 也不能误入 KFS pipeline。
...
| Dataset 绑定 | KFS Space 与 Dify Dataset 相互独立 | 无法使用 Dataset 可见性、RBAC、`enable_api` 作为权威依据 | P1 |
| 产品列表 | KFS 按自身 member/policy/API Access 过滤空间；Dify 原有 Dataset 列表按 Dify 权限过滤 | 两套授权源会产生可见性不一致，KFS 列表不能作为产品列表 | P4 |
...
| API Access | KFS 默认关闭，覆盖 service API/API key/MCP/agent；Dify `Dataset.enable_api` 默认开启，内部 agent retrieval 还会绕过该开关 | 默认值和通道语义均不一致，直接迁移可能意外开放 | P1、P4、P8 |
| API Key | KFS 有 per-space Key；Dify 当前 `ApiToken` 主要是 tenant 级，部分 controller 又假设已不存在的 `dataset_id` | 不能直接用现有 Dify Token 替代 | P1、P4 |
...
| --- | --- | --- | --- | --- |
| 知识库列表 | Dify Dataset 列表 API | Dify | Dify Dataset + KFS 批量技术摘要 | 仅 Dify 可调的批量内部接口 |
| 知识库详情 | Dify Dataset 详情 API | Dify | 产品字段在 Dify；技术字段在 KFS | Dify BFF 聚合 |
| 创建知识库 | Dify Dataset 创建 API | Dify | Dify Dataset + 绑定；KFS Space 技术壳 | Dify saga 调用 |
| 删除知识库 | Dify Dataset 删除 API | Dify | Dify 删除状态；KFS durable deletion | Dify saga 调用 |
| 名称、图标、描述 | Dify | Dify | Dify Dataset | 当前 KFS 必填字段仅作非权威技术镜像 |
| 成员、权限、可见范围 | Dify | Dify | Dify membership、Dataset permission、RBAC | KFS 不提供产品管理接口 |
...
| 文档列表、详情、Outline、Chunk | Dify BFF | Dify | KFS | 普通 BFF |
...
### 6.1 Dataset 与 KnowledgeSpace 一一绑定

新增 Dify-owned 绑定表，建议命名为 `dataset_knowledge_fs_bindings`：

| 字段 | 说明 |
| --- | --- |
| `id` | Dify 主键 |
| `tenant_id` | Dify Workspace ID |
| `dataset_id` | Dify Dataset ID，唯一且外键，`ON DELETE RESTRICT` |
| `knowledge_space_id` | KnowledgeFS Space ID；`provisioning` 时允许为空，绑定成功后不可变 |
| `provisioning_key` | 创建技术 Space 的稳定幂等键，唯一且不可复用 |
...
| `binding_version` | 绑定和状态转换的 CAS revision |
| `attempt_count` / `last_attempt_at` | worker 重试和 stuck 检测 |
| `last_error_code` | 可观测、可重试的稳定错误码 |
| `last_error_message` | 脱敏后的诊断信息 |
| `last_synced_at` | 技术摘要最近同步时间 |
| `created_at` / `updated_at` | 审计时间 |

约束：

- 唯一索引 `(tenant_id, dataset_id)`。
- `knowledge_space_id` 非空时唯一索引 `(tenant_id, knowledge_space_id)`。PostgreSQL 使用 `WHERE knowledge_space_id IS NOT NULL` 的 partial unique index；MySQL/TiDB 使用允许多个 `NULL` 的 unique key，并用状态机保证 active 时非空。
- 所有查找必须带 `tenant_id`。
- 不允许仅凭浏览器传入的 `knowledge_space_id` 反向获取资源。
- 新增 `DatasetRuntimeMode.KNOWLEDGE_FS`，避免普通 Dataset 被误路由到 KFS。
- Dataset 的物理删除必须被 `RESTRICT`，只能在 KFS deletion terminal 后由 lifecycle service 解除 binding；禁止 ORM cascade 绕过 durable deletion。
- Workspace 删除必须枚举所有 KnowledgeFS Dataset，逐个进入 deleting 状态并等待或记录可恢复的 cleanup ledger，之后才能完成 Workspace 数据清理。

### 6.2 创建采用 saga/outbox

Dify 数据库与 KnowledgeFS 数据库之间没有分布式事务，创建流程必须幂等：

1. Dify 在本地事务中创建 Dataset、`provisioning` binding 和 outbox event。
2. provision worker 只能在 Capability v2 verifier、integrated provision route 和 KFS legacy ACL mutation freeze 就绪后启用；此前只部署 schema，不能发远端流量。
3. worker 使用稳定 `provisioning_key` 和 namespace-level `knowledge_spaces.provision` service Capability 调用 KFS integrated provision operation。
4. integrated provision 只创建技术 Space，绝不 bootstrap owner/member/policy/API Access/API Key。
5. KFS 创建空空间并保存用户所选模型/profile 标识，但不调用 plugin-daemon。
6. 当前 KFS create schema 必填的 name/slug/description/icon 只保存 Dify 产品字段的非权威技术镜像。Dify 是读取事实源；Dify 更新后通过单向 outbox 最终同步，漂移时以 Dify 为准。后续可用 additive schema 去除 KFS 不必要的必填产品字段。
7. Dify 将 `knowledge_space_id` 写入 binding 并切换为 `active`。
8. KFS 超时或失败时 binding 进入可重试状态，不能创建第二个 Dataset。
9. 若 KFS 已成功但 Dify 回写失败，重试必须通过 `provisioning_key` 找回同一 Space。

### 6.3 删除采用双阶段状态

1. Dify 完成删除权限判断并把 Dataset/binding 标为 `deleting`，立即从普通列表隐藏。
2. Dify outbox 调用 KFS durable deletion，携带 dataset/space 精确资源 Capability 和 idempotency key。
3. KFS 执行 tombstone、任务取消策略、Source sync/OAuth 引用清理、索引/对象清理、重试和 terminal audit。
4. Dify 轮询或消费回调，把 binding 标为 `deleted`，再按现有产品规则处理 Dataset 行。
5. 回滚只允许恢复尚未进入 KFS 不可逆清理点的请求；越过该点后只能重新导入，不能伪恢复。
6. Dify 的 Dataset、批量清理和 Workspace 删除入口都必须调用同一个 lifecycle service，数据库 FK 负责阻止旁路硬删除。
7. outbox/cleanup ledger 的保留期必须覆盖最大删除重试窗口；orphan reconciler 定期对比 deleting binding、KFS tombstone 和残留对象。

### 6.4 API Access 和 API Token

不能直接把当前 `Dataset.enable_api` 等同于设计稿的“Service API 和 Agent Access”：

- 明确 caller channel：`interactive`、`service_api`、`agent`、`workflow`、`mcp`、`internal_worker`。
- Dify 新增统一的 external-access policy service；由它决定各 channel 是否允许签发 KFS Capability。
- 设计稿的单一开关固定为以下语义：Console `interactive` 不受影响；`service_api`、`agent` 和使用知识库的 `workflow` 受控；`internal_worker` 继承已准入 job grant，不重新解释该开关；MCP 在没有独立产品策略前默认 fail-closed。
- 新建 KnowledgeFS Dataset 的 external access 默认 `false`。该默认值只作用于 KnowledgeFS runtime，不修改普通 Dataset 的既有 `enable_api` 行为。
- 旧 KFS API Access 默认 `false`，Dify 默认 `true`。迁移时以已有 KFS 显式值为准；未知值和未绑定资源一律迁为 `false`。
- P4 必须修正当前会绕过 `enable_api`/用户权限的 `api/services/knowledge_retrieval_inner_service.py`，并检查 `api/controllers/service_api/wraps.py` 等 service API 入口，统一在 Capability 签发前调用 external-access policy。
- 修正 Dify resource API Token schema：Token 必须能绑定 Dataset/KnowledgeSpace，包含 principal、状态、expiry、revision 和 revoke 信息。
- 长期凭据只存在 Dify；Dify 验证凭据后为单次 KFS operation 兑换短期 Capability。
...
- Dataset visibility、maintainer、partial members 和 resource RBAC 变化递增 `dataset_acl_epoch`。
- API Access caller policy 变化递增 `external_access_epoch`。
- resource credential 更新、过期或撤销递增 credential revision。
- 子资源 scope policy 变化递增 `content_policy_revision`。

这些 revision 由 Dify 事务内更新并写 outbox。它们用于审计、shadow diff、显式 revoke 的有序 CAS 和故障对账，不要求 KnowledgeFS 保存成员或反查 Dify 权限表。
...
- Task、Document、UploadSession 等子资源必须同时绑定其父 `knowledge_space_id`。
...
所有通道都必须先验证 Dataset runtime mode、active binding、operation action/resource，并持久化脱敏 issuance audit；principal-specific 准入规则如下：

| Principal | Dify 准入规则 | `sub` |
| --- | --- | --- |
| Interactive account | 验证登录、Workspace membership、Dataset visibility/maintainer、operation RBAC | `dify-account:<account-id>` |
| Dataset-bound service credential | 验证 credential 状态、expiry、resource binding、operation allowlist、external access | `dify-api-credential:<credential-id>` |
| Agent/Workflow app | 验证 app 所属 Workspace、app↔Dataset 绑定、运行权限、external access；不伪装 owner | `dify-app:<app-id>` |
| Internal worker | 验证已持久化 job/grant、限定内部 operation，不重新使用用户 Cookie | `dify-worker:<service-id>` |
| MCP | 独立策略未实现前不签发 | 不适用 |

Dify 必须从数据库绑定和已验证 principal 构造 namespace、Dataset、Space、Task、content scopes 和 revision；不得采用客户端传入的最终值。
...
- Dify 在创建任务时生成稳定 `grant_id`，写入 membership、Dataset ACL、external-access、credential 和 content-policy revision；KFS 保存不可伪造的 grant provenance，但不保存原始 Token。
...
优先扩展现有 Dataset controller/service，而不是建立一套平行的 `/knowledge-fs/knowledge-spaces` 产品模型：

- Dataset 列表、详情、创建、修改、删除。
- Dataset permission、partial members、maintainer 和 API Access。
- KnowledgeFS 技术设置子资源：模型、Embedding/vector-space、Rerank、retrieval profile。
- 文档、Source、查询、Trace、Research 任务等 typed BFF 路由。
- 大文件上传 Capability issuance。
- Research/Query stream Capability issuance。
...
- 按显式 Space ID 集合批量查询 `document_count`、`index_state`、`model_profile`、`last_job_state`。
- 幂等的 integrated provision：只创建技术 Space，不 bootstrap owner/member/policy/API Access/API Key。
- 更新模型和 retrieval profile。
- 文档/Source/Query/Trace/Research 的业务 API。
...
1. Dify 使用现有 Dataset 查询逻辑先应用 Workspace membership、RBAC 和 Dataset permission。
2. 只收集当前页可见、runtime mode 为 KnowledgeFS、binding active 的 Space ID。
3. Dify 用 batch-status Capability 向 KFS 请求技术摘要。
4. Dify 按 `dataset_id` 聚合名称、描述、权限和技术状态后返回前端。
5. KFS 超时不应扩大可见范围；Dify 返回产品字段和 `technical_status=unavailable`，并记录降级指标。

验收重点：分页和可见性必须在 KFS 调用之前完成，禁止“先取全部空间再在内存过滤”。

### 9.2 创建空知识库

1. Dify 验证 Dataset create 权限和用户选择的模型标识格式。
2. 创建 Dataset + provisioning binding + outbox。
3. KFS 创建空 Space，并持久化模型/provider/vector-space 意图配置。
4. 不调用 plugin-daemon，不阻塞空知识库创建。
5. binding active 后返回产品可用状态。
...
- 基本信息、可见范围、成员和 API Access 只更新 Dify。
...
1. 前端向 Dify 请求某 Dataset 的 upload capability。
...
5. KFS 创建 logical document、asset 和 compilation job。
...
Dify 在 query admission 时检查 Dataset 可见性和 API Access channel，签发具体 Space 的 `queries.create` Capability。KFS 再按最终模式执行：
...
- Source、Document、Node、Graph 的技术归属过滤；若启用子资源产品权限，只使用 Capability 中 Dify 已解析的 opaque scope IDs 和 policy revision，不依赖 KFS 本地成员表。
...
- Research job
- Document compilation requester binding
- Durable deletion
- Profile migration
...
```mermaid
flowchart LR
    P0["P0 契约与运行基线"] --> P1A["P1A 绑定 schema"]
...
### P1：Dify Dataset 绑定和产品状态模型

目标：建立 Dify 产品资源与 KFS 技术资源的一一对应关系。

P1 分为 schema 和启用两段，避免 provisioning worker 在正式认证和 integrated provision 就绪前调用 KFS。

P1A 任务（只部署 additive schema，worker 保持关闭）：

- [ ] `P1A-01` 新增 `DatasetRuntimeMode.KNOWLEDGE_FS`，并在所有原生 indexing/retrieval/delete dispatcher 中显式拒绝或路由该模式。
- [ ] `P1A-02` 新增 nullable Space binding model、migration、repository、CAS state service 和 `ON DELETE RESTRICT`。
- [ ] `P1A-03` 新增 provisioning/deletion outbox schema、稳定 operation/idempotency key、attempt 和保留策略；worker 默认 disabled。
- [ ] `P1A-04` 新增 Dify-owned external-access policy/revision schema，分类 interactive/service/agent/workflow/MCP。
- [ ] `P1A-05` 修正 per-dataset API Token schema/controller，使长期凭据可以精确绑定资源、过期、revision 和撤销。
- [ ] `P1A-06` 增加 binding 管理命令：dry-run、backfill、repair、orphan report。

P1B 任务（P2 和 P3 完成后才允许发流量）：

- [ ] `P1B-01` 启用 integrated provision saga，定义 Dataset 创建、KFS 创建、失败重试和孤儿 Space reconciliation。
- [ ] `P1B-02` 启用 deletion saga，要求 KFS durable grant/revoke/publication fence 已切到 P3。
- [ ] `P1B-03` 统一 Dataset、批量清理和 Workspace 删除入口，禁止绕过 lifecycle service。
- [ ] `P1B-04` 为已创建的 KnowledgeFS Dataset 保留永久可用的 reconciler/delete path；关闭产品 feature flag 不得让资源失去清理入口。

产出：Dataset ↔ Space binding 和可恢复生命周期。

依赖：P1A 依赖 P0；P1B provision 依赖 P2 + P3 integrated provision，P1B deletion 还依赖 P3 durable provenance/fence。

验收：

- 同一 Dataset/Space 无法重复绑定。
- 任一侧超时后重试不会创建重复资源。
- 非 KnowledgeFS Dataset 不会进入 KFS client。
- KnowledgeFS Dataset 不会进入 Dify 原生 indexing/retrieval/delete pipeline。
- binding error 可由后台安全重试或人工修复。
- Dataset/Workspace 旁路硬删除会被数据库约束阻止。

回滚：P1A 是 additive migration。关闭 runtime-mode 产品入口后，已创建的 KnowledgeFS Dataset、outbox、reconciler 和删除入口必须继续运行，直到所有绑定资源进入安全终态。
...
### P4：Dify 产品 API、列表与权限控制面

目标：前端只消费 Dify-owned 产品 API，KnowledgeFS 不再决定谁能看到知识库。

任务：

- [ ] `P4-01` 实现 KFS batch technical status 及 Dify aggregator；Dataset 列表先执行现有可见性/RBAC，再发送显式 Space ID 集合。
- [ ] `P4-02` Dataset 详情聚合产品字段和 KFS 模型/索引/文档技术摘要。
- [ ] `P4-03` 创建、删除、基本信息、permission、partial members、maintainer 全部落 Dify。
- [ ] `P4-04` 将 Enterprise resource RBAC 的 `dataset_id` 传入具体 operation 授权，不能只做 Workspace 粗粒度检查。
- [ ] `P4-05` 实现 external-access policy 并修复 KnowledgeFS Dataset 的默认值/语义迁移；修改 `knowledge_retrieval_inner_service.py` 和 service API wrappers，使 Service API、Agent、Workflow 在签发前统一受控，且不改变普通 Dataset 既有默认行为。
- [ ] `P4-06` 修复 per-dataset API Token 创建、查询、撤销和缓存失效。
- [ ] `P4-07` 将 raw KFS list/create proxy 标为 deprecated，前端迁到 Dataset 产品 API。
...
- partial member 只能在 Dify 列表看到被授权的 Dataset。
- KFS 不可用时不会扩大列表可见范围。
- 任意具体资源操作都有 Dataset binding + resource RBAC。
...
- [ ] `P5-01` 为模型/profile、状态、文档、Outline、Chunk、Source、Query、Trace、Research JSON 接口建立 typed DTO。
- [ ] `P5-02` 扩展 operation registry：method、path、action、resource resolver、RBAC、billing、request/response limit、stream kind。
- [ ] `P5-03` 每个 route 由 Dataset ID 解析 binding，不接受前端直接选择 namespace。
- [ ] `P5-04` 接入模型/profile update，并保持首次文档异步 preflight 和动态维度规则。
- [ ] `P5-05` 接入文档 CRUD、bulk/reindex、job status/cancel。
...
- [ ] `P6-02` 只在 KFS 新增 upload session repository，以及 Postgres/TiDB 对称 migration；Dify 只保存 Capability issuance audit 和必要的 Dataset/Space 映射，不复制 session 状态。
- [ ] `P6-03` Dify 新增 Dataset resource-scoped upload capability endpoint。
...
- [ ] `P8A-01` 生成只读 dry-run 报告：Dataset、Space、owner、visibility、API Access、API Key、授权 revision、任务和孤儿资源。
- [ ] `P8A-02` 建立 Dataset ↔ Space binding；冲突进入人工队列，不自动覆盖。
- [ ] `P8A-03` visibility 映射：`only_me → only_me`、`all_members → all_team_members`、`partial_members → partial_members`。
- [ ] `P8A-04` 将可解析账号映射到 Dify Dataset permission；无法映射的主体不授予访问并进入 quarantine report。
- [ ] `P8A-05` API Access 以旧 KFS 显式值为准；未知值迁为 `false`，不得采用 Dify `true` 默认值。
- [ ] `P8A-06` KFS per-space Key 盘点后由用户轮换成 Dify resource credential，不迁移明文 secret。
...
- [ ] `P8B-02` 应用 final delta，验证 binding、permission、external access、credential 和任务水位。
...
- 每条 binding、权限映射、revision 和任务处理都有审计记录。
...
- `api/models/dataset.py`：runtime mode、Dataset 产品字段语义。
- `api/models/` + `api/migrations/versions/`：binding、external access、resource token、outbox、issuance audit。
- `api/services/knowledge_fs_proxy.py`：迁移期 transport、operation registry 和安全 header。
- `api/controllers/console/knowledge_fs_proxy.py`：迁移期 catch-all；逐步由 typed controller 替代。
- 新增 `api/services/knowledge_fs/`：client、binding、capability issuer、aggregator、lifecycle saga。
- 现有 Dataset controllers/services：列表、详情、创建、删除、permissions、API access 聚合。
- `api/services/knowledge_retrieval_inner_service.py` 和 `api/controllers/service_api/wraps.py`：Agent/Workflow/Service API external-access admission。
...
- Query、Research、Document、Source、Quality、Deletion route/runtime：grant provenance v2。
...
- Dify Dataset 可见性到 KFS ID 集合的映射。
- binding unique/CAS/state transition/idempotency。
...
- Dataset、批量和 Workspace 删除无法绕过 durable deletion；orphan reconciler 可收敛残留。
...
- AnswerTrace/Evidence 只可由当前有权限的 Dataset 调用方读取。
...
- binding provisioning/deleting/error 数量与停留时间。
...
- binding `provisioning` 或 `deleting` 超过约定时间。
- 列表技术摘要降级率持续超阈值。
...
- `KNOWLEDGE_FS_DATASET_BINDING_ENABLED`
...
2. 发布 P1A binding/outbox schema、P2 issuer、P3 integrated provision/provenance 和 P4–P7 代码，但 worker 和产品 route 全部保持关闭。
3. 执行 P8A dry-run、binding/权限/API Access 回填、final-delta 方案和 shadow authorization。
...
| 风险 | 影响 | 预防/缓解 |
| --- | --- | --- |
| 没有 binding 就先删 KFS ACL | 所有资源失去权威授权来源 | P1A schema 先落地，P8A 完成绑定后才能 P8B/P9 |
...
| `enable_api` 默认值相反 | 迁移后意外开放 API/Agent | 显式映射，未知值 false，shadow diff |
| Dify per-dataset Token schema不完整 | 无法替代 KFS per-space Key | P1/P4 先修 schema 和撤销模型 |
...
| Workspace/Dataset cascade 删除 | 绕过 KFS durable deletion，产生孤儿数据 | FK RESTRICT、统一 lifecycle、orphan reconciler |
...
| Dify 提供创建、删除入口 | 6.2、6.3 | P1A-02～P1A-03、P1B、P4-03 | saga 故障注入测试 |
| Dify 管理成员、权限、可见范围 | 1、5、7.4 | P4-03、P4-04 | Dataset/RBAC 越权矩阵 |
| Dify 管理 API Access | 6.4 | P1A-04、P1A-05、P4-05、P4-06 | caller channel 测试 |
...
| KFS 保留文档、Embedding、PageIndex、Graph | 3、9.4、9.5 | P5、P6 | 文档编译与索引集成测试 |
...
2. `PR-02` Dataset runtime mode + nullable binding/outbox/external-access additive schema（P1A，worker disabled）。
...
7. `PR-07` Binding lifecycle saga/outbox/reconciliation（P1B，仍由产品 flag 关闭）。
8. `PR-08` Dify list/detail aggregator + KFS batch status API。
9. `PR-09` Dify permissions/external access/resource API token。
10. `PR-10` Typed settings/document/source BFF。
...
每个 PR 必须包含：迁移兼容性、测试、指标、feature flag、回滚说明和文档更新。不得把 binding、授权源切换、旧表删除放进同一个 PR。
...
- [ ] Dataset 与 KFS Space 有唯一、可修复、可审计的 binding。
...
- [ ] 文档、PageIndex、Graph、AnswerTrace、SSE、durable deletion 和任务链路全部通过集成测试。
...
- [ ] Dataset/Workspace 删除不能通过 cascade 绕过 KFS durable deletion。
