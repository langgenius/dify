...
The root retains engine aggregate usage. Synthetic retry-attempt and agent-log spans carry `metrics_from_parent=true`; enterprise metric generation counts their original logical node instead. Container totals are not another model call. Workflow-agent output logs are converted once into their explicit round/model/tool children; live log events receive explicitly observed wall-clock timestamps, while output-only historical logs retain unavailable timing. Native exporters that require timestamps use a marked, zero-duration point anchored to a captured endpoint; they validate all spans before sending any. [Workflow persistence](../../api/core/app/workflow/layers/persistence.py) now only persists execution records; it no longer reconstructs or enqueues an OPS workflow tree.
...
Successful, failed and cancelled trace bodies are deleted immediately by default after a terminal result is committed. Set `OPS_TRACE_SUCCESS_RETENTION_SECONDS` or `OPS_TRACE_FAILURE_RETENTION_SECONDS` to retain bodies for debugging. Periodic cleanup retries deletion failures and applies the configured retention. Receipts and rows remain while their original message/run or dependent delivery exists, with a minimum 30-day metadata age before ordinary row cleanup. Workspace-only rows use that finite age. Cancelled-upload tombstones are the explicit exception above. Cleanup is idempotent and never deletes pending/sending objects.
...
The settings fingerprint changes with the tenant, authentication configuration or deployment key. Fingerprinting requires a nonempty `SECRET_KEY`; rotating that key invalidates pending destination snapshots instead of reusing their old authorization. Pending exports require the exact captured destination revision and enabled selection. `tracing_revision` protects concurrent configuration saves; `tracing_destination_revision` changes only when the effective destination changes. No-op saves preserve ciphertext, and inactive-provider edits leave pending exports valid. Changes cancel old work with `configuration_changed`, rather than sending old input/output to a new project. App and enterprise work are independent. Disable/delete takes effect before an attempt's authorization check; an already authorized request may finish. Re-enabling increments the revision and does not revive old work.

Provider-owned deployment headers, transport protocols and TLS credentials also enter the fingerprint. Their environment precedence stays in each provider directory. Renewable credentials bind their captured authentication source instead of the issued token, allowing normal refresh without cancelling pending exports; static credentials remain bound by content. Certificate contents are copied outside database sessions; changing a certificate at the same filename changes the fingerprint. The delivery worker compares the current snapshot, then passes those exact checked contents to an independently owned client. Only the fingerprint crosses the queue boundary, and the client does not reread mutable files after authorization. Capture and delivery processes must resolve the same deployment settings.
...
Langfuse uses SDK v4 attribute builders and its propagation span processor with local OTel spans. Each export owns its tracer provider and span collector, then sends the completed tree synchronously through the SSRF-aware HTTP client to the Langfuse OTLP endpoint. Observation input/output replaces deprecated trace input/output. The SDK is pinned to the tested version because the adapter uses internal helpers to preserve historical timestamps and isolate concurrent destinations. Providers use explicit native ingestion/call APIs or OTLP according to captured deployment settings. Phoenix/Arize and cloud OTLP destinations use deterministic OTLP trace/span IDs. MLflow/Databricks use explicit HTTP endpoints and authentication. No export or verification calls global login/init, changes tracking environment variables, or initializes a global Langfuse client or implicit global OTel provider. Unused provider SDK dependencies remain removed.

MLflow artifact uploads require HTTP or HTTPS, including the `mlflow-artifacts:/` proxy used by FileStore tracking servers with artifact serving enabled. Direct filesystem, S3 and GCS artifact writes are intentionally excluded; deployments using those experiment locations must expose them through the MLflow artifact server before rollout. Artifact exports use linked traces for late operations to preserve completed parent artifacts. See [MLflow artifact deployment guidance](../../api/providers/trace/README.md#mlflow-artifacts).
...
2. Apply [migration `74f13a2c08b9`](../../api/migrations/versions/2026_09_09_1200-74f13a2c08b9_add_ops_trace_deliveries.py): delivery table/indexes, configuration and destination revisions, and app/provider uniqueness. It deterministically removes byte-equivalent duplicate configurations. Conflicting rows stop migration with app/provider IDs for resolution; it never guesses which secret/project to retain. Existing encrypted configurations remain in place.
