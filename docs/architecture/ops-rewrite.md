...
The settings fingerprint changes with the tenant, credential contents or deployment key. Fingerprinting requires a nonempty `SECRET_KEY`; rotating that key invalidates pending destination snapshots instead of reusing their old authorization. Pending exports require the exact captured destination revision and enabled selection. `tracing_revision` protects concurrent configuration saves; `tracing_destination_revision` changes only when the effective destination changes. No-op saves preserve ciphertext, and inactive-provider edits leave pending exports valid. Changes cancel old work with `configuration_changed`, rather than sending old input/output to a new project. App and enterprise work are independent. Disable/delete takes effect before an attempt's authorization check; an already authorized request may finish. Re-enabling increments the revision and does not revive old work.

Provider-owned deployment headers, transport protocols and TLS credentials also enter the fingerprint. Their environment precedence stays in each provider directory. Certificate contents are copied outside database sessions; changing a certificate at the same filename changes the fingerprint. The delivery worker compares the current snapshot, then passes those exact checked contents to an independently owned client. Only the fingerprint crosses the queue boundary, and the client does not reread mutable files after authorization. Capture and delivery processes must resolve the same deployment settings.
