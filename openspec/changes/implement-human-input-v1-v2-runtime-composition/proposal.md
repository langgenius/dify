...
本 change 只修复 workflow node runtime 与 HITL callback composition：建立严格版本分派、独立 v2 node class、基于 workflow run/node execution 的 runtime owner，以及可注入、可重入的 v2 callback boundary。v2 runtime application 通过一个深的 `FormSending` 接口隐藏 recipient resolution、sender selection、fanout 和 Email/IM delivery policy；callback 不接收 sender list 或 raw delivery capability snapshots。为移除错误的 form-owned pause identity，本 change 同时更新内部 submission/resume correlation；Controller、OTP、delivery worker 和对外 API wiring 仍由后续 change 负责。
...
- v2 runtime application 先按 runtime owner reload；只有 owner 尚无 form 时才调用 `FormSending.send`。`FormSending` 内部通过 create-once 建立完整 form、grant 和 endpoint graph，只有竞争 winner 在 graph ready 后执行 sender resolution 与 fanout；单事务提交完整 graph 是推荐实现而非硬性 contract。
- 由 composition root 注入 v2 runtime application、runtime form persistence 和 `FormSending`；Node 和 callback 不直接读取 ORM、数据库 session、recipient snapshots、provider capabilities、sender list、Celery 或 service locator。
...
- 主要影响：
  ...
  - v2 runtime application 与 `FormSending` composition
...
- 保持不变：
  ...
  - delivery crash-recovery、outbox 与 Form lifecycle state；本 change 不为 form commit 后、`FormSending` 完成前的进程退出增加恢复状态
