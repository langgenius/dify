...
- transport MUST 与 frontend migration adapter boundary 对齐：request body 使用 `nodes: [{ node_id, node_data }]`，成功响应使用 `data: [{ node_id, node_data }]`，整批失败响应使用 `blockers` 返回 node-scoped blocker；HTTP adapter 只负责把成功或失败状态映射到 frontend discriminated result。
