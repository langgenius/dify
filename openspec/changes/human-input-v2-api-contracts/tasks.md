...
- [ ] 3.2 Complete the canonical IM Pydantic DTOs and transport mapping for `integration_id + config_version` CAS, read-only effective deployment event transport projection, latest-only reads, `page / limit / total`, one real result bucket, `finished_at`, provider-user-ID search, binding writes, and workspace override reset; reject Integration-level mode input and do not expose tenant-selectable supported modes.
- [ ] 3.3 Replace the remaining IM Integration management 501 workspace handlers with thin service-backed implementations that own workspace authentication/scope, request validation, operation metadata, response projection, and stable domain-error-to-HTTP mapping. The reconciliation-backed sync create/latest/latest-results, identity search, Organization binding and workspace override handlers are owned and delivered by `implement-im-contact-sync-api`; do not duplicate them here.
- [ ] 3.4 Add workspace IM Integration management controller tests for permissions, DTO validation, CAS conflicts and proof that those remaining routes no longer return the generic 501 response. Latest-only paging, provider-neutral identity search, unmatched-result side-effect prohibition, binding/override behavior and the corresponding non-501 assertions are owned by `implement-im-contact-sync-api`.
...
- [ ] 6.2 Consume the Dify-owned Integration management boundary, including read-only effective deployment event transport mode and safe operational projection, from `implement-human-input-v2-im-provider-foundation`, and consume the Sync/Organization binding boundaries from `implement-im-contact-sync-api`; keep deployment mode configuration、provider client lifecycle、directory adapters、reconciliation、worker、repositories 与 transaction ownership in their respective implementation changes.
