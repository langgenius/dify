...
- 以 Dify Human Input application service 作为所有 edition 的 IM provider、sync orchestration、reconciliation、persistence 与 binding logic 的唯一实现 owner；本 change 独占相应 Pydantic DTO、workspace Flask handler、trusted internal HTTP controller、鉴权/scope/metadata mapping、HTTP error mapping 与 controller tests。
- 以 `implement-ee-human-input-admin-api` 作为 EE Dashboard Kratos HTTP facade 与 typed Dify client 的跨仓协调入口；EE repository-owned delivery artifact与代码仍是EE实现owner，且EE不复制provider、sync worker、reconciler、Contact projection或Human Input persistence。
...
- `human-input-ee-admin-api`: EE 管理后台的 Organization 级 IM integration / sync 与 Organization Contact IM binding Protobuf-defined Kratos HTTP façade contract。
...
- `openspec/changes/implement-human-input-v2-im-provider-foundation/`
