...
- [ ] 1.2 Add the `api/core/human_input_v2/{shared,contact_directory,approval,im_integration}/` package structure with module docstrings defining responsibilities and dependency direction; name inner modules after hidden business knowledge instead of mechanically creating identical `entities/policies/ports/errors` files.
...
- [ ] 1.6 Add a behavior-ownership test matrix proving local lifecycle rules remain on rich state models, cross-snapshot decisions remain pure domain services, application handlers own I/O ordering, and repository adapters alone own database atomicity.
...
- [ ] 4.1 Add failing tests that keep `ApproverGrant`, `DeliveryEndpoint`, raw credential, `VerifiedProof`, and `SubmissionActor` distinct and prevent endpoint tokens or historical snapshots from becoming submission authority.
- [ ] 4.2 Implement the rich `HumanInputForm` aggregate, approver grants, endpoint snapshots, active-state checks, action validation, and first-success transition decisions without introducing a separate `FormLifecycle` object or claiming persistence success before commit.
...
- [ ] 4.4 Add failing proof-verification and authorization tests for current Account session, trusted EndUser, Email OTP, and IM callback evidence, including rejection of raw credentials at the authorization boundary and correct resolution to Account, EndUser, or EmailAddress submission actors.
...
- [ ] 4.6 Implement immutable `AuthorizationContext` plus `SubmissionAuthorizer`; load current facts through one coherent repository operation, and codify that concurrent Contact, Email, membership, or binding changes after the context is read do not invalidate the current transaction.
...
- [ ] 4.10 Define a submit application handler that verifies proof, invokes the Authorizer and Form aggregate, requests one atomic commit, and exposes workflow resume as an idempotent post-commit port.
...
- [ ] 5.5 Add failing tests proving an Integration can have at most one active sync run, concurrent triggers cannot create two active runs, and retries of the same `sync_run_id` are idempotent.
- [ ] 5.6 Implement `IMSyncRun`, pure `SyncReconciler`, `ReconciliationPlan`, and immutable sync result facts while keeping provider reads and persistence apply outside the reconciler.
- [ ] 5.7 Add failing tests for effective binding priority `workspace override > organization binding > Email fallback`, reset-to-global behavior, and mismatched integration/provider rejection.
- [ ] 5.8 Implement effective IM binding resolution and binding invariants without exposing provider credentials or ORM identity records to recipient resolution.
- [ ] 5.9 Define IM persistence ports for CAS configuration writes, Integration-locked active-run creation, revision-guarded reconciliation, current identity/binding snapshots, and append-only sync results.
...
- [ ] 7.5 Add repository contract tests for coherent submission `AuthorizationContext`, Form row locking, authorization audit, unique submission insert, form status transition, full rollback, accepted concurrent identity changes after context load, and no resume dispatch before commit.
- [ ] 7.6 Implement the SQLAlchemy approval adapter with coherent context loading and one atomic `commit_authorized_submission_once` operation; use a Form row lock and translate unique-conflict races into the already-completed domain result without adding Contact or Binding versions.
- [ ] 7.7 Add repository contract tests for IM Integration CAS, Integration-locked single active sync creation, configuration revision capture, provider replacement cleanup, credential rotation preservation, idempotent run retry, and revision-guarded sync reconciliation.
- [ ] 7.8 Implement the SQLAlchemy IM adapter and ensure concurrent triggers create at most one active run while stale reconciliation can append diagnostic results without mutating current identities or bindings.
- [ ] 7.9 Implement submit-handler post-commit orchestration so enqueue never occurs before commit, enqueue failure never rolls back Submission, failures log tenant/form/workflow identifiers, and repeated resume requests use an idempotent identity.
- [ ] 7.10 Add query-count and eager-loading assertions for aggregate loads so logical relationships with `lazy="raise"` cannot trigger hidden N+1 queries in application services.

## 8. Verify Concurrency, Compatibility, And Handoff

- [ ] 8.1 Add CI-only PostgreSQL integration coverage for concurrent Email/IM submission and prove exactly one submission, one authorized audit event, and one form transition commit; separately prove a Contact change committed after `AuthorizationContext` load does not retroactively reject that transaction.
...
- [ ] 8.3 Add CI-only PostgreSQL integration coverage for concurrent IM Integration CAS writes, concurrent sync triggers with at most one active run, idempotent retry of one run, and stale sync reconciliation.
...
- [ ] 8.5 Add application-handler tests proving commit precedes resume enqueue, enqueue failure leaves the Form submitted and emits actionable identifiers, and duplicate resume dispatch is safe through the port contract.
- [ ] 8.6 Run targeted Human Input v2 domain, mapper, repository, migration, application-handler, and existing Human Input v1 regression tests; document any CI-only suites that cannot run locally.
- [ ] 8.7 Run backend formatting, linting, and type checking, then remove compatibility shims that are no longer referenced while preserving existing API stub imports.
- [ ] 8.8 Re-read and update all affected module, class, and method docstrings so they describe final responsibilities, invariants, transaction boundaries, raised domain errors, snapshot semantics, and excluded concerns.
- [ ] 8.9 Verify no controller stub, provider adapter, Celery task, EE protobuf, transactional outbox, sync-generation mechanism, Contact/Binding versioning, or node-data migration implementation entered this change, and record discovered follow-up work in the appropriate downstream OpenSpec change.
- [ ] 8.10 Validate `implement-human-input-v2-domain-core` with OpenSpec and confirm every requirement scenario is mapped to at least one domain, repository, application-handler, or PostgreSQL integration test.
