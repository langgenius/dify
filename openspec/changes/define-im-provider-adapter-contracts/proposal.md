...
- 定义所有初始 Provider 必备的 Basic Messaging，以及仅 Slack、Feishu/Lark 与 Microsoft Teams 提供的可选 Dynamic Card Messaging capabilities；DingTalk 与 WeCom 本期不暴露 Dynamic Card Messaging。Dynamic Card Messaging 通过无副作用 Card Assessment 对完整 HITL-aligned card intent 做 Provider-specific representability judgment。每次方法调用至多执行一次 side-effecting Provider operation，并返回 Provider acceptance 与精确 Provider-owned message reference。
...
- Capability presence 是支持情况的唯一事实来源：Slack 与 Feishu/Lark 暴露 Webhook Events 和 STREAM Events，Microsoft Teams 仅暴露 Webhook Events；DingTalk 与 WeCom 本期不暴露任何入站 event capability；不支持的 Provider 不实现 dummy capability。
