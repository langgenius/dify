...
**Reason**: Old Integration aggregate/service ownership is replaced by `IMChannelService` plus the independent `IMChannelReader` / `IMChannelWriter` prerequisite。

**Migration**: Controller keeps the confirmed Channel API and delegates to `IMChannelService`。The Service owns Provider preparation、operation selection、Channel construction、transaction span and safe failure mapping。
