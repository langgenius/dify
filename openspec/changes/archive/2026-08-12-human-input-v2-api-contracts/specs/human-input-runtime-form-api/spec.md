...
#### Scenario: One-time Email or Dynamic Email token 不能用于 authenticated Contact page
- **WHEN** a one-time Email or Dynamic Email grant token is presented to the authenticated Contact approval surface
- **THEN** the console handler MUST reject it without invoking Contact-session approval logic or forwarding the request to the public web handler
...
#### Scenario: Email overlap does not rewrite actor semantics
- **WHEN** a one-time Email or Dynamic Email approver submits successfully after a current Contact begins using the same normalized email
- **THEN** the runtime MUST still persist an EmailAddress-backed authorization result rather than rewriting it into Contact-backed audit data
