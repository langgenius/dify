...
Human Input v2 已经具备稳定的 Form、Approver Grant、IM Delivery Endpoint、delivery attempt、IM identity proof 与 first-success submission 抽象，但还没有一条真正发送 IM card、消费 provider card action 并提交同一 Form 的运行时链路。独立的 `implement-human-input-v2-im-provider-foundation` 已统一拥有 Integration/credential/client lifecycle，以及由deployment runtime policy选择的`WEBHOOK / STREAM` verification、ack、lease/fencing与authenticated event transport；Card Interaction应复用这些基础，而不是再次实现或选择ingress runtime。
...
- 注册 Card-owned `AuthenticatedIMEventSink`，消费 Foundation 从 `WEBHOOK` 或 `STREAM` 交付的同一种 `AuthenticatedIMEventEnvelope`；provider normalizer 只把 payload 差异收敛为 authenticated provider actor、endpoint capability、selected action 与 bounded inputs，不执行 Dify authorization。
...
- 增加 provider-neutral interaction processor：解析 endpoint capability，将 authenticated provider actor 转换为 `VerifiedIMIdentityProof` evidence，并把 current binding、Contact、Account、workspace 与 Form authorization 留给共享 HITL v2 submission transaction；复用共享 form validation、first-success submission 与 workflow resume。
...
- Affected persistence: stable card delivery operations、provider card handles、encrypted short-lived capability escrow、Card interaction inbox/deduplication 与 terminal update operations；不修改 deployment event transport policy、Integration credential/revision或stream lease state。
