...
本 change 只覆盖 Contacts 领域中的 Organization 级 Channels 管理、IM platform 绑定与通讯录同步详情。它不涉及 Agent 发布、Agent Roster、App access point 或 workflow node binding。Figma 文件虽然名为 “Agent Roster”，但用户已经明确这些节点表达的是 Contacts 的 channel 管理 UI。

已有 `hitl-im-contact-domain-discovery` change 提供了基础领域约束；本次 Channels 设计用“每个 provider 一份 active 配置”取代原来的全局单 active IM platform 限制，其余约束继续复用：

- Email channel 与 IM channels 可以同时配置；同一 provider 只保留一份 active 配置。
...
| 范围                | Figma node                                          |
| ------------------- | --------------------------------------------------- |
| Channels 与配置界面 | `1649:6572`、`1613:5906`、`1646:5214`、`2188:34008` |
...
- 在 Contacts 管理区域提供从未连接、配置、mock 授权、连接异常到已连接的完整 Channels 管理体验。
- 支持 Email channel 的 Resend 专用配置，以及已配置 channel 的 Configure / Delete 生命周期。
...
- 不允许同一 Organization 为同一个 provider 创建多份 active 配置；不同 channel 可以并存。
...
不采用完全动态 JSON form renderer，因为 Email、App ID、OAuth、callback、权限说明等交互差异较大；也不为每个 IM provider 复制完整 overlay，以避免状态和错误处理分叉。

### 4. 已配置 channel 使用明确的 Configure / Delete 操作
...
### 8. Sync details 以 `sync_run_id` 为稳定上下文
...
### 9. Summary 与 detail 使用统一结果 taxonomy
...
- [Figma 中 provider-specific 差异可能继续调整] → 保留共享 channel card、IM provider shell 与 Email adapter 边界，将视觉差异限制在 adapter 内。
...
1. 扩展 Contacts-owned view model、repository interface、query keys 和命名 mock scenarios，使 Email 与 IM channel 可并存。
2. 在 mock repository 上完成 Channels 列表、Email 配置、已配置项操作、连接状态、手动同步与同步详情 UI。
...
- 多个 IM provider 同时配置时的真实 directory sync 聚合语义留待后端 contract 明确；当前前端沿用确定性 mock 摘要。
