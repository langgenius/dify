...
已有 `hitl-im-contact-domain-discovery` change 提供了基础领域约束；本次 Channels 设计在保留全局单 active IM platform 限制的同时增加独立 Email channel，其余约束继续复用：
...
- 不允许同一 Organization 同时存在多个尚未删除的 IM provider binding；Email 可独立共存。
...
1. 扩展 Contacts-owned view model、repository interface、query keys 和命名 mock scenarios，使 Email 可与单一 active IM binding 共存。
...
本 change 不包含数据迁移，也不要求任何后端部署步骤。该代码尚未上线，因此不兼容历史上的多 IM provider mock 状态；回滚时关闭或移除前端入口即可。
