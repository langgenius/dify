...
本 change 只覆盖 Contacts 领域的前端管理体验：联系人列表、联系人详情、添加 External contact、Add from Platform、选择并移除可移除联系人，以及移除 workspace member 时的 Contacts 影响提示。Figma 文件虽然名为 “Agent Roster”，这些节点在本 change 中只作为 Contacts UI 验收来源，不引入任何 Agent 领域能力。
...
- Platform contact 是 Contacts 中的一种 Contact type。EE 暂时保留 Add from Platform 流程，但前端只查询“可添加的 Platform contacts”，不建立 OrganizationCandidate 领域模型。
...
- CE / SaaS 移除 member 后，当前 workspace Contact identity 直接删除，不保留 inactive identity；重新加入时创建新的 Contact identity。EE 移除 member 时可选择保留为 Platform contact。
...
本 change 严格限定为前端。后端数据结构尚未完成，因此目录、详情、可添加 Platform contacts、External contact 创建、联系人移除和 member 移除均由集中、类型安全、确定性的 mock repository 驱动。mock 权限与 mutation 只用于展示和测试，不构成真实安全或持久化边界。
...
- 支持管理员创建 External contact，EE 管理员从 mock 可添加 Platform-contact 数据源多选添加 Platform contact，并从当前目录多选和移除 Platform / External contact；workspace contact 始终不可选择或移除。
...
- 不执行真实 member 删除、Contact 持久化、Platform-contact 数据源查询或权限校验。
- 不实现 OrganizationCandidate 前端类型、query key、hook、repository method、fixture 或 i18n namespace；Add from Platform 使用 Platform-contact 语义命名。
- 不实现 Organization 级 IM platform 绑定和同步详情；该能力属于 `add-im-platform-binding-ui`。
- 不实现 Contact 在 HITL 节点中的选择器、通知投递、审批鉴权或历史审计。
- 不实现未由本批 Figma 页面明确覆盖的批量编辑、Contact 合并、workspace contact 删除、workspace IM override 或自由文本 IM identity 编辑。
...
Contacts 建立独立 feature，拥有目录页面、详情 surface、External contact 表单、EE Platform contact picker、联系人选择 / 移除状态、repository 和 query keys。Members feature 继续拥有成员列表和移除入口，只调用 Contacts 提供的 `MemberRemovalContactImpactDialog` 或等价窄组件。
...
- 可添加 Platform contact 搜索无结果、已有联系人排除、多选成功和失败。
- Platform / External contact 多选移除成功和失败，以及 workspace contact 不可移除。
...
External contact、添加 Platform contact、联系人移除和移除 member 使用 mutation；成功后精确失效目录、相关详情和可添加 Platform-contact query。表单草稿、列表选择和 overlay open state 保持局部，不引入全局 store。
...
表单至少拥有 display name 与必填 Email。前端先做必填和格式校验；mock repository 使用 lower-case 完整 Email 在当前 Contacts 与可添加 Platform-contact 数据中进行身份匹配，并返回 typed result：
...
### 7. Add from Platform 使用 Platform-contact 语义建模

EE shell 继续展示 Add from Platform 操作。可添加 Platform contact 使用独立 query、搜索输入和 selection state；已属于当前 workspace Contacts 的对象不能再次选择。确认后 mock mutation 将所选对象加入目录，并以 `kind: platform` 展示。

前端类型、query key、hook、repository method、fixture、scenario 和 i18n namespace 均使用 `AvailablePlatformContact` / `platformPicker` 等 Platform-contact 语义，不出现 OrganizationCandidate 概念。可添加对象池不直接混入普通 Contacts 列表。

### 8. 批量选择只包含可移除 Contact

具备 Contact 管理权限时，目录展示 checkbox 列。workspace contact checkbox 固定禁用；Platform contact 与 External contact 可选择。表头 checkbox 只计算当前可见且可移除的行，并正确表达全选与部分选择状态。

有选择时按 Figma 展示选中数量以及 Remove / Cancel 操作。Remove 只向 typed mock repository 发送已选择的 Platform / External contact id；repository 再次校验类型，不能移除 workspace contact。成功后清空选择并刷新目录 / 详情，失败时保留选择与现有行。

### 9. Member 移除只使用一个确认流程

现有成员菜单的简单 AlertDialog 应扩展或替换为 Contacts-aware 确认 surface：

- 对 pending invitation 的取消保持原有简单确认，不展示 Contacts 去留。
- CE / SaaS active member 展示“将同时删除当前 workspace Contact identity”的不可选影响说明。
...
- [可添加 Platform-contact 数据集较大] → mock 仍使用分页 / 增量查询 contract，不把全量可添加对象一次性载入浏览器。
- [选择状态包含不可移除联系人] → UI 和 repository 双层按 Contact kind 限制，表头全选只覆盖 Platform / External 行。
...
3. 完成列表与详情，再完成 External contact、EE Platform contact picker、联系人批量移除和 member removal dialog。
...
- 列表展示名称、Email、类型、只读 channel summary 与加入时间；详情不提供没有
  typed action 的编辑、删除、合并或 IM override 操作。列表批量移除仅作用于
  Platform / External contact，workspace 行 checkbox 禁用。
- Platform contact 在目录和详情中只作为 Contact type 展示；Add from Platform
  暂时保留，但前端使用 Platform-contact 语义的数据结构，不建立
  OrganizationCandidate 代码。
