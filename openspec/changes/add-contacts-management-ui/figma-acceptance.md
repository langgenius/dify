...
| `1282:62739` | Directory with details        | Keep the current directory visible and open a 320px right detail panel. Search, filter, and pagination context remain unchanged while the panel is open. The table exposes selection checkboxes and the selected-count Remove / Cancel action bar; workspace rows remain disabled because only Platform and External contacts are removable. |
...
| `1459:31142` | EE directory                  | Add the Platform filter and retain the Add contact menu with Add from Platform and External contact. Platform remains a normal Contact type in the directory; the frontend does not expose an OrganizationCandidate concept.                                                                                                                 |
| `1459:32562` | Platform contact picker       | Search available Platform contacts, exclude existing Contacts, keep explicit selection, and add one or more Platform contacts. The approved OpenSpec multi-select behavior takes precedence over the inspected single-select visual variant.                                                                                                 |
...
- Deployment: CE, SaaS, and EE share the page shell. EE adds the Platform
  filter, Add from Platform picker, and member-retention choice.
