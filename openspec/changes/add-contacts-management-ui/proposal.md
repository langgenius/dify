...
- 增加 External contact 创建流程，覆盖必填字段、Email 格式、大小写归一后的重复校验、命中当前目录内 workspace 或 Platform contact 时的冲突提示、pending、失败和成功状态。
- Platform contact 在 Contacts 目录中只作为一种 Contact type 展示；暂时保留 EE 的 Add from Platform 入口、选择和添加流程，但前端使用可添加 Platform contact 的模型与查询，不引入 `OrganizationCandidate` 领域代码。
- 增加联系人批量选择与移除交互；只有 Platform contact 和 External contact 可被选择和移除，workspace contact 的 checkbox 固定禁用。
...
- `contacts-directory-browsing`: Contacts 列表、搜索筛选、类型展示、权限状态、可移除联系人选择，以及 CE / SaaS 与 EE 的目录差异。
...
- 需要为列表、详情、权限、External contact 校验、Add from Platform、Platform / External contact 选择与移除、member 移除分支和可访问性补充 Vitest / Testing Library 测试。
- 设计验收来源：
