...
- [ ] 1.5 Confirm the Dify upstream dependencies are split correctly: `initialize-human-input-contact-projection` owns only the idempotent version-upgrade command `flask data-migrate human-input-contacts --apply`; `implement-contact-projection-lifecycle-maintenance` owns authoritative Account write-through and availability; `complete-human-input-im-channel-management` owns provider configuration and connected diagnostics; `integrate-im-contact-sync-runtime` owns manual sync eligibility, dispatch, provider directory execution, and Contact read-only reconciliation; `complete-human-input-contact-binding-api` owns current Contact/identity queries and binding application boundaries. `joined_at` remains projected from `Account.created_at`, and Organization Contact reads/manual sync MUST NOT trigger initialization or repair.
