...
- [ ] 1.5 Add shared codec conformance tests proving every private payload model exposes `encode() -> str` and `decode(value: str) -> Self`, every encoded locator is URL-safe Base64, and every decoder rejects invalid alphabet, malformed padding and invalid length without re-encoding decoded bytes.
- [ ] 1.6 Add source-boundary tests proving adapters and helpers contain no duplicate model-to-JSON-to-URL-safe-Base64 or reverse conversion logic.
- [ ] 1.7 Base64-decode one encoded locator per Provider in tests and assert the JSON object explicitly contains `v` and `p`, uses the exact required field set, and does not obtain either discriminator solely from a Pydantic default.
...
- [ ] 4.2 Add Feishu/Lark property-based tests over the shared payload model verifying `payload.decode(payload.encode()) == payload`; retain examples for missing `v`/`p`, wrong/unknown Provider, unknown version, malformed Base64, malformed JSON, strict/extra validation, secret absence and scalar round trip across adapter recreation.
- [ ] 4.3 Require the Feishu/Lark adapter to call `_FeishuLarkLocatorPayload.decode()`, verify `payload.p == self._provider`, and return `INVALID_REFERENCE` without Provider I/O for cross-Feishu/Lark locators; preserve exact-message replacement without private wrapper identity or a process-local registry.
