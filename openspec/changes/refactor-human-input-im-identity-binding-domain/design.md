...
- `get/get_for_contact/list_all` for default Bindings;
...
Identity ports define one root `IMIdentityRepositoryError` with narrow already-exists, not-found, in-use, and stale-write errors. Binding ports define one root `IMBindingRepositoryError` with conflict, Identity-not-found, and stale-write errors. Both roots derive directly from `Exception`.
