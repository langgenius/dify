...
### Requirement: Account-backed Contact 解析 MUST 使用当前成员与可用状态
...
- **THEN** 系统 MUST 在同一事务中 hard-delete its `HumanInputExternalContactProfile`、`HumanInputContactIdentity` and current bindings，并 MUST 在 current reads 返回 `404`
...
- **THEN** current Contact reads MUST return the new Account-owned values without updating `HumanInputContactIdentity` or allocating a new Contact ID
