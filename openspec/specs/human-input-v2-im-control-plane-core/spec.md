...
## Purpose
Define Channel-scoped synchronization, reconciliation, effective Binding resolution and credential-envelope invariants for Human Input V2 IM.
## Requirements
...
- **WHEN** an IM repository loads Channel, Identity, Binding or sync records
...
`IMChannelService` MUST distinguish confirmed credential rotation from provider or provider-tenant replacement.
...
- **THEN** Channel ID、current identities and bindings MUST be preserved while configuration version advances

#### Scenario: Provider tenant is replaced
- **WHEN** provider or provider-tenant identity changes
- **THEN** `IMChannelService` MUST replace the current Channel with a new Channel ID
...
### Requirement: Each IM Channel MUST have at most one active sync run
The persistence contract MUST prevent concurrent triggers from creating more than one active synchronization run for the same Channel revision.
...
- **WHEN** a trigger arrives while the Channel has an active run
...
The pure reconciler MUST match provider entries against current snapshots and produce an immutable plan. Persistence apply MUST compare the captured Channel revision again before mutating current identities or bindings.
...
- **WHEN** a plan changes the captured Channel version or provider for an existing sync run
- **THEN** apply MUST reject the plan before checking or mutating current state

#### Scenario: Reconciliation is stale
- **WHEN** current Channel revision differs from the run's captured revision
...
#### Scenario: Binding belongs to another Channel
- **WHEN** a binding or identity belongs to a different Channel than the requested Channel
- **THEN** resolution MUST return a stable invalid-binding result and MUST NOT expose the binding to consumers

#### Scenario: Workspace requests another owner's Channel
- **WHEN** owner-bound composition cannot load the requested Channel for the current workspace
- **THEN** resolution MUST reject the request before loading Channel-scoped identities and bindings

### Requirement: IM persistence ports MUST expose transaction-oriented operations
IM persistence ports MUST provide Channel-bound run creation, snapshot loading, revision-guarded plan apply and append-only results rather than generic CRUD per table. Channel configuration persistence MUST remain owned by `IMChannelReader` and `IMChannelWriter`.
...
### Requirement: IM Channel credentials MUST use one versioned opaque persistence envelope
The IM credential owner MUST serialize the complete validated resolved credential payload, including its provider discriminator, and encrypt it once with an owner-bounded cipher before persistence. Workspace Channel composition MUST bind that cipher to the owning tenant and its existing provisioned key. Deployment cipher provisioning is deferred; production MUST reject deployment configuration when no deployment-bounded cipher is explicitly injected. `IMEncryptedCredentials` MUST inherit `BaseModel` directly and declare `ConfigDict(extra="forbid", frozen=True, strict=True, validate_default=True)`. It MUST contain exactly `version: Literal[1]` and a non-empty `ciphertext`. `HumanInputIMChannel.encrypted_credentials` MUST use `FrozenPydanticModelColumn(IMEncryptedCredentials)`. The column MUST NOT use a provider-specific model, `TypeAdapter`, discriminated union or `model_types`. The Channel provider, provider tenant identity and safe `app_identifier` metadata MUST remain separately persisted.

#### Scenario: A new workspace IM Channel is configured
- **WHEN** a workspace create, credential rotation or replacement accepts complete credentials for any supported IM provider
- **THEN** the resulting Channel MUST contain one recognized envelope version and one ciphertext
- **AND** every submitted provider configuration field MUST be recoverable only from that ciphertext
- **AND** the Channel MUST persist the provider and a safe `app_identifier` outside the ciphertext
...
- **WHEN** SQLAlchemy loads `HumanInputIMChannel.encrypted_credentials`
...
- **WHEN** a Channel contains an unknown envelope version, undecryptable ciphertext or a decrypted value that is not a valid credential object
...
The credential loader MUST decrypt an envelope with an owner-bounded cipher and validate the recovered payload with the resolved credential discriminated union. Workspace runtime MUST bind the cipher using the owner context that loaded the current Channel. Deployment runtime requires an explicitly injected deployment-bounded cipher. The provider discriminator inside the recovered payload MUST equal the persisted Channel provider.

#### Scenario: Envelope credentials are recovered for runtime work
- **WHEN** IM synchronization, delivery or event runtime needs an adapter for a Channel with a recognized envelope
...
- **WHEN** runtime composes a deployment-owned Channel without an explicitly injected deployment-bounded cipher
- **THEN** composition MUST fail before decryption, adapter construction or provider I/O

#### Scenario: Encrypted provider discriminator does not match the Channel
- **WHEN** decrypted payload provider differs from the persisted Channel provider
