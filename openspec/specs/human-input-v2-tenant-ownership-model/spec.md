# human-input-v2-tenant-ownership-model Specification

## Purpose

Defines canonical tenant-owner terminology and boundary mapping for HITLv2 domain, persistence, and serialized contracts.
...
`HumanInputIMChannel.owner_key` MUST 仅表示 Dify Channel owner slot。它 MUST NOT 替代或重解释 `provider_tenant_id` 或其他 Provider-native identifier。
...
Tenant terminology migration 本身 MUST NOT 要求无关的 schema migration。除 IM Channel 外，HITLv2 ORM owner columns 已使用 `tenant_id`，且当前没有需要迁移的 HITLv2 持久化记录或异步消息，因此实现 MUST 复用现有表、列、索引和 owner predicates。独立的 IM Channel Repository MAY 将未发布的 nullable `tenant_id` owner column 替换为 non-null unique `owner_key`，因为该列用于强制 workspace/deployment singleton persistence；没有已发布的 Channel row 需要回填。除该明确例外外，该 change MUST NOT 创建 schema migration、payload migration 或数据回填。
